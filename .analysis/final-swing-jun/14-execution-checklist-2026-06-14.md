...
current incremental update: entry module and source-file queries now require exact-one matches instead of first-match selection; malformed async `Promise`/async-union return plans now report unsupported diagnostics instead of inventing `void`/`object` awaited types; named intersection alias targets now report unsupported diagnostics before emitting traversal placeholders; C# private JS runtime type rendering now uses full `sourceRuntimeName` identities instead of simple-name maps; stale CLI “TypeScript compilation failed” wording now says Tsonic source analysis; focused validation green with @tsonic/frontend build, @tsonic/cli build, and @tsonic/csharp-emitter package tests `14 passing / 0 failing`
...
| 8.4 | C# emitter tests | Plan renderer tests green | Green after async/intersection renderer hardening | `14 passing / 0 failing` for @tsonic/csharp-emitter package tests |
...
| Renderer broad type placeholders | Array literals, source primitives, unions, intersections, missing alias targets, malformed async return types, and unsupported type plans could silently render broad `object?` placeholders | These renderer paths now report unsupported diagnostics before emitting traversal placeholders; valid opaque runtime carriers still map to `object?` by source fact | @tsonic/csharp-emitter build green; @tsonic/csharp-emitter test package `14 passing / 0 failing` |
