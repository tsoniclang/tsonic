...
latest pushed commit before this checkpoint: 7a13a6cc Remove arbitrary first semantic fallbacks
current local state: declaration-owner selection in lowering now uses value declarations, exact-one declarations, exact-one runtime type declarations, exact-one class/interface declarations, or unique runtime visibility; direct first-declaration and first-signature owner selection searches are clean
...
| Binding access over unions | Destructured binding type lookup selected the first non-nullish union arm | Binding access now evaluates all non-nullish union arms and preserves a union result or exact shared result; it never picks the first arm | Lowering focused subset `5 passing / 0 failing` |
