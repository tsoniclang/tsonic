...
| 8.4 | C# emitter tests | Plan renderer tests green | Green after async/intersection/missing-return renderer hardening | `15 passing / 0 failing` for @tsonic/csharp-emitter package tests |
...
| Renderer broad type placeholders | Array literals, source primitives, unions, intersections, missing alias targets, malformed async return types, missing function return types, and unsupported type plans could silently render broad `object?`/`void` placeholders | These renderer paths now report unsupported diagnostics before emitting traversal placeholders; valid opaque runtime carriers still map to `object?` by source fact | @tsonic/csharp-emitter build green; @tsonic/csharp-emitter test package `15 passing / 0 failing` |
