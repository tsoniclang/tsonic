...
latest pushed commit before this checkpoint: cb41b8ee Require proven runtime type identities
...
| Renderer broad type placeholders | Array literals, source primitives, unions, intersections, missing alias targets, and unsupported type plans could silently render broad `object?` placeholders | These renderer paths now report unsupported diagnostics before emitting traversal placeholders; valid opaque runtime carriers still map to `object?` by source fact | @tsonic/csharp-emitter build green; @tsonic/csharp-emitter test package `12 passing / 0 failing` |
