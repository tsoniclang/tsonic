...
| 4.4 | Source files/modules | Runtime closure and semantic support files are separated | Expanded | `TsonicProgram.workspaceGraph` now carries the single discovery snapshot; focused Program Creation group 48 passing / 0 failing |
...
| 6.1 | Module graph/diagnostics | TSTS owns semantic module graph | Expanded | Public old dependency-graph constructor deleted; lowering graph is built from the already-created TSTS program |
...
| 7.2 | Delete old graph extraction | No product import/export semantic walkers as public entrypoints | Expanded | `buildModuleDependencyGraph`, `ModuleDependencyGraphResult`, and public `dependency-graph` path removed; remaining import-closure seeding is internal and pending final replacement |
...
| 7.5 | Delete old inference/binding | No local checker/generic/call inference owner | Expanded | Standalone generic-function helper deleted; unused source-function signature fallback selector deleted; source extension is single fact owner |
