...
latest pushed commit before this checkpoint: f88ecc1a Require exact runtime heritage lowering
current local state: source-front-end construction is now a single TSTS analysis path: no public transpile surface, no caller-supplied extension list, no semantic/check toggle, explicit module-resolution paths, `.temp/`-rooted frontend test scratch, and exact runtime heritage lowering remains in place
