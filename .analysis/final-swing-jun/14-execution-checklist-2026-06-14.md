...
latest pushed commit before this checkpoint: 59159fd8 Narrow TSTS source frontend boundary
current local state: C# expression rendering no longer invents missing identifier names, member names, literal text, array callbacks, intrinsic arguments, or element-index expressions; binding-pattern variable plans carry the AST pattern text for diagnostics instead of synthetic `value`
