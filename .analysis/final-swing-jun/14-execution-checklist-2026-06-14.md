...
| External binding target identity | Override-accessibility metadata treated a named type whose source name contained `.` as if it were a target-qualified runtime name | External binding lookup now uses proven `sourceRuntimeName` or metadata simple-name lookup only; dotted source names are not target identity | @tsonic/csharp-emitter build green; product search for dotted type-name target inference is clean |
