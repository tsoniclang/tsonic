...
audit validation: product TSC import search clean outside vendored TSTS; frontend CLR/C#/System target leakage search clean; old IR/source-text emission decision search clean except diagnostics/token labels; message-substring capability gating removed; product duplicate `Record` named-type checks removed outside source fact production; unused source-function signature fallback selector symbols removed from CLI; lowering fallback `unknown` search now reports only real source `unknown` and declaration-kind label; product search for `pickPackageFolder`, first package-folder indexing, source-signature first-choice fallback, and exact-nullish fallback is clean
