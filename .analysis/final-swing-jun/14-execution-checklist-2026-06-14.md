...
| 8.4 | C# emitter tests | Plan renderer tests green | Green after async/intersection/missing-return renderer hardening | `16 passing / 0 failing` for @tsonic/csharp-emitter package tests |
...
| External binding target identity | Override-accessibility metadata treated a named type whose source name contained `.` or whose source simple name matched an external type as if it had target identity | External binding lookup now requires proven `sourceRuntimeName`; dotted names, simple-name maps, and known-name renderer maps are not target identity | @tsonic/csharp-emitter build green; @tsonic/csharp-emitter test package `12 passing / 0 failing`; product search for dotted/simple-name target inference is clean |
