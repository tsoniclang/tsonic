...
const renderConstructor = (
  ...
  const body = superCall
    ? removeLeadingSuperConstructorCall(plan.body)
    : plan.body;
  const baseInitializer =
    superCall && superCall.arguments.length > 0
      ...
      : "";
  const renderedBody = renderFunctionBody(
    body,
    context,
    undefined,
    plan.parameters
...
const leadingSuperConstructorCall = (
  ...
  const first = body?.statementKind === "block" ? body.statements[0] : body;
...
const renderClass = (
  ...
  return withTypeParameterScope(context, plan.typeParameters, () => {
    ...
    const declarationKeyword =
      plan.sourceTypeKind === "struct" ? "struct" : "class";
    ...
    const synthesizedConstructor = !hasConstructor
...
const renderVariable = (
  ...
  if (
    ...
    const rendered = [
      ...
      `public static ${delegateName} ${sanitizeIdentifier(declarationName)} = ${renderExpression(
        initializer,
        context
      )};`,
    ].join("\n");
