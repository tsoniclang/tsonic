...
const isNativeArrayReceiverType = (
  type: IrType | undefined,
  context: EmitterContext
): boolean =>
  !!type && resolveTypeAlias(stripNullish(type), context).kind === "arrayType";
...
export const tryEmitJsSurfaceArrayLikeLengthAccess = (
  ...
  const hasArrayLengthBinding =
    !!expr.memberBinding &&
    surfaceMemberReadsArrayLength(expr.memberBinding, context);
  if (
    expr.isComputed ||
    typeof expr.property !== "string" ||
    (!hasArrayLengthBinding && expr.property !== "length")
  ) {
    return undefined;
  }

  const directStorageType = resolveDirectStorageIrType(expr.object, context);
  const effectiveObjectType = directStorageType ?? objectType;
  const nativeArrayReceiver = isNativeArrayReceiverType(
    effectiveObjectType,
    context
  );
  if (!hasArrayLengthBinding && !nativeArrayReceiver) {
    return undefined;
  }
