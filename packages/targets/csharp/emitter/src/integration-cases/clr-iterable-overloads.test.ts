...
describe("Integration: CLR iterable overloads", () => {
  ...
  it("routes source-package iterable return values through metadata-backed external iterable overloads", () => {
    const csharp = compileProjectToCSharp(
      {
        "package.json": JSON.stringify(
          { name: "emitter-test-project", version: "1.0.0", type: "module" },
        ...
        "node_modules/@fixture/js/src/uint8-array.ts": [
          ...
          "export function createBytes(): Uint8Array {",
          "  return new Uint8Array();",
          "}",
        ].join("\n"),
        "src/index.ts": [
          'import { Assert } from "xunit-types/Xunit.js";',
          'import { createBytes } from "@fixture/js/uint8-array.js";',
          "",
          "export function run(): void {",
          "  const left = createBytes();",
          "  const right = createBytes();",
          "  Assert.Equal(left, right);",
          "}",
        ].join("\n"),
      },
    ...
    const csharp = compileProjectToCSharp(
      {
        ...
        "src/index.ts": [
          ...
          "  | undefined;",
          "",
          "export function run(received: RuntimeValue): void {",
          "  Assert.Equal(42, received);",
          "}",
        ].join("\n"),
      },
