...
describe("End-to-End Integration", () => {
  describe("Regression Coverage", () => {
    ...
    it("uses runtime storage property targets for broad-union object literal initializers", () => {
      const source = `
        type RuntimeValue = string | number | boolean | object | null | undefined;

        class Options {
          detail: RuntimeValue = undefined;
        }

        export function run(detail: string): Options {
          return { detail };
        }
      `;
      ...
      const initializerLine = csharp
        .split("\n")
        .find((line) => line.includes("new Options"));

      expect(initializerLine).to.include("detail = detail");
      expect(initializerLine).to.not.include("Union");
    });

    it("materializes broad runtime-value numeric assertions before comparison", () => {
      const source = `
        type RuntimeValue =
          | string
          | number
        ...
        export function run(left: RuntimeValue, right: RuntimeValue): boolean {
      ...
      expect(csharp).to.include("left switch { int ");
      expect(csharp).to.include("right switch { int ");
    ...
    it("preserves Array.isArray narrowed element types in for-of loops", () => {
      const source = `
        type QueryStringValue = string | string[] | null | undefined;

        const convertToString = (value: string | null | undefined): string =>
          value === null || value === undefined ? "" : String(value);

        export function stringifyValue(value: QueryStringValue): string {
          let result = "";
          if (Array.isArray(value)) {
            for (const item of value) {
              result = result + convertToString(item);
            }
            return result;
          }
      ...
      expect(csharp).to.include("foreach (var item in (value.As1()))");
      expect(csharp).to.include("convertToString(item)");
      expect(csharp).to.not.include("Enumerable.Select<string, object?>");
      expect(csharp).to.not.include("convertToString((object?)item)");
    });
