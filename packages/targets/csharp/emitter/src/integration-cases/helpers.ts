...
import { findTsonicWorkspaceRoot } from "../testing/workspace-root.js";
...
const tsonicWorkspaceRoot = findTsonicWorkspaceRoot(helpersDir);
const siblingRepositoriesRoot = path.resolve(tsonicWorkspaceRoot, "..");
const workspaceNodeModulesRoot = path.join(tsonicWorkspaceRoot, "node_modules");
...
const findAuthoritativePackageRoot = (
  ...
  const candidates = [
    path.join(siblingRepositoriesRoot, shortName, "versions", "10"),
    path.join(siblingRepositoriesRoot, shortName),
    path.join(siblingRepositoriesRoot, shortName, "packages", shortName),
  ];
