/**
 * Union-narrowing guard emission cases for if-statements.
 * Handles predicate, in-operator, property-truthiness, discriminant-equality,
 * and negated-predicate guards.
 */
...
/**
 * Try to emit a property-truthiness guard narrowing for
 * `if (result.success) { ... }` or `if (!result.success) { ... }`.
 * Returns undefined if the condition is not a matching property-truthiness guard.
 */
