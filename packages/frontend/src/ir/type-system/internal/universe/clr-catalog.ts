...
const dtsTypeNodeToIrType = (
  ...
  if (ts.isTypeReferenceNode(node)) {
    ...
    // preserve extension scopes across fluent chains. For Tsonic's IR/runtime typing,
    // it must erase to the new shape.
    if (baseName === "Rewrap" && node.typeArguments?.length === 2) {
      const result = node.typeArguments[1];
      return result
        ? dtsTypeNodeToIrType(result, inScopeTypeParams, tsNameToTypeId)
        : { kind: "unknownType" };
    }
