 ...
 * Example: "Length|:provider int32|static=false|accessor=get"
 *
 * Format for indexer properties: "Name|[IndexType]:ReturnType|static=bool|accessor=get"
 * Example: "Chars|[provider int32]:provider char|static=false|accessor=get"
 */
export const parsePropertyType = (normalizedSig: string): IrType => {
  // Try indexer format first: Chars|[provider int32]:provider char|...
  const indexerMatch = normalizedSig.match(/\|\[[^\]]*\]:([^|]+)\|/);
  if (indexerMatch && indexerMatch[1]) {
    return parseExternalTypeString(indexerMatch[1]);
  }

  // Try regular property format: Length|:provider int32|...
  const colonMatch = normalizedSig.match(/\|:([^|]+)\|/);
  if (colonMatch && colonMatch[1]) {
    return parseExternalTypeString(colonMatch[1]);
  }
 ...
 * Example: "Empty|provider string|static=true|const=false"
 */
export const parseFieldType = (normalizedSig: string): IrType => {
  const parts = normalizedSig.split("|");
  if (parts.length >= 2 && parts[1]) {
    return parseExternalTypeString(parts[1]);
  }
  return { kind: "unknownType" };
};
 ...
 * Example: "Substring|(provider int32,provider int32):provider string|static=false"
 */
export const parseMethodSignature = (
  ...
  const returnType =
    returnMatch && returnMatch[1]
      ? parseExternalTypeString(returnMatch[1])
      : { kind: "voidType" as const };
  ...
  if (paramsMatch && paramsMatch[1]) {
    ...
    for (let i = 0; i < paramTypes.length; i++) {
      ...
      parameters.push({
        ...
        type: parseExternalTypeString(paramType),
        mode,
        isOptional: false,
        isRest: false,
      });
...
export const convertRawType = (
  ...
  if (!stableId) {
    throw new Error(
      `Missing canonical provider identity for type: ${rawType.targetName}`
    );
  }
  ...
  const typeId = makeTypeId(
    ...
    tsbindgenTargetTypeNameToTsTypeName(rawType.targetName)
  );
  ...
  for (const prop of rawType.properties) {
    const propTsName = prop.targetName;
  ...
  for (const field of rawType.fields) {
    const fieldTsName = field.targetName;
    const memberEntry: MemberEntry = {
      tsName: fieldTsName,
  ...
  for (const method of rawType.methods) {
    const existing = methodsByName.get(method.targetName) ?? [];
    existing.push(method);
    methodsByName.set(method.targetName, existing);
  }
  ...
  for (const iface of rawType.interfaces ?? []) {
    heritage.push({
      ...
      typeArguments: (iface.typeArguments ?? []).map(parseExternalTypeString),
    });
  }
  ...
  return {
    ...
    members,
    origin: "external",
    sourcePrimitiveName: rawType.sourcePrimitive,
    asyncWrapper: rawType.asyncWrapper,
    iterableShape: rawType.iterableShape,
    accessibility,
