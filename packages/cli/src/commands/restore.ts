...
export const restoreCommand = (
  ...
  if (packageReferencesAll.length > 0) {
    ...
    try {
      assets = JSON.parse(
        readFileSync(assetsPathResult.value, "utf-8")
      ) as ProjectAssets;
    } catch (error) {
      return {
        ok: false,
    ...
    if (roots.length === 0) {
      ...
      for (const libKey of topo) {
        ...
        if (seedDlls.length === 0) {
          ...
          const packageName = defaultBindingsPackageNameForNuget(
            node.packageId
          );
          ...
          for (const depDir of Array.from(libDirs).sort((a, b) =>
            a.localeCompare(b)
          )) {
  ...
  if (dllLibraries.length > 0) {
    const dllAbs = dllLibraries.map((p) =>
      resolveFromProjectRoot(workspaceRoot, p)
    );
