...
import { resolveRestoreParallelism } from "./parallel.js";
...
type NugetBindingWorkItem = {
  readonly libKey: string;
  readonly dependencies: ReadonlySet<string>;
  readonly run: () => Promise<Result<void, string>>;
};
...
const runNugetBindingWorkItems = async (
  workItems: readonly NugetBindingWorkItem[]
): Promise<Result<void, string>> => {
  const completed = new Set<string>();
  const started = new Set<string>();

  while (completed.size < workItems.length) {
    const ready = workItems.filter(
      (item) =>
        !started.has(item.libKey) &&
        Array.from(item.dependencies).every((dep) => completed.has(dep))
    );

    if (ready.length === 0) {
      const pending = workItems
        .filter((item) => !completed.has(item.libKey))
        .map((item) => item.libKey)
        .join("\n");
      return {
        ok: false,
        error: `Cycle detected in NuGet bindings work graph:\n${pending}`,
      };
    }

    for (const item of ready) started.add(item.libKey);

    const queue = [...ready];
    const failures: string[] = [];
    const parallelism = resolveRestoreParallelism(queue.length);
...
export const generateNugetBindingsAsync = async ({
  workspaceRoot,
  dotnetLib,
  runtimes,
  tsbindgenDll,
  options,
  plan,
}: GenerateNugetBindingsOptions): Promise<Result<void, string>> => {
  const bindingsDirByLibKey = new Map(plan.bindingsDirByLibKey);
  const typesLibDirByPkgId = new Map<string, string>();
  ...
  const workItems: Array<Omit<NugetBindingWorkItem, "dependencies">> = [];
  ...
  const taskKeys = new Set(workItems.map((item) => item.libKey));
  return runNugetBindingWorkItems(
    workItems.map((item) => ({
      ...item,
      dependencies: new Set(
        Array.from(plan.transitiveDeps.get(item.libKey) ?? []).filter(
          (depKey) => taskKeys.has(depKey)
        )
      ),
    }))
  );
