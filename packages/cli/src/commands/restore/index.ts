...
import {
  ...
  generateFrameworkBindingsAsync,
} from "./framework-bindings.js";
import {
  restoreNugetBindings,
  restoreNugetBindingsAsync,
} from "./nuget-bindings.js";
import {
...
export const restoreCommandAsync = async (
  configPath: string,
  options: RestoreOptions = {}
  ...
  const frameworkBindingsPromise = generateFrameworkBindingsAsync({
    frameworkReferences: frameworkReferencesResult.value,
    runtimes: runtimesResult.value,
    workspaceRoot,
    dotnetLib: dotnetRoot.value,
  ...
  const nugetBindingsResult = await restoreNugetBindingsAsync({
    workspaceRoot,
    targetFramework: config.dotnetVersion,
    nugetConfigFile: nugetConfigResult.value,
    packageReferencesAll: packageReferencesResult.value,
  ...
  const frameworkBindingsResult = await frameworkBindingsPromise;
  if (!frameworkBindingsResult.ok) return frameworkBindingsResult;
  if (!nugetBindingsResult.ok) return nugetBindingsResult;
