...
import {
  generateNugetBindings,
  generateNugetBindingsAsync,
} from "./nuget-generation.js";
...
export const restoreNugetBindingsAsync = async ({
  workspaceRoot,
  targetFramework,
  nugetConfigFile,
  packageReferencesAll,
  ...
  tsbindgenDll,
  options,
}: RestoreNugetBindingsOptions): Promise<Result<void, string>> => {
  if (packageReferencesAll.length === 0) {
    return { ok: true, value: undefined };
  }
  ...
  return generateNugetBindingsAsync({
    workspaceRoot,
    dotnetLib,
    runtimes,
    tsbindgenDll,
