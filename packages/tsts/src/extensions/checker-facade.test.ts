...
type SemanticQuerySnapshot = {
  ...
  readonly declaredValueUnionMembers: readonly string[];
  readonly maybeNonNullishMembers: readonly string[];
  readonly valueSymbolName: string;
  readonly valueDeclarationCount: number;
  readonly valueDeclarationTypeText: string;
  ...
  readonly identityReturnType: string;
  readonly userAliasSymbolName: string | undefined;
  readonly userTypeSymbolName: string | undefined;
  readonly userNamePropertyType: string;
  readonly stringIndexType: string;
  readonly numberIndexType: string;
  readonly namesIsArray: boolean;
  readonly tupleIsTuple: boolean;
  readonly boxConstructSignatureCount: number;
  readonly moduleExports: readonly string[];
  readonly scopeHasIdentity: boolean;
  readonly typeNodeWasCreated: boolean;
};
...
const readSemanticSnapshot = (
  ...
  let declaredValueParameter: GoPtr<Node>;
  let declaredValueName: GoPtr<Node>;
  let maybeName: GoPtr<Node>;
  let userName: GoPtr<Node>;
  let recordName: GoPtr<Node>;
  let numericRecordName: GoPtr<Node>;
  let namesName: GoPtr<Node>;
  let tupleName: GoPtr<Node>;
  let boxName: GoPtr<Node>;
  ...
  visitSourceFile(must(context.sourceFile, "checked source file was missing"), (node) => {
    ...
    if (
      getTstsIdentifierText(node) === "numericRecord" &&
  ...
  const valueDeclaration = must(
    declaredValueName,
    "value declaration was not discovered",
  );
  const declaredValueType = context.checker.getDeclaredTypeOfSymbol(
    context.checker.getSymbolAtLocation(valueDeclaration),
  );
  const maybeType = context.checker.getTypeAtLocation(
    must(maybeName, "maybe declaration was not discovered"),
  );
  const userType = context.checker.getTypeAtLocation(
    must(userName, "user declaration was not discovered"),
  );
  const recordType = context.checker.getTypeAtLocation(
    must(recordName, "record identifier was not discovered"),
  );
  const numericRecordType = context.checker.getTypeAtLocation(
    must(numericRecordName, "numericRecord identifier was not discovered"),
  );
  const namesType = context.checker.getTypeAtLocation(
    must(namesName, "names identifier was not discovered"),
  );
  const tupleType = context.checker.getTypeAtLocation(
    must(tupleName, "tuple identifier was not discovered"),
  );
  const boxType = context.checker.getTypeAtLocation(
    must(boxName, "Box identifier was not discovered"),
  );
  ...
  return {
    ...
    valueDeclarationTypeText: context.checker.typeToString(
      context.checker.getTypeFromTypeNode(valueDeclaredTypeNode),
    ),
    ...
    identityReturnType: context.checker.typeToString(
      context.checker.getReturnTypeOfSignature(identitySignature),
    ),
    ...
    stringIndexType: context.checker.typeToString(
      context.checker.getStringIndexType(recordType),
    ),
    numberIndexType: context.checker.typeToString(
      context.checker.getNumberIndexType(numericRecordType),
    ),
    namesIsArray: context.checker.isArrayType(namesType),
    tupleIsTuple: context.checker.isTupleType(tupleType),
    boxConstructSignatureCount:
      context.checker.getConstructSignatures(boxType).length,
    moduleExports:
      context.checker
        .getExportsOfModule(sourceFileSymbol)
        .map((symbol) => context.checker.getSymbolName(symbol))
        .sort(),
    scopeHasIdentity: scopeSymbols.some(
      (symbol) => context.checker.getSymbolName(symbol) === "identity",
    ),
    typeNodeWasCreated:
      context.checker.typeToTypeNode(userType, context.sourceFile, FlagsNone) !==
      undefined,
  };
};
