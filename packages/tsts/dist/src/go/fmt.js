...
// ---------------------------------------------------------------------------
function hasStringMethod(v) {
    return v !== null && typeof v === "object" && typeof v.String === "function";
}
function hasErrorMethod(v) {
    return v !== null && typeof v === "object" && typeof v.Error === "function";
}
...
function defaultString(v, plusV, goSyntax) {
    if (v === undefined || v === null) {
    ...
    switch (typeof v) {
        ...
        case "object": {
            ...
            return formatStruct(v, plusV, goSyntax);
        }
        default:
            return globalThis.String(v);
    }
}
function formatStruct(obj, plusV, goSyntax) {
...
function renderInteger(v, base, upper, spec) {
    const n = toInteger(v);
    if (globalThis.Number.isNaN(n)) {
...
function signPrefix(negative, spec) {
    if (negative) {
        return "-";
    }
    if (spec.plus) {
...
function altIntPrefix(base, upper, spec) {
    if (!spec.hash) {
        return "";
    }
    if (base === 16) {
...
function padNumeric(prefix, digits, spec) {
...
function renderFloat(v, spec) {
    const n = typeof v === "bigint" ? globalThis.Number(v) : v;
...
function normalizeExponent(s, upper) {
    const normalized = s.replace(/e([+-])(\d+)/i, (_m, sign, exp) => {
        const padded = exp.length < 2 ? exp.padStart(2, "0") : exp;
        return `e${sign}${padded}`;
    });
    return upper ? normalized.replace("e", "E") : normalized;
}
function formatG(magnitude, precision, upper) {
...
function applyVerb(spec, arg, hasArg) {
    if (!hasArg) {
    ...
    switch (spec.verb) {
        ...
        case "w": {
            ...
            const wrapped = arg instanceof globalThis.Error ? arg : hasErrorMethod(arg) ? toGoError(arg) : undefined;
...
function renderHex(arg, upper, spec) {
    ...
    if (globalThis.Array.isArray(arg)) {
        const hex = arg.map((b) => (b & 0xff).toString(16).padStart(2, "0")).join("");
        return padString(upper ? hex.toUpperCase() : hex, spec);
    }
...
const pointerHandles = new globalThis.WeakMap();
let pointerCounter = 0xc000000000;
function pointerHandle(v) {
...
function isAsciiDigitAt(s, i) {
    const ch = s[i];
    return ch !== undefined && ch >= "0" && ch <= "9";
}
function formatCore(format, args) {
    const out = [];
    let argIndex = 0;
    ...
    while (i < format.length) {
        ...
        const verb = format[i] ?? "";
        i++;
        const spec = { minus, plus, space, hash, zero, width, precision, verb };
...
// appended.
export function Sprintln(...args) {
    return joinOperands(args, true) + "\n";
}
function joinOperands(args, alwaysSpace) {
    const out = [];
...
export function Fprintf(w, format, ...args) {
...
export function Fprintln(w, ...args) {
    return writeToWriter(w, joinOperands(args, true) + "\n");
}
function writeToWriter(w, text) {
    const bytes = utf8Bytes(text);
    const [, err] = w.Write(bytes);
    return [bytes.length, err];
}
export function Sscanf(str, format, ...targets) {
...
function syntaxScanError(str, pos) {
