...
export function NewBuffer(bytes) {
    return new BytesBuffer(bytes);
}
export function NewReader(bytes) {
    return toBytes(bytes);
}
export function ReplaceAll(source, oldValue, newValue) {
...
const indexOf = (source, search) => {
    if (search.length === 0) {
        return 0;
    }
    outer: for (let index = 0; index <= source.length - search.length; index++) {
