...
import { Index, IndexFunc, Contains, ContainsFunc, Equal, EqualFunc, Compare, CompareFunc, Clone, Concat, Repeat, Grow, Clip, Delete, DeleteFunc, Insert, Replace, Reverse, Compact, CompactFunc, Sort, SortFunc, SortStableFunc, Sorted, SortedFunc, IsSorted, BinarySearch, BinarySearchFunc, Values, Collect, AppendSeq, } from "./slices.js";
...
test("slices.IndexFunc / ContainsFunc", () => {
    const s = [1, 2, 3, 4];
    assert.equal(IndexFunc(s, (e) => e > 2), 2);
    assert.equal(IndexFunc(s, (e) => e > 100), -1);
    assert.equal(ContainsFunc(s, (e) => e === 3), true);
    assert.equal(ContainsFunc(s, (e) => e === 99), false);
});
test("slices.Equal / EqualFunc", () => {
    assert.equal(Equal([1, 2, 3], [1, 2, 3]), true);
    assert.equal(Equal([1, 2], [1, 2, 3]), false);
    assert.equal(Equal([], []), true);
    assert.equal(EqualFunc([1, 2], ["1", "2"], (a, b) => globalThis.String(a) === b), true);
});
test("slices.Compare / CompareFunc", () => {
    ...
    assert.equal(CompareFunc([1, 2], [1, 2], (a, b) => a - b), 0);
});
...
test("slices.Compact / CompactFunc collapse consecutive equals", () => {
    ...
    assert.deepEqual(CompactFunc([1, 3, 2, 4, 6], (a, b) => a % 2 === b % 2), [1, 2]);
});
...
test("slices.SortFunc / SortStableFunc use tri-state comparator", () => {
    ...
    assert.deepEqual(s.map((e) => e.k), [1, 2, 3]);
    const stable = [
        { k: 1, id: "a" },
        { k: 1, id: "b" },
        { k: 0, id: "c" },
    ...
    assert.deepEqual(stable.map((e) => e.id), ["c", "a", "b"]);
});
...
test("slices.Sorted / SortedFunc collect-and-sort from a seq", () => {
    ...
    assert.deepEqual(SortedFunc(seq, (a, b) => b - a), [3, 2, 1]);
});
test("slices.BinarySearch / BinarySearchFunc", () => {
    ...
    assert.deepEqual(BinarySearchFunc(s, 7, (e, t) => e - t), [3, true]);
});
...
//# sourceMappingURL=slices.test.js.map