...
import { Sprintf, Sprint, Sprintln, Errorf, Fprint, Fprintf, Fprintln, Println, } from "./fmt.js";
...
test("fmt.Sprintf calls Stringer for %v and %s", () => {
    const stringer = { String: () => "I am stringer" };
...
test("fmt.Errorf returns an error with the formatted message", () => {
    ...
    assert.equal(err.message, "failed: io (7)");
});
test("fmt.Errorf %w wraps an error so errors.Is sees it", () => {
    ...
    assert.equal(wrapped.message, "context: root");
...
//# sourceMappingURL=fmt.test.js.map