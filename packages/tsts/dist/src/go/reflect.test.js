...
import { TypeOf, ValueOf, DeepEqual, Bool, Float64, Int64, Interface, Struct, String as StringKind, Slice, Map as MapKind, Invalid, Int, NewType, TypeFor, TypeAssert, MakeSlice, Append, Zero, VisibleFields, } from "./reflect.js";
...
test("reflect.DeepEqual arrays/objects/maps", () => {
    ...
    const m1 = new globalThis.Map([["a", 1]]);
    const m2 = new globalThis.Map([["a", 1]]);
    const m3 = new globalThis.Map([["a", 2]]);
...
test("reflect static-type helpers use explicit metadata without runtime guessing", () => {
    ...
    assert.equal(TypeFor().Kind(), Interface);
    assert.deepEqual(TypeAssert(ValueOf("x")), ["x", true]);
