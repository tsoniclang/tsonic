...
const surrogate = /[\uD800-\uDFFF]/;
const encodeScalar = (bytes, codePoint) => {
    if (codePoint < 0x80) {
...
const asciiStringByteView = { ascii: true };
const stringByteViewCache = new globalThis.Map();
const stringByteViewCacheBudget = 64 * 1024 * 1024;
const stringByteViewCacheState = { bytes: 0 };
export function GetStringByteView(s) {
    ...
    const view = ascii ? asciiStringByteView : { ascii, bytes: encode(s), hasSurrogate };
    if (s.length >= 4096) {
...
export function StringByteLen(s) {
    const view = GetStringByteView(s);
    return StringByteViewLen(s, view);
}
export function StringByteViewLen(s, view) {
...
export function StringByteViewAt(s, view, i) {
    return view.ascii ? s.charCodeAt(i) : view.bytes[i];
}
export function StringByteSlice(s, start, end) {
    const view = GetStringByteView(s);
    return StringByteViewSlice(s, view, start, end);
}
export function StringByteViewSlice(s, view, start, end) {
    if (view.ascii) {
...
export function StringByteViewHasPrefix(s, view, start, prefix) {
...
export function StringByteViewIndexByte(s, view, start, b) {
    if (view.ascii) {
        return s.indexOf(globalThis.String.fromCharCode(b), start);
    }
...
export function StringByteViewIndex(s, view, start, needle) {
...
export function StringByteViewUTF16Len(s, view, start, end) {
    if (view.ascii) {
        return (end - start);
    }
    let length = 0;
...
export function StringUtf8Bytes(s) {
    const view = GetStringByteView(s);
    if (!view.ascii) {
        return view.bytes;
    }
...
const decodeRuneBytes = (bytes, i) => {
    ...
    if ((b0 & 0xf8) === 0xf0) {
        ...
        const r = ((b0 & 0x07) << 18) | ((b1 & 0x3f) << 12) | ((b2 & 0x3f) << 6) | (b3 & 0x3f);
...
export function DecodeRuneInBytesAt(bytes, i) {
    return decodeRuneBytes(bytes, i);
}
export function DecodeLastRuneInBytesBefore(bytes, end) {
    if (end <= 0) {
        return [RuneError, 0];
    }
    let start = end - 1;
...
export function DecodeRuneInStringAt(s, i) {
    const view = GetStringByteView(s);
    return DecodeRuneInStringViewAt(s, view, i);
}
export function DecodeRuneInStringViewAt(s, view, i) {
    if (view.ascii) {
...
export function DecodeLastRuneInStringBefore(s, end) {
    const view = GetStringByteView(s);
    return DecodeLastRuneInStringViewBefore(s, view, end);
}
export function DecodeLastRuneInStringViewBefore(s, view, end) {
    if (view.ascii) {
