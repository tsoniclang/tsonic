...
const StdPaddingValue = 0x3d; // '='
const NoPaddingValue = -1;
...
export class Encoding {
    ...
    constructor(encoder) {
        ...
        const alphabet = new globalThis.TextEncoder().encode(encoder);
    ...
    Encode(dst, src) {
        ...
        let di = 0;
        let si = 0;
        const n = globalThis.Math.trunc(src.length / 3) * 3;
        while (si < n) {
            // Convert 3x 8bit source bytes into 4 bytes.
            const val = ((src[si + 0] << 16) | (src[si + 1] << 8) | src[si + 2]) >>> 0;
    ...
    EncodeToString(src) {
        const buf = new globalThis.Array(this.EncodedLen(src.length)).fill(0);
        this.Encode(buf, src);
    ...
    DecodeString(s) {
        const src = globalThis.Array.from(new globalThis.TextEncoder().encode(s));
        const dbuf = new globalThis.Array(this.DecodedLen(src.length)).fill(0);
    ...
    decodeQuantum(dst, dstOff, src, si) {
        const dbuf = new globalThis.Uint8Array(4);
        let dlen = 4;
        let err = undefined;
        for (let j = 0; j < 4; j++) {
            if (src.length === si) {
            ...
            const inByte = src[si];
            si++;
            const out = this.decodeMap[inByte];
            if (out !== 0xff) {
                dbuf[j] = out;
                continue;
            }
        ...
        const val = (((dbuf[0] << 18) | (dbuf[1] << 12) | (dbuf[2] << 6) | dbuf[3]) >>> 0);
        const b2 = (val >>> 0) & 0xff;
        const b1 = (val >>> 8) & 0xff;
...
export const StdEncoding = new Encoding("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/");
...
class encoder {
    ...
    enc;
    w;
    buf = new globalThis.Uint8Array(3);
    nbuf = 0;
    out = new globalThis.Uint8Array(1024);
    constructor(enc, w) {
        this.enc = enc;
        this.w = w;
    }
    Write(p) {
        ...
        if (this.nbuf > 0) {
            ...
            const outChunk = globalThis.Array.from(this.out);
        ...
        while (p.length >= 3) {
            let nn = globalThis.Math.trunc(this.out.length / 4) * 3;
            if (nn > p.length) {
                nn = p.length;
                nn -= nn % 3;
            }
...
export function NewEncoder(enc, w) {
