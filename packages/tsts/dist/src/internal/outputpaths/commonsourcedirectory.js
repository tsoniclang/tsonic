import { EnsureTrailingDirectorySeparator, GetCanonicalFileName, GetDirectoryPath, GetNormalizedPathComponents, GetPathFromPathComponents, } from "../tspath/path.js";
/**
...
const computeNextComponents = (commonPathComponents, sourceFile, currentDirectory, useCaseSensitiveFileNames) => {
    ...
    const foundIdx = globalThis.Array.from({ length: n }, (_, i) => i).findIndex((i) => GetCanonicalFileName(currentCommonPathComponents[i], useCaseSensitiveFileNames) !==
...
export function computeCommonSourceDirectoryOfFilenames(fileNames, currentDirectory, useCaseSensitiveFileNames) {
    const commonPathComponents = fileNames.reduce((acc, sourceFile) => computeNextComponents(acc, sourceFile, currentDirectory, useCaseSensitiveFileNames), undefined);
    if (commonPathComponents === null) {
        // Failed to find any common path component
        return "";
    }
 ...
 * }
 */
export function GetComputedCommonSourceDirectory(emittedFiles, currentDirectory, useCaseSensitiveFileNames) {
 ...
 * }
 */
export function GetCommonSourceDirectory(options, files, currentDirectory, useCaseSensitiveFileNames) {
    const rawCommonSourceDirectory = options.RootDir !== ""
        ? options.RootDir
...
//# sourceMappingURL=commonsourcedirectory.js.map