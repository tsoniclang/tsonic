 ...
 * }
 */
export function Pattern_IsValid(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Pattern_Matches(receiver, candidate) {
    const p = receiver;
    if (p.StarIndex === -1) {
        return p.Text === candidate;
    }
    return (byteLen(candidate) >= byteLen(p.Text) - 1 &&
 ...
 * }
 */
export function Pattern_MatchedText(receiver, candidate) {
    const p = receiver;
