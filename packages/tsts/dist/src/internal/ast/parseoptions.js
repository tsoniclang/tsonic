import { CompilerOptions_GetEmitModuleDetectionKind, CompilerOptions_GetEmitModuleKind, JsxEmitReactJSX, JsxEmitReactJSXDev, ModuleDetectionKindAuto, ModuleDetectionKindForce, ModuleDetectionKindLegacy, ModuleKindESNext, } from "../core/compileroptions.js";
import { ExtensionCjs, ExtensionCts, ExtensionMjs, ExtensionMts, FileExtensionIsOneOf, IsDeclarationFileName, } from "../tspath/extension.js";
import { AsImportEqualsDeclaration } from "./generated/casts.js";
import { IsExportAssignment, IsExportDeclaration, IsExternalModuleReference, IsImportDeclaration, IsImportEqualsDeclaration, IsJsxFragment, IsJsxOpeningElement, } from "./generated/predicates.js";
...
import { GetImpliedNodeFormatForEmitWorker, HasSyntacticModifier, IsImportMeta, } from "./utilities.js";
 ...
 * }
 */
export function GetExternalModuleIndicatorOptions(fileName, options, metadata) {
 ...
 */
export const isFileForcedToBeModuleByFormatExtensions = [ExtensionCjs, ExtensionCts, ExtensionMjs, ExtensionMts];
/**
 ...
 * }
 */
export function isFileForcedToBeModuleByFormat(fileName, options, metadata) {
    // Excludes declaration files - they still require an explicit `export {}` or the like
    // for back compat purposes. The only non-declaration files _not_ forced to be a module are `.js` files
    // that aren't esm-mode (meaning not in a `type: module` scope).
 ...
 * }
 */
export function SetExternalModuleIndicator(file, opts) {
    file.ExternalModuleIndicator = getExternalModuleIndicator(file, opts);
}
/**
 ...
 * }
 */
export function getExternalModuleIndicator(file, opts) {
 ...
 * }
 */
export function isAnExternalModuleIndicatorNode(node) {
    return HasSyntacticModifier(node, ModifierFlagsExport) ||
 ...
 * }
 */
export function getImportMetaIfNecessary(sourceFile) {
 ...
 * }
 */
export function findChildNode(root, check) {
 ...
 * }
 */
export function isFileModuleFromUsingJSXTag(file) {
 ...
 * }
 */
export function walkTreeForJSXTags(node) {
    const container = { found: undefined };
    const visitor = (n) => {
