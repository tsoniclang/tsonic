...
function parseTypeScript(text, jsx) {
    ...
    const file = ParseSourceFile({ FileName: fileName, Path: fileName }, text, GetScriptKindFromFileName(fileName));
    return file;
}
...
for (const rec of data) {
    test(`Clone ${rec.title}`, () => {
        ...
        let work = [{ original: file, copy: clone }];
        while (work.length > 0) {
            const nextWork = [];
...
//# sourceMappingURL=deepclone.test.js.map