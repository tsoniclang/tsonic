...
const utilUtf8Encoder = new globalThis.TextEncoder();
const utilUtf8Decoder = new globalThis.TextDecoder("utf-8");
const utilUtf8ByteInfoCache = new globalThis.Map();
const utilUtf8ByteInfoCacheBudget = 64 * 1024 * 1024; // total cached bytes
const utilUtf8ByteInfoCacheState = { bytes: 0 };
const utilGetUtf8ByteInfo = (s) => {
    ...
    const info = {
        ascii,
        bytes: ascii ? undefined : utilUtf8Encoder.encode(s),
    };
    if (s.length >= 4096) {
...
export function findImportOrRequire(text, start) {
    ...
    const n = utilByteLen(text);
    let index = globalThis.Math.max(start, 0);
    while (index < n) {
        const next = strings.IndexAny(utilByteSlice(text, index), "ir");
        if (next < 0) {
            return [-1, 0];
        }
        const newIndex = (index + next);
 ...
 * }
 */
export function IsExpandoInitializer(initializer) {
    ...
    if (IsInJSFile(initializer)) {
        return (IsClassExpression(initializer) || (IsObjectLiteralExpression(initializer) && (Node_Properties(initializer)?.length ?? 0) === 0));
    }
    return false;
}
/**
 ...
 * }
 */
export function IsImplicitlyExportedJSTypeAlias(node) {
    return (IsJSTypeAliasDeclaration(node) && IsSourceFile(node.Parent) && IsExternalOrCommonJSModule(AsSourceFile(node.Parent)));
}
/**
