 ...
 * }
 */
export function getDeepCloneVisitor(f, syntheticLocation) {
    let visitor;
    const syntheticNodeLocation = () => NewTextRange(-1, -1);
    const trailingCommaLocation = () => NewTextRange(-2, -2);
    visitor = NewNodeVisitor((node) => {
        ...
        VisitModifiers: (nodes, nodeVisitor) => {
            ...
            const newList = visited !== nodes ? visited : ModifierList_Clone(nodes, nodeVisitor.Factory);
            if (syntheticLocation) {
                newList.Loc = syntheticNodeLocation();
                if (NodeList_HasTrailingComma(nodes)) {
                    newList.Nodes[newList.Nodes.length - 1].Loc = trailingCommaLocation();
 ...
 * }
 */
export function NodeFactory_DeepCloneNode(receiver, node) {
    return NodeVisitor_VisitNode(getDeepCloneVisitor(receiver, true), node);
}
/**
 ...
 * }
 */
export function NodeFactory_DeepCloneReparse(receiver, node) {
    if (node !== undefined) {
        const reparsed = NodeVisitor_VisitNode(getDeepCloneVisitor(receiver, false), node);
 ...
 * }
 */
export function NodeFactory_DeepCloneReparseModifiers(receiver, modifiers) {
    return NodeVisitor_VisitModifiers(getDeepCloneVisitor(receiver, false), modifiers);
}
