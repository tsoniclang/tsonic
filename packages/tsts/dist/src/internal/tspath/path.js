 ...
 * )
 */
export const DirectorySeparator = 0x2f; // '/'
export const urlSchemeSeparator = "://";
/**
 ...
 * }
 */
export function isAnyDirectorySeparator(char) {
 ...
 * }
 */
export function CombinePaths(firstPath, ...paths) {
    // TODO (drosen): There is potential for a fast path here.
    // In the case where we find the last absolute path and just path.Join from there.
    firstPath = NormalizeSlashes(firstPath);
 ...
 * }
 */
export function pathComponents(path, rootLength) {
 ...
 * }
 */
export function IsVolumeCharacter(char) {
 ...
 * }
 */
export function getFileUrlVolumeSeparatorEnd(url, start) {
    if (url.length <= start) {
        return -1;
    }
 ...
 * }
 */
export function GetEncodedRootLength(path) {
 ...
 * }
 */
export function Path_GetDirectoryPath(receiver) {
 ...
 * }
 */
export function ResolveTripleslashReference(moduleName, containingFile) {
 ...
 * }
 */
export function getNormalizedPathComponentsFromCombined(path) {
    const rootLength = GetRootLength(path);
    // Always include the root component (empty string for relative paths).
    const components = [path.slice(0, rootLength)];
 ...
 * }
 */
export function simpleNormalizePath(path) {
    // Most paths don't require normalization
    if (!hasRelativePathSegment(path)) {
        return [path, true];
    }
 ...
 * }
 */
export function hasRelativePathSegment(p) {
    const n = p.length;
    if (n === 0) {
        return false;
    }
 ...
 * }
 */
export function GetCanonicalFileName(fileName, useCaseSensitiveFileNames) {
    if (useCaseSensitiveFileNames) {
        return fileName;
    }
    return ToFileNameLowerCase(fileName);
 ...
 * }
 */
export function ToFileNameLowerCase(fileName) {
    const IWithDot = 0x0130;
    let ascii = true;
    ...
    if (ascii) {
        ...
        const b = new globalThis.Array(fileNameLen);
        for (let i = 0; i < fileNameLen; i++) {
 ...
 * }
 */
export function ToPath(fileName, basePath, useCaseSensitiveFileNames) {
 ...
 * }
 */
export function Path_RemoveTrailingDirectorySeparator(receiver) {
 ...
 * }
 */
export function RemoveTrailingDirectorySeparators(path) {
 ...
 * }
 */
export function Path_EnsureTrailingDirectorySeparator(receiver) {
 ...
 * }
 */
export function GetPathComponentsRelativeTo(from_, to, options) {
    ...
    const result = new globalThis.Array(1 + numDotDotSlashes + toComponents.length - start);
    result[0] = "";
    let i = 1;
    // Add all the relative components until we hit a common directory.
 ...
 * }
 */
export function GetRelativePathFromDirectory(fromDirectory, to, options) {
    if (GetRootLength(fromDirectory) > 0 !== GetRootLength(to) > 0) {
 ...
 * }
 */
export function GetRelativePathFromFile(from_, to, options) {
 ...
 * }
 */
export function ConvertToRelativePath(absoluteOrRelativePath, options) {
    if (!IsRootedDiskPath(absoluteOrRelativePath)) {
        return absoluteOrRelativePath;
    }
    return GetRelativePathToDirectoryOrUrl(options.CurrentDirectory, absoluteOrRelativePath, false /*isAbsolutePathAnUrl*/, options);
 ...
 * }
 */
export function GetRelativePathToDirectoryOrUrl(directoryPathOrUrl, relativeOrAbsolutePath, isAbsolutePathAnUrl, options) {
 ...
 * }
 */
export function getAnyExtensionFromPathWorker(path, extensions, stringEqualityComparer) {
 ...
 * }
 */
export function tryGetExtensionFromPath(path, extension, stringEqualityComparer) {
    if (!strings.HasPrefix(extension, ".")) {
        extension = "." + extension;
    }
 ...
 * }
 */
export function ComparePathsOptions_GetComparer(receiver) {
 ...
 * }
 */
export function ComparePathsOptions_getEqualityComparer(receiver) {
 ...
 * }
 */
export function ComparePaths(a, b, options) {
    a = CombinePaths(options.CurrentDirectory, a);
    b = CombinePaths(options.CurrentDirectory, b);
    if (a === b) {
        return 0;
 ...
 * }
 */
export function ComparePathsCaseSensitive(a, b, currentDirectory) {
 ...
 * }
 */
export function ComparePathsCaseInsensitive(a, b, currentDirectory) {
 ...
 * }
 */
export function ContainsPath(parent, child, options) {
    parent = CombinePaths(options.CurrentDirectory, parent);
    child = CombinePaths(options.CurrentDirectory, child);
    if (parent === "" || child === "") {
        return false;
 ...
 * }
 */
export function Path_ContainsPath(receiver, child) {
 ...
 * }
 */
export function ForEachAncestorDirectoryStoppingAtGlobalCache(globalCacheLocation, directory, callback) {
    const [result] = ForEachAncestorDirectory(directory, (ancestorDirectory) => {
 ...
 * }
 */
export function ForEachAncestorDirectoryPath(directory, callback) {
 ...
 * }
 */
export function GetCommonParents(paths, minComponents, getPathComponents, options) {
    if (minComponents < 1) {
    ...
    if (paths.length === 1) {
        if (reducePathComponents(getPathComponents(paths[0], options.CurrentDirectory)).length < minComponents) {
            const ignoredSingle = new globalThis.Map();
            ignoredSingle.set(paths[0], {});
            return [[], ignoredSingle];
        }
        return [paths, new globalThis.Map()];
    }
    const ignored = new globalThis.Map();
    ...
    const resultPaths = new globalThis.Array(results.length);
 ...
 * }
 */
export function getCommonParentsWorker(componentGroups, minComponents, options) {
            ...
            for (let j = 0; j < rest.length; j++) {
                ...
                if (!equality(candidate, comps[lastCommonIndex])) {
                    ...
                    if (lastCommonIndex < minComponents) {
                        // Not enough components, we need to fan out
                        const orderedGroups = [];
                        const newGroups = new globalThis.Map();
 ...
 * }
 */
export function StartsWithDirectory(fileName, directoryName, useCaseSensitiveFileNames) {
    if (directoryName === "") {
        return false;
    }
 ...
 * }
 */
export function CompareNumberOfDirectorySeparators(path1, path2) {
    return cmp.Compare(strings.Count(path1, "/"), strings.Count(path2, "/"));
}
