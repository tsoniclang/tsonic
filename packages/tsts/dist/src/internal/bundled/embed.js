 ...
 */
export let __eb693fb3_0 = wrappedFS_as_vfs_FS(undefined);
export function wrappedFS_as_vfs_FS(receiver) {
    return {
        UseCaseSensitiveFileNames: () => wrappedFS_UseCaseSensitiveFileNames(receiver),
        FileExists: (path) => wrappedFS_FileExists(receiver, path),
        ReadFile: (path) => wrappedFS_ReadFile(receiver, path),
        WriteFile: (path, data) => wrappedFS_WriteFile(receiver, path, data),
        AppendFile: (path, data) => wrappedFS_AppendFile(receiver, path, data),
        Remove: (path) => wrappedFS_Remove(receiver, path),
        Chtimes: (path, aTime, mTime) => wrappedFS_Chtimes(receiver, path, aTime, mTime),
        DirectoryExists: (path) => wrappedFS_DirectoryExists(receiver, path),
        GetAccessibleEntries: (path) => wrappedFS_GetAccessibleEntries(receiver, path),
        Stat: (path) => wrappedFS_Stat(receiver, path),
        WalkDir: (root, walkFn) => wrappedFS_WalkDir(receiver, root, walkFn),
        Realpath: (path) => wrappedFS_Realpath(receiver, path),
    };
}
/**
 ...
 * }
 */
export function wrappedFS_UseCaseSensitiveFileNames(receiver) {
 ...
 * }
 */
export function wrappedFS_FileExists(receiver, path) {
    const [rest, ok] = splitPath(path);
    if (ok) {
        return embeddedContents.has(rest);
    }
 ...
 * }
 */
export function wrappedFS_ReadFile(receiver, path) {
    const [rest, ok] = splitPath(path);
    if (ok) {
 ...
 * }
 */
export function wrappedFS_DirectoryExists(receiver, path) {
    const [rest, ok] = splitPath(path);
    if (ok) {
        return rest === "libs";
    }
 ...
 * }
 */
export function wrappedFS_GetAccessibleEntries(receiver, path) {
    const [rest, ok] = splitPath(path);
    if (ok) {
        const result = { Files: [], Directories: [], Symlinks: undefined };
        if (rest === "") {
 ...
 * }
 */
export let rootEntries = [
    fileInfo_as_io_fs_DirEntry({ name: "libs", mode: ModeDir, size: 0 }),
];
export let libsEntries = LibNames.map((name) => fileInfo_as_io_fs_DirEntry({
    name,
    mode: 0,
    size: Buffer.byteLength(embeddedContents.get(`libs/${name}`) ?? "", "utf8"),
}));
/**
 ...
 * }
 */
export function wrappedFS_Stat(receiver, path) {
    const [rest, ok] = splitPath(path);
    if (ok) {
        if (rest === "" || rest === "libs") {
            return fileInfo_as_io_fs_FileInfo({ name: rest, mode: ModeDir, size: 0 });
        }
        const lib = embeddedContents.get(rest);
        if (lib !== undefined) {
            const [libName] = strings.CutPrefix(rest, "libs/");
            return fileInfo_as_io_fs_FileInfo({ name: libName, mode: 0, size: Buffer.byteLength(lib, "utf8") });
        }
        return undefined;
    }
 ...
 * }
 */
export function wrappedFS_WalkDir(receiver, root, walkFn) {
 ...
 * }
 */
export function wrappedFS_walkDir(receiver, rest, walkFn) {
    let entries;
    switch (rest) {
        case "":
            entries = rootEntries;
 ...
 * }
 */
export function wrappedFS_Realpath(receiver, path) {
    const [, ok] = splitPath(path);
    if (ok) {
        return path;
    }
 ...
 * }
 */
export function wrappedFS_WriteFile(receiver, path, data) {
    const [, ok] = splitPath(path);
    if (ok) {
 ...
 * }
 */
export function wrappedFS_AppendFile(receiver, path, data) {
    const [, ok] = splitPath(path);
    if (ok) {
        throw new globalThis.Error("cannot write to embedded file system");
    }
 ...
 * }
 */
export function wrappedFS_Remove(receiver, path) {
    const [, ok] = splitPath(path);
    if (ok) {
 ...
 * }
 */
export function wrappedFS_Chtimes(receiver, path, aTime, mTime) {
    const [, ok] = splitPath(path);
    if (ok) {
 ...
 * )
 */
export let ____85348954_0 = fileInfo_as_io_fs_FileInfo(undefined);
export let ____85348954_1 = fileInfo_as_io_fs_DirEntry(undefined);
export function fileInfo_as_io_fs_FileInfo(receiver) {
    return {
        Name: () => fileInfo_Name(receiver),
        Size: () => fileInfo_Size(receiver),
        Mode: () => fileInfo_Mode(receiver),
        ModTime: () => fileInfo_ModTime(receiver),
        IsDir: () => fileInfo_IsDir(receiver),
        Sys: () => fileInfo_Sys(receiver),
    };
}
export function fileInfo_as_io_fs_DirEntry(receiver) {
    return {
        Name: () => fileInfo_Name(receiver),
        IsDir: () => fileInfo_IsDir(receiver),
        Type: () => fileInfo_Type(receiver),
        Info: () => fileInfo_Info(receiver),
    };
}
/**
 ...
 * }
 */
export function fileInfo_IsDir(receiver) {
    return ((receiver.mode & ModeDir) !== 0);
}
/**
 ...
 * }
 */
export function fileInfo_ModTime(_receiver) {
    return new Time();
}
/**
 ...
 * }
 */
export function fileInfo_Mode(receiver) {
 ...
 * }
 */
export function fileInfo_Size(receiver) {
 ...
 * }
 */
export function fileInfo_Sys(_receiver) {
    return undefined;
}
/**
 ...
 * }
 */
export function fileInfo_Info(receiver) {
 ...
 * }
 */
export function fileInfo_Type(receiver) {
    return (receiver.mode & (ModeDir | ModeSymlink | ModeIrregular));
}
