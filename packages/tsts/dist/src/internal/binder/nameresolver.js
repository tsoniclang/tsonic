...
import { IsConstAssertion, IsModuleOrEnumDeclaration, IsGlobalSourceFile, IsFunctionLike, IsBindingPattern, IsParameterLike, IsPartOfParameterDeclaration, IsClassLike, IsClassElement, IsExternalOrCommonJSModule, IsGlobalScopeAugmentation, GetDeclarationOfKind, IsStatic, FindConstructorDeclaration, IsNullishCoalesce, IsOptionalChain, IsTypeNode, IsFunctionLikeDeclaration, HasStaticModifier, GetImmediatelyInvokedFunctionExpression, HasSyntacticModifier, IsInJSFile, IsRequireCall, GetSourceFileOfNode, FindAncestor, } from "../ast/utilities.js";
 ...
 * }
 */
export function NameResolver_Resolve(receiver, location, name, meaning, nameNotFoundMessage, isUse, excludeGlobals) {
    let result = undefined;
    let lastLocation = undefined;
    let lastSelfReferenceLocation = undefined;
    let propertyWithInvalidInitializer = undefined;
    let associatedDeclarationForContainingInitializerOrBindingName = undefined;
            ...
            case KindEnumDeclaration: {
                ...
                if (result !== undefined) {
                    if (nameNotFoundMessage !== undefined && CompilerOptions_GetIsolatedModules(receiver.CompilerOptions) && (location.Flags & NodeFlagsAmbient) === 0 && GetSourceFileOfNode(location) !== GetSourceFileOfNode(result.ValueDeclaration)) {
                        ...
                        NameResolver_error(receiver, originalLocation, Cannot_access_0_from_another_file_without_qualification_when_1_is_enabled_Use_2_instead, name, isolatedModulesLikeFlagName, enumSymbol.Name + "." + name);
                    }
                    break loop;
                }
                break;
 ...
 * }
 */
export function NameResolver_useOuterVariableScopeInParameter(receiver, result, location, lastLocation) {
    if (IsParameterDeclaration(lastLocation)) {
        ...
        if (body !== undefined && result.ValueDeclaration !== undefined && Node_Pos(result.ValueDeclaration) >= Node_Pos(body) && Node_End(result.ValueDeclaration) <= Node_End(body)) {
            ...
            let declarationRequiresScopeChange = TSUnknown;
 ...
 * }
 */
export function NameResolver_requiresScopeChange(receiver, node) {
    const d = AsParameterDeclaration(node);
    return (NameResolver_requiresScopeChangeWorker(receiver, Node_Name(node)) || (d.Initializer !== undefined && NameResolver_requiresScopeChangeWorker(receiver, d.Initializer)));
}
/**
 ...
 * }
 */
export function NameResolver_requiresScopeChangeWorker(receiver, node) {
    switch (node.Kind) {
        ...
        default:
            if (IsNullishCoalesce(node) || IsOptionalChain(node)) {
                return (CompilerOptions_GetEmitScriptTarget(receiver.CompilerOptions) < ScriptTargetES2020);
            }
            if (IsBindingElement(node) && AsBindingElement(node).DotDotDotToken !== undefined && IsObjectBindingPattern(node.Parent)) {
                return (CompilerOptions_GetEmitScriptTarget(receiver.CompilerOptions) < ScriptTargetES2017);
            }
            if (IsTypeNode(node)) {
                return false;
            }
 ...
 * }
 */
export function NameResolver_error(receiver, location, message, ...args) {
 ...
 * }
 */
export function NameResolver_getSymbolOfDeclaration(receiver, node) {
 ...
 * }
 */
export function NameResolver_lookup(receiver, symbols, name, meaning) {
 ...
 * }
 */
export function NameResolver_argumentsSymbol(receiver) {
    if (receiver.ArgumentsSymbol === undefined) {
        // Default implementation synthesizes a transient symbol for `arguments`
        receiver.ArgumentsSymbol = { Name: "arguments", Flags: SymbolFlagsProperty | SymbolFlagsTransient };
    }
 ...
 * }
 */
export function GetLocalSymbolForExportDefault(symbol_) {
 ...
 * }
 */
export function isExportDefaultSymbol(symbol_) {
    const declarations = symbol_?.Declarations ?? [];
    return (declarations.length > 0 && HasSyntacticModifier(declarations[0], ModifierFlagsDefault));
}
/**
 ...
 * }
 */
export function getIsDeferredContext(location, lastLocation) {
    if (location.Kind !== KindArrowFunction && location.Kind !== KindFunctionExpression) {
        ...
        return (IsTypeQueryNode(location) ||
            (IsFunctionLikeDeclaration(location) || (location.Kind === KindPropertyDeclaration && !IsStatic(location))) &&
                (lastLocation === undefined || lastLocation !== Node_Name(location)));
    }
    ...
    return (GetImmediatelyInvokedFunctionExpression(location) === undefined);
}
/**
 ...
 * }
 */
export function isTypeParameterSymbolDeclaredInContainer(symbol_, container) {
 ...
 * }
 */
export function isSelfReferenceLocation(node, lastLocation) {
    switch (node.Kind) {
        case KindParameter:
            return (lastLocation !== undefined && lastLocation === Node_Name(node));
        case KindFunctionDeclaration:
        case KindClassDeclaration:
        case KindInterfaceDeclaration:
        case KindEnumDeclaration:
