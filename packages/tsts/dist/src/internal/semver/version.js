 ...
 */
export const versionRegexp = regexp.MustCompile(`(?i)^(0|[1-9]\\d*)(?:\\.(0|[1-9]\\d*)(?:\\.(0|[1-9]\\d*)(?:-([a-z0-9-.]+))?(?:\\+([a-z0-9-.]+))?)?)?$`);
/**
 ...
 * )
 */
export const prereleaseRegexp = regexp.MustCompile(`(?i)^(?:0|[1-9]\\d*|[a-z-][a-z0-9-]*)(?:\\.(?:0|[1-9]\\d*|[a-zA-Z-][a-zA-Z0-9-]*))*$`);
export const prereleasePartRegexp = regexp.MustCompile(`(?i)^(?:0|[1-9]\\d*|[a-z-][a-z0-9-]*)$`);
/**
 ...
 * )
 */
export const buildRegExp = regexp.MustCompile(`(?i)^[a-z0-9-]+(?:\\.[a-z0-9-]+)*$`);
export const buildPartRegExp = regexp.MustCompile(`(?i)^[a-z0-9-]+$`);
/**
 ...
 */
export const numericIdentifierRegExp = regexp.MustCompile(`^(?:0|[1-9]\\d*)$`);
/**
 ...
 * }
 */
export function Version_incrementMajor(receiver) {
    const v = receiver;
    return {
        major: v.major + 1,
        minor: 0,
 ...
 * }
 */
export function Version_incrementMinor(receiver) {
    const v = receiver;
    return {
        major: v.major,
        minor: v.minor + 1,
 ...
 * }
 */
export function Version_incrementPatch(receiver) {
    const v = receiver;
    return {
        major: v.major,
        minor: v.minor,
 ...
 * }
 */
export function Version_Compare(receiver, b) {
    const a = receiver;
    // https://semver.org/#spec-item-11
    // > Precedence is determined by the first difference when comparing each of these
    // > identifiers from left to right as follows: Major, minor, and patch versions are
    ...
    let r = cmp.Compare(a.major, b.major);
    if (r !== 0) {
        return r;
    }
    r = cmp.Compare(a.minor, b.minor);
 ...
 * }
 */
export function comparePreReleaseIdentifiers(left, right) {
    // https://semver.org/#spec-item-11
    // > When major, minor, and patch are equal, a pre-release version has lower precedence
    // > than a normal version.
    if (left.length === 0) {
 ...
 * }
 */
export function comparePreReleaseIdentifier(left, right) {
    // https://semver.org/#spec-item-11
    // > Precedence for two pre-release versions with the same major, minor, and patch version
    // > MUST be determined by comparing each dot separated identifier from left to right until
    // > a difference is found [...]
    const compareResult = strings.Compare(left, right);
    if (compareResult === 0) {
        return compareResult;
    }
    const leftIsNumeric = numericIdentifierRegExp.MatchString(left);
    const rightIsNumeric = numericIdentifierRegExp.MatchString(right);
    if (leftIsNumeric || rightIsNumeric) {
        // https://semver.org/#spec-item-11
        // > Numeric identifiers always have lower precedence than non-numeric identifiers.
        if (!rightIsNumeric) {
        ...
        if (leftErr !== undefined || rightErr !== undefined) {
            ...
            const lenCompare = cmp.Compare(leftLen, rightLen);
            if (lenCompare === 0) {
                return compareResult;
            }
            else {
 ...
 * }
 */
export function Version_String(receiver) {
    const v = receiver;
 ...
 * }
 */
export function SemverParseError_Error(receiver) {
    const e = receiver;
    return fmt.Sprintf("Could not parse version string from %q", e.origInput);
}
/**
 ...
 * }
 */
export function TryParseVersion(text) {
    const result = {
        major: 0,
        minor: 0,
        patch: 0,
    ...
    const match = versionRegexp.FindStringSubmatch(text);
    if (match === undefined) {
 ...
 * }
 */
export function MustParse(text) {
 ...
 * }
 */
export function getUintComponent(text) {
