...
import { DeclarationMeaningGetAccessor, DeclarationMeaningSetAccessor, DeclarationMeaningPropertyAssignment, DeclarationMeaningMethod, DeclarationMeaningGetOrSetAccessor, someType, everyType, createDiagnosticForNode, } from "./checker/state.js";
 ...
 * }
 */
export function getIdentifierFromEntityNameExpression(node) {
    switch (node.Kind) {
        case KindIdentifier:
            return node;
                    ...
                    for (const j of Node_EagerJSDoc(node, file)) {
                        ...
                        for (const tag of AsJSDoc(j).Tags.Nodes) {
                            if (tag.Kind === KindJSDocAugmentsTag) {
                                ...
                                const targetName = getIdentifierFromEntityNameExpression(target.Expression);
                                const sourceName = getIdentifierFromEntityNameExpression(source.Expression);
                                if (targetName !== undefined && sourceName !== undefined && Node_Text(targetName) !== Node_Text(sourceName)) {
                                    return Checker_grammarErrorOnNode(receiver, sourceName, JSDoc_0_1_does_not_match_the_extends_2_clause, Node_Text(Node_TagName(tag)), Node_Text(sourceName), Node_Text(targetName));
                                }
                            }
                        }
                    }
            ...
            if (IsInTopLevelContext(asNode)) {
                if (!Checker_hasParseDiagnostics(receiver, sourceFile)) {
                    if (!IsEffectiveExternalModule(sourceFile, receiver.compilerOptions)) {
                        Checker_addDiagnostic(receiver, createDiagnosticForNode(forInOrOfStatement.AwaitModifier, X_for_await_loops_are_only_allowed_at_the_top_level_of_a_file_when_that_file_is_a_module_but_this_file_has_no_imports_or_exports_Consider_adding_an_empty_export_to_make_this_file_a_module));
                    }
                    ...
                    if (isNodeModuleKind2) {
                        ...
                        if (sourceFileMetaData.ImpliedNodeFormat === ModuleKindCommonJS) {
                            Checker_addDiagnostic(receiver, createDiagnosticForNode(forInOrOfStatement.AwaitModifier, The_current_file_is_a_CommonJS_module_and_cannot_use_await_at_the_top_level));
                    ...
                    if (!skipTopLevelForAwaitError) {
                        Checker_addDiagnostic(receiver, createDiagnosticForNode(forInOrOfStatement.AwaitModifier, Top_level_for_await_loops_are_only_allowed_when_the_module_option_is_set_to_es2022_esnext_system_node16_node18_node20_nodenext_or_preserve_and_the_target_option_is_set_to_es2017_or_higher));
                    }
                }
            }
            else {
