...
import { SymbolFlagsAlias, SymbolFlagsBlockScopedVariable, SymbolFlagsClass, SymbolFlagsEnum, SymbolFlagsFunction, SymbolFlagsFunctionScopedVariable, SymbolFlagsInterface, SymbolFlagsMethod, SymbolFlagsNamespace, SymbolFlagsNamespaceModule, SymbolFlagsProperty, SymbolFlagsPrototype, SymbolFlagsType, SymbolFlagsTypeAlias, SymbolFlagsValueModule, } from "../ast/generated/flags.js";
...
import { GetExtendsHeritageClauseElements, GetImplementsHeritageClauseElements, } from "../ast/utilities.js";
 ...
 * }
 */
export function NodeBuilderImpl_expandSymbolForHover(receiver, symbol_) {
    let results = [];
 ...
 * }
 */
export function NodeBuilderImpl_expandEnumDecl(receiver, symbol_) {
    const name = SymbolName(symbol_);
    receiver.ctx.approximateLength += 9 + name.length;
    const memberProps = Filter(Checker_getPropertiesOfType(receiver.ch, Checker_getTypeOfSymbol(receiver.ch, symbol_)), (p) => ((p.Flags & SymbolFlagsEnumMember) !== 0));
    let members = [];
    ...
    const constModifier = isConstEnumSymbol(symbol_) ? ModifierFlagsConst : ModifierFlagsNone;
    let mods;
    if (constModifier !== 0) {
 ...
 * }
 */
export function NodeBuilderImpl_enumMemberInitializer(receiver, p) {
    const memberDecl = Find(p.Declarations ?? [], IsEnumMember);
    ...
    if (typeof val === "string") {
        return NewStringLiteral(receiver.f, val, 0);
    }
    if (typeof val === "number") {
        return NewNumericLiteral(receiver.f, val.toString(), 0);
    }
    return undefined;
}
/**
 ...
 * }
 */
export function NodeBuilderImpl_expandClassDecl(receiver, symbol_) {
    const name = SymbolName(symbol_);
    receiver.ctx.approximateLength += 9 + name.length;
    ...
    const classType = Checker_getTypeWithThisArgument(receiver.ch, declaredType, undefined, false);
    ...
    const isClass = (staticType.symbol !== undefined && staticType.symbol.ValueDeclaration !== undefined && IsClassLike(staticType.symbol.ValueDeclaration));
    let staticBaseType;
    if (isClass) {
    ...
    instanceMembers = NodeBuilderImpl_addClassModifiers(receiver, instanceMembers, false);
    const staticProps = Filter(Checker_getPropertiesOfType(receiver.ch, staticType), (p) => ((p.Flags & SymbolFlagsPrototype) === 0 && p.Name !== "prototype" && !NodeBuilderImpl_isNamespaceMember(receiver, p)));
    let staticMembers = [];
    ...
    staticMembers = NodeBuilderImpl_addClassModifiers(receiver, staticMembers, true);
    let privateMembers = [];
    if (hasPrivate) {
 ...
 * }
 */
export function NodeBuilderImpl_addClassModifiers(receiver, members, isStatic) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (memberName !== undefined) {
            const sym = receiver.idToSymbol.get(memberName);
            if (sym !== undefined) {
                memberSymbol = sym;
            }
        }
 ...
 * }
 */
export function typeElementsToClassElements(f, members) {
 ...
 * }
 */
export function NodeBuilderImpl_expandInterfaceDecl(receiver, symbol_) {
    const name = SymbolName(symbol_);
 ...
 * }
 */
export function NodeBuilderImpl_hoverHeritageClauses(receiver, declarations) {
    let extendsTypes = [];
    let implementsTypes = [];
 ...
 * }
 */
export function NodeBuilderImpl_serializePropertiesWithTruncation(receiver, properties, elements) {
    const filtered = Filter(properties, (p) => ((p.Flags & SymbolFlagsPrototype) === 0));
    let result = [...elements];
    for (let i = 0; i < filtered.length; i++) {
        ...
        if (NodeBuilderImpl_checkTruncationLengthIfExpanding(receiver) && (i + 3 < filtered.length - 1)) {
            ...
            result = NodeBuilderImpl_addPropertyToElementList(receiver, filtered[filtered.length - 1], result);
            break;
        }
        result = NodeBuilderImpl_addPropertyToElementList(receiver, p, result);
    }
    return result;
}
/**
 ...
 * }
 */
export function NodeBuilderImpl_serializeConstructors(receiver, staticType, staticBaseType, isClass, symbol_) {
    const isNonConstructable = (!isClass &&
        symbol_.ValueDeclaration !== undefined &&
        IsInJSFile(symbol_.ValueDeclaration) &&
        Checker_getSignaturesOfType(receiver.ch, staticType, SignatureKindConstruct).length === 0);
    if (isNonConstructable) {
    ...
    if (staticBaseType !== undefined) {
        ...
        if (baseSigs.length === 0 && Every(signatures, (sig) => (sig.parameters.length === 0))) {
            return [];
        }
        if (baseSigs.length === signatures.length) {
            let allMatch = true;
            for (let i = 0; i < baseSigs.length; i++) {
                if (Checker_compareSignaturesIdentical(receiver.ch, signatures[i], baseSigs[i], false, false, true, (s, t) => Checker_compareTypesIdentical(receiver.ch, s, t)) !== TernaryTrue) {
                    allMatch = false;
                    break;
                }
            }
    ...
    else if (Every(signatures, (sig) => (sig.parameters.length === 0))) {
        return [];
    }
    let result = [];
    for (const sig of signatures) {
 ...
 * }
 */
export function NodeBuilderImpl_serializeIndexSignaturesOfType(receiver, input, baseType) {
    let result = [];
 ...
 * }
 */
export function NodeBuilderImpl_serializeNamespaceMember(receiver, resolved, name) {
    if ((resolved.Flags & SymbolFlagsTypeAlias) !== 0) {
        ...
        return NewVariableStatement(receiver.f, undefined, NewVariableDeclarationList(receiver.f, NodeFactory_NewNodeList(receiver.f, [
            NewVariableDeclaration(receiver.f, NewIdentifier(receiver.f, name), undefined, NodeBuilderImpl_serializeTypeForDeclaration(receiver, undefined, t, resolved, true), undefined),
        ]), NodeFlagsLet));
    }
}
/**
 ...
 * }
 */
export function NodeBuilderImpl_expandModuleDecl(receiver, symbol_) {
    ...
    receiver.ctx.flags = (receiver.ctx.flags | FlagsWriteTypeParametersInQualifiedName | SymbolFormatFlagsUseOnlyExternalAliasing);
    const localName = NodeBuilderImpl_symbolToNode(receiver, symbol_, SymbolFlagsAll);
    receiver.ctx.flags = oldFlags;
    let bodyStmts = [];
    const emittedLocals = NewSetWithSizeHint(0);
    for (let i = 0; i < members.length; i++) {
        const m = members[i];
        if (NodeBuilderImpl_checkTruncationLengthIfExpanding(receiver) && i + 3 < members.length - 1) {
            receiver.ctx.expansionTruncated = true;
            bodyStmts = [...bodyStmts, { node: NewExpressionStatement(receiver.f, NewIdentifier(receiver.f, `... (${members.length - i - 1} more) ...`)), isLocal: false }];
            i = members.length - 2;
            continue;
        }
            ...
            if (target !== undefined) {
                if ((target.Flags & (SymbolFlagsBlockScopedVariable | SymbolFlagsFunctionScopedVariable | SymbolFlagsProperty)) !== 0) {
                    if (Set_AddIfAbsent(emittedLocals, target)) {
                        ...
                        const localStmt = NewVariableStatement(receiver.f, undefined, NewVariableDeclarationList(receiver.f, NodeFactory_NewNodeList(receiver.f, [
                            NewVariableDeclaration(receiver.f, NewIdentifier(receiver.f, target.Name), undefined, NodeBuilderImpl_serializeTypeForDeclaration(receiver, undefined, localType, target, true), undefined),
                        ]), NodeFlagsLet));
                        bodyStmts = [...bodyStmts, { node: localStmt, isLocal: true }];
                    }
                }
                ...
                const stmt = NewExportDeclaration(receiver.f, undefined, false, NewNamedExports(receiver.f, NodeFactory_NewNodeList(receiver.f, [
                    NewExportSpecifier(receiver.f, false, propertyName, NewIdentifier(receiver.f, m.Name)),
                ])), undefined, undefined);
                bodyStmts = [...bodyStmts, { node: stmt, isLocal: false }];
                continue;
            }
        }
        ...
        if ((resolved.Flags & (SymbolFlagsFunction | SymbolFlagsMethod)) !== 0) {
            ...
            for (const sig of sigs) {
                ...
                bodyStmts = [...bodyStmts, { node: decl, isLocal: false }];
            }
            ...
            if (!hasModuleExports) {
                bodyStmts = [...bodyStmts, { node: NewModuleDeclaration(receiver.f, undefined, KindNamespaceKeyword, NewIdentifier(receiver.f, m.Name), NewModuleBlock(receiver.f, NodeFactory_NewNodeList(receiver.f, []))), isLocal: false }];
            }
            continue;
        }
        const node = NodeBuilderImpl_serializeNamespaceMember(receiver, resolved, m.Name);
        if (node !== undefined) {
            bodyStmts = [...bodyStmts, { node, isLocal: false }];
        }
    }
 ...
 * }
 */
export function NodeBuilderImpl_serializeTypeAliasForNamespace(receiver, symbol_, name) {
    ...
    receiver.ctx.flags = (receiver.ctx.flags | FlagsInTypeAlias);
 ...
 * }
 */
export function NodeBuilderImpl_filterInheritedProperties(receiver, t, baseTypes, properties) {
    if (baseTypes.length === 0) {
        return properties;
    }
    const propsByName = new globalThis.Map();
    for (const p of properties) {
        propsByName.set(p.Name, p);
    }
    const inherited = NewSetWithSizeHint(0);
    for (const base of baseTypes) {
        const baseWithThis = Checker_getTypeWithThisArgument(receiver.ch, base, Type_AsInterfaceType(Checker_getTargetType(receiver.ch, t)).thisType, false);
 ...
 * }
 */
export function NodeBuilderImpl_isNamespaceMember(receiver, p) {
    return ((p.Flags & (SymbolFlagsType | SymbolFlagsNamespace | SymbolFlagsAlias)) !== 0 ||
 ...
 * }
 */
export function isHashPrivate(s) {
    return (s.ValueDeclaration !== undefined && Node_Name(s.ValueDeclaration) !== undefined && IsPrivateIdentifier(Node_Name(s.ValueDeclaration)));
}
//# sourceMappingURL=nodebuilder_hover.js.map