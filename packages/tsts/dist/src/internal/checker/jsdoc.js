 ...
 * }
 */
export function Checker_checkUnmatchedJSDocParameters(receiver, node) {
    const jsdocParameters = [];
    ...
    const parameters = NewSetWithSizeHint(0);
    const excludedParameters = NewSetWithSizeHint(0);
    ...
    if (Checker_containsArgumentsReference(receiver, node)) {
        ...
        for (let index = 0; index < jsdocParameters.length; index++) {
            ...
            if (IsQualifiedName(name)) {
                if (isJs) {
                    Checker_error(receiver, name, Qualified_name_0_is_not_allowed_without_a_leading_param_object_1, entityNameToString(name), entityNameToString(AsQualifiedName(name).Left));
                }
            }
            else {
                if (!isNameFirst) {
                    Checker_errorOrSuggestion(receiver, isJs, name, JSDoc_param_tag_has_name_0_but_there_is_no_parameter_with_that_name, Node_Text(name));
                }
            }
        }
    }
 ...
 * }
 */
export function getAllJSDocTags(node) {
    if (node === undefined) {
        return [];
    }
