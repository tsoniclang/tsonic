 ...
 * }
 */
export function Checker_IsTypeSymbolAccessible(receiver, typeSymbol, enclosingDeclaration) {
 ...
 * }
 */
export function Checker_IsValueSymbolAccessible(receiver, symbol_, enclosingDeclaration) {
 ...
 * }
 */
export function Checker_IsSymbolAccessibleByFlags(receiver, symbol_, enclosingDeclaration, flags) {
 ...
 * }
 */
export function Checker_IsAnySymbolAccessible(receiver, symbols, enclosingDeclaration, initialSymbol, meaning, shouldComputeAliasesToMakeVisible, allowModules) {
    if (symbols.length === 0) {
        return undefined;
    }
    let hadAccessibleChain = undefined;
 ...
 * }
 */
export function hasNonGlobalAugmentationExternalModuleSymbol(declaration) {
 ...
 * }
 */
export function getQualifiedLeftMeaning(rightMeaning) {
    // If we are looking in value space, the parent meaning is value, other wise it is namespace
 ...
 * }
 */
export function Checker_getWithAlternativeContainers(receiver, container, symbol_, enclosingDeclaration, meaning) {
    const additionalContainers = container.Declarations?.flatMap((d) => {
        const s = Checker_getFileSymbolIfFileSymbolExportEqualsContainer(receiver, d, container);
        return s !== undefined ? [s] : [];
    }) ?? [];
    let reexportContainers = [];
    ...
    if (enclosingDeclaration !== undefined &&
        ...
        // This order expresses a preference for the real container if it is in scope
        let res = [container, ...additionalContainers, ...reexportContainers];
    ...
    if ((meaning === SymbolFlagsValue &&
        ...
        Checker_someSymbolTableInScope(receiver, enclosingDeclaration, (t, _tableId, _ignoreQ, _isLocal, _node) => {
            let found = false;
            for (const [, s] of t ?? []) {
    ...
    let res = [...variableMatches, ...additionalContainers, container];
    if (objectLiteralContainer !== undefined) {
        res = [...res, objectLiteralContainer];
    }
 ...
 * }
 */
export function Checker_getAlternativeContainingModules(receiver, symbol_, enclosingDeclaration) {
    if (enclosingDeclaration === undefined) {
        return [];
    }
    const containingFile = GetSourceFileOfNode(enclosingDeclaration);
    const id = GetNodeId(containingFile);
    const links = LinkStore_Get(receiver.symbolContainerLinks, symbol_);
    if (links.extendedContainersByFile === undefined) {
        links.extendedContainersByFile = new globalThis.Map();
    }
    ...
    let results = [];
    const imports = SourceFile_Imports(containingFile);
    ...
    for (const file of otherFiles) {
        ...
        const sym = Checker_getSymbolOfDeclaration(receiver, file);
 ...
 * }
 */
export function Checker_getVariableDeclarationOfObjectLiteral(receiver, symbol_, meaning) {
    // If we're trying to reference some object literal in, eg `var a = { x: 1 }`, the symbol for the literal, `__object`, is distinct
    // from the symbol of the declaration it is being assigned to. Since we can use the declaration to refer to the literal, however,
    // we'd like to make that connection here - potentially causing us to paint the declaration's visibility, and therefore the literal.
 ...
 * }
 */
export function hasExternalModuleSymbol(declaration) {
 ...
 * }
 */
export function Checker_getExternalModuleContainer(receiver, declaration) {
 ...
 * }
 */
export function Checker_getFileSymbolIfFileSymbolExportEqualsContainer(receiver, d, container) {
 ...
 * }
 */
export function Checker_getContainersOfSymbol(receiver, symbol_, enclosingDeclaration, meaning) {
    ...
    for (const d of (symbol_.Declarations ?? [])) {
        ...
        if (IsClassExpression(d) && IsBinaryExpression(d.Parent) && AsBinaryExpression(d.Parent).OperatorToken.Kind === KindEqualsToken && IsAccessExpression(AsBinaryExpression(d.Parent).Left) && IsEntityNameExpression(Node_Expression(AsBinaryExpression(d.Parent).Left))) {
            if (IsModuleExportsAccessExpression(AsBinaryExpression(d.Parent).Left) || IsExportsIdentifier(Node_Expression(AsBinaryExpression(d.Parent).Left))) {
                const sym = Checker_getSymbolOfDeclaration(receiver, GetSourceFileOfNode(d));
                if (sym !== undefined && !candidates.includes(sym)) {
                    candidates = [...candidates, sym];
                }
                continue;
            ...
            const sym = LinkStore_Get(receiver.symbolNodeLinks, Node_Expression(AsBinaryExpression(d.Parent).Left)).resolvedSymbol;
            if (sym !== undefined && !candidates.includes(sym)) {
                candidates = [...candidates, sym];
            }
            continue;
 ...
 * }
 */
export function Checker_getAliasForSymbolInContainer(receiver, container, symbol_) {
 ...
 * }
 */
export function Checker_getAccessibleSymbolChain(receiver, symbol_, enclosingDeclaration, meaning, useOnlyExternalAliasing) {
 ...
 * )
 */
export const stKindLocals = 0; // 0 << 61
export const stKindExports = 0x2000000000000000; // 1 << 61
export const stKindMembers = 0x4000000000000000; // 2 << 61
export const stKindGlobals = 0x6000000000000000; // 3 << 61
export const stKindResolvedExports = 0x8000000000000000; // 4 << 61, resolved/derived exports from getExportsOfSymbol, distinct from raw sym.Exports
// stKindMask extracts the kind bits from a symbolTableID.
export const stKindMask = 0x8000000000000000; // (5 - 1) << 61
/**
 ...
 * }
 */
export function symbolTableIDFromLocals(node) {
 ...
 * }
 */
export function symbolTableIDFromExports(sym) {
 ...
 * }
 */
export function symbolTableIDFromResolvedExports(sym) {
 ...
 * }
 */
export function symbolTableIDFromMembers(sym) {
 ...
 * }
 */
export function symbolTableIDFromGlobals() {
    return stKindGlobals;
}
/**
 ...
 * }
 */
export function Checker_getAccessibleSymbolChainEx(receiver, ctx) {
    ...
    // Go from enclosingDeclaration to the first scope we check, so the cache is keyed off the scope and thus shared more
    let firstRelevantLocation = undefined;
    Checker_someSymbolTableInScope(receiver, ctx.enclosingDeclaration, (_t, _tableId, _iq, _il, node) => {
        firstRelevantLocation = node;
        return true;
    });
    const links = LinkStore_Get(receiver.symbolContainerLinks, ctx.symbol);
    const linkKey = { useOnlyExternalAliasing: ctx.useOnlyExternalAliasing, location: firstRelevantLocation, meaning: ctx.meaning };
    if (links.accessibleChainCache === undefined) {
        links.accessibleChainCache = NewGoStructMap();
    }
    ...
    let result = [];
    Checker_someSymbolTableInScope(receiver, ctx.enclosingDeclaration, (t, tableId, ignoreQualification, isLocalNameLookup, _node) => {
 ...
 * }
 */
export function Checker_getAccessibleSymbolChainFromSymbolTable(receiver, ctx, t, tableId, ignoreQualification, isLocalNameLookup) {
    ...
    if (visitedSymbolTables === undefined) {
        visitedSymbolTables = new globalThis.Map();
 ...
 * }
 */
export function Checker_getSymbolTableAliases(receiver, symbols, tableId) {
    ...
    if (kind === stKindGlobals || kind === stKindExports || kind === stKindResolvedExports) {
        if (receiver.symbolTableAliasCache === undefined) {
            receiver.symbolTableAliasCache = new globalThis.Map();
        }
 ...
 * }
 */
export function Checker_trySymbolTable(receiver, ctx, symbols, tableId, ignoreQualification, isLocalNameLookup) {
 ...
 * }
 */
export function Checker_compareSymbolChainsWorker(receiver, a, b) {
 ...
 * }
 */
export function isUMDExportSymbol(symbol_) {
 ...
 * }
 */
export function isNamespaceReexportDeclaration(node) {
 ...
 * }
 */
export function Checker_getCandidateListForSymbol(receiver, ctx, symbolFromSymbolTable, resolvedImportedSymbol, ignoreQualification) {
 ...
 * }
 */
export function Checker_isAccessible(receiver, ctx, symbolFromSymbolTable, resolvedAliasSymbol, ignoreQualification) {
 ...
 * }
 */
export function Checker_canQualifySymbol(receiver, ctx, symbolFromSymbolTable, meaning) {
    // If the symbol is equivalent and doesn't need further qualification, this symbol is accessible
 ...
 * }
 */
export function Checker_needsQualification(receiver, symbol_, enclosingDeclaration, meaning) {
    let qualify = false;
    Checker_someSymbolTableInScope(receiver, enclosingDeclaration, (symbolTable, _tableId, _iq, _il, _node) => {
        // If symbol of this name is not available in the symbol table we are ok
 ...
 * }
 */
export function isPropertyOrMethodDeclarationSymbol(symbol_) {
    if (symbol_.Declarations !== undefined && symbol_.Declarations.length > 0) {
 ...
 * }
 */
export function Checker_someSymbolTableInScope(receiver, enclosingDeclaration, callback) {
 ...
 * }
 */
export function Checker_IsSymbolAccessible(receiver, symbol_, enclosingDeclaration, meaning, shouldComputeAliasesToMakeVisible) {
 ...
 * }
 */
export function Checker_isSymbolAccessibleWorker(receiver, symbol_, enclosingDeclaration, meaning, shouldComputeAliasesToMakeVisible, allowModules) {
    if (symbol_ !== undefined && enclosingDeclaration !== undefined) {
        ...
        // This could be a symbol that is not exported in the external module
        // or it could be a symbol from different external module that is not aliased and hence cannot be named
        const symbolExternalModule = FirstNonNil(symbol_.Declarations ?? [], (d) => Checker_getExternalModuleContainer(receiver, d));
...
//# sourceMappingURL=symbolaccessibility.js.map