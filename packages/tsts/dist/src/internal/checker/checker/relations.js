...
import { IsAccessExpression, IsAmbientModule, GetSourceFileOfNode, IsEntityNameExpression, NewHasFileName, SkipParentheses, } from "../../ast/utilities.js";
import { IsArrayLiteralExpression, IsBinaryExpression, IsCallExpression, IsComputedPropertyName, IsConstructorDeclaration, IsElementAccessExpression, IsExportAssignment, IsIdentifier, IsModuleDeclaration, IsNamespaceImport, IsObjectLiteralExpression, IsOmittedExpression, IsParameterDeclaration, IsPrivateIdentifier, IsPropertyAccessExpression, IsPropertyAssignment, IsPropertyDeclaration, IsPropertySignatureDeclaration, IsShorthandPropertyAssignment, IsSourceFile, IsSpreadAssignment, IsSpreadElement, IsVariableDeclaration, } from "../../ast/generated/predicates.js";
...
import { CheckModeNormal, CachedTypeKindEquivalentBaseType, IterationUseDestructuring, IterationUsePossiblyOutOfBounds, ReferenceHintExportAssignment, TypeFactsIsUndefined, TypeFactsNEUndefined, WideningKindNormal, everyType, getBooleanLiteralValue, getTypeListKey, getVerbatimModuleSyntaxErrorMessage, isTupleType, isUnitType, thisAssignmentDeclarationConstructor, thisAssignmentDeclarationMethod, thisAssignmentDeclarationTyped, getBaseTypeNodeOfClass, } from "./state.js";
...
import { ObjectFlagsClass, ObjectFlagsClassOrInterface, ObjectFlagsIdenticalBaseTypeCalculated, ObjectFlagsReference, InterfaceType_TypeParameters, Type_AsInterfaceType, Type_Target, Type_Types, TypeFlagsAnyOrUnknown, TypeFlagsBigInt, TypeFlagsBigIntLike, TypeFlagsBigIntLiteral, TypeFlagsBooleanLike, TypeFlagsBooleanLiteral, TypeFlagsDefinitelyNonNullable, TypeFlagsESSymbol, TypeFlagsNever, TypeFlagsNonPrimitive, TypeFlagsNull, TypeFlagsNullable, TypeFlagsNumber, TypeFlagsNumberLike, TypeFlagsNumberLiteral, TypeFlagsObject, TypeFlagsString, TypeFlagsStringLike, TypeFlagsStringLiteral, TypeFlagsStringMapping, TypeFlagsIntersection, TypeFlagsInstantiableNonPrimitive, TypeFlagsTemplateLiteral, TypeFlagsTypeParameter, TypeFlagsStructuredOrInstantiable, TypeFlagsUndefined, TypeFlagsUnion, TypeFlagsUniqueESSymbol, TypeFlagsVoid, } from "../types.js";
 ...
 * }
 */
export function Checker_checkInheritedPropertiesAreIdentical(receiver, t, typeNode) {
 ...
 * }
 */
export function Checker_isPropertyIdenticalTo(receiver, sourceProp, targetProp) {
    return Checker_compareProperties(receiver, sourceProp, targetProp, (s, t) => Checker_compareTypesIdentical(receiver, s, t)) !== TernaryFalse;
}
/**
 ...
 * }
 */
export function Checker_checkExportAssignment(receiver, node) {
    const isExportEquals = AsExportAssignment(node).IsExportEquals;
    const illegalContextMessage = IfElse(isExportEquals, An_export_assignment_must_be_at_the_top_level_of_a_file_or_module_declaration, A_default_export_must_be_at_the_top_level_of_a_file_or_module_declaration);
    ...
    const isIllegalExportDefaultInCJS = (!isExportEquals && (node.Flags & NodeFlagsAmbient) === 0 && compilerOptions.VerbatimModuleSyntax === TSTrue && receiver.program.GetEmitModuleFormatOfFile(sourceFileName) === ModuleKindCommonJS);
        ...
        if (sym !== undefined) {
            ...
            if ((Checker_getSymbolFlags(receiver, sym) & SymbolFlagsValue) !== 0) {
                ...
                if (!isIllegalExportDefaultInCJS && (node.Flags & NodeFlagsAmbient) === 0 && compilerOptions.VerbatimModuleSyntax === TSTrue && typeOnlyDeclaration !== undefined) {
                    const message = IfElse(isExportEquals, An_export_declaration_must_reference_a_real_value_when_verbatimModuleSyntax_is_enabled_but_0_resolves_to_a_type_only_declaration, An_export_default_must_reference_a_real_value_when_verbatimModuleSyntax_is_enabled_but_0_resolves_to_a_type_only_declaration);
                ...
                const message = IfElse(isExportEquals, An_export_declaration_must_reference_a_value_when_verbatimModuleSyntax_is_enabled_but_0_only_refers_to_a_type, An_export_default_must_reference_a_value_when_verbatimModuleSyntax_is_enabled_but_0_only_refers_to_a_type);
                Checker_error(receiver, id, message, Node_Text(id));
            }
            if (!isIllegalExportDefaultInCJS && (node.Flags & NodeFlagsAmbient) === 0 && CompilerOptions_GetIsolatedModules(compilerOptions) && (sym.Flags & SymbolFlagsValue) === 0) {
                ...
                if ((sym.Flags & SymbolFlagsAlias) !== 0 && (nonLocalMeanings & SymbolFlagsType) !== 0 && (nonLocalMeanings & SymbolFlagsValue) === 0 && (typeOnlyDeclaration === undefined || GetSourceFileOfNode(typeOnlyDeclaration) !== sourceFile)) {
                    const message = IfElse(isExportEquals, X_0_resolves_to_a_type_and_must_be_marked_type_only_in_this_file_before_re_exporting_when_1_is_enabled_Consider_using_import_type_where_0_is_imported, X_0_resolves_to_a_type_and_must_be_marked_type_only_in_this_file_before_re_exporting_when_1_is_enabled_Consider_using_export_type_0_as_default);
 ...
 * }
 */
export function Checker_areDeclarationFlagsIdentical(receiver, left, right) {
    if ((IsParameterDeclaration(left) && IsVariableDeclaration(right)) || (IsVariableDeclaration(left) && IsParameterDeclaration(right))) {
        return true;
    }
    if (isOptionalDeclaration(left) !== isOptionalDeclaration(right)) {
        return false;
 ...
 * }
 */
export function Checker_checkSatisfiesExpression(receiver, node) {
    const typeNode = Node_Type(node);
 ...
 * }
 */
export function Checker_checkDestructuringAssignment(receiver, node, sourceType, checkMode, rightIsThis) {
    let target;
    ...
    if (IsBinaryExpression(target) && AsBinaryExpression(target).OperatorToken.Kind === KindEqualsToken) {
        ...
        target = AsBinaryExpression(target).Left;
 ...
 * }
 */
export function Checker_checkObjectLiteralAssignment(receiver, node, sourceType, rightIsThis) {
 ...
 * }
 */
export function Checker_checkObjectLiteralDestructuringPropertyAssignment(receiver, node, objectLiteralType, propertyIndex, allProperties, rightIsThis) {
    ...
    if (IsPropertyAssignment(property) || IsShorthandPropertyAssignment(property)) {
        ...
        if (isTypeUsableAsPropertyName(exprType)) {
            ...
            if (prop !== undefined) {
                ...
                Checker_checkPropertyAccessibility(receiver, property, false, true, objectLiteralType, prop);
            }
        }
        const elementType = Checker_getIndexedAccessTypeEx(receiver, objectLiteralType, exprType, (AccessFlagsExpressionPosition | (Checker_hasDefaultValue(receiver, property) ? AccessFlagsAllowMissing : 0)), name, undefined);
        const t = Checker_getFlowTypeOfDestructuring(receiver, property, elementType);
        let expr = property;
        if (IsPropertyAssignment(property)) {
            expr = Node_Initializer(property);
        }
        return Checker_checkDestructuringAssignment(receiver, expr, t, CheckModeNormal, false);
    }
    if (IsSpreadAssignment(property)) {
        ...
        return Checker_checkDestructuringAssignment(receiver, Node_Expression(property), t, CheckModeNormal, false);
    }
 ...
 * }
 */
export function Checker_checkArrayLiteralAssignment(receiver, node, sourceType, checkMode) {
    const elements = Node_Elements(node);
    const possiblyOutOfBoundsType = OrElse(Checker_checkIteratedTypeOrElementType(receiver, (IterationUseDestructuring | IterationUsePossiblyOutOfBounds), sourceType, receiver.undefinedType, node), receiver.errorType);
    let inBoundsType = IfElse(receiver.compilerOptions.NoUncheckedIndexedAccess === TSTrue, undefined, possiblyOutOfBoundsType);
    for (let i = 0; i < elements.length; i++) {
        let t = possiblyOutOfBoundsType;
        if (elements[i].Kind === KindSpreadElement) {
            if (inBoundsType === undefined) {
                inBoundsType = OrElse(Checker_checkIteratedTypeOrElementType(receiver, IterationUseDestructuring, sourceType, receiver.undefinedType, node), receiver.errorType);
            }
            t = inBoundsType;
        }
 ...
 * }
 */
export function Checker_checkArrayLiteralDestructuringElementAssignment(receiver, node, sourceType, elementIndex, elementType, checkMode) {
    ...
    if (!IsOmittedExpression(element)) {
        if (!IsSpreadElement(element)) {
            ...
            if (Checker_isArrayLikeType(receiver, sourceType)) {
                const accessFlags = (AccessFlagsExpressionPosition | (Checker_hasDefaultValue(receiver, element) ? AccessFlagsAllowMissing : 0));
                let localElementType = OrElse(Checker_getIndexedAccessTypeOrUndefined(receiver, sourceType, indexType, accessFlags, Checker_createSyntheticExpression(receiver, element, indexType, false, undefined), undefined), receiver.errorType);
                ...
                return Checker_checkDestructuringAssignment(receiver, element, t, checkMode, false);
            }
            return Checker_checkDestructuringAssignment(receiver, element, elementType, checkMode, false);
        }
        if (elementIndex < elements.Nodes.length - 1) {
            ...
            if (IsBinaryExpression(restExpression) && AsBinaryExpression(restExpression).OperatorToken.Kind === KindEqualsToken) {
                Checker_error(receiver, AsBinaryExpression(restExpression).OperatorToken, A_rest_element_cannot_have_an_initializer, undefined);
            }
            else {
                ...
                let t;
                if (everyType(sourceType, isTupleType)) {
                    t = Checker_mapType(receiver, sourceType, (type) => Checker_sliceTupleType(receiver, type, elementIndex, 0));
                }
                else {
                    t = Checker_createArrayType(receiver, elementType);
                }
                return Checker_checkDestructuringAssignment(receiver, restExpression, t, checkMode, false);
            }
        }
    }
    return undefined;
 ...
 * }
 */
export function Checker_checkReferenceAssignment(receiver, target, sourceType, checkMode) {
 ...
 * }
 */
export function Checker_checkAssignmentOperator(receiver, left, operator, right, leftType, rightType) {
    if (IsAssignmentOperator(operator)) {
        if (IsDeclarationNode(left.Parent) && GetAssignmentDeclarationKind(left.Parent) === JSDeclarationKindExportsProperty) {
            const symbol_ = LinkStore_Get(receiver.symbolNodeLinks, left).resolvedSymbol;
        ...
        if (IsCompoundAssignment(operator) && IsPropertyAccessExpression(left)) {
            localLeftType = Checker_checkPropertyAccessExpression(receiver, left, CheckModeNormal, true);
        }
 ...
 * }
 */
export function Checker_isTypeEqualityComparableTo(receiver, source, target) {
 ...
 * }
 */
export function Checker_checkPropertyAssignment(receiver, node, checkMode) {
    if (IsComputedPropertyName(Node_Name(node))) {
        Checker_checkComputedPropertyName(receiver, Node_Name(node));
    }
    const initializerType = Checker_checkExpressionForMutableLocation(receiver, Node_Initializer(node), checkMode);
    const typeNode = Node_Type(node);
    if (typeNode !== undefined) {
        const t = Checker_getTypeFromTypeNode(receiver, typeNode);
        Checker_checkTypeAssignableToAndOptionallyElaborate(receiver, initializerType, t, node, Node_Initializer(node), undefined, undefined);
        return t;
    }
    return initializerType;
}
 ...
 * }
 */
export function Checker_checkShorthandPropertyAssignment(receiver, node, inDestructuringPattern, checkMode) {
    let expr;
    if (!inDestructuringPattern) {
        expr = AsShorthandPropertyAssignment(node).ObjectAssignmentInitializer;
    }
    if (expr === undefined) {
        expr = Node_Name(node);
    }
 ...
 * }
 */
export function Checker_isReadonlyAssignmentDeclaration(receiver, node) {
    ...
    if (valueType !== undefined) {
        ...
        if (writableProp !== undefined) {
            ...
            if (writableProp.ValueDeclaration !== undefined && IsPropertyAssignment(writableProp.ValueDeclaration)) {
                writableType = Checker_checkExpression(receiver, Node_Initializer(writableProp.ValueDeclaration));
            }
            else {
    ...
    return (Checker_getTypeOfPropertyOfType(receiver, propertyDescriptorType, "set") === undefined);
}
/**
 ...
 * }
 */
export function Checker_getTargetOfExportAssignment(receiver, node) {
 ...
 * }
 */
export function Checker_getWidenedTypeForAssignmentDeclaration(receiver, symbol_) {
    let t;
    ...
    if (symbol_.ValueDeclaration !== undefined && IsInJSFile(symbol_.ValueDeclaration) &&
        Checker_filterType(receiver, t, (ct) => ((ct.flags & ~TypeFlagsNullable) !== 0)) === receiver.neverType) {
 ...
 * }
 */
export function Checker_getAssignmentDeclarationInitializerType(receiver, node) {
    if (IsBinaryExpression(node)) {
        ...
        return Checker_checkExpressionForMutableLocation(receiver, AsBinaryExpression(node).Right, CheckModeNormal);
    }
    if (IsCallExpression(node)) {
        const args = Node_Arguments(node);
 ...
 * }
 */
export function Checker_removeSubtypes(receiver, types, hasObjectTypes) {
    if (types.length < 2) {
        return types;
    }
    ...
    const hasEmptyObject = hasObjectTypes && types.some((t) => (t.flags & TypeFlagsObject) !== 0 &&
    ...
    while (i > 0) {
        ...
        if (hasEmptyObject || (source.flags & TypeFlagsStructuredOrInstantiable) !== 0) {
            ...
            for (const target of types) {
                ...
                if (count === 100000) {
                    ...
                    if (estimatedCount > 1000000) {
                        if (receiver.tracer !== undefined) {
                            Tracer_Instant(receiver.tracer, PhaseCheckTypes, "removeSubtypes_DepthLimit", new globalThis.Map([["estimatedCount", estimatedCount]]));
                        }
                ...
                if (Checker_isTypeRelatedTo(receiver, source, target, receiver.strictSubtypeRelation) &&
 ...
 * }
 */
export function Checker_removeRedundantSupertypes(receiver, types, includes) {
    let result = types;
    let i = result.length;
    while (i > 0) {
        i--;
 ...
 * }
 */
export function Checker_isAssignmentToReadonlyEntity(receiver, expr, symbol_, assignmentKind) {
    if (assignmentKind === AssignmentKindNone) {
        return false;
    }
    ...
    if (IsAccessExpression(expr)) {
        ...
        if (IsIdentifier(node)) {
            ...
            if ((expressionSymbol.Flags & SymbolFlagsAlias) !== 0) {
                ...
                return (declaration !== undefined && IsNamespaceImport(declaration));
            }
        }
    }
    return false;
 ...
 * }
 */
export function Checker_allTypesAssignableToKind(receiver, source, kind) {
 ...
 * }
 */
export function Checker_allTypesAssignableToKindEx(receiver, source, kind, strict) {
    if ((source.flags & TypeFlagsUnion) !== 0) {
        return Every(Type_Types(source), (subType) => {
 ...
 * }
 */
export function Checker_isTypeAssignableToKind(receiver, source, kind) {
 ...
 * }
 */
export function Checker_isTypeAssignableToKindEx(receiver, source, kind, strict) {
    if ((source.flags & kind) !== 0) {
        return true;
    }
    if (strict && (source.flags & (TypeFlagsAnyOrUnknown | TypeFlagsVoid | TypeFlagsUndefined | TypeFlagsNull)) !== 0) {
 ...
 * }
 */
export function Checker_getSingleBaseForNonAugmentingSubtype(receiver, t) {
    if ((t.objectFlags & ObjectFlagsReference) === 0 || (Type_Target(t).objectFlags & ObjectFlagsClassOrInterface) === 0) {
        return undefined;
    }
    const key = { kind: CachedTypeKindEquivalentBaseType, typeId: t.id };
    if ((t.objectFlags & ObjectFlagsIdenticalBaseTypeCalculated) !== 0) {
 ...
 * }
 */
export function Checker_markExportAssignmentAliasReferenced(receiver, location) {
 ...
 * }
 */
export function Checker_getContextualTypeForAssignmentExpression(receiver, binary) {
