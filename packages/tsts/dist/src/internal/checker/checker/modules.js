 ...
 * }
 */
export function Checker_mergeModuleAugmentation(receiver, moduleName) {
 ...
 * }
 */
export function Checker_checkModuleAugmentationElement(receiver, node) {
    switch (node.Kind) {
 ...
 * }
 */
export function Checker_getTargetOfModuleDefault(receiver, moduleSymbol, node, dontResolveAlias) {
    ...
    if (isShorthandAmbientModuleSymbol(moduleSymbol)) {
        ...
        receiver.program.GetImpliedNodeFormatForEmit(NewHasFileName(SourceFile_FileName(file), SourceFile_Path(file))) === ModuleKindESNext) {
 ...
 * }
 */
export function Checker_getEmitSyntaxForModuleSpecifierExpression(receiver, usage) {
 ...
 * }
 */
export function Checker_resolveExternalModule(receiver, location, moduleReference, moduleNotFoundError, errorNode, isForAugmentation) {
    ...
    if (IsStringLiteralLike(location) ||
        ...
        contextSpecifier = AsLiteralTypeNode(AsImportTypeNode(location).Argument).Literal;
    }
        ...
        if (errorNode !== undefined) {
            if (resolvedModule.ResolvedUsingTsExtension && IsDeclarationFileName(moduleReference)) {
                if (FindAncestor(location, IsEmittableImport) !== undefined) {
                    ...
                    Checker_error(receiver, errorNode, A_declaration_file_cannot_be_imported_without_import_type_Did_you_mean_to_import_an_implementation_file_0_instead, Checker_getSuggestedImportSource(receiver, moduleReference, tsExtension, mode));
                }
            }
                ...
                if (!resolvedModule.ResolvedUsingTsExtension && shouldRewrite) {
                    const relativeToSourceFile = GetRelativePathFromFile(GetNormalizedAbsolutePath(SourceFile_FileName(importingSourceFile), receiver.program.GetCurrentDirectory()), resolvedModule.ResolvedFileName, {
                    ...
                    Checker_error(receiver, errorNode, This_import_uses_a_0_extension_to_resolve_to_an_input_TypeScript_file_but_will_not_be_rewritten_during_emit_because_it_is_not_a_relative_path, GetAnyExtensionFromPath(moduleReference, [], false));
                }
                else if (resolvedModule.ResolvedUsingTsExtension && shouldRewrite) {
                    ...
                    if (redirect !== undefined) {
                        ...
                        if (rootDirPath !== outDirPath) {
                            Checker_error(receiver, errorNode, This_import_path_is_unsafe_to_rewrite_because_it_resolves_to_another_project_and_the_relative_path_between_the_projects_output_files_is_not_the_same_as_the_relative_path_between_its_input_files);
                        }
                    }
                }
            }
        }
        const sourceFileSymbol = Node_Symbol(sourceFile);
        if (sourceFileSymbol !== undefined) {
            if (errorNode !== undefined) {
                ...
                if (receiver.moduleKind === ModuleKindNode16 || receiver.moduleKind === ModuleKindNode18) {
                    const isSyncImport = (receiver.program.GetDefaultResolutionModeForFile(importingSourceFile) === ModuleKindCommonJS &&
                    ...
                    if (isSyncImport &&
                        ...
                        if (FindAncestorKind(location, KindImportEqualsDeclaration) !== undefined) {
                            Checker_error(receiver, errorNode, Module_0_cannot_be_imported_using_this_construct_The_specifier_only_resolves_to_an_ES_module_which_cannot_be_imported_with_require_Use_an_ECMAScript_import_instead, moduleReference);
                        }
                        else {
    ...
    if ((ResolvedModule_IsResolved(resolvedModule) && !resolutionExtensionIsTSOrJson(resolvedModule.Extension) && resolutionDiagnostic === undefined) ||
        ...
        if (isForAugmentation) {
            Checker_error(receiver, errorNode, Invalid_module_name_in_augmentation_Module_0_resolves_to_an_untyped_module_at_1_which_cannot_be_augmented, moduleReference, resolvedModule.ResolvedFileName);
        }
        else {
 ...
 * }
 */
export function Checker_createModuleNotFoundChain(receiver, resolvedModule, errorNode, moduleReference, mode, packageName) {
 ...
 * }
 */
export function Checker_tryFindAmbientModule(receiver, moduleName, withAugmentations) {
 ...
 * }
 */
export function Checker_GetAmbientModules(receiver) {
    receiver.ambientModulesOnce.Do(() => {
        for (const [sym, global] of receiver.globals) {
