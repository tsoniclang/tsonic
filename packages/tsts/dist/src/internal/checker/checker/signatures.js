import { beginSelectedCallEvidenceFrame, commitSelectedCallEvidenceFrame, journalSelectedCallEvidence, rollbackSelectedCallEvidenceFrame, } from "./selected-call-evidence-transaction.js";
 ...
 * }
 */
export function Checker_GetTypeAliasTypeParameters(receiver, symbol_) {
    ...
    return LinkStore_Get(receiver.typeAliasLinks, symbol_).typeParameters;
}
/**
 ...
 * }
 */
export function Checker_checkTypeParameter(receiver, node) {
    ...
    if (constraintType !== undefined && defaultType !== undefined) {
        Checker_checkTypeAssignableTo(receiver, defaultType, Checker_getTypeWithThisArgument(receiver, Checker_instantiateType(receiver, constraintType, newSimpleTypeMapper(typeParameter, defaultType)), defaultType, false), typeParameterNode.DefaultType, Type_0_does_not_satisfy_the_constraint_1);
    }
 ...
 * }
 */
export function Checker_checkTypeParameterDeferred(receiver, node) {
    if (IsInterfaceDeclaration(node.Parent) || IsClassLike(node.Parent) || IsTypeOrJSTypeAliasDeclaration(node.Parent)) {
        const typeParameter = Checker_getDeclaredTypeOfTypeParameter(receiver, Checker_getSymbolOfDeclaration(receiver, node));
        const modifiers = (Checker_getTypeParameterModifiers(receiver, typeParameter) & (ModifierFlagsIn | ModifierFlagsOut));
        if (modifiers !== 0) {
            ...
            if (IsTypeOrJSTypeAliasDeclaration(node.Parent) && (Checker_getDeclaredTypeOfSymbol(receiver, symbol_).objectFlags & (ObjectFlagsAnonymous | ObjectFlagsMapped)) === 0) {
                ...
                const popTrace = receiver.tracer !== undefined
                    ? Tracer_Push(receiver.tracer, PhaseCheckTypes, "checkTypeParameterDeferred", new globalThis.Map([["parent", Checker_getDeclaredTypeOfSymbol(receiver, symbol_).id], ["id", typeParameter.id]]), false)
                    : undefined;
                try {
 ...
 * }
 */
export function Checker_checkParameter(receiver, node) {
    Checker_checkGrammarModifiers(receiver, node);
 ...
 * }
 */
export function Checker_checkPropertySignature(receiver, node) {
 ...
 * }
 */
export function Checker_checkSignatureDeclaration(receiver, node) {
    switch (node.Kind) {
 ...
 * }
 */
export function Checker_checkAsyncFunctionReturnType(receiver, node, returnTypeNode) {
    const returnType = Checker_getTypeFromTypeNode(receiver, returnTypeNode);
    ...
    if (globalPromiseType !== receiver.emptyGenericType && !Checker_isReferenceToType(receiver, returnType, globalPromiseType)) {
        Checker_error(receiver, returnTypeNode, The_return_type_of_an_async_function_or_method_must_be_the_global_Promise_T_type_Did_you_mean_to_write_Promise_0, Checker_TypeToString(receiver, core.OrElse(Checker_getAwaitedTypeNoAlias(receiver, returnType), receiver.voidType)));
        return;
    }
 ...
 * }
 */
export function Checker_checkConstructorDeclaration(receiver, node) {
    ...
    if (superCall !== undefined) {
        ...
        const superCallShouldBeRootLevel = (!receiver.emitStandardClassFields &&
            (core.Some(Node_Members(node.Parent) ?? [], isInstancePropertyWithInitializerOrPrivateIdentifierProperty) ||
                core.Some(Node_Parameters(node) ?? [], (p) => HasSyntacticModifier(p, ModifierFlagsParameterPropertyModifier))));
        if (superCallShouldBeRootLevel) {
 ...
 * }
 */
export function Checker_findFirstSuperCall(receiver, node) {
    let superCall = undefined;
    const visit = (n) => {
        if (isSuperCall(n)) {
            superCall = n;
            return true;
 ...
 * }
 */
export function Checker_checkTypeArgumentConstraints(receiver, node, typeParameters) {
 ...
 * }
 */
export function Checker_checkThisType(receiver, node) {
 ...
 * }
 */
export function Checker_checkFunctionOrConstructorSymbol(receiver, symbol_) {
 ...
 * }
 */
export function Checker_checkFunctionOrConstructorSymbolWorker(receiver, symbol_) {
    const flagsToCheck = (ModifierFlagsExport | ModifierFlagsAmbient | ModifierFlagsPrivate | ModifierFlagsProtected | ModifierFlagsAbstract);
    let someNodeFlags = 0;
    let allNodeFlags = flagsToCheck;
    let someHaveQuestionToken = false;
    let allHaveQuestionToken = true;
    let hasOverloads = false;
    let bodyDeclaration = undefined;
    let lastSeenNonAmbientDeclaration = undefined;
    let previousDeclaration = undefined;
    const declarations = symbol_.Declarations ?? [];
    const isConstructor = ((symbol_.Flags & SymbolFlagsConstructor) !== 0);
    let duplicateFunctionDeclaration = false;
    let multipleConstructorImplementation = false;
    let hasNonAmbientClass = false;
    const functionDeclarations = [];
    const getCanonicalOverload = (overloads, implementation) => {
    ...
    const checkFlagAgreementBetweenOverloads = (overloads, implementation, checkFlags, someOverloadFlags, allOverloadFlags) => {
        ...
        if (someButNotAllOverloadFlags !== 0) {
            ...
            for (const overloadsInFile of groups.values()) {
                ...
                for (const overload of overloadsInFile) {
                    const deviation = (Checker_getEffectiveDeclarationFlags(receiver, overload, checkFlags) ^ canonicalFlags);
                    const deviationInFile = (Checker_getEffectiveDeclarationFlags(receiver, overload, checkFlags) ^ canonicalFlagsForFile);
    ...
    const checkQuestionTokenAgreementBetweenOverloads = (overloads, implementation, someOverloadHasQuestionToken, allOverloadsHaveQuestionToken) => {
    ...
    const reportImplementationExpectedError = (node) => {
        const name = Node_Name(node);
        if (name !== undefined && NodeIsMissing(name)) {
            return;
        }
        let seen = false;
        const subsequentNode = { node: undefined };
        Node_ForEachChild(node.Parent, (child) => {
            if (seen) {
        ...
        if (subsequentNode.node !== undefined && Node_Pos(subsequentNode.node) === Node_End(node)) {
            if (subsequentNode.node.Kind === node.Kind) {
                ...
                if (name !== undefined && subsequentName !== undefined &&
                    ...
                    const reportError = ((IsMethodDeclaration(node) || IsMethodSignatureDeclaration(node)) && IsStatic(node) !== IsStatic(subsequentNode.node));
                    if (reportError) {
    ...
    for (const node of declarations) {
        const inAmbientContext = ((node.Flags & NodeFlagsAmbient) !== 0);
        const inAmbientContextOrInterface = (inAmbientContext || (node.Parent !== undefined && (IsInterfaceDeclaration(node.Parent) || IsTypeLiteralNode(node.Parent))));
        if (inAmbientContextOrInterface) {
        ...
        if (IsFunctionDeclaration(node) || IsMethodDeclaration(node) || IsMethodSignatureDeclaration(node) || IsConstructorDeclaration(node)) {
            ...
            someNodeFlags = (someNodeFlags | currentNodeFlags);
            allNodeFlags = (allNodeFlags & currentNodeFlags);
            someHaveQuestionToken = (someHaveQuestionToken || isOptionalDeclaration(node));
            allHaveQuestionToken = (allHaveQuestionToken && isOptionalDeclaration(node));
            ...
            else if (previousDeclaration !== undefined &&
    ...
    if (lastSeenNonAmbientDeclaration !== undefined &&
 ...
 * }
 */
export function Checker_isImplementationCompatibleWithOverload(receiver, implementation, overload) {
 ...
 * }
 */
export function Checker_isUnwrappedReturnTypeUndefinedVoidOrAny(receiver, fn, returnType) {
 ...
 * }
 */
export function Checker_checkTypeParameterListsIdentical(receiver, symbol_) {
 ...
 * }
 */
export function Checker_areTypeParametersIdentical(receiver, declarations, targetParameters, getTypeParameterDeclarations) {
 ...
 * }
 */
export function Checker_getTypeWithoutSignatures(receiver, t) {
    if ((t.flags & TypeFlagsObject) !== 0) {
        ...
        return Checker_getIntersectionType(receiver, core.Map(Type_Types(t), (type_) => Checker_getTypeWithoutSignatures(receiver, type_)));
    }
    return t;
}
/**
 ...
 * }
 */
export function Checker_checkIndexConstraintForIndexSignature(receiver, t, checkInfo) {
    ...
    for (const info of indexInfos) {
        ...
        if (errorNode === undefined && interfaceDeclaration !== undefined && !core.Some(Checker_getBaseTypes(receiver, t), (base) => (Checker_getIndexInfoOfType(receiver, base, checkInfo.keyType) !== undefined && Checker_getIndexTypeOfType(receiver, base, info.keyType) !== undefined))) {
            errorNode = interfaceDeclaration;
        }
        if (errorNode !== undefined && !Checker_isTypeAssignableTo(receiver, checkInfo.valueType, info.valueType)) {
            Checker_error(receiver, errorNode, X_0_index_type_1_is_not_assignable_to_2_index_type_3, Checker_TypeToString(receiver, checkInfo.keyType), Checker_TypeToString(receiver, checkInfo.valueType), Checker_TypeToString(receiver, info.keyType), Checker_TypeToString(receiver, info.valueType));
        }
    }
}
/**
 ...
 * }
 */
export function Checker_checkClassOrInterfaceForDuplicateIndexSignatures(receiver, node) {
    const links = LinkStore_Get(receiver.declaredTypeLinks, Checker_getSymbolOfDeclaration(receiver, node));
 ...
 * }
 */
export function Checker_checkTypeForDuplicateIndexSignatures(receiver, node) {
 ...
 * }
 */
export function Checker_isPropertyInitializedInConstructor(receiver, propName, propType, constructor_) {
    let reference;
    if (IsComputedPropertyName(propName)) {
        reference = NewElementAccessExpression(receiver.factory, NewKeywordExpression(receiver.factory, KindThisKeyword), undefined, Node_Expression(propName), NodeFlagsNone);
    }
    else {
        reference = NewPropertyAccessExpression(receiver.factory, NewKeywordExpression(receiver.factory, KindThisKeyword), undefined, propName, NodeFlagsNone);
    }
 ...
 * }
 */
export function Checker_getIterationTypeOfGeneratorFunctionReturnType(receiver, typeKind, returnType, isAsyncGenerator) {
    if (IsTypeAny(returnType)) {
        return undefined;
    }
 ...
 * }
 */
export function Checker_getIterationTypesOfGeneratorFunctionReturnType(receiver, t, isAsyncGenerator) {
    if (IsTypeAny(t)) {
 ...
 * }
 */
export function Checker_getBuiltinIteratorReturnType(receiver) {
 ...
 * }
 */
export function Checker_checkTypeParameters(receiver, typeParameterDeclarations) {
 ...
 * }
 */
export function Checker_checkTypeParametersNotReferenced(receiver, root, typeParameters, index) {
    const visit = (node) => {
 ...
 * }
 */
export function Checker_checkUnusedLocalsAndParameters(receiver, node) {
    const variableParents = new globalThis.Set();
    const importClauses = new globalThis.Map();
    for (const local of Node_Locals(node)?.values() ?? []) {
        const referenceKinds = LinkStore_Get(receiver.symbolReferenceLinks, local).referenceKinds ?? SymbolFlagsNone;
 ...
 * }
 */
export function Checker_reportUnusedParameters(receiver, node) {
 ...
 * }
 */
export function Checker_checkUnusedInferTypeParameter(receiver, node) {
 ...
 * }
 */
export function Checker_checkUnusedTypeParameters(receiver, node) {
 ...
 * }
 */
export function Checker_isUnreferencedTypeParameter(receiver, typeParameter) {
    return ((LinkStore_Get(receiver.symbolReferenceLinks, Checker_getMergedSymbol(receiver, Node_Symbol(typeParameter))).referenceKinds ?? SymbolFlagsNone) & SymbolFlagsTypeParameter) === 0 &&
 ...
 * }
 */
export function Checker_getReturnTypeOfSingleNonGenericSignature(receiver, funcType, kind) {
 ...
 * }
 */
export function Checker_getReturnTypeOfSingleNonGenericSignatureOfCallChain(receiver, expr) {
 ...
 * }
 */
export function Checker_instantiateTypeWithSingleGenericCallSignature(receiver, node, t, checkMode) {
    if ((checkMode & (CheckModeInferential | CheckModeSkipGenericFunctions)) === 0) {
        return t;
    }
 ...
 * }
 */
export function Checker_getOuterInferenceTypeParameters(receiver) {
    let result = [];
 ...
 * }
 */
export function Checker_getUniqueTypeParameters(receiver, context, typeParameters) {
    ...
    for (const tp of (typeParameters ?? [])) {
        ...
        if (hasTypeParameterByName(context.inferredTypeParameters, name) || hasTypeParameterByName(result, name)) {
            ...
            oldTypeParameters = [...(oldTypeParameters ?? []), tp];
            newTypeParameters = [...(newTypeParameters ?? []), newTypeParameter];
            result = [...result, newTypeParameter];
        }
        else {
            result = [...result, tp];
        }
 ...
 * }
 */
export function Checker_isInConstructorArgumentInitializer(receiver, node, constructorDecl) {
    return FindAncestorOrQuit(node, (n) => {
 ...
 * }
 */
export function Checker_checkImportCallExpression(receiver, node) {
 ...
 * }
 */
export function Checker_checkCallExpression(receiver, node, checkMode) {
    ...
    if ((returnType.flags & TypeFlagsESSymbolLike) !== 0 && Checker_isSymbolOrSymbolForCall(receiver, node)) {
        return Checker_getESSymbolLikeTypeForNode(receiver, WalkUpParenthesizedExpressions(node.Parent));
    }
 ...
 * }
 */
export function Checker_isSymbolOrSymbolForCall(receiver, node) {
...
export function Checker_finalizeResolvedCallEvidence(receiver, node, sourceResultType) {
    ...
    const finalized = Object.freeze({ ...selection, sourceResultType });
    journalSelectedCallEvidence(receiver, links);
    ...
    if (callSignatures.length === 0) {
        if (numConstructSignatures !== 0) {
            ...
            if ((Node_Arguments(node) ?? []).length === 1) {
                ...
                const options = { StopAfterLineBreak: true, StopAtComments: false, InJSDoc: false };
                if (IsLineBreak(text.charCodeAt(SkipTriviaEx(text, Node_End(Node_Expression(node)), options) - 1))) {
 ...
 * }
 */
export function Checker_isConstructorAccessible(receiver, node, signature) {
...
function buildResolvedCallSelectedSignatureParameters(receiver, selectedSignature) {
...
function Checker_resolveCallWithSelectedArguments(receiver, node, signatures, candidatesOutArray, checkMode, callChainFlags, headMessage, output) {
    ...
    try {
        ...
        LinkStore_Get(receiver.signatureLinks, node).resolvedSignature = result;
        if (reportErrors) {
 ...
 * }
 */
export function Checker_reorderCandidates(receiver, signatures, callChainFlags) {
    let lastParent;
    let lastSymbol;
    let index = 0;
    let cutoffIndex = 0;
 ...
 * }
 */
export function Checker_getOptionalCallSignature(receiver, signature, callChainFlags) {
    if ((signature.flags & SignatureFlagsCallChainFlags) === callChainFlags) {
        return signature;
    }
    const key = { sig: signature, key: core.IfElse(callChainFlags === SignatureFlagsIsInnerCallChain, SignatureKeyInner, SignatureKeyOuter) };
    const cached = receiver.cachedSignatures.get(key);
    if (cached !== undefined) {
        return cached;
    }
    const result = Checker_cloneSignature(receiver, signature);
    result.flags = (result.flags | callChainFlags);
    receiver.cachedSignatures.set(key, result);
    return result;
}
/**
        ...
        try {
            ...
            if (candidate.typeParameters.length !== 0) {
                ...
                if (s.typeArguments.length !== 0) {
                    typeArgumentTypes = Checker_checkTypeArguments(receiver, candidate, s.typeArguments, false, undefined);
                    ...
                    typeArgumentTypes = Checker_inferTypeArguments(receiver, s.node, candidate, s.args, (s.argCheckMode | CheckModeSkipGenericFunctions), inferenceContext);
                    if ((inferenceContext.flags & InferenceFlagsSkippedGenericFunction) !== 0) {
                        s.argCheckMode = (s.argCheckMode | CheckModeSkipGenericFunctions);
                    }
                }
                let inferredTypeParameters;
            ...
            if (s.argCheckMode !== 0) {
                ...
                try {
                    preliminaryApplicable = Checker_isSignatureApplicableWithSelectedArgumentTypes(receiver, s.node, s.args, checkCandidate, relation, s.argCheckMode, false, undefined, undefined);
                }
                finally {
...
function retainApplicableCallSelection(receiver, node, output, signature, selectedArguments) {
    if (output === undefined) {
        return;
    }
 ...
 * }
 */
export function Checker_hasCorrectArity(receiver, node, args, signature, signatureHelpTrailingComma) {
 ...
 * }
 */
export function Checker_getDecoratorArgumentCount(receiver, node, signature) {
 ...
 * }
 */
export function Checker_getLegacyDecoratorArgumentCount(receiver, node, signature) {
    switch (node.Parent.Kind) {
        case KindClassDeclaration:
 ...
 * }
 */
export function Checker_hasCorrectTypeArgumentArity(receiver, signature, typeArguments) {
 ...
 * }
 */
export function Checker_checkTypeArguments(receiver, signature, typeArgumentNodes, reportErrors, headMessage) {
    const isJavaScript = IsInJSFile(signature.declaration);
    const typeParameters = signature.typeParameters;
    const typeArgumentTypes = Checker_fillMissingTypeArguments(receiver, core.Map(typeArgumentNodes, (node) => Checker_getTypeFromTypeNode(receiver, node)), typeParameters, Checker_getMinTypeArgumentCount(receiver, typeParameters), isJavaScript);
    let mapper = undefined;
    for (let index = 0; index < typeArgumentNodes.length; index++) {
        ...
        if (constraint !== undefined) {
            ...
            const diags = [];
            if (!Checker_checkTypeAssignableToEx(receiver, typeArgument, Checker_getTypeWithThisArgument(receiver, Checker_instantiateType(receiver, constraint, mapper), typeArgument, false), errorNode, typeArgumentHeadMessage, diags)) {
                if (diags.length !== 0) {
    ...
    if (restType !== undefined) {
        ...
        if (reportErrors) {
            if (restArgCount === 0) {
                ...
                errorNode = Checker_createSyntheticExpression(receiver, node, spreadType, false, undefined);
 ...
 * }
 */
export function Checker_getThisArgumentOfCall(receiver, node) {
 ...
 * }
 */
export function Checker_getThisArgumentType(receiver, node) {
    if (node === undefined) {
        return receiver.voidType;
    }
 ...
 * }
 */
export function Checker_inferTypeArguments(receiver, node, signature, args, checkMode, context) {
    ...
    if (!IsDecorator(node) && !IsBinaryExpression(node)) {
        const skipBindingPatterns = core.Every(signature.typeParameters, (p) => Checker_getDefaultFromTypeParameter(receiver, p) !== undefined);
    ...
    if (restType !== undefined) {
        argCount = Math.min(Checker_getParameterCount(receiver, signature) - 1, argCount);
    }
    if (restType !== undefined && (restType.flags & TypeFlagsTypeParameter) !== 0) {
        const info = core.Find(context.inferences, (inference) => inference.typeParameter === restType);
        if (info !== undefined) {
    ...
    if (restType !== undefined && receiver.couldContainTypeVariables(restType)) {
        const spreadType = Checker_getSpreadArgumentType(receiver, args, argCount, args.length, restType, context, checkMode);
 ...
 * }
 */
export function Checker_getCandidateForOverloadFailure(receiver, node, candidates, args, hasCandidatesOutArray, checkMode) {
    Checker_checkNodeDeferred(receiver, node);
    if (hasCandidatesOutArray || candidates.length === 1 || core.Some(candidates, (s) => s.typeParameters.length !== 0)) {
 ...
 * }
 */
export function Checker_pickLongestCandidateSignature(receiver, node, candidates, args, checkMode) {
    // Go: len(nil) == 0.
    let argCount = (args ?? []).length;
    if (receiver.apparentArgumentCount !== undefined) {
        argCount = receiver.apparentArgumentCount;
    }
    const bestIndex = Checker_getLongestCandidateIndex(receiver, candidates, argCount);
 ...
 * }
 */
export function Checker_getLongestCandidateIndex(receiver, candidates, argsCount) {
 ...
 * }
 */
export function Checker_getTypeArgumentsFromNodes(receiver, typeArgumentNodes, typeParameters) {
    let nodes = typeArgumentNodes;
    if ((nodes ?? []).length > (typeParameters ?? []).length) {
        nodes = (nodes ?? []).slice(0, (typeParameters ?? []).length);
    }
    const typeArguments = core.Map(nodes, (n) => Checker_getTypeFromTypeNode(receiver, n));
    let result = typeArguments ?? [];
 ...
 * }
 */
export function Checker_inferSignatureInstantiationForOverloadFailure(receiver, node, typeParameters, candidate, args, checkMode) {
 ...
 * }
 */
export function Checker_createUnionOfSignaturesForOverloadFailure(receiver, candidates) {
    const thisParameters = core.MapNonNil(candidates, (candidate) => candidate.thisParameter);
    let thisParameter;
    if (thisParameters.length !== 0) {
        thisParameter = Checker_createCombinedSymbolFromTypes(receiver, thisParameters, core.Map(thisParameters, (symbol_) => Checker_getTypeOfParameter(receiver, symbol_)));
    }
    const [minArgumentCount, maxNonRestParam] = minAndMax(candidates, getNonRestParameterCount);
    const parameters = [];
    for (let i = 0; i < maxNonRestParam; i++) {
        const index = i;
        const symbols = core.MapNonNil(candidates, (signature) => {
        ...
        parameters.push(Checker_createCombinedSymbolFromTypes(receiver, symbols, core.MapNonNil(candidates, (signature) => Checker_tryGetTypeAtPosition(receiver, signature, index))));
    }
    const restParameterSymbols = core.MapNonNil(candidates, (signature) => {
        if (signatureHasRestParameter(signature)) {
            return core.LastOrNil(signature.parameters);
        }
        return undefined;
    ...
    if (restParameterSymbols.length !== 0) {
        const restType = Checker_createArrayType(receiver, Checker_getUnionTypeEx(receiver, core.MapNonNil(candidates, (signature) => Checker_tryGetRestTypeOfSignature(receiver, signature)), UnionReductionSubtype, undefined, undefined));
    ...
    return Checker_newSignature(receiver, flags, candidates[0].declaration, [], thisParameter, parameters, Checker_getIntersectionType(receiver, core.Map(candidates, (signature) => Checker_getReturnTypeOfSignature(receiver, signature))), undefined, minArgumentCount);
}
/**
 ...
 * }
 */
export function Checker_createCombinedSymbolForOverloadFailure(receiver, sources, t) {
 ...
 * }
 */
export function Checker_getRestTypeOfSignature(receiver, signature) {
 ...
 * }
 */
export function Checker_tryGetRestTypeOfSignature(receiver, signature) {
    if (!signatureHasRestParameter(signature)) {
        return undefined;
    }
 ...
 * }
 */
export function Checker_isUntypedFunctionCall(receiver, funcType, apparentFuncType, numCallSignatures, numConstructSignatures) {
    return IsTypeAny(funcType) || IsTypeAny(apparentFuncType) && (funcType.flags & TypeFlagsTypeParameter) !== 0 ||
        numCallSignatures === 0 && numConstructSignatures === 0 && (apparentFuncType.flags & TypeFlagsUnion) === 0 &&
 ...
 * }
 */
export function Checker_inferFromAnnotatedParametersAndReturn(receiver, sig, context, inferenceContext) {
 ...
 * }
 */
export function Checker_getContextualSignature(receiver, node) {
    ...
    for (const current of Type_Types(t)) {
        ...
        if (signature !== undefined) {
            if (signatureList.length !== 0 && Checker_compareSignaturesIdentical(receiver, signatureList[0], signature, false, true, true, (source, target) => Checker_compareTypesIdentical(receiver, source, target)) === TernaryFalse) {
                return undefined;
            }
 ...
 * }
 */
export function Checker_createUnionSignature(receiver, sig, unionSignatures) {
 ...
 * }
 */
export function Checker_getContextualCallSignature(receiver, t, node) {
    const signatures = Checker_getSignaturesOfType(receiver, t, SignatureKindCall);
    const applicableByArity = core.Filter(signatures, (s) => !Checker_isAritySmaller(receiver, s, node));
 ...
 * }
 */
export function Checker_getIntersectedSignatures(receiver, signatures) {
    if (!receiver.noImplicitAny) {
        return undefined;
    }
 ...
 * }
 */
export function Checker_isAritySmaller(receiver, signature, target) {
 ...
 * }
 */
export function Checker_assignContextualParameterTypes(receiver, sig, context) {
    ...
    if (signatureHasRestParameter(sig)) {
        ...
        if ((parameter.ValueDeclaration !== undefined && Node_Type(parameter.ValueDeclaration) === undefined) ||
 ...
 * }
 */
export function Checker_assignNonContextualParameterTypes(receiver, signature) {
 ...
 * }
 */
export function Checker_assignParameterType(receiver, parameter, contextualType) {
    const links = LinkStore_Get(receiver.valueSymbolLinks, parameter);
 ...
 * }
 */
export function Checker_checkExpressionWithTypeArguments(receiver, node) {
 ...
 * }
 */
export function Checker_isMethodAccessForCall(receiver, node) {
    let cur = node;
 ...
 * }
 */
export function Checker_getEnclosingClassFromThisParameter(receiver, node) {
 ...
 * }
 */
export function Checker_getContextualThisParameterType(receiver, fn) {
    if (IsArrowFunction(fn)) {
        return undefined;
    }
    ...
    if (receiver.noImplicitThis || inJs) {
        ...
        const parent = WalkUpParenthesizedExpressions(fn.Parent);
 ...
 * }
 */
export function Checker_tryGetThisTypeAt(receiver, node) {
 ...
 * }
 */
export function Checker_TryGetThisTypeAtEx(receiver, node, includeGlobalThis, container) {
...
export function Checker_tryGetThisTypeAtEx(receiver, node, includeGlobalThis, container) {
    ...
    if (IsSourceFile(effectiveContainer)) {
        const sourceFile = effectiveContainer;
 ...
 * }
 */
export function Checker_isInParameterInitializerBeforeContainingFunction(receiver, node) {
 ...
 * }
 */
export function Checker_isIndirectCall(receiver, node) {
 ...
 * }
 */
export function Checker_isValidConstAssertionArgument(receiver, node) {
    switch (node.Kind) {
 ...
 * }
 */
export function Checker_newParameter(receiver, name, t) {
    const symbol_ = Checker_newSymbol(receiver, SymbolFlagsFunctionScopedVariable, name);
    LinkStore_Get(receiver.valueSymbolLinks, symbol_).resolvedType = t;
    return symbol_;
}
/**
 ...
 * }
 */
export function Checker_hasSignatures(receiver, t) {
 ...
 * }
 */
export function Checker_lateBindIndexSignature(receiver, parent, earlySymbols, lateSymbols, decl) {
 ...
 * }
 */
export function Checker_getTypeOfVariableOrParameterOrProperty(receiver, symbol_) {
    const links = LinkStore_Get(receiver.valueSymbolLinks, symbol_);
    if (links.resolvedType === undefined) {
 ...
 * }
 */
export function Checker_isParameterOfContextSensitiveSignature(receiver, symbol_) {
 ...
 * }
 */
export function Checker_getTypeOfVariableOrParameterOrPropertyWorker(receiver, symbol_) {
 ...
 * }
 */
export function Checker_getBaseConstructorTypeOfClass(receiver, t) {
    ...
    if ((baseConstructorType.flags & TypeFlagsAny) === 0 && baseConstructorType !== receiver.nullWideningType && !Checker_isConstructorType(receiver, baseConstructorType)) {
        ...
        if ((baseConstructorType.flags & TypeFlagsTypeParameter) !== 0) {
            ...
            let ctorReturn = receiver.unknownType;
            if (constraint !== undefined) {
            ...
            if (baseConstructorType.symbol.Declarations !== undefined) {
                Diagnostic_AddRelatedInfo(err, NewDiagnosticForNode(baseConstructorType.symbol.Declarations[0], Did_you_mean_for_0_to_be_constrained_to_type_new_args_Colon_any_1, Checker_symbolToString(receiver, baseConstructorType.symbol), Checker_TypeToString(receiver, ctorReturn)));
            }
        }
        if (data.resolvedBaseConstructorType === undefined) {
            data.resolvedBaseConstructorType = receiver.errorType;
 ...
 * }
 */
export function Checker_isConstructorType(receiver, t) {
 ...
 * }
 */
export function Checker_isMixinConstructorType(receiver, t) {
 ...
 * }
 */
export function Checker_getTypeOfParameter(receiver, symbol_) {
    const declaration = symbol_.ValueDeclaration;
 ...
 * }
 */
export function Checker_getConstraintOfTypeParameter(receiver, typeParameter) {
 ...
 * }
 */
export function Checker_getConstraintFromTypeParameter(receiver, t) {
    if ((t.flags & TypeFlagsTypeParameter) === 0) {
        return undefined;
    }
 ...
 * }
 */
export function Checker_getConstraintOrUnknownFromTypeParameter(receiver, t) {
 ...
 * }
 */
export function Checker_getInferredTypeParameterConstraint(receiver, t, omitTypeReferences) {
    const inferences = [];
    if (t.symbol !== undefined && (t.symbol.Declarations ?? []).length !== 0) {
        for (const declaration of t.symbol.Declarations ?? []) {
            if (IsInferTypeNode(declaration.Parent)) {
                ...
                if (IsTypeReferenceNode(parent) && !omitTypeReferences) {
                    ...
                    if (typeParameters !== undefined) {
                        ...
                        if (index >= 0 && index < typeParameters.length) {
                            ...
                            if (declaredConstraint !== undefined) {
                                const mapper = newDeferredTypeMapper(typeParameters, core.MapIndex(typeParameters, (_typeParameter, typeParameterIndex) => {
                                    return () => Checker_getEffectiveTypeArgumentAtIndex(receiver, parent, typeParameters, typeParameterIndex);
                                }));
                ...
                else if ((IsParameterDeclaration(parent) && AsParameterDeclaration(parent).DotDotDotToken !== undefined) ||
                ...
                else if (IsMappedTypeNode(parent) &&
                    ...
                    const mapper = newSimpleTypeMapper(Checker_getDeclaredTypeOfTypeParameter(receiver, Checker_getSymbolOfDeclaration(receiver, checkMappedTypeParameter)), core.IfElse(checkMappedTypeParameterDeclaration.Constraint !== undefined, Checker_getTypeFromTypeNode(receiver, checkMappedTypeParameterDeclaration.Constraint), receiver.stringNumberSymbolType));
 ...
 * }
 */
export function Checker_getTypeParametersForTypeReferenceOrImport(receiver, node) {
 ...
 * }
 */
export function Checker_getTypeParametersForTypeAndSymbol(receiver, t, symbol_) {
    if (!Checker_isErrorType(receiver, t)) {
        if ((symbol_.Flags & SymbolFlagsTypeAlias) !== 0) {
 ...
 * }
 */
export function Checker_getEffectiveTypeArgumentAtIndex(receiver, node, typeParameters, index) {
 ...
 * }
 */
export function Checker_getRestType(receiver, source, properties, symbol_) {
    source = Checker_filterType(receiver, source, (t) => ((t.flags & TypeFlagsNullable) === 0));
    if ((source.flags & TypeFlagsNever) !== 0) {
        return receiver.emptyObjectType;
    }
    if ((source.flags & TypeFlagsUnion) !== 0) {
        return Checker_mapType(receiver, source, (t) => Checker_getRestType(receiver, t, properties, symbol_));
    }
    let omitKeyType = Checker_getUnionType(receiver, core.Map(properties, (property) => Checker_getLiteralTypeFromPropertyName(receiver, property)));
    const spreadableProperties = [];
    const unspreadableToRestKeys = [];
    for (const prop of Checker_getPropertiesOfType(receiver, source)) {
        const literalTypeFromProperty = Checker_getLiteralTypeFromProperty(receiver, prop, TypeFlagsStringOrNumberLiteralOrUnique, false);
        if (!Checker_isTypeAssignableTo(receiver, literalTypeFromProperty, omitKeyType) &&
    ...
    for (const prop of spreadableProperties) {
        members.set(prop.Name, Checker_getSpreadSymbol(receiver, prop, false));
    }
    const result = Checker_newAnonymousType(receiver, symbol_, members, [], [], Checker_getIndexInfosOfType(receiver, source));
    result.objectFlags = (result.objectFlags | ObjectFlagsObjectRestType);
    return result;
}
/**
 ...
 * }
 */
export function Checker_isConstructorDeclaredThisProperty(receiver, symbol_) {
    ...
    let kind = thisAssignmentDeclarationNone;
    if (allThis) {
 ...
 * }
 */
export function Checker_isGlobalSymbolConstructor(receiver, node) {
 ...
 * }
 */
export function Checker_getSignaturesOfType(receiver, t, kind) {
 ...
 * }
 */
export function Checker_getSignaturesOfStructuredType(receiver, t, kind) {
    if ((t.flags & TypeFlagsStructuredType) === 0) {
        return [];
    }
    const resolved = Checker_resolveStructuredTypeMembers(receiver, t);
 ...
 * }
 */
export function Checker_getApplicableIndexInfo(receiver, t, keyType) {
 ...
 * }
 */
export function Checker_getApplicableIndexInfoForName(receiver, t, name) {
    if (isLateBoundName(name)) {
 ...
 * }
 */
export function Checker_findApplicableIndexInfo(receiver, indexInfos, keyType) {
    let stringIndexInfo = undefined;
    const applicableInfos = [];
    for (const info of (indexInfos ?? [])) {
    ...
    let isReadonly = true;
    const types = applicableInfos.map(info => {
        if (!info.isReadonly) {
            isReadonly = false;
        }
 ...
 * }
 */
export function Checker_isApplicableIndexType(receiver, source, target) {
 ...
 * }
 */
export function Checker_getInstantiatedConstructorsForTypeArguments(receiver, t, typeArgumentNodes, location) {
    const signatures = Checker_getConstructorsForTypeArguments(receiver, t, typeArgumentNodes, location);
    const typeArguments = core.Map(typeArgumentNodes, (node) => Checker_getTypeFromTypeNode(receiver, node));
    return core.SameMap(signatures, (sig) => {
        if (sig.typeParameters.length !== 0) {
            return Checker_getSignatureInstantiation(receiver, sig, typeArguments, IsInJSFile(location), undefined);
        }
        return sig;
    });
}
 ...
 * }
 */
export function Checker_getConstructorsForTypeArguments(receiver, t, typeArgumentNodes, location) {
    const typeArgCount = typeArgumentNodes.length;
    return core.Filter(Checker_getSignaturesOfType(receiver, t, SignatureKindConstruct), (sig) => typeArgCount >= Checker_getMinTypeArgumentCount(receiver, sig.typeParameters) && typeArgCount <= sig.typeParameters.length);
}
/**
 ...
 * }
 */
export function Checker_getSignatureInstantiation(receiver, sig, typeArguments, isJavaScript, inferredTypeParameters) {
 ...
 * }
 */
export function Checker_cloneSignature(receiver, sig) {
    const result = Checker_newSignature(receiver, (sig.flags & SignatureFlagsPropagatingFlags), sig.declaration, sig.typeParameters, sig.thisParameter, sig.parameters, undefined, undefined, sig.minArgumentCount);
    result.target = sig;
    result.mapper = sig.mapper;
    result.composite = sig.composite;
    return result;
 ...
 * }
 */
export function Checker_getSignatureInstantiationWithoutFillingInTypeArguments(receiver, sig, typeArguments) {
    const key = { sig, key: getTypeListKey(typeArguments) };
    let instantiation = receiver.cachedSignatures.get(key);
    if (instantiation === undefined) {
        instantiation = Checker_createSignatureInstantiation(receiver, sig, typeArguments);
        receiver.cachedSignatures.set(key, instantiation);
    }
    return instantiation;
}
/**
 ...
 * }
 */
export function Checker_createSignatureInstantiation(receiver, sig, typeArguments) {
 ...
 * }
 */
export function Checker_createSignatureTypeMapper(receiver, sig, typeArguments) {
 ...
 * }
 */
export function Checker_getTypeParametersForMapper(receiver, sig) {
    return core.SameMap(sig.typeParameters, (tp) => Checker_instantiateType(receiver, tp, Type_Mapper(tp)));
}
/**
 ...
 * }
 */
export function Checker_getSingleCallSignature(receiver, t) {
 ...
 * }
 */
export function Checker_getSingleCallOrConstructSignature(receiver, t) {
 ...
 * }
 */
export function Checker_getSingleSignature(receiver, t, kind, allowMembers) {
    if ((t.flags & TypeFlagsObject) !== 0) {
        const resolved = Checker_resolveStructuredTypeMembers(receiver, t);
 ...
 * }
 */
export function Checker_getOrCreateTypeFromSignature(receiver, sig) {
 ...
 * }
 */
export function Checker_getErasedSignature(receiver, signature) {
    if (signature.typeParameters.length === 0) {
        return signature;
    }
    const key = { sig: signature, key: SignatureKeyErased };
    let erased = receiver.cachedSignatures.get(key);
    if (erased === undefined) {
        ...
        receiver.cachedSignatures.set(key, erased);
    }
    return erased;
}
/**
 ...
 * }
 */
export function Checker_getCanonicalSignature(receiver, signature) {
    if (signature.typeParameters.length === 0) {
        return signature;
    }
    const key = { sig: signature, key: SignatureKeyCanonical };
    let canonical = receiver.cachedSignatures.get(key);
    if (canonical === undefined) {
        canonical = Checker_createCanonicalSignature(receiver, signature);
        receiver.cachedSignatures.set(key, canonical);
    }
    return canonical;
}
/**
 ...
 * }
 */
export function Checker_createCanonicalSignature(receiver, signature) {
    const typeArguments = core.Map(signature.typeParameters, (tp) => {
    ...
    return Checker_getSignatureInstantiation(receiver, signature, typeArguments, IsInJSFile(signature.declaration), undefined);
}
/**
 ...
 * }
 */
export function Checker_getBaseSignature(receiver, signature) {
    const typeParameters = signature.typeParameters;
    if (typeParameters.length === 0) {
        return signature;
    }
    const key = { sig: signature, key: SignatureKeyBase };
    const cached = receiver.cachedSignatures.get(key);
    if (cached !== undefined) {
        return cached;
    }
    const baseConstraintMapper = newTypeMapper(typeParameters, core.Map(typeParameters, (tp) => core.OrElse(Checker_getConstraintOfTypeParameter(receiver, tp), receiver.unknownType)));
    let baseConstraints = core.Map(typeParameters, (tp) => Checker_instantiateType(receiver, tp, baseConstraintMapper));
 ...
 * }
 */
export function Checker_instantiateSignatureInContextOf(receiver, signature, contextualSignature, inferenceContext, compareTypes) {
    ...
    Checker_applyToParameterTypes(receiver, sourceSignature, signature, (source, target) => {
        Checker_inferTypes(receiver, context.inferences, source, target, InferencePriorityNone, false);
    });
    if (inferenceContext === undefined) {
        Checker_applyToReturnTypes(receiver, contextualSignature, signature, (source, target) => {
 ...
 * }
 */
export function Checker_areAllOuterTypeParametersApplied(receiver, t) {
 ...
 * }
 */
export function Checker_getTypeWithThisArgument(receiver, t, thisArgument, needApparentType) {
    if ((t.objectFlags & ObjectFlagsReference) !== 0) {
        ...
        const newTypes = core.SameMap(types, (t2) => Checker_getTypeWithThisArgument(receiver, t2, thisArgument, needApparentType));
        if (core.Same(newTypes, types)) {
            return t;
        }
 ...
 * }
 */
export function Checker_getSignaturesOfSymbol(receiver, symbol_) {
    if (symbol_ === undefined) {
        return [];
    }
    const result = [];
 ...
 * }
 */
export function Checker_getSignatureFromDeclaration(receiver, declaration) {
    const links = LinkStore_Get(receiver.signatureLinks, declaration);
    ...
    const isUntypedSignatureInJSFile = iife === undefined
        ...
        && core.Every(Node_Parameters(declaration), (param) => Node_Type(param) === undefined)
        && Checker_getContextualType(receiver, declaration, ContextFlagsSignature) === undefined;
    if (isUntypedSignatureInJSFile) {
        flags = (flags | SignatureFlagsIsUntypedSignatureInJSFile);
    }
    for (const [i, param] of (Node_Parameters(declaration) ?? []).entries()) {
        ...
        if (typeNode !== undefined && typeNode.Kind === KindLiteralType) {
            flags = (flags | SignatureFlagsHasLiteralTypes);
        }
    ...
    if ((IsConstructorTypeNode(declaration) && HasSyntacticModifier(declaration, ModifierFlagsAbstract))
        ...
        flags = (flags | SignatureFlagsAbstract);
    }
 ...
 * }
 */
export function Checker_getTypeParametersFromDeclaration(receiver, declaration) {
 ...
 * }
 */
export function Checker_getAnnotatedAccessorThisParameter(receiver, accessor) {
 ...
 * }
 */
export function Checker_getAccessorThisParameter(receiver, accessor) {
 ...
 * }
 */
export function Checker_hasLateBindableIndexSignature(receiver, node) {
 ...
 * }
 */
export function Checker_isLateBindableIndexSignature(receiver, node) {
    if (!isLateBindableAST(node)) {
        return false;
    }
    ...
    return Checker_isTypeUsableAsIndexSignatureDeclaration(receiver, Checker_checkExpressionCached(receiver, AsElementAccessExpression(node).ArgumentExpression));
}
/**
 ...
 * }
 */
export function Checker_isTypeUsableAsIndexSignatureDeclaration(receiver, t) {
 ...
 * }
 */
export function Checker_getReturnTypeOfSignature(receiver, sig) {
    ...
    if (sig.target !== undefined) {
        ...
        t = Checker_instantiateType(receiver, Checker_getUnionOrIntersectionType(receiver, core.Map(sig.composite.signatures, (signature) => Checker_getReturnTypeOfSignature(receiver, signature)), sig.composite.isUnion, UnionReductionSubtype), sig.mapper);
    }
    else {
 ...
 * }
 */
export function Checker_getNonCircularReturnTypeOfSignature(receiver, sig) {
 ...
 * }
 */
export function Checker_getReturnTypeFromAnnotation(receiver, declaration) {
    if (IsConstructorDeclaration(declaration)) {
 ...
 * }
 */
export function Checker_getSignatureOfFullSignatureType(receiver, node) {
 ...
 * }
 */
export function Checker_getParameterTypeOfFullSignature(receiver, node, parameter) {
 ...
 * }
 */
export function Checker_getReturnTypeOfFullSignature(receiver, node) {
    const signature = Checker_getSignatureOfFullSignatureType(receiver, node);
    if (signature !== undefined) {
        return Checker_getReturnTypeOfSignature(receiver, signature);
    }
 ...
 * }
 */
export function Checker_getReturnTypeFromBody(receiver, fn, checkMode) {
    ...
    let yieldType;
    let nextType;
    let fallbackReturnType = receiver.voidType;
    if (!IsBlock(body)) {
        returnType = Checker_checkExpressionCachedEx(receiver, body, (checkMode & ~CheckModeSkipGenericFunctions));
        ...
        if ((types ?? []).length === 0) {
            ...
            const emptyReturnType = contextualReturnType !== undefined &&
                someType(core.OrElse(Checker_unwrapReturnType(receiver, contextualReturnType, functionFlags), receiver.voidType), (t) => (t.flags & TypeFlagsUndefined) !== 0)
 ...
 * }
 */
export function Checker_createPromiseReturnType(receiver, fn, promisedType) {
    const promiseType = Checker_createPromiseType(receiver, promisedType);
    if (promiseType === receiver.unknownType) {
        Checker_error(receiver, fn, core.IfElse(IsImportCall(fn), A_dynamic_import_call_returns_a_Promise_Make_sure_you_have_a_declaration_for_Promise_or_include_ES2015_in_your_lib_option, An_async_function_or_method_must_return_a_Promise_Make_sure_you_have_a_declaration_for_Promise_or_include_ES2015_in_your_lib_option));
        return receiver.errorType;
    }
    if (receiver.getGlobalPromiseConstructorSymbol() === undefined) {
        Checker_error(receiver, fn, core.IfElse(IsImportCall(fn), A_dynamic_import_call_in_ES5_requires_the_Promise_constructor_Make_sure_you_have_a_declaration_for_the_Promise_constructor_or_include_ES2015_in_your_lib_option, An_async_function_or_method_in_ES5_requires_the_Promise_constructor_Make_sure_you_have_a_declaration_for_the_Promise_constructor_or_include_ES2015_in_your_lib_option));
    }
    return promiseType;
}
/**
 ...
 * }
 */
export function Checker_unwrapReturnType(receiver, returnType, functionFlags) {
    const isGenerator = (functionFlags & FunctionFlagsGenerator) !== 0;
    const isAsync = (functionFlags & FunctionFlagsAsync) !== 0;
    if (isGenerator) {
 ...
 * }
 */
export function Checker_getWidenedLiteralLikeTypeForContextualReturnTypeIfNeeded(receiver, t, contextualSignatureReturnType, isAsync) {
    if (t !== undefined && isUnitType(t)) {
        let contextualType = undefined;
 ...
 * }
 */
export function Checker_checkIfExpressionRefinesAnyParameter(receiver, fn, expr) {
 ...
 * }
 */
export function Checker_checkIfExpressionRefinesParameter(receiver, fn, expr, param, initType) {
    let antecedent = getFlowNodeOfNode(expr);
    if (antecedent === undefined && IsReturnStatement(expr.Parent)) {
        antecedent = getFlowNodeOfNode(expr.Parent);
    }
    if (antecedent === undefined) {
        antecedent = { Flags: FlowFlagsStart, Node: undefined, Antecedent: undefined, Antecedents: undefined };
    }
    const trueCondition = { Flags: FlowFlagsTrueCondition, Node: expr, Antecedent: antecedent, Antecedents: undefined };
    const trueType = Checker_getFlowTypeOfReferenceEx(receiver, Node_Name(param), initType, initType, fn, trueCondition);
    if (trueType === initType) {
        return undefined;
    }
    const falseCondition = { Flags: FlowFlagsFalseCondition, Node: expr, Antecedent: antecedent, Antecedents: undefined };
 ...
 * }
 */
export function Checker_instantiateSignature(receiver, sig, m) {
 ...
 * }
 */
export function Checker_instantiateSignatureEx(receiver, sig, m, eraseTypeParameters) {
    let freshTypeParameters = [];
    if (sig.typeParameters.length !== 0 && !eraseTypeParameters) {
        freshTypeParameters = core.Map(sig.typeParameters, (tp) => Checker_cloneTypeParameter(receiver, tp));
    ...
    const result = Checker_newSignature(receiver, (sig.flags & SignatureFlagsPropagatingFlags), sig.declaration, freshTypeParameters, Checker_instantiateSymbol(receiver, sig.thisParameter, m), Checker_instantiateSymbols(receiver, sig.parameters, m), undefined, undefined, sig.minArgumentCount);
    result.target = sig;
    result.mapper = m;
    return result;
}
 ...
 * }
 */
export function Checker_getDefaultConstructSignatures(receiver, classType) {
    ...
    if (baseSignatures.length === 0) {
        ...
        return [Checker_newSignature(receiver, flags, undefined, InterfaceType_LocalTypeParameters(Type_AsInterfaceType(classType)), undefined, [], classType, undefined, 0)];
    }
 ...
 * }
 */
export function Checker_getArrayMemberCallSignatures(receiver, t) {
    let memberName = "";
    const types = Type_Types(t);
    ...
    const arrayArg = Checker_mapType(receiver, t, (type_) => {
    ...
    const arrayType = Checker_createArrayTypeEx(receiver, arrayArg, someType(t, (type_) => Checker_isReadonlyArraySymbol(receiver, type_.symbol.Parent)));
 ...
 * }
 */
export function Checker_getUnionSignatures(receiver, signatureLists) {
    let result = [];
    let indexWithLengthOverOne = 0;
    let countLengthOverOne = 0;
    for (let i = 0; i < signatureLists.length; i++) {
        ...
        for (const signature of signatureLists[i]) {
            ...
            if (result.length === 0 || Checker_findMatchingSignature(receiver, result, signature, false, false, true) === undefined) {
                const unionSignatures = Checker_findMatchingSignatures(receiver, signatureLists, signature, i);
                if (unionSignatures !== undefined && unionSignatures.length !== 0) {
                    let s = signature;
                    // Union the result types when more than one signature matches
                    if (unionSignatures.length > 1) {
                        let thisParameter = signature.thisParameter;
                        const firstThisParameterOfUnionSignatures = core.FirstNonNil(unionSignatures, (sig) => {
                            return sig.thisParameter;
                        });
                        if (firstThisParameterOfUnionSignatures !== undefined) {
                            const thisType = Checker_getIntersectionType(receiver, core.MapNonNil(unionSignatures, (sig) => {
    ...
    if (result.length === 0 && countLengthOverOne <= 1) {
        ...
        for (const signatures of signatureLists) {
            if (!core.Same(signatures, masterList)) {
                ...
                if (signature.typeParameters.length !== 0 && core.Some(results, (s) => {
 ...
 * }
 */
export function Checker_combineUnionOrIntersectionMemberSignatures(receiver, left, right, isUnion) {
    ...
    let flags = ((left.flags | right.flags) & (SignatureFlagsPropagatingFlags & ~SignatureFlagsHasRestParameter));
 ...
 * }
 */
export function Checker_combineUnionOrIntersectionParameters(receiver, left, right, mapper, isUnion) {
    ...
    const params = new Array(longestCount + core.IfElse(needsExtraRestElement, 1, 0));
    for (let i = 0; i < longestCount; i++) {
        let longestParamType = Checker_tryGetTypeAtPosition(receiver, longest, i);
        if (longest === right) {
            longestParamType = Checker_instantiateType(receiver, longestParamType, mapper);
        }
        let shorterParamType = core.OrElse(Checker_tryGetTypeAtPosition(receiver, shorter, i), receiver.unknownType);
        if (shorter === right) {
        ...
        let rightName = "";
        if (i < leftCount) {
            leftName = Checker_getParameterNameAtPosition(receiver, left, i);
        }
        if (i < rightCount) {
            rightName = Checker_getParameterNameAtPosition(receiver, right, i);
        }
        let paramName = "";
        switch (true) {
            case leftName === rightName:
        ...
        const paramSymbol = Checker_newSymbolEx(receiver, (SymbolFlagsFunctionScopedVariable | core.IfElse(isOptional && !isRestParam, SymbolFlagsOptional, 0)), paramName, core.IfElse(isRestParam, CheckFlagsRestParameter, core.IfElse(isOptional, CheckFlagsOptionalParameter, 0)));
        const links = LinkStore_Get(receiver.valueSymbolLinks, paramSymbol);
        if (isRestParam) {
    ...
    if (needsExtraRestElement) {
        const restParamSymbol = Checker_newSymbolEx(receiver, SymbolFlagsFunctionScopedVariable, "args", CheckFlagsRestParameter);
        const links = LinkStore_Get(receiver.valueSymbolLinks, restParamSymbol);
 ...
 * }
 */
export function Checker_appendSignatures(receiver, signatures, newSignatures) {
    let result = signatures ?? [];
    for (const sig of (newSignatures ?? [])) {
        if (result.length === 0 || core.Every(result, (s) => Checker_compareSignaturesIdentical(receiver, s, sig, false, false, false, (source, target) => Checker_compareTypesIdentical(receiver, source, target)) === TernaryFalse)) {
            result = [...result, sig];
        }
    }
    return result;
 ...
 * }
 */
export function Checker_getTypeArguments(receiver, t) {
        ...
        if (node !== undefined) {
            switch (node.Kind) {
                case KindTypeReference:
                    typeArguments = [...InterfaceType_OuterTypeParameters(targetInterface), ...Checker_getEffectiveTypeArguments(receiver, node, InterfaceType_LocalTypeParameters(targetInterface))];
                    break;
                case KindArrayType:
                    typeArguments = [Checker_getTypeFromTypeNode(receiver, AsArrayTypeNode(node).ElementType)];
                    break;
                case KindTupleType:
                    typeArguments = core.Map(Node_Elements(node) ?? [], (element) => Checker_getTypeFromTypeNode(receiver, element));
                    break;
                default:
 ...
 * }
 */
export function Checker_getEffectiveTypeArguments(receiver, node, typeParameters) {
    return Checker_fillMissingTypeArguments(receiver, core.Map(Node_TypeArguments(node) ?? [], (n) => Checker_getTypeFromTypeNode(receiver, n)), typeParameters, Checker_getMinTypeArgumentCount(receiver, typeParameters), IsInJSFile(node));
}
/**
 ...
 * }
 */
export function Checker_getMinTypeArgumentCount(receiver, typeParameters) {
 ...
 * }
 */
export function Checker_hasTypeParameterDefault(receiver, t) {
    return t.symbol !== undefined && core.Some(t.symbol.Declarations ?? [], (d) => IsTypeParameterDeclaration(d) && AsTypeParameterDeclaration(d).DefaultType !== undefined);
}
/**
 ...
 * }
 */
export function Checker_fillMissingTypeArguments(receiver, typeArguments, typeParameters, minTypeArgumentCount, isJavaScriptImplicitAny) {
    ...
    if (isJavaScriptImplicitAny || numTypeArguments < numTypeParameters) {
        const result = new Array(numTypeParameters);
 ...
 * }
 */
export function Checker_getDefaultTypeArgumentType(receiver, isInJavaScriptFile) {
    if (isInJavaScriptFile) {
        return receiver.anyType;
    }
 ...
 * }
 */
export function Checker_getDefaultFromTypeParameter(receiver, t) {
    if ((t.flags & TypeFlagsTypeParameter) === 0) {
        return undefined;
    }
 ...
 * }
 */
export function Checker_getResolvedTypeParameterDefault(receiver, t) {
    ...
    if (d.resolvedDefaultType === undefined) {
        if (d.target !== undefined) {
            ...
            if (t.symbol !== undefined) {
                const defaultDeclaration = core.FirstNonNil(t.symbol.Declarations ?? [], (decl) => {
 ...
 * }
 */
export function Checker_getDefaultOrUnknownFromTypeParameter(receiver, t) {
 ...
 * }
 */
export function Checker_isTypeParameterPossiblyReferenced(receiver, tp, node) {
    const containsReference = (current) => {
        switch (current.Kind) {
            case KindThisType:
                return Type_AsTypeParameter(tp).isThisType;
            case KindTypeReference:
                if (!Type_AsTypeParameter(tp).isThisType &&
                    (Node_TypeArguments(current)?.length ?? 0) === 0 &&
            ...
            case KindTypeQuery: {
                ...
                if (!IsThisIdentifier(firstIdentifier)) {
                    ...
                    let typeParameterScope = undefined;
                    ...
                    if (typeParameterScope !== undefined) {
                        return (core.Some(firstIdentifierSymbol.Declarations ?? [], (declaration) => isNodeDescendantOf(declaration, typeParameterScope)) || core.Some(Node_TypeArguments(current), containsReference));
                    }
                }
                return true;
            }
            ...
            case KindMethodSignature: {
                ...
                return ((returnType === undefined && Node_Body(current) !== undefined) ||
                    core.Some(Node_TypeParameters(current), containsReference) ||
                    core.Some(Node_Parameters(current), containsReference) ||
 ...
 * }
 */
export function Checker_cloneTypeParameter(receiver, tp) {
 ...
 * }
 */
export function Checker_getTypeParameterFromMappedType(receiver, t) {
 ...
 * }
 */
export function Checker_forEachMappedTypePropertyKeyTypeAndIndexSignatureKeyType(receiver, t, include, stringsOnly, cb) {
 ...
 * }
 */
export function Checker_instantiateSignatures(receiver, signatures, m) {
 ...
 * }
 */
export function Checker_getTypeFromThisTypeNode(receiver, node) {
 ...
 * }
 */
export function Checker_getThisType(receiver, node) {
 ...
 * }
 */
export function Checker_getTypeFromTypeLiteralOrFunctionOrConstructorTypeNode(receiver, node) {
    const links = LinkStore_Get(receiver.typeNodeLinks, node);
    if (links.resolvedType === undefined) {
        // Deferred resolution of members is handled by resolveObjectTypeMembers
 ...
 * }
 */
export function Checker_getTypeArgumentsFromNode(receiver, node) {
    return core.Map(Node_TypeArguments(node) ?? [], (n) => Checker_getTypeFromTypeNode(receiver, n));
}
/**
 ...
 * }
 */
export function Checker_checkNoTypeArguments(receiver, node, symbol_) {
 ...
 * }
 */
export function Checker_getTypeReferenceArity(receiver, t) {
 ...
 * }
 */
export function Checker_getTypeArgumentsForAliasSymbol(receiver, symbol_) {
    if (symbol_ !== undefined) {
 ...
 * }
 */
export function Checker_getOuterTypeParametersOfClassOrInterface(receiver, symbol_) {
    let declaration = symbol_.ValueDeclaration;
    if ((symbol_.Flags & (SymbolFlagsClass | SymbolFlagsFunction)) === 0) {
        declaration = core.Find(symbol_.Declarations ?? [], (d) => {
 ...
 * }
 */
export function Checker_getOuterTypeParameters(receiver, node, includeThisTypes) {
    let cur = node;
    for (;;) {
        cur = cur.Parent;
        if (cur === undefined) {
        ...
        switch (kind) {
            ...
            case KindConditionalType: {
                ...
                if ((kind === KindFunctionExpression || kind === KindArrowFunction || IsObjectLiteralMethod(cur)) && Checker_isContextSensitive(receiver, cur)) {
                    ...
                    if (signature !== undefined && (signature.typeParameters ?? []).length !== 0) {
                        return [...(outerTypeParameters ?? []), ...signature.typeParameters];
                    }
                }
                if (kind === KindMappedType) {
                    return [...(outerTypeParameters ?? []), Checker_getDeclaredTypeOfTypeParameter(receiver, Checker_getSymbolOfDeclaration(receiver, AsMappedTypeNode(cur).TypeParameter))];
                }
                if (kind === KindConditionalType) {
                    return [...(outerTypeParameters ?? []), ...(Checker_getInferTypeParameters(receiver, cur) ?? [])];
                }
                ...
                if (thisType !== undefined) {
                    return [...(outerAndOwnTypeParameters ?? []), thisType];
                }
                return outerAndOwnTypeParameters;
            }
        }
 ...
 * }
 */
export function Checker_getInferTypeParameters(receiver, node) {
    let result = [];
 ...
 * }
 */
export function Checker_getLocalTypeParametersOfClassOrInterfaceOrTypeAlias(receiver, symbol_) {
 ...
 * }
 */
export function Checker_appendLocalTypeParametersOfClassOrInterfaceOrTypeAlias(receiver, types, symbol_) {
    let result = types;
 ...
 * }
 */
export function Checker_appendTypeParameters(receiver, typeParameters, declarations) {
 ...
 * }
 */
export function Checker_getDeclaredTypeOfTypeParameter(receiver, symbol_) {
    const links = LinkStore_Get(receiver.declaredTypeLinks, symbol_);
 ...
 * }
 */
export function Checker_getTypeFromRestTypeNode(receiver, node) {
 ...
 * }
 */
export function Checker_getRestrictiveTypeParameter(receiver, t) {
    const typeParameter = Type_AsTypeParameter(t);
    ...
    const key = { kind: CachedTypeKindRestrictiveTypeParameter, typeId: t.id };
 ...
 * }
 */
export function Checker_getRestTypeOfTupleType(receiver, t) {
 ...
 * }
 */
export function Checker_getUniqueLiteralTypeForTypeParameter(receiver, t) {
    if ((t.flags & TypeFlagsTypeParameter) !== 0) {
 ...
 * }
 */
export function Checker_newTypeParameter(receiver, symbol_) {
    const data = {
 ...
 * }
 */
export function Checker_newSignature(receiver, flags, declaration, typeParameters, thisParameter, parameters, resolvedReturnType, resolvedTypePredicate, minArgumentCount) {
 ...
 * }
 */
export function Checker_getSuggestionForNonexistentIndexSignature(receiver, objectType, expr, keyedType) {
    const hasProp = (name) => {
 ...
 * }
 */
export function Checker_isThisPropertyAccessInConstructor(receiver, node, prop) {
 ...
 * }
 */
export function Checker_getDeclaringConstructor(receiver, symbol_) {
    for (const declaration of symbol_.Declarations ?? []) {
 ...
 * }
 */
export function Checker_isStringIndexSignatureOnlyTypeWorker(receiver, t) {
    return ((t.flags & TypeFlagsObject) !== 0 &&
        ...
        ((t.flags & TypeFlagsUnionOrIntersection) !== 0 && core.Every(Type_Types(t), (type_) => receiver.isStringIndexSignatureOnlyType(type_)));
}
/**
 ...
 * }
 */
export function Checker_expandSignatureParametersWithTupleMembers(receiver, signature, restType, restIndex, restSymbol) {
    ...
    const expanded = signature.parameters.slice(0, restIndex);
 ...
 * }
 */
export function Checker_typeHasCallOrConstructSignatures(receiver, t) {
 ...
 * }
 */
export function Checker_getParameterTypeNodeForDecoratorCheck(receiver, node) {
 ...
 * }
 */
export function Checker_getTypeOfFirstParameterOfSignature(receiver, signature) {
 ...
 * }
 */
export function Checker_getTypeOfFirstParameterOfSignatureWithFallback(receiver, signature, fallbackType) {
 ...
 * }
 */
export function Checker_getTypeOfPropertyOrIndexSignatureOfType(receiver, t, name) {
 ...
 * }
 */
export function Checker_getContextuallyTypedParameterType(receiver, parameter) {
    ...
    if (iife !== undefined) {
        ...
        const links = LinkStore_Get(receiver.signatureLinks, iife);
        const cached = links.resolvedSignature;
...
function Checker_getSpreadArgumentTypeWithSelectedArgumentTypes(receiver, args, index, argCount, restType, context, checkMode, selectedArguments) {
    ...
    if (argCount > 0 && index >= argCount - 1) {
        ...
        if (isSpreadArgument(arg)) {
            ...
            if (IsSyntheticExpression(arg)) {
                spreadType = AsSyntheticExpression(arg).Type;
            }
            else {
    ...
    for (let argIndex = index; argIndex < argCount; argIndex++) {
        ...
        let t;
        const info = { flags: ElementFlagsRequired, labeledDeclaration: undefined };
        if (isSpreadArgument(arg)) {
            let spreadType;
            if (IsSyntheticExpression(arg)) {
                spreadType = AsSyntheticExpression(arg).Type;
    ...
    return Checker_createTupleTypeEx(receiver, types, infos, inConstContext && !someType(restType, (t) => Checker_isMutableArrayLikeType(receiver, t)));
}
/**
 ...
 * }
 */
export function Checker_getContextualReturnType(receiver, functionDecl, contextFlags) {
    ...
    if (signature !== undefined && !Checker_isResolvingReturnTypeOfSignature(receiver, signature)) {
        ...
        if ((functionFlags & FunctionFlagsGenerator) !== 0) {
            return Checker_filterType(receiver, returnType, (type_) => (((type_.flags & (TypeFlagsAnyOrUnknown | TypeFlagsVoid | TypeFlagsInstantiableNonPrimitive)) !== 0) ||
                Checker_checkGeneratorInstantiationAssignabilityToReturnType(receiver, type_, functionFlags, undefined)));
        }
        if ((functionFlags & FunctionFlagsAsync) !== 0) {
            return Checker_filterType(receiver, returnType, (type_) => (((type_.flags & (TypeFlagsAnyOrUnknown | TypeFlagsVoid | TypeFlagsInstantiableNonPrimitive)) !== 0) ||
                Checker_getAwaitedTypeOfPromise(receiver, type_) !== undefined));
        }
        return returnType;
    }
 ...
 * }
 */
export function Checker_checkGeneratorInstantiationAssignabilityToReturnType(receiver, returnType, functionFlags, errorNode) {
 ...
 * }
 */
export function Checker_getContextualSignatureForFunctionLikeDeclaration(receiver, node) {
 ...
 * }
 */
export function Checker_getContextualTypeForArgument(receiver, callTarget, arg) {
 ...
 * }
 */
export function Checker_getContextualTypeForArgumentAtIndex(receiver, callTarget, argIndex) {
    ...
    let signature;
    if (LinkStore_Get(receiver.signatureLinks, callTarget).resolvedSignature === receiver.resolvingSignature) {
...
function Checker_getEffectiveCallArgumentsWithOrigins(receiver, node, output) {
    ...
    if (IsTaggedTemplateExpression(node)) {
        ...
        const args = new globalThis.Array(spans.length + 1);
        args[0] = firstArg;
 ...
 * }
 */
export function Checker_getSpreadArgumentIndex(receiver, args) {
    return core.FindIndex(args, isSpreadArgument);
}
/**
 ...
 * }
 */
export function Checker_getEffectiveDecoratorArguments(receiver, node) {
    const expr = Node_Expression(node);
    const signature = Checker_getDecoratorCallSignature(receiver, node);
    if (signature !== undefined) {
        const args = new Array(signature.parameters.length);
 ...
 * }
 */
export function Checker_getDecoratorCallSignature(receiver, decorator) {
 ...
 * }
 */
export function Checker_getLegacyDecoratorCallSignature(receiver, decorator) {
    ...
    if (links.decoratorSignature === undefined) {
        ...
        switch (node.Kind) {
            ...
            case KindClassExpression: {
                ...
                links.decoratorSignature = Checker_newCallSignature(receiver, [], undefined, [targetParam], Checker_getUnionType(receiver, [targetType, receiver.voidType]));
                break;
            }
            case KindParameter: {
                ...
                links.decoratorSignature = Checker_newCallSignature(receiver, [], undefined, [targetParam, keyParam, indexParam], receiver.voidType);
                break;
            }
            case KindMethodDeclaration:
            case KindGetAccessor:
            ...
            case KindPropertyDeclaration: {
                ...
                if (hasPropDesc) {
                    ...
                    links.decoratorSignature = Checker_newCallSignature(receiver, [], undefined, [targetParam, keyParam, descriptorParam], Checker_getUnionType(receiver, [returnType, receiver.voidType]));
                }
                else {
                    links.decoratorSignature = Checker_newCallSignature(receiver, [], undefined, [targetParam, keyParam], Checker_getUnionType(receiver, [returnType, receiver.voidType]));
                }
                break;
            }
        }
 ...
 * }
 */
export function Checker_getESDecoratorCallSignature(receiver, decorator) {
    const node = decorator.Parent;
    const links = LinkStore_Get(receiver.signatureLinks, node);
    if (links.decoratorSignature === undefined) {
        links.decoratorSignature = receiver.anySignature;
 ...
 * }
 */
export function Checker_newESDecoratorCallSignature(receiver, targetType, contextType, nonOptionalReturnType) {
    const targetParam = Checker_newParameter(receiver, "target", targetType);
    const contextParam = Checker_newParameter(receiver, "context", contextType);
    const returnType = Checker_getUnionType(receiver, [nonOptionalReturnType, receiver.voidType]);
    return Checker_newCallSignature(receiver, [], undefined, [targetParam, contextParam], returnType);
}
/**
 ...
 * }
 */
export function Checker_newCallSignature(receiver, typeParameters, thisParameter, parameters, returnType) {
 ...
 * }
 */
export function Checker_getIndexSignaturesAtLocation(receiver, node) {
    let signatures = [];
 ...
 * }
 */
export function Checker_isThisPropertyAndThisTyped(receiver, node) {
 ...
 * }
 */
export function Checker_getThisTypeOfObjectLiteralFromContextualType(receiver, containingLiteral, contextualType) {
 ...
 * }
 */
export function Checker_getThisTypeFromContextualType(receiver, t) {
 ...
 * }
 */
export function Checker_getThisTypeArgument(receiver, t) {
 ...
 * }
 */
export function Checker_getApplicableIndexInfos(receiver, t, keyType) {
    return core.Filter(Checker_getIndexInfosOfType(receiver, t), (info) => Checker_isApplicableIndexType(receiver, keyType, info.keyType));
}
/**
...
export function Checker_getApplicableIndexSymbol(receiver, t, keyType) {
    ...
    if (info.indexSymbol === undefined) {
        ...
        if ((declarations ?? []).length !== 0) {
            ...
            LinkStore_Get(receiver.valueSymbolLinks, symbol_).resolvedType = info.valueType;
 ...
 * }
 */
export function Checker_containsArgumentsReference(receiver, node) {
