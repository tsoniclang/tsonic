...
export function Checker_maybeMappedType(receiver, node, symbol_) {
    ...
    if (IsTypeLiteralNode(node) && (Node_Members(node) ?? []).length === 1) {
        ...
        return ((t.flags & TypeFlagsUnion) !== 0 && Checker_allTypesAssignableToKind(receiver, t, TypeFlagsStringOrNumberLiteral));
    }
    return false;
}
/**
    ...
    if ((tupleTarget.combinedFlags & ElementFlagsVariadic) !== 0) {
        for (let index = 0; index < elementTypes.length; index++) {
            ...
            if (index < tupleTarget.elementInfos.length &&
                ...
                const checkTypes = core.MapIndex(elementTypes, (type_, typeIndex) => {
                ...
                if (Checker_checkCrossProductUnion(receiver, checkTypes)) {
                    return Checker_mapType(receiver, elementType, (type_) => Checker_createNormalizedTupleType(receiver, target, core.ReplaceElement(elementTypes, index, type_)));
                }
            }
        }
    }
...
export function Checker_getNumberLiteralType(receiver, value) {
    ...
    const isNaNKey = globalThis.Number.isNaN(value);
