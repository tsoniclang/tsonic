...
export function Checker_checkAssertion(receiver, node, checkMode) {
    if (node.Kind === KindTypeAssertionExpression) {
        if (Checker_shouldCheckErasableSyntax(receiver, node)) {
            ...
            DiagnosticsCollection_Add(receiver.diagnostics, NewDiagnostic(sourceFile, NewTextRange(SkipTrivia(SourceFile_Text(sourceFile), Node_Pos(node)), Node_Pos(Node_Expression(node))), This_syntax_is_not_allowed_when_erasableSyntaxOnly_is_enabled));
        }
    }
