...
import { NodeBuilderImpl_checkTypeExpandability, NodeBuilderImpl_getTypeFromTypeNode, NodeBuilderImpl_newIdentifier, NodeBuilderImpl_serializeTypeName, NodeBuilderImpl_setTextRange, NodeBuilderImpl_typeParameterToName, NodeBuilderImpl_typeToTypeNode, } from "./nodebuilderimpl.js";
 ...
 * }
 */
export function NodeBuilderImpl_reuseName(receiver, node) {
...
export function getExistingNodeTreeVisitor(b, bound) {
    ...
    const tryVisitIndexedAccess = (node) => {
        ...
        return NodeBuilderImpl_setTextRange(b, NewIndexedAccessTypeNode(b.f, resultObjectType, visitNode(indexed.IndexType)), node);
    };
    const tryVisitKeyOf = (node) => {
        ...
        return NodeBuilderImpl_setTextRange(b, NewTypeOperatorNode(b.f, typeOperator.Operator, typeNode), node);
    };
    const tryVisitTypeQuery = (node) => {
        ...
        if (!introducesError) {
            return NodeBuilderImpl_setTextRange(b, NewTypeQueryNode(b.f, exprName, visitNodes(typeQuery.TypeArguments)), node);
        }
    ...
    const tryVisitTypeReference = (node) => {
        ...
        if (!introducesError) {
            return NodeBuilderImpl_setTextRange(b, NewTypeReferenceNode(b.f, newName, typeArguments), node);
        }
    ...
    const visitExistingNodeTreeSymbolsWorker = (node) => {
        ...
        if (IsLiteralImportTypeNode(node)) {
            ...
            return NewImportTypeNode(factory, importType.IsTypeOf, arg, visitNode(importType.Attributes), visitNode(importType.Qualifier), visitNodes(importType.TypeArguments));
        }
        ...
        if ((IsFunctionLike(node) && Node_Type(node) === undefined) ||
            ...
            if (IsCallSignatureDeclaration(visited)) {
                ...
                return NewCallSignatureDeclaration(factory, call.TypeParameters, call.Parameters, newType);
            }
            if (IsJSDocSignature(visited)) {
                const jsdocSig = AsJSDocSignature(visited);
                return NewJSDocSignature(factory, jsdocSig.TypeParameters, jsdocSig.Parameters, newType);
            }
            if (IsConstructSignatureDeclaration(visited)) {
                const construct = AsConstructSignatureDeclaration(visited);
                return NewConstructSignatureDeclaration(factory, construct.TypeParameters, construct.Parameters, newType);
            }
            ...
            if (IsFunctionTypeNode(visited)) {
                ...
                return NewFunctionTypeNode(factory, fn.TypeParameters, fn.Parameters, newType);
            }
        ...
        if (IsComputedPropertyName(node) && IsEntityNameExpression(AsComputedPropertyName(node).Expression)) {
            ...
            if (!introducesError) {
                return NewComputedPropertyName(factory, result);
            }
            recoveryBoundary_markError(bound, undefined);
            return visitEachChild(node);
        }
        if (IsTypePredicateNode(node)) {
            ...
            return NodeBuilderImpl_setTextRange(b, NewTypePredicateNode(factory, visitNode(predicate.AssertsModifier), parameterName, visitNode(predicate.Type)), node);
        }
        if (IsConditionalTypeNode(node)) {
            ...
            return NewConditionalTypeNode(factory, checkType, extendsType, trueType, falseType);
        }
