...
import { PseudoObjectElement_AsPseudoGetAccessor, PseudoObjectElement_AsPseudoObjectMethod, PseudoObjectElement_AsPseudoPropertyAssignment, PseudoObjectElement_AsPseudoSetAccessor, PseudoObjectElement_Signature, PseudoObjectElementKindGetAccessor, PseudoObjectElementKindMethod, PseudoObjectElementKindPropertyAssignment, PseudoObjectElementKindSetAccessor, PseudoType_AsPseudoTypeDirect, PseudoType_AsPseudoTypeInferred, PseudoType_AsPseudoTypeLiteral, PseudoType_AsPseudoTypeMaybeConstLocation, PseudoType_AsPseudoTypeNoResult, PseudoType_AsPseudoTypeObjectLiteral, PseudoType_AsPseudoTypeSingleCallSignature, PseudoType_AsPseudoTypeTuple, PseudoType_AsPseudoTypeUnion, PseudoTypeKindAny, PseudoTypeKindBigInt, PseudoTypeKindBigIntLiteral, PseudoTypeKindBoolean, PseudoTypeKindDirect, PseudoTypeKindFalse, PseudoTypeKindInferred, PseudoTypeKindMaybeConstLocation, PseudoTypeKindNull, PseudoTypeKindNoResult, PseudoTypeKindNumber, PseudoTypeKindNumericLiteral, PseudoTypeKindObjectLiteral, PseudoTypeKindSingleCallSignature, PseudoTypeKindString, PseudoTypeKindStringLiteral, PseudoTypeKindTrue, PseudoTypeKindTuple, PseudoTypeKindUndefined, PseudoTypeKindUnion, } from "../pseudochecker/type.js";
...
import { Checker_getSignatureFromDeclaration, Checker_getSingleCallSignature, Checker_getTypeArguments, Checker_getTypeOfParameter, Checker_getReturnTypeOfSignature, } from "./checker/signatures.js";
import { Checker_getPropertyOfType, Checker_getTypeOfSymbol, Checker_getWriteTypeOfSymbol, } from "./checker/symbols.js";
import { Checker_getContextualType, Checker_getPropertiesOfType, Checker_getRegularTypeOfExpression, Checker_getRegularTypeOfLiteralType, Checker_getTypeOfExpression, Checker_getTypeFromTypeNode, Checker_getTypeWithFacts, Checker_getUnionType, Checker_getWidenedType, Checker_instantiateContextualType, Checker_isLiteralOfContextualType, Checker_removeMissingType, } from "./checker/types.js";
 ...
 * }
 */
export function NodeBuilderImpl_pseudoTypeToNodeWithCheckerFallback(receiver, t, checkerType) {
    const b = receiver;
 ...
 * }
 */
export function NodeBuilderImpl_pseudoTypeToNode(receiver, t) {
    const b = receiver;
            ...
            case PseudoTypeKindMaybeConstLocation: {
                ...
                if (!isInConstContext && IsInConstContext(d.Node)) {
                    ...
                    if (typeFromPseudo !== undefined &&
                        Checker_isLiteralOfContextualType(b.ch, typeFromPseudo, Checker_instantiateContextualType(b.ch, contextualType, d.Node, ContextFlagsNone))) {
                        isInConstContext = true;
                    }
                }
            ...
            case PseudoTypeKindSingleCallSignature: {
                ...
                if (d.TypeParameters.length > 0) {
                    ...
                    for (const tp of d.TypeParameters) {
                        res.push(NodeBuilderImpl_reuseNode(b, tp));
                    }
                ...
                const fnResult = NewFunctionTypeNode(b.f, typeParams, params, returnType);
                cleanup();
                return fnResult;
            }
                ...
                for (const element of elements) {
                    ...
                    switch (elementNode.Kind) {
                        case PseudoObjectElementKindMethod: {
                            ...
                            if (isConst) {
                                newProp = NewPropertySignatureDeclaration(b.f, modifiers, NodeBuilderImpl_reuseName(b, elementNode.Name, false), undefined, NewFunctionTypeNode(b.f, typeParams, NodeBuilderImpl_pseudoParametersToNodeList(b, d.Parameters), NodeBuilderImpl_pseudoTypeToNode(b, d.ReturnType)), undefined);
                                break;
                            }
                            newProp = NewMethodSignatureDeclaration(b.f, modifiers, NodeBuilderImpl_reuseName(b, elementNode.Name, true), undefined, typeParams, NodeBuilderImpl_pseudoParametersToNodeList(b, d.Parameters), NodeBuilderImpl_pseudoTypeToNode(b, d.ReturnType));
                            break;
                        }
                        case PseudoObjectElementKindPropertyAssignment: {
                            const d = PseudoObjectElement_AsPseudoPropertyAssignment(elementNode);
                            newProp = NewPropertySignatureDeclaration(b.f, modifiers, NodeBuilderImpl_reuseName(b, elementNode.Name, false), undefined, NodeBuilderImpl_pseudoTypeToNode(b, d.Type), undefined);
                            break;
                        }
                        case PseudoObjectElementKindSetAccessor: {
                            const d = PseudoObjectElement_AsPseudoSetAccessor(elementNode);
                            newProp = NewSetAccessorDeclaration(b.f, undefined, NodeBuilderImpl_reuseName(b, elementNode.Name, false), undefined, NodeFactory_NewNodeList(b.f, [NodeBuilderImpl_pseudoParameterToNode(b, d.Parameter)]), undefined, undefined, undefined);
                            break;
                        }
                        case PseudoObjectElementKindGetAccessor: {
                            const d = PseudoObjectElement_AsPseudoGetAccessor(elementNode);
                            newProp = NewGetAccessorDeclaration(b.f, undefined, NodeBuilderImpl_reuseName(b, elementNode.Name, false), undefined, undefined, NodeBuilderImpl_pseudoTypeToNode(b, d.Type), undefined, undefined);
                            break;
                        }
                    }
 ...
 * }
 */
export function NodeBuilderImpl_pseudoParametersToNodeList(receiver, params) {
    const b = receiver;
    const res = [];
 ...
 * }
 */
export function NodeBuilderImpl_pseudoParameterToNode(receiver, p) {
    const b = receiver;
    let dotDotDot = undefined;
    ...
    const parameter = NewParameterDeclaration(b.f, undefined, dotDotDot, 
    // matches strada behavior of always reserializing param names from scratch
    NodeBuilderImpl_parameterToParameterDeclarationName(b, Node_Symbol(p.Name.Parent), p.Name.Parent), questionMark, NodeBuilderImpl_pseudoTypeToNode(b, p.Type), undefined);
 ...
 * }
 */
export function NodeBuilderImpl_pseudoTypeEquivalentToType(receiver, t, type_, isOptionalAnnotated, reportErrors) {
    const b = receiver;
 ...
 * }
 */
export function NodeBuilderImpl_pseudoParametersEquivalentToParameters(receiver, params, targetSig, reportErrors, nonParamErrorLocation) {
    const b = receiver;
    ...
    if (targetSig.thisParameter !== undefined && remainingParams.length === 0) {
        ...
        if (!NodeBuilderImpl_pseudoTypeEquivalentToType(b, remainingParams[0].Type, paramType, remainingParams[0].Optional, false)) {
            if (reportErrors) {
    ...
    for (let i = 0; i < remainingParams.length; i++) {
        ...
        const paramType = Checker_getTypeOfParameter(b.ch, targetParam);
        if (!NodeBuilderImpl_pseudoTypeEquivalentToType(b, p.Type, paramType, p.Optional, false)) {
            if (reportErrors) {
                b.ctx.tracker.ReportInferenceFallback(p.Name.Parent);
            }
            return false;
 ...
 * }
 */
export function isStructuralPseudoType(t) {
    switch (t.Kind) {
        case PseudoTypeKindObjectLiteral:
        case PseudoTypeKindTuple:
        case PseudoTypeKindSingleCallSignature:
 ...
 * }
 */
export function NodeBuilderImpl_pseudoReturnTypeMatchesPredicate(receiver, rt, predicate) {
    const b = receiver;
 ...
 * }
 */
export function NodeBuilderImpl_pseudoTypeToType(receiver, t) {
    const b = receiver;
    // !!! TODO: only literal types currently mapped because this is only used to determine if literal contextual typing need apply to the pseudotype
    // If this is used more broadly, the implementation needs to be filled out more to handle the structural pseudotypes - signatures, objects, tuples, etc
...
//# sourceMappingURL=pseudotypenodebuilder.js.map