...
import { ForEachDynamicImportOrRequireCall, GetExternalModuleName, HasSyntacticModifier, IsAmbientModule, IsAnyImportOrReExport, IsExternalModule, IsInJSFile, SetImportsOfSourceFile, } from "../ast/utilities.js";
 ...
 * }
 */
export function collectExternalModuleReferences(file) {
    ...
    if ((sf.Flags & NodeFlagsPossiblyContainsDynamicImport) !== 0 || IsInJSFile(file)) {
        ForEachDynamicImportOrRequireCall(sf, /*includeTypeSpaceImports*/ true, /*requireStringLiteralLikeArgument*/ true, (node, moduleSpecifier) => {
 ...
 * }
 */
export function collectModuleReferences(file, node, inAmbientModule) {
    const sf = AsSourceFile(file);
