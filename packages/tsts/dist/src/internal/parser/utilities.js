...
import { KindArrowFunction, KindExportSpecifier, KindFunctionExpression, KindGreaterThanToken, KindIdentifier, KindParameter, KindParenthesizedExpression, KindTypeParameter, KindVariableDeclaration, } from "../ast/generated/kinds.js";
...
export const lastNewlineBefore = (text, end) => {
 ...
 * }
 */
export function getLanguageVariant(scriptKind) {
    switch (scriptKind) {
        case ScriptKindTSX:
        case ScriptKindJSX:
        case ScriptKindJS:
 ...
 * }
 */
export function GetJSDocCommentRanges(f, commentRanges, node, text) {
    switch (node.Kind) {
        case KindParameter:
        ...
        case KindExportSpecifier:
            GetTrailingCommentRanges(f, text, Node_Pos(node))((commentRange) => {
                commentRanges.push(commentRange);
                return true;
            });
            GetLeadingCommentRanges(f, text, Node_Pos(node))((commentRange) => {
                commentRanges.push(commentRange);
                return true;
            });
            break;
    ...
    // Keep if the comment starts with '/**' but not if it is '/**/'
    return DeleteFunc(commentRanges, (comment) => {
        ...
        return (TextRange_End(comment) > Node_End(node) ||
            ...
            byteAt(text, commentStart + 3) === 0x2f /* '/' */);
    });
}
/**
 ...
 * }
 */
export function isKeywordOrPunctuation(token) {
 ...
 * }
 */
export function isJSDocLikeText(text) {
    return (byteLen(text) >= 4 && byteAt(text, 1) === 0x2a /* '*' */ && byteAt(text, 2) === 0x2a /* '*' */ && byteAt(text, 3) !== 0x2f /* '/' */);
}
