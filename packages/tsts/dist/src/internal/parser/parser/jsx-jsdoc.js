...
import { NewBinaryExpression, NewJsxAttribute, NewJsxAttributes, NewJsxClosingElement, NewJsxClosingFragment, NewJsxElement, NewJsxExpression, NewJsxFragment, NewJsxNamespacedName, NewJsxOpeningElement, NewJsxOpeningFragment, NewJsxSelfClosingElement, NewJsxSpreadAttribute, NewJsxText, NewJSDocAllType, NewJSDocNonNullableType, NewJSDocNullableType, NewJSDocOptionalType, NewJSDocVariadicType, NewKeywordExpression, NewPropertyAccessExpression, NewToken, } from "../../ast/generated/factory.js";
import { NodeFlagsJavaScriptFile, NodeFlagsNone } from "../../ast/generated/flags.js";
import { KindCloseBraceToken, KindColonToken, KindCommaToken, KindConflictMarkerTrivia, KindDotDotDotToken, KindDotToken, KindEndOfFile, KindEqualsToken, KindGreaterThanToken, KindJsxElement, KindJsxOpeningElement, KindJsxOpeningFragment, KindJsxSelfClosingElement, KindJsxText, KindJsxTextAllWhiteSpaces, KindLessThanSlashToken, KindLessThanToken, KindOpenBraceToken, KindSlashToken, KindStringLiteral, KindThisKeyword, } from "../../ast/generated/kinds.js";
...
import { Expected_corresponding_closing_tag_for_JSX_fragment, Expected_corresponding_JSX_closing_tag_for_0, JSX_element_0_has_no_corresponding_closing_tag, JSX_expressions_must_have_one_parent_element, JSX_fragment_has_no_corresponding_closing_tag, X_0_expected, X_or_JSX_element_expected, } from "../../diagnostics/generated/messages.js";
import { GetTextOfNodeFromSourceText, } from "../../scanner/utilities.js";
import { Scanner_ReScanJsxToken, Scanner_ScanJSDocCommentTextToken, Scanner_ScanJSDocToken, Scanner_ScanJsxAttributeValue, Scanner_ScanJsxIdentifier, Scanner_ScanJsxToken, Scanner_SetSkipJSDocLeadingAsterisks, Scanner_TokenValue, SkipTrivia, TokenToString, } from "../../scanner/scanner.js";
...
import { Parser_finishNode, Parser_nodePos, Parser_parseOptional, } from "./support.js";
...
import { PCJsxAttributes, PCJsxChildren, } from "./state.js";
import { Parser_parseExpectedWithDiagnostic, Parser_parseExpectedWithoutAdvancing, Parser_parseRightSideOfDot, } from "./statements-declarations.js";
import { Parser_nextToken, Parser_parseErrorAtCurrentToken, Parser_parseExpected, Parser_parseIdentifierNameErrorOnUnicodeEscapeSequence, Parser_parseOptionalToken, Parser_parseTokenNode, } from "./tokens-speculation.js";
import { Parser_parseNonArrayType, Parser_parseType, Parser_parseTypeArguments, Parser_parseTypeOrTypePredicate, } from "./types.js";
/**
 ...
 * }
 */
export function Parser_nextTokenJSDoc(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_nextJSDocCommentTextToken(receiver, inBackticks) {
    const p = receiver;
 ...
 * }
 */
export function Parser_createJSDocCache(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_parseExpectedJSDoc(receiver, kind) {
    const p = receiver;
    if (p.token === kind) {
 ...
 * }
 */
export function Parser_parseExpectedTokenJSDoc(receiver, kind) {
    const p = receiver;
    const optional = Parser_parseOptionalTokenJSDoc(p, kind);
 ...
 * }
 */
export function Parser_parseOptionalTokenJSDoc(receiver, kind) {
    const p = receiver;
    if (p.token === kind) {
 ...
 * }
 */
export function Parser_parseJSDocAllType(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    Parser_nextToken(p);
 ...
 * }
 */
export function Parser_parseJSDocNonNullableType(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    Parser_nextToken(p);
 ...
 * }
 */
export function Parser_parseJSDocNullableType(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    // skip the ?
    Parser_nextToken(p);
 ...
 * }
 */
export function Parser_parseJSDocType(receiver) {
    const p = receiver;
    Scanner_SetSkipJSDocLeadingAsterisks(p.scanner, true);
    const pos = Parser_nodePos(p);
    const hasDotDotDot = Parser_parseOptional(p, KindDotDotDotToken);
    const t = Parser_parseTypeOrTypePredicate(p);
    Scanner_SetSkipJSDocLeadingAsterisks(p.scanner, false);
    const tWithVariadic = hasDotDotDot
 ...
 * }
 */
export function Parser_parseJsxElementOrSelfClosingElementOrFragment(receiver, inExpressionContext, topInvalidNodePosition, openingTag, mustBeUnary) {
    const p = receiver;
    const pos = Parser_nodePos(receiver);
    const opening = Parser_parseJsxOpeningOrSelfClosingElementOrOpeningFragment(receiver, inExpressionContext);
    let result;
    switch (opening.Kind) {
        case KindJsxOpeningElement: {
            let children = Parser_parseJsxChildren(receiver, opening);
            let closingElement;
            ...
            if (lastChild !== undefined && lastChild.Kind === KindJsxElement &&
                ...
                // restructure (<div>(...<span>...</div>)) --> (<div>(...<span>...</>)</div>)
                // (no need to error; the parent will error)
                const end = NodeList_End(Node_Children(lastChild));
                ...
                const newLast = Parser_finishNodeWithEnd(receiver, NewJsxElement(p.factory, AsJsxElement(lastChild).OpeningElement, Node_Children(lastChild), newClosingElement), Node_Pos(AsJsxElement(lastChild).OpeningElement), end);
                // force reset parent pointers from discarded parse result
    ...
    if (!mustBeUnary && inExpressionContext && p.token === KindLessThanToken) {
        let topBadPos = topInvalidNodePosition;
        if (topBadPos < 0) {
            topBadPos = Node_Pos(result);
        }
        const invalidElement = Parser_parseJsxElementOrSelfClosingElementOrFragment(receiver, /*inExpressionContext*/ true, topBadPos, undefined, false);
 ...
 * }
 */
export function Parser_parseJsxChildren(receiver, openingTag) {
    const p = receiver;
    const pos = Parser_nodePos(receiver);
 ...
 * }
 */
export function Parser_parseJsxChild(receiver, openingTag, token) {
    const p = receiver;
    switch (token) {
        case KindEndOfFile:
            // If we hit EOF, issue the error at the tag that lacks the closing element
            ...
            if (IsJsxOpeningFragment(openingTag)) {
                ...
                const start = Math.min(SkipTrivia(p.sourceText, Node_Pos(tag)), Node_End(tag));
                Parser_parseErrorAt(receiver, start, Node_End(tag), JSX_element_0_has_no_corresponding_closing_tag, GetTextOfNodeFromSourceText(p.sourceText, Node_TagName(openingTag), false /*includeTrivia*/));
            }
            return undefined;
        ...
        case KindLessThanToken:
            return Parser_parseJsxElementOrSelfClosingElementOrFragment(receiver, false /*inExpressionContext*/, -1 /*topInvalidNodePosition*/, openingTag, false);
    }
 ...
 * }
 */
export function Parser_parseJsxText(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    const result = NewJsxText(p.factory, Scanner_TokenValue(p.scanner), p.token === KindJsxTextAllWhiteSpaces);
 ...
 * }
 */
export function Parser_parseJsxExpression(receiver, inExpressionContext) {
    const p = receiver;
    const pos = Parser_nodePos(p);
 ...
 * }
 */
export function Parser_scanJsxText(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_scanJsxIdentifier(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_scanJsxAttributeValue(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_parseJsxClosingElement(receiver, open, inExpressionContext) {
    const p = receiver;
    const pos = Parser_nodePos(receiver);
    Parser_parseExpected(receiver, KindLessThanSlashToken);
    const tagName = Parser_parseJsxElementName(receiver);
 ...
 * }
 */
export function Parser_parseJsxOpeningOrSelfClosingElementOrOpeningFragment(receiver, inExpressionContext) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    ...
    const tagName = Parser_parseJsxElementName(p);
    const typeArguments = (p.contextFlags & NodeFlagsJavaScriptFile) === 0 ? Parser_parseTypeArguments(p) : undefined;
    const attributes = Parser_parseJsxAttributes(p);
    if (p.token === KindGreaterThanToken) {
        // Closing tag, so scan the immediately-following text with the JSX scanning instead
        // of regular scanning to avoid treating illegal characters (e.g. '#') as immediate
        // scanning errors
 ...
 * }
 */
export function Parser_parseJsxElementName(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    // JsxElement can have name in the form of
    //      propertyAccessExpression
    ...
    // We can't just simply use parseLeftHandSideExpressionOrHigher because then we will start consider class,function etc as a keyword
    // We only want to consider "this" as a primaryExpression
    const initialExpression = Parser_parseJsxTagName(p);
    ...
    while (Parser_parseOptional(p, KindDotToken)) {
        expression = Parser_finishNode(p, NewPropertyAccessExpression(p.factory, expression, undefined, Parser_parseRightSideOfDot(p, true /*allowIdentifierNames*/, false /*allowPrivateIdentifiers*/, false /*allowUnicodeEscapeSequenceInIdentifierName*/), NodeFlagsNone), pos);
    }
    return expression;
}
/**
 ...
 * }
 */
export function Parser_parseJsxTagName(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    Parser_scanJsxIdentifier(p);
    const isThis = p.token === KindThisKeyword;
    const tagName = Parser_parseIdentifierNameErrorOnUnicodeEscapeSequence(p);
    ...
    if (isThis) {
        const result = NewKeywordExpression(p.factory, KindThisKeyword);
        return Parser_finishNode(p, result, pos);
    }
    return tagName;
}
 ...
 * }
 */
export function Parser_parseJsxAttributes(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
 ...
 * }
 */
export function Parser_parseJsxAttribute(receiver) {
    const p = receiver;
 ...
 * }
 */
export function Parser_parseJsxSpreadAttribute(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    Parser_parseExpected(p, KindOpenBraceToken);
    Parser_parseExpected(p, KindDotDotDotToken);
    const expression = Parser_parseExpression(p);
    Parser_parseExpected(p, KindCloseBraceToken);
 ...
 * }
 */
export function Parser_parseJsxAttributeName(receiver) {
    const p = receiver;
    const pos = Parser_nodePos(p);
    Parser_scanJsxIdentifier(p);
    const attrName = Parser_parseIdentifierNameErrorOnUnicodeEscapeSequence(p);
    if (Parser_parseOptional(p, KindColonToken)) {
        Parser_scanJsxIdentifier(p);
 ...
 * }
 */
export function Parser_parseJsxAttributeValue(receiver) {
    const p = receiver;
    if (p.token === KindEqualsToken) {
 ...
 * }
 */
export function Parser_parseJsxClosingFragment(receiver, inExpressionContext) {
    const p = receiver;
    const pos = Parser_nodePos(p);
...
//# sourceMappingURL=jsx-jsdoc.js.map