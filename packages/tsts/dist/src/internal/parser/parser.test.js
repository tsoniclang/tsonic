...
`;
    const file = ParseSourceFile({ FileName: "/index.js", Path: "/index.js" }, sourceText, ScriptKindJS);
    ...
    for (let i = 1; i < reparsedClones.length; i++) {
        ...
        assert.ok(!(Node_Pos(a) === Node_Pos(b) && Node_End(a) === Node_End(b) && a.Kind === b.Kind), `duplicate ReparsedClones at [${i - 1}] and [${i}]: kind=${a.Kind} pos=${Node_Pos(a)} end=${Node_End(a)}`);
    }
    for (const imp of SourceFile_Imports(file) ?? []) {
        const reparsed = GetReparsedNodeForNode(imp);
        assert.ok(GetSourceFileOfNode(reparsed) !== undefined, `reparsed import at pos=${Node_Pos(imp)} has broken parent chain`);
    }
});
