...
import { NewExpressionWithTypeArguments, NewJSDoc, NewJSDocAugmentsTag, NewJSDocCallbackTag, NewJSDocDeprecatedTag, NewJSDocImplementsTag, NewJSDocImportTag, NewJSDocLink, NewJSDocLinkCode, NewJSDocLinkPlain, NewJSDocNameReference, NewJSDocOverloadTag, NewJSDocOverrideTag, NewJSDocParameterOrPropertyTag, NewJSDocPrivateTag, NewJSDocProtectedTag, NewJSDocPublicTag, NewJSDocReadonlyTag, NewJSDocReturnTag, NewJSDocSatisfiesTag, NewJSDocSeeTag, NewJSDocSignature, NewJSDocTemplateTag, NewJSDocText, NewJSDocThisTag, NewJSDocThrowsTag, NewJSDocTypeLiteral, NewJSDocTypeExpression, NewJSDocTypeTag, NewJSDocTypedefTag, NewJSDocUnknownTag, NewModuleDeclaration, NewPropertyAccessExpression, NewQualifiedName, NewTypeParameterDeclaration, } from "../ast/generated/factory.js";
import { KindArrayType, KindAsteriskToken, KindAtToken, KindBacktickToken, KindCloseBraceToken, KindCloseBracketToken, KindCommaToken, KindDotToken, KindEqualsToken, KindEndOfFile, KindIdentifier, KindJSDocCommentTextToken, KindJSDocParameterTag, KindJSDocPropertyTag, KindJSDocReturnTag, KindJSDocTemplateTag, KindJSDocTypeTag, KindNamespaceKeyword, KindNewLineTrivia, KindObjectKeyword, KindOpenBraceToken, KindOpenBracketToken, KindPrivateIdentifier, KindTypeKeyword, KindWhitespaceTrivia, } from "../ast/generated/kinds.js";
import { NodeFlagsHasAsyncFunctions, NodeFlagsHasJSDoc, NodeFlagsIdentifierIsInJSDocNamespace, NodeFlagsJavaScriptFile, NodeFlagsJSDoc, NodeFlagsNestedNamespace, NodeFlagsNone, NodeFlagsOptionalChain, NodeFlagsPossiblyContainsDeprecatedTag, } from "../ast/generated/flags.js";
import { Some, IfElse } from "../core/core.js";
import { A_JSDoc_template_tag_may_not_follow_a_typedef_callback_or_overload_tag, A_JSDoc_typedef_comment_may_not_contain_multiple_type_tags, Identifier_expected, Identifier_expected_0_is_a_reserved_word_that_cannot_be_used_here, The_tag_was_first_specified_here, Unexpected_token_A_type_parameter_name_was_expected_without_curly_braces, X_0_tag_already_specified, } from "../diagnostics/generated/messages.js";
...
import { Scanner_CanFollowJSDocAt, Scanner_HasPrecedingLineBreak, Scanner_Mark, Scanner_Rewind, Scanner_ResetPos, Scanner_ReScanHashToken, Scanner_SetSkipJSDocLeadingAsterisks, Scanner_SetTextEnd, Scanner_TokenEnd, Scanner_TokenFullStart, Scanner_TokenStart, Scanner_TokenText, Scanner_TokenValue, } from "../scanner/scanner.js";
...
import { Parser_finishNode, Parser_initializeState, Parser_isJavaScript, Parser_lookAhead, Parser_mark, Parser_nodePos, Parser_parseModifiersEx, Parser_parseOptional, Parser_rewind, getParser, isReservedWord, putParser, } from "./parser/support.js";
...
import { Parser_createMissingIdentifier, Parser_internIdentifier, Parser_isIdentifier, Parser_newIdentifier, Parser_nextTokenIsIdentifierOrKeyword, Parser_parseErrorAtCurrentToken, Parser_parseExpected, Parser_parseIdentifier, Parser_parseIdentifierName, Parser_parseOptionalToken, Parser_setContextFlags, } from "./parser/tokens-speculation.js";
 ...
 * }
 */
export function parseJSDocForNode(sourceFile, node) {
    const sf = sourceFile;
    const p = getParser();
    try {
 ...
 * )
 */
export const jsdocStateBeginningOfLine = 0;
export const jsdocStateSawAsterisk = 1;
export const jsdocStateSavingComments = 2;
export const jsdocStateSavingBackticks = 3;
/**
 ...
 * )
 */
export const propertyLikeParseProperty = 1 << 0;
export const propertyLikeParseParameter = 1 << 1;
export const propertyLikeParseCallbackParameter = 1 << 2;
/**
 ...
 * }
 */
export function Parser_withJSDoc(receiver, node, info) {
    if ((info & jsdocScannerInfoHasJSDoc) === 0) {
        return undefined;
    }
    // For TS/TSX files, defer JSDoc parsing to first access, unless the comment
    ...
    const jsdoc = Arena_NewSlice(receiver.nodeSliceArena, ranges.length).slice(0, 0);
    let pos = Node_Pos(node);
    for (const comment of ranges) {
    ...
    if (jsdoc.length !== 0) {
        ...
        receiver.jsdocInfos = [...receiver.jsdocInfos, { parent: node, jsDocs: jsdoc }];
        return jsdoc;
    }
    return undefined;
}
 ...
 * }
 */
export function Parser_parseJSDocTypeExpression(receiver, mayOmitBraces) {
 ...
 * }
 */
export function Parser_parseJSDocNameReference(receiver) {
    const pos = Parser_nodePos(receiver);
 ...
 * }
 */
export function Parser_parseJSDocComment(receiver, parent, start, end, fullStart) {
    if (end === -1) {
    ...
    const saveScannerState = Scanner_Mark(receiver.scanner);
 ...
 * }
 */
export function Parser_parseJSDocCommentWorker(receiver, start, end, fullStart, indent) {
    // Initially we can parse out a tag.  We also have seen a starting asterisk.
    // This is so that /** * @type */ doesn't parse.
    let tags = Arena_NewSlice(receiver.nodeSliceArena, 1).slice(0, 0);
    let tagsPos = -1;
    let tagsEnd = -1;
    let state = jsdocStateSawAsterisk;
    let backtickCount = 0;
    let inFencedCodeBlock = false;
    let commentParts = Arena_NewSlice(receiver.nodeSliceArena, 1).slice(0, 0);
    let comments = receiver.jsdocCommentsSpace;
    let commentsPos = -1;
    let linkEnd = start;
    let margin = -1;
        ...
        switch (receiver.token) {
            ...
            case KindOpenBraceToken: {
                ...
                if (link !== undefined) {
                    ...
                    const jsdocText = Parser_finishNodeWithEnd(receiver, NewJSDocText(receiver.factory, Arena_Clone(receiver.stringSliceArena, comments)), linkEnd, commentEnd);
    ...
    if (comments.length > 0) {
        ...
        const jsdocText = Parser_finishNodeWithEnd(receiver, NewJSDocText(receiver.factory, Arena_Clone(receiver.stringSliceArena, comments)), linkEnd, commentsPos);
        commentParts.push(jsdocText);
    }
    let tagsNodeList;
    if (tagsPos !== -1) {
 ...
 * }
 */
export function removeLeadingNewlines(comments) {
    let i = 0;
...
export function trimEnd(s) {
    return TrimRightFunc(s, (r) => IsWhiteSpaceLike(r));
}
/**
 ...
 * }
 */
export function removeTrailingWhitespace(comments) {
 ...
 * }
 */
export function Parser_isNextNonwhitespaceTokenEndOfFile(receiver) {
    for (;;) {
        Parser_nextTokenJSDoc(receiver);
 ...
 * }
 */
export function Parser_skipWhitespace(receiver) {
    if (receiver.token === KindWhitespaceTrivia || receiver.token === KindNewLineTrivia) {
        if (Parser_lookAhead(receiver, (p) => Parser_isNextNonwhitespaceTokenEndOfFile(p))) {
            return;
            // Don't skip whitespace prior to EoF (or end of comment) - that shouldn't be included in any node's range
        }
    }
 ...
 * }
 */
export function Parser_skipWhitespaceOrAsterisk(receiver) {
    if (receiver.token === KindWhitespaceTrivia || receiver.token === KindNewLineTrivia) {
        if (Parser_lookAhead(receiver, (p) => Parser_isNextNonwhitespaceTokenEndOfFile(p))) {
            return "";
            // Don't skip whitespace prior to EoF (or end of comment) - that shouldn't be included in any node's range
 ...
 */
export function Parser_parseTag(receiver, tags, margin) {
    ...
    switch (Node_Text(tagName)) {
        ...
        case "extends":
            ...
            break;
        case "public":
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocPublicTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "private":
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocPrivateTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "protected":
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocProtectedTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "readonly":
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocReadonlyTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "override":
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocOverrideTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "deprecated":
            receiver.hasDeprecatedTag = true;
            tag = Parser_parseSimpleTag(receiver, start, (tn, comments) => NewJSDocDeprecatedTag(receiver.factory, tn, comments), tagName, margin, indentText);
            break;
        case "this":
 ...
 * }
 */
export function Parser_parseTrailingTagComments(receiver, pos, end, margin, indentText) {
    // some tags, like typedef and callback, have already parsed their comments earlier
    let m = margin;
 ...
 */
export function Parser_parseTagComments(receiver, indent, initialMargin) {
    const commentsPos = Parser_nodePos(receiver);
    let comments = receiver.jsdocTagCommentsSpace;
    receiver.jsdocTagCommentsSpace = [];
    let parts = receiver.jsdocTagCommentsPartsSpace;
    receiver.jsdocTagCommentsPartsSpace = [];
    let linkEnd = -1;
    let state = jsdocStateBeginningOfLine;
    let backtickCount = 0;
    let inFencedCodeBlock = false;
    let margin = -1;
    const pushComment = (text) => {
        ...
        switch (tok) {
            ...
            case KindOpenBraceToken: {
                ...
                if (link !== undefined) {
                    ...
                    const text = Parser_finishNodeWithEnd(receiver, NewJSDocText(receiver.factory, Arena_Clone(receiver.stringSliceArena, comments)), commentStart, commentEnd);
                    parts.push(text);
                    parts.push(link);
                    comments = comments.slice(0, 0);
                    linkEnd = Scanner_TokenEnd(receiver.scanner);
    ...
    if (comments.length > 0) {
        const commentStart = linkEnd > -1 ? linkEnd : commentsPos;
        const text = Parser_finishNode(receiver, NewJSDocText(receiver.factory, Arena_Clone(receiver.stringSliceArena, comments)), commentStart);
        parts.push(text);
    }
    receiver.jsdocTagCommentsPartsSpace = parts.slice(0, 0);
    if (parts.length > 0) {
        return Parser_newNodeList(receiver, NewTextRange(commentsPos, Scanner_TokenEnd(receiver.scanner)), Arena_Clone(receiver.nodeSliceArena, parts));
    }
    return undefined;
}
/**
 ...
 * }
 */
export function Parser_parseJSDocLink(receiver, start) {
 ...
 * }
 */
export function Parser_parseJSDocLinkName(receiver) {
 ...
 * }
 */
export function Parser_parseJSDocLinkPrefix(receiver) {
 ...
 * }
 */
export function isJSDocLinkTag(kind) {
    return kind === "link" || kind === "linkcode" || kind === "linkplain";
}
/**
 ...
 * }
 */
export function Parser_parseUnknownTag(receiver, start, tagName, indent, indentText) {
 ...
 * }
 */
export function Parser_tryParseTypeExpression(receiver) {
    Parser_skipWhitespaceOrAsterisk(receiver);
 ...
 * }
 */
export function Parser_parseBracketNameInPropertyAndParamTag(receiver, target) {
    // Looking for something like '[foo]', 'foo', '[foo.bar]' or 'foo.bar'
    ...
    const name = Parser_parseJSDocEntityName(receiver, IfElse(target === propertyLikeParseParameter, undefined, Identifier_expected));
    if (isBackquoted) {
 ...
 */
export function isObjectOrObjectArrayTypeReference(node) {
    switch (node.Kind) {
        case KindObjectKeyword:
            return true;
        case KindArrayType:
            return isObjectOrObjectArrayTypeReference(AsArrayTypeNode(node).ElementType);
        default:
 ...
 */
export function Parser_parseParameterOrPropertyTag(receiver, start, tagName, target, indent) {
    ...
    const indentText = Parser_skipWhitespaceOrAsterisk(receiver);
    if (isNameFirst && Parser_lookAhead(receiver, (p) => { const [, ok] = Parser_parseJSDocLinkPrefix(p); return !ok; })) {
    ...
    const kind = IfElse(target === propertyLikeParseProperty, KindJSDocPropertyTag, KindJSDocParameterTag);
    const result = NewJSDocParameterOrPropertyTag(receiver.factory, kind, tagName, name, isBracketed, typeExpression, isNameFirst || nestedTypeLiteral !== undefined, comment);
 ...
 */
export function Parser_parseNestedTypeLiteral(receiver, typeExpression, name, target, indent) {
    if (typeExpression !== undefined && isObjectOrObjectArrayTypeReference(Node_Type(typeExpression))) {
        const pos = Parser_nodePos(receiver);
        const children = [];
        for (;;) {
            const state = Parser_mark(receiver);
        ...
        if (children.length > 0) {
            ...
            return Parser_finishNode(receiver, NewJSDocTypeExpression(receiver.factory, literal), pos);
        }
    }
    return undefined;
}
 ...
 */
export function Parser_parseReturnTag(receiver, previousTags, start, tagName, indent, indentText) {
    ...
    return Parser_finishNode(receiver, NewJSDocReturnTag(receiver.factory, tagName, typeExpression, Parser_parseTrailingTagComments(receiver, start, Parser_nodePos(receiver), indent, indentText)), start);
}
/**
 ...
 * Go source: (uses tagName.Text() and parseTrailingTagComments)
 */
export function Parser_parseTypeTag(receiver, previousTags, start, tagName, indent, indentText) {
 ...
 */
export function Parser_parseSeeTag(receiver, start, tagName, indent, indentText) {
    const hasNameReference = (Parser_isIdentifier(receiver) && !hasAsciiPrefixAt(receiver.sourceText, Scanner_TokenEnd(receiver.scanner), "://")) ||
        (receiver.token === KindOpenBraceToken && Parser_lookAhead(receiver, (p) => Parser_nextTokenIsIdentifierOrKeyword(p)));
    let nameExpression;
    if (hasNameReference) {
    ...
    return Parser_finishNode(receiver, NewJSDocSeeTag(receiver.factory, tagName, nameExpression, comments), start);
}
/**
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseImplementsTag(receiver, start, tagName, margin, indentText) {
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseAugmentsTag(receiver, start, tagName, margin, indentText) {
    const className = Parser_parseExpressionWithTypeArgumentsForAugments(receiver);
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseSatisfiesTag(receiver, start, tagName, margin, indentText) {
    ...
    return Parser_finishNode(receiver, NewJSDocSatisfiesTag(receiver.factory, tagName, typeExpression, comments), start);
}
/**
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseThrowsTag(receiver, start, tagName, margin, indentText) {
    const typeExpression = Parser_tryParseTypeExpression(receiver);
    const comment = Parser_parseTrailingTagComments(receiver, start, Parser_nodePos(receiver), margin, indentText);
    return Parser_finishNode(receiver, NewJSDocThrowsTag(receiver.factory, tagName, typeExpression, comment), start);
}
/**
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseImportTag(receiver, start, tagName, margin, indentText) {
 ...
 * }
 */
export function Parser_parseExpressionWithTypeArgumentsForAugments(receiver) {
 ...
 * }
 */
export function Parser_parsePropertyAccessEntityNameExpression(receiver) {
    const pos = Parser_nodePos(receiver);
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseSimpleTag(receiver, start, createTag, tagName, margin, indentText) {
 ...
 * Go source: (uses parseTrailingTagComments)
 */
export function Parser_parseThisTag(receiver, start, tagName, margin, indentText) {
    const typeExpression = Parser_parseJSDocTypeExpression(receiver, true);
    Parser_skipWhitespace(receiver);
    const result = NewJSDocThisTag(receiver.factory, tagName, typeExpression, Parser_parseTrailingTagComments(receiver, start, Parser_nodePos(receiver), margin, indentText));
    return Parser_finishNode(receiver, result, start);
}
/**
 ...
 * }
 */
export function Parser_parseJSDocTypeNameWithNamespace(receiver, nested) {
    const start = Scanner_TokenStart(receiver.scanner);
    if (!tokenIsIdentifierOrKeyword(receiver.token)) {
        return undefined;
    }
    ...
    if (Parser_parseOptionalJsdoc(receiver, KindDotToken)) {
        const body = Parser_parseJSDocTypeNameWithNamespace(receiver, true);
        const jsDocNamespaceNode = NewModuleDeclaration(receiver.factory, undefined, // modifiers
        KindNamespaceKeyword, // keyword
        typeNameOrNamespaceName, body);
        if (nested) {
 ...
 */
export function Parser_parseTypedefTag(receiver, start, tagName, indent, indentText) {
    let typeExpression = Parser_tryParseTypeExpression(receiver);
    Parser_skipWhitespaceOrAsterisk(receiver);
    ...
    if (typeExpression === undefined || isObjectOrObjectArrayTypeReference(Node_Type(typeExpression))) {
        ...
        if (hasChildren) {
            ...
            if (childTypeTag !== undefined && childTypeTag.TypeExpression !== undefined && !isObjectOrObjectArrayTypeReference(Node_Type(childTypeTag.TypeExpression))) {
 ...
 */
export function Parser_parseCallbackTagParameters(receiver, indent) {
    let child;
    const parameters = [];
    const pos = Parser_nodePos(receiver);
    for (;;) {
 ...
 */
export function Parser_parseJSDocSignature(receiver, start, indent) {
    ...
    return Parser_finishNode(receiver, NewJSDocSignature(receiver.factory, undefined, parameters, returnTag), start);
}
/**
 ...
 */
export function Parser_parseCallbackTag(receiver, start, tagName, indent, indentText) {
    let fullName = Parser_parseJSDocTypeNameWithNamespace(receiver, false);
    if (fullName === undefined) {
        fullName = Parser_parseJSDocIdentifierName(receiver, Identifier_expected);
    }
    ...
    return Parser_finishNodeWithEnd(receiver, NewJSDocCallbackTag(receiver.factory, tagName, typeExpression, fullName, comment), start, end);
}
/**
 ...
 */
export function Parser_parseOverloadTag(receiver, start, tagName, indent, indentText) {
    Parser_skipWhitespace(receiver);
    let comment = Parser_parseTagComments(receiver, indent, undefined);
    ...
    const end = comment !== undefined ? Parser_nodePos(receiver) : Node_End(typeExpression);
    return Parser_finishNodeWithEnd(receiver, NewJSDocOverloadTag(receiver.factory, tagName, typeExpression, comment), start, end);
}
/**
...
export function textsEqual(a, b) {
    ...
    return (Node_Text(a) === Node_Text(b));
}
/**
 ...
 * }
 */
export function Parser_parseChildPropertyTag(receiver, indent) {
 ...
 */
export function Parser_parseChildParameterOrPropertyTag(receiver, target, indent, name) {
 ...
 */
export function Parser_tryParseChildTag(receiver, target, indent) {
 ...
 */
export function Parser_parseTemplateTagTypeParameter(receiver) {
    ...
    return Parser_finishNode(receiver, NewTypeParameterDeclaration(receiver.factory, modifiers, name, undefined, undefined, defaultType), typeParameterPos);
}
/**
 ...
 */
export function Parser_parseTemplateTagTypeParameters(receiver) {
    const pos = Parser_nodePos(receiver);
    const nodes = [];
    do {
        Parser_skipWhitespace(receiver);
 ...
 */
export function Parser_parseTemplateTag(receiver, start, tagName, indent, indentText) {
    let constraint;
    if (receiver.token === KindOpenBraceToken) {
 ...
 * }
 */
export function Parser_parseOptionalJsdoc(receiver, t) {
    if (receiver.token === t) {
        Parser_nextTokenJSDoc(receiver);
        return true;
    }
 ...
 * }
 */
export function Parser_parseJSDocEntityName(receiver, diagnosticMessage) {
    let entity = Parser_parseJSDocIdentifierName(receiver, diagnosticMessage);
 ...
 * }
 */
export function Parser_parseJSDocIdentifierName(receiver, diagnosticMessage) {
    if (!tokenIsIdentifierOrKeyword(receiver.token)) {
