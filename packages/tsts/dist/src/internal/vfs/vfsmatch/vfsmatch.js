 ...
 * )
 */
export const UsageFiles = 0;
export const UsageDirectories = 1;
export const UsageExclude = 2;
/**
 ...
 */
export const UnlimitedDepth = MaxInt;
/**
 ...
 * }
 */
export function ReadDirectory(host, currentDir, path, extensions, excludes, includes, depth) {
    return matchFiles(path, extensions, excludes, includes, host.UseCaseSensitiveFileNames(), currentDir, depth, host);
}
/**
 ...
 * }
 */
export function IsImplicitGlob(lastPathComponent) {
    return !lastPathComponent.includes(".") && !lastPathComponent.includes("*") && !lastPathComponent.includes("?");
}
/**
 ...
 */
export let wildcardCharCodes = [0x2a, 0x3f]; // '*', '?'
/**
 ...
 * }
 */
export function getIncludeBasePath(absolute) {
 ...
 * }
 */
export function getBasePaths(path, includes, useCaseSensitiveFileNames) {
    const basePaths = [path];
    const includeList = includes ?? [];
    if (includeList.length > 0) {
        const comparePathsOptions = { CurrentDirectory: path, UseCaseSensitiveFileNames: useCaseSensitiveFileNames };
 ...
 * )
 */
export const kindLiteral = 0;
export const kindWildcard = 1;
export const kindDoubleAsterisk = 2;
/**
 ...
 * )
 */
export const segLiteral = 0;
export const segStar = 1;
export const segQuestion = 2;
/**
 ...
 * }
 */
export function compileGlobPattern(spec, basePath, usage, caseSensitive) {
 ...
 * }
 */
export function parseComponent(s, isInclude) {
    if (s === "**") {
 ...
 * }
 */
export function globPattern_matches(receiver, path) {
    return globPattern_matchPathParts(receiver, path, "", 0, 0, false);
}
/**
 ...
 * }
 */
export function globPattern_matchesParts(receiver, prefix, suffix) {
    return globPattern_matchPathParts(receiver, prefix, suffix, 0, 0, false);
}
/**
 ...
 * }
 */
export function globPattern_matchesPrefixParts(receiver, prefix, suffix) {
    return globPattern_matchPathParts(receiver, prefix, suffix, 0, 0, true);
}
/**
 ...
 * }
 */
export function globPattern_matchPathParts(receiver, prefix, suffix, pathOffset, compIdx, prefixOnly) {
    ...
    while (true) {
        ...
        if (comp.kind === kindDoubleAsterisk) {
            if (globPattern_matchPathParts(receiver, prefix, suffix, curPathOffset, curCompIdx + 1, prefixOnly)) {
                return true;
            }
 ...
 * }
 */
export function globPattern_patternSatisfied(receiver, compIdx) {
 ...
 * }
 */
export function nextPathPartSingle(s, offset) {
    let off = offset;
    if (off >= s.length) {
        return ["", off, false];
    }
    ...
    if (idx >= 0) {
        return [rest.slice(0, idx), (off + idx), true];
    }
 ...
 * }
 */
export function nextPathPartParts(prefix, suffix, offset) {
    // Fast paths: keep the hot single-string scan tight.
    if (suffix.length === 0) {
        return nextPathPartSingle(prefix, offset);
    }
 ...
 * }
 */
export function globPattern_matchWildcard(receiver, segs, s) {
    // Include patterns: wildcards at start cannot match hidden files
 ...
 * }
 */
export function globPattern_matchSegments(receiver, segs, s) {
    let segIdx = 0;
    let sIdx = 0;
    let starSegIdx = -1;
    let starSIdx = 0;
 ...
 * }
 */
export function globPattern_shouldIncludeMinJs(receiver, filename, segs) {
 ...
 * }
 */
export function globPattern_hasMinJsSuffix(receiver, filename) {
    const minJs = ".min.js";
    if (receiver.caseSensitive) {
        return filename.endsWith(minJs);
    }
    if (filename.length < minJs.length) {
        return false;
    }
    // case-insensitive suffix comparison
    return (filename.slice(filename.length - minJs.length).toLowerCase() === minJs);
}
/**
 ...
 * }
 */
export function globPattern_patternMentionsMinSuffix(receiver, segs) {
 ...
 * }
 */
export function globPattern_stringsEqual(receiver, a, b) {
    if (receiver.caseSensitive) {
        return (a === b);
    }
 ...
 * }
 */
export function isHiddenPath(name) {
 ...
 * }
 */
export function isPackageFolder(name) {
    switch (name.length) {
        case "node_modules".length:
            return (name.toLowerCase() === "node_modules");
        case "jspm_packages".length:
            return (name.toLowerCase() === "jspm_packages");
        case "bower_components".length:
            return (name.toLowerCase() === "bower_components");
    }
    return false;
}
/**
 ...
 * }
 */
export function newGlobMatcher(includeSpecs, excludeSpecs, basePath, caseSensitive, usage) {
 ...
 * }
 */
export function globMatcher_matchesFileParts(receiver, prefix, suffix) {
 ...
 * }
 */
export function globMatcher_matchesDirectoryParts(receiver, prefix, suffix) {
    for (let i = 0; i < receiver.excludes.length; i++) {
        if (globPattern_matchesParts(receiver.excludes[i], prefix, suffix)) {
            return false;
        }
 ...
 * }
 */
export function globVisitor_visit(receiver, path, absolutePath, depth, resolvedRealPath) {
    // Detect symlink cycles
    let realPath;
    if (resolvedRealPath !== "") {
        realPath = resolvedRealPath;
    ...
    for (const dir of entries.Directories) {
        ...
        // If Symlinks is nil, the FS doesn't track symlinks;
        // leave childRealPath empty to call Realpath (preserving old behavior).
        globVisitor_visit(receiver, pathPrefix + dir, absDir, curDepth, childRealPath);
    }
}
/**
 ...
 * }
 */
export function matchFiles(path, extensions, excludes, includes, useCaseSensitiveFileNames, currentDirectory, depth, host) {
 ...
 * }
 */
export function SpecMatcher_MatchString(receiver, path) {
 ...
 * }
 */
export function SpecMatcher_MatchIndex(receiver, path) {
    for (let i = 0; i < receiver.patterns.length; i++) {
        if (globPattern_matches(receiver.patterns[i], path)) {
            return i;
        }
 ...
 * }
 */
export function NewSpecMatcher(specs, basePath, usage, useCaseSensitiveFileNames) {
    if (specs.length === 0) {
        return undefined;
    }
    const patterns = [];
...
//# sourceMappingURL=vfsmatch.js.map