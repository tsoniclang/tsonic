...
import { compileGlobPattern, globPattern_matches, IsImplicitGlob, NewSpecMatcher, ReadDirectory, SpecMatcher_MatchIndex, SpecMatcher_MatchString, UnlimitedDepth, UsageExclude, UsageFiles, } from "./vfsmatch.js";
function caseInsensitiveHost() {
...
function runReadDirectoryCase(testCase) {
    const host = testCase.host();
    const got = ReadDirectory(host, "/", testCase.path ?? "/dev", testCase.extensions ?? [], testCase.excludes ?? [], testCase.includes ?? [], testCase.depth ?? UnlimitedDepth);
    testCase.expect(got);
}
...
test("SpecMatcher mirrors upstream match and index behavior", () => {
    const matcher = NewSpecMatcher(["*.ts", "*.tsx"], "/project", UsageFiles, true);
    ...
    const recursive = NewSpecMatcher(["**/*.ts"], "/project", UsageFiles, true);
    ...
    const exclude = NewSpecMatcher(["node_modules"], "/project", UsageExclude, true);
    ...
    assert.equal(NewSpecMatcher([], "/project", UsageFiles, true), undefined);
    assert.equal(NewSpecMatcher(["**"], "/project", UsageFiles, true), undefined);
});
test("compileGlobPattern mirrors upstream segment backtracking behavior", () => {
    const [question, questionOk] = compileGlobPattern("a?b", "/", UsageFiles, true);
    ...
    const [star, starOk] = compileGlobPattern("*a*b*c", "/", UsageFiles, true);
...
//# sourceMappingURL=vfsmatch.test.js.map