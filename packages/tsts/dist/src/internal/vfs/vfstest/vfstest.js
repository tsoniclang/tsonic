 ...
 * }
 */
export function clockImpl_Now(receiver) {
    return Now();
}
/**
 ...
 * }
 */
export function clockImpl_SinceStart(receiver) {
 ...
 * )
 */
export let ____34464f57_0 = MapFS_as_iovfs_RealpathFS(undefined);
export let ____34464f57_1 = MapFS_as_iovfs_WritableFS(undefined);
export function MapFS_as_io_fs_FS(receiver) {
    return {
        Open: (name) => MapFS_Open(receiver, name),
    };
}
export function MapFS_as_iovfs_RealpathFS(receiver) {
    return {
        ...MapFS_as_io_fs_FS(receiver),
        Realpath: (path) => MapFS_Realpath(receiver, path),
    };
}
export function MapFS_as_iovfs_WritableFS(receiver) {
    return {
        ...MapFS_as_io_fs_FS(receiver),
        WriteFile: (path, data, perm) => MapFS_WriteFile(receiver, path, data, perm),
        AppendFile: (path, data, perm) => MapFS_AppendFile(receiver, path, data, perm),
        MkdirAll: (path, perm) => MapFS_MkdirAll(receiver, path, perm),
        Remove: (path) => MapFS_Remove(receiver, path),
        Chtimes: (path, aTime, mTime) => MapFS_Chtimes(receiver, path, aTime, mTime),
    };
}
function MapFS_as_iovfs_FS(receiver) {
    return {
 ...
 * }
 */
export function FromMap(m, useCaseSensitiveFileNames) {
    const clockObj = { start: Now() };
    return FromMapWithClock(m, useCaseSensitiveFileNames, {
        Now() { return clockImpl_Now(clockObj); },
        SinceStart() { return clockImpl_SinceStart(clockObj); },
    });
}
/**
 ...
 * }
 */
export function FromMapWithClock(m, useCaseSensitiveFileNames, clock) {
    let posix = false;
    let windows = false;
    const checkPath = (p) => {
 ...
 * }
 */
export function convertMapFS(input, useCaseSensitiveFileNames, clock) {
    const clockObj = { start: Now() };
    const clockVal = clock !== undefined ? clock : {
        Now() { return clockImpl_Now(clockObj); },
        SinceStart() { return clockImpl_SinceStart(clockObj); },
    };
    const m = {
 ...
 * }
 */
export function comparePathsByParts(a, b) {
    let aVar = a;
    let bVar = b;
    for (;;) {
 ...
 * }
 */
export function MapFS_getCanonicalPath(receiver, p) {
 ...
 * }
 */
export function MapFS_open(receiver, p) {
    const internalMap = receiver.m;
    ...
    if (file === undefined) {
        ...
        file = { Data: new Uint8Array(0), Mode: ModeDir, ModTime: undefined, Sys: undefined };
    }
    let offset = 0;
    const bytes = file.Data;
    // Build a synthetic fs.File backed by the internal map file.
    const fi = {
        Name() { return baseName(p); },
        Sys() { return file.Sys; },
        Mode() { return file.Mode; },
        IsDir() { return (file.Mode & ModeDir) !== 0; },
        IsRegular() { return (file.Mode & (ModeDir | ModeSymlink)) === 0; },
    };
    const f = {
        Stat() { return [fi, undefined]; },
        Read(buffer) {
        ...
        ReadDir(_n) {
            ...
            for (const [k, v] of internalMap.entries()) {
                if (k !== p && k.startsWith(prefix) && k.slice(prefix.length).indexOf("/") < 0) {
                    ...
                    const entryFi = {
                        Name() { return entryName; },
                        Sys() { return entryFile.Sys; },
                        Mode() { return entryFile.Mode; },
                        IsDir() { return (entryFile.Mode & ModeDir) !== 0; },
                        IsRegular() { return (entryFile.Mode & (ModeDir | ModeSymlink)) === 0; },
                    };
                    entries.push({
                        Info() { return [entryFi, undefined]; },
                    });
                }
            }
 ...
 * }
 */
export function MapFS_remove(receiver, path) {
 ...
 * }
 */
export function Symlink(target) {
    const f = {
 ...
 * }
 */
export function MapFS_getFollowingSymlinks(receiver, p) {
...
class BrokenSymlinkErrorImpl extends globalThis.Error {
    "from";
    to;
    constructor(from, to) {
 ...
 * }
 */
export function brokenSymlinkError_Error(receiver) {
 ...
 * }
 */
export function isBrokenSymlinkError(err) {
    const [, ok] = AsType(err, (e) => e instanceof BrokenSymlinkErrorImpl);
    return ok;
}
/**
 ...
 * }
 */
export function MapFS_getFollowingSymlinksWorker(receiver, p, symlinkFrom, symlinkTo) {
    const internalMap = receiver.m;
    ...
    for (const [other, symTarget] of receiver.symlinks.entries()) {
        if (other.length < p.length && other === p.slice(0, other.length) && p[other.length] === "/") {
            return MapFS_getFollowingSymlinksWorker(receiver, (symTarget + p.slice(other.length)), other, symTarget);
        }
    }
    let err = ErrNotExist;
    if (symlinkFrom !== "") {
 ...
 * }
 */
export function MapFS_set(receiver, p, file) {
    const internalMap = receiver.m;
    internalMap.set(p, file);
}
/**
 ...
 * }
 */
export function MapFS_setEntry(receiver, realpath, canonical, file) {
    if (realpath === "" || canonical === "") {
 ...
 * }
 */
export function splitPath(s, offset) {
 ...
 * }
 */
export function MapFS_mkdirAll(receiver, p, perm) {
    if (p === "") {
        throw new globalThis.Error("empty path");
    }
    // Fast path; already exists.
    ...
    if (fastErr === undefined) {
        const fastFile = fastOther;
        if ((fastFile.Mode & ModeDir) === 0) {
    ...
    for (;;) {
        ...
        if (err !== undefined) {
            ...
            if (err !== ErrNotExist && err.message !== ErrNotExist?.message) {
                return err;
            }
            toCreate.push(dir);
        }
 ...
 * }
 */
export function fileInfo_Name(receiver) {
 ...
 * }
 */
export function fileInfo_Sys(receiver) {
    return receiver.sys;
}
/**
 ...
 * }
 */
export function file_Stat(receiver) {
 ...
 * }
 */
export function readDirFile_Stat(receiver) {
    return [receiver.fileInfo, undefined];
}
/**
 ...
 * }
 */
export function readDirFile_ReadDir(receiver, n) {
    const embedded = receiver.__tsgoEmbedded0;
    const [list, err] = embedded.ReadDir(n);
    if (err !== undefined) {
        return [[], err];
    }
    const entries = new globalThis.Array(list.length);
    for (let i = 0; i < list.length; i++) {
        const entry = list[i];
        const [infoVal, infoErr] = entry.Info();
        const info = must(infoVal, infoErr);
        const [newInfo, ok] = convertInfo(info);
        if (!ok) {
            const infoInternal = info;
 ...
 * }
 */
export function MapFS_Open(receiver, name) {
    receiver.mu.RLock();
    try {
        ...
        const info = must(statVal, statErr);
        const infoForConvert = info;
        ...
        if (!ok) {
            ...
            const rdfResult = {
                ...
                Stat: () => readDirFile_Stat(rdfResult),
                Read: (buffer) => f.Read(buffer),
                Close: () => f.Close(),
                ReadDir: (n) => readDirFile_ReadDir(rdfResult, n),
            };
            return [rdfResult, undefined];
        }
        ...
        const fileResult = {
            __tsgoEmbedded0: f,
            fileInfo: newInfo,
            Stat: () => file_Stat(fileResult),
            Read: (buffer) => f.Read(buffer),
            Close: () => f.Close(),
        };
        return [fileResult, undefined];
    }
    finally {
        receiver.mu.RUnlock();
    }
 ...
 * }
 */
export function MapFS_Realpath(receiver, name) {
    receiver.mu.RLock();
    try {
        ...
        const internalFile = file;
        const sysData = internalFile.Sys;
 ...
 * }
 */
export function convertInfo(info) {
    const internalInfo = info;
    ...
    const typedSys = sysObj;
    return [makeFileInfo(info, typedSys.original, typedSys.realpath), true];
}
function makeFileInfo(info, sysValue, realpath) {
    const result = {
        __tsgoEmbedded0: info,
        sys: sysValue,
        realpath,
        Name: () => fileInfo_Name(result),
        Size: () => info.Size(),
        Mode: () => info.Mode(),
        ModTime: () => info.ModTime(),
        IsDir: () => info.IsDir(),
        Sys: () => fileInfo_Sys(result),
    };
    return result;
}
/**
 ...
 * }
 */
export function MapFS_MkdirAll(receiver, path, perm) {
    receiver.mu.Lock();
    try {
 ...
 * }
 */
export function MapFS_AddSymlink(receiver, path, target) {
    receiver.mu.Lock();
    try {
        const canonical = MapFS_getCanonicalPath(receiver, path);
        const f = {
 ...
 * }
 */
export function MapFS_WriteFile(receiver, path, data, perm) {
    receiver.mu.Lock();
    try {
        ...
        if (parent !== "") {
            ...
            const parentInternal = parentFile;
            if ((parentInternal.Mode & ModeDir) === 0) {
        ...
        if (err !== undefined) {
            ...
            const fileInternal = file;
            if ((fileInternal.Mode & (ModeDir | ModeSymlink)) !== 0) {
 ...
 * }
 */
export function MapFS_AppendFile(receiver, path, data, perm) {
    receiver.mu.Lock();
    try {
        const parent = dirName(path);
        if (parent !== "") {
 ...
 * }
 */
export function MapFS_Chtimes(receiver, path, aTime, mTime) {
    receiver.mu.Lock();
    try {
        const canonical = MapFS_getCanonicalPath(receiver, path);
        const internalMap = receiver.m;
 ...
 * }
 */
export function MapFS_GetTargetOfSymlink(receiver, path) {
 ...
 * }
 */
export function MapFS_GetModTime(receiver, path) {
 ...
 * }
 */
export function MapFS_Entries(receiver) {
    return (yieldValue) => {
        receiver.mu.RLock();
        try {
            const internalMap = receiver.m;
 ...
 * }
 */
export function MapFS_GetFileInfo(receiver, path) {
    const pathVar = path.startsWith("/") ? path.slice(1) : path;
    receiver.mu.RLock();
    try {
        const canonical = MapFS_getCanonicalPath(receiver, pathVar);
        const internalMap = receiver.m;
        return internalMap.get(canonical);
    }
    finally {
        receiver.mu.RUnlock();
    }
