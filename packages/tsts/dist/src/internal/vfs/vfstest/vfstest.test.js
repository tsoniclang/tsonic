...
import { convertMapFS, FromMap, MapFS_as_io_fs_FS, MapFS_Realpath, Symlink, } from "./vfstest.js";
function bytes(text) {
    return new TextEncoder().encode(text);
}
function assertErrorContains(error, expected) {
...
function dirEntriesToNames(entries) {
...
test("convertMapFS mirrors TS-Go case-insensitive lookup and realpath behavior", () => {
    ...
    const mapFS = convertMapFS(map, false, nilClock());
...
test("convertMapFS mirrors TS-Go case-sensitive lookup behavior", () => {
    ...
    const fs = MapFS_as_io_fs_FS(convertMapFS(map, true, nilClock()));
    assert.deepEqual(ReadFile(fs, "foo/bar/baz"), ["bar", undefined]);
...
test("convertMapFS mirrors TS-Go duplicate and parent-file panics", () => {
    assert.throws(() => convertMapFS(new Map([
        ["foo", { Data: bytes("bar") }],
        ["Foo", { Data: bytes("baz") }],
    ]), false, nilClock()), /duplicate path: "Foo" and "foo" have the same canonical path/);
    assert.doesNotThrow(() => convertMapFS(new Map([
        ["foo", { Data: bytes("bar") }],
        ["Foo", { Data: bytes("baz") }],
    ]), true, nilClock()));
    assert.throws(() => convertMapFS(new Map([
    ...
    ]), false, nilClock()), /failed to create intermediate directories for "foo\/oops": mkdir "foo": path exists but is not a directory/);
});
test("FromMap mirrors TS-Go read/write/delete and path validation behavior", () => {
    const fs = FromMap(new Map(), false);
    ...
    assert.throws(() => FromMap(new Map([["string", "hello, world"]]), false), /non-rooted path "string"/);
    assert.throws(() => FromMap(new Map([["/string/", "hello, world"]]), false), /non-normalized path "\/string\/"/);
    assert.throws(() => FromMap(new Map([["/string/../foo", "hello, world"]]), false), /non-normalized path "\/string\/\.\.\/foo"/);
    assert.throws(() => FromMap(new Map([["/string", 1234]]), false), /invalid file type/);
    assert.throws(() => FromMap(new Map([
        ["/string", "hello, world"],
        ["c:/bytes", bytes("hello, world")],
    ]), false), /mixed posix and windows paths/);
});
test("FromMap mirrors TS-Go POSIX, Windows, and BOM read behavior", () => {
    const posix = FromMap(new Map([
        ["/string", "hello, world"],
        ["/bytes", bytes("hello, world")],
        ["/mapfile", { Data: bytes("hello, world") }],
    ]), false);
    ...
    const windows = FromMap(new Map([
        ["c:/string", "hello, world"],
        ["d:/bytes", bytes("hello, world")],
        ["e:/mapfile", { Data: bytes("hello, world") }],
    ]), false);
    ...
    assert.deepEqual(FromMap(new Map([["/foo.ts", utf16be]]), true).ReadFile("/foo.ts"), ["hi", true]);
    assert.deepEqual(FromMap(new Map([["/foo.ts", utf16le]]), true).ReadFile("/foo.ts"), ["hi", true]);
    assert.deepEqual(FromMap(new Map([["/foo.ts", utf8Bom]]), true).ReadFile("/foo.ts"), ["hi", true]);
});
...
//# sourceMappingURL=vfstest.test.js.map