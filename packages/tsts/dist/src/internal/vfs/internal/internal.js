...
export function Common_ReadFile(receiver, path) {
    ...
    const [b, err] = fs_ReadFile(fsys, rest);
    if (err !== undefined) {
        return ["", false];
    }
...
export function decodeBytes(s) {
    ...
    if (bytes.length >= 2) {
        ...
        if (bom0 === 0xFF && bom1 === 0xFE) {
            return [decodeUtf16(bytes.subarray(2), LittleEndian), true];
        }
        if (bom0 === 0xFE && bom1 === 0xFF) {
            return [decodeUtf16(bytes.subarray(2), BigEndian), true];
        }
    }
 ...
 * }
 */
export function decodeUtf16(bytes, order) {
    const codeUnits = [];
