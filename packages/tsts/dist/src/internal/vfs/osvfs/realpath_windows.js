...
export function realpath(path) {
    ...
    if (path.length < 248) {
        ...
        // For long paths, defer to os.Open to run the path through fixLongPath.
        const [f, openErr] = goOs.Open(path);
...
function realpathWithHandle(h, _path) {
    const _VOLUME_NAME_DOS = 0;
    let buf = new Array(310);
    for (;;) {
        const [n, err] = windows.GetFinalPathNameByHandle(h, buf[0], buf.length, _VOLUME_NAME_DOS);
        if (err !== undefined) {
            return ["", err];
        }
        if (n < buf.length) {
    ...
    let s = syscall.UTF16ToString(buf);
 ...
 * }
 */
export function openMetadata(path) {
    // based on https://github.com/microsoft/go-winio/blob/3c9576c9346a1892dee136329e7e15309e82fb4f/pkg/fs/resolve.go#L113
    const [pathUTF16, pathUTF16Err] = windows.UTF16PtrFromString(path);
    if (pathUTF16Err !== undefined) {
        return [windows.InvalidHandle, pathUTF16Err];
    }
    ...
    const [h, createErr] = windows.CreateFile(pathUTF16, _FILE_ANY_ACCESS, (_FILE_SHARE_READ | _FILE_SHARE_WRITE | _FILE_SHARE_DELETE), undefined, _OPEN_EXISTING, _FILE_FLAG_BACKUP_SEMANTICS, 0);
    if (createErr !== undefined) {
        return [0, new globalThis.Error(`CreateFile ${path}: ${createErr.message}`)];
    }
    return [h, undefined];
}
//# sourceMappingURL=realpath_windows.js.map