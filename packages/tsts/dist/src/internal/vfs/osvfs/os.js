 ...
 * )
 */
export let blockingOpSema = NewLimitedSemaphore(128);
export let readSema = NewLimitedSemaphore(128);
export let writeSema = NewLimitedSemaphore(32);
/**
...
export let osVFS = {
    ...
    ReadFile: (path) => osFS_ReadFile(_osFS, path),
    DirectoryExists: (path) => osFS_DirectoryExists(_osFS, path),
    FileExists: (path) => osFS_FileExists(_osFS, path),
    GetAccessibleEntries: (path) => osFS_GetAccessibleEntries(_osFS, path),
    Stat: (path) => osFS_Stat(_osFS, path),
    WalkDir: (root, walkFn) => osFS_WalkDir(_osFS, root, walkFn),
    Realpath: (path) => osFS_Realpath(_osFS, path),
    WriteFile: (path, content) => osFS_WriteFile(_osFS, path, content),
    AppendFile: (path, content) => osFS_AppendFile(_osFS, path, content),
    Remove: (path) => osFS_Remove(_osFS, path),
    Chtimes: (path, aTime, mTime) => osFS_Chtimes(_osFS, path, aTime, mTime),
};
/**
 ...
 * }()
 */
export let isFileSystemCaseSensitive = (process.platform !== "win32" && process.platform !== "darwin");
/**
 ...
 * }
 */
export function osFS_UseCaseSensitiveFileNames(receiver) {
    return isFileSystemCaseSensitive;
}
/**
 ...
 * }
 */
export function osFS_ReadFile(receiver, path) {
 ...
 * }
 */
export function osFS_DirectoryExists(receiver, path) {
 ...
 * }
 */
export function osFS_FileExists(receiver, path) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
 ...
 * }
 */
export function osFS_GetAccessibleEntries(receiver, path) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
 ...
 * }
 */
export function osFS_Stat(receiver, path) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
 ...
 * }
 */
export let limitedWalkDirFuncPool = new Pool();
limitedWalkDirFuncPool.New = () => {
    const w = { inner: undefined, walk: undefined };
    w.walk = ((path, d, err) => limitedWalkDirFunc_walker(w, path, d, err));
    return w;
};
/**
 ...
 * }
 */
export function getLimitedWalkDirFunc(walkFn) {
    const w = limitedWalkDirFuncPool.Get();
    w.inner = walkFn;
    return w;
}
/**
 ...
 * }
 */
export function limitedWalkDirFunc_walker(receiver, path, d, err) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
    return receiver.inner(path, d, err);
}
/**
 ...
 * }
 */
export function osFS_WalkDir(receiver, root, walkFn) {
 ...
 * }
 */
export function osFS_Realpath(receiver, path) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
    return osFSRealpath(path);
}
/**
 ...
 * }
 */
export function osFSRealpath(path) {
 ...
 * }
 */
export function osFS_writeFileWithFlag(receiver, path, content, flag) {
    // defer writeSema.Acquire()() — no-op in single-threaded TS
    const [file, openErr] = goOs.OpenFile(path, flag, 0o666);
    if (openErr !== undefined) {
        return openErr;
    }
    try {
        const [, writeErr] = file.WriteString(content);
 ...
 * }
 */
export function osFS_ensureDirectoryExists(receiver, directoryPath) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
    return goOs.MkdirAll(directoryPath, 0o777);
}
/**
 ...
 * }
 */
export function osFS_writeFileEnsuringDir(receiver, path, content, flag) {
    void RootLength(path); // Assert path is rooted
 ...
 * }
 */
export function osFS_WriteFile(receiver, path, content) {
    return osFS_writeFileEnsuringDir(receiver, path, content, (goOs.O_WRONLY | goOs.O_CREATE | goOs.O_TRUNC));
}
/**
 ...
 * }
 */
export function osFS_AppendFile(receiver, path, content) {
    return osFS_writeFileEnsuringDir(receiver, path, content, (goOs.O_WRONLY | goOs.O_CREATE | goOs.O_APPEND));
}
/**
 ...
 * }
 */
export function osFS_Remove(receiver, path) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
 ...
 * }
 */
export function osFS_Chtimes(receiver, path, aTime, mTime) {
    // defer blockingOpSema.Acquire()() — no-op in single-threaded TS
 ...
 * }
 */
export function GetGlobalTypingsCacheLocation() {
    const [userCacheDir, userCacheDirErr] = goOs.UserCacheDir();
    const cacheDir = userCacheDirErr !== undefined ? goOs.TempDir() : userCacheDir;
    const subdir = runtime.GOOS === "windows" ? "Microsoft/TypeScript" : "typescript";
