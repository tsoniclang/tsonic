 ...
 * })
 */
export const hasFGetPath = OnceValue(() => {
    // Verify that F_GETPATH is supported by this kernel version.
    const buf = new globalThis.Array(unix.PathMax).fill(0);
    const [fd, err] = unix.Open(".", unix.O_EVTONLY | unix.O_NONBLOCK | unix.O_CLOEXEC, 0);
    if (err !== undefined) {
        return false;
    }
    try {
        const [, getPathErr] = fcntlGetPath(fd, buf);
        return (getPathErr === undefined);
    }
    finally {
        unix.Close(fd);
    }
 ...
 * }
 */
export function fcntlGetPath(fd, buf) {
 ...
 * }
 */
export function fcntlGetPathPtr(fd, buf) {
    return unix.FcntlInt(fd, unix.F_GETPATH, buf);
}
/**
...
export function realpath(path) {
    ...
    const [fd, err] = unix.Open(path, unix.O_EVTONLY | unix.O_NONBLOCK | unix.O_CLOEXEC, 0);
    if (err !== undefined) {
        return ["", err];
    }
    try {
        ...
        return [unix.ByteSliceToString(buf), undefined];
    }
    finally {
        unix.Close(fd);
    }
}
//# sourceMappingURL=realpath_darwin.js.map