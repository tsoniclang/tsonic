 ...
 * })
 */
export const hasProcSelfFD = OnceValue(() => {
    const stat = {}; // var stat unix.Stat_t
    return (unix.Stat(_procSelfFD, stat) === undefined);
});
/**
...
export function realpath(path) {
    ...
    try {
        ...
        for (;;) {
            const [nn, readlinkErr] = ignoringEINTR(() => {
                return unix.Readlink(procPath, buf);
            });
            ...
            if (nn < buf.length) {
                return [new TextDecoder("utf-8").decode(buf.subarray(0, nn)), undefined];
            }
...
//# sourceMappingURL=realpath_linux.js.map