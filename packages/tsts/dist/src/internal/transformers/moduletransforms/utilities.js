 ...
 * }
 */
export function isDeclarationNameOfEnumOrNamespace(emitContext, node) {
    ...
    if (original !== undefined && original.Parent !== undefined) {
        switch (original.Parent.Kind) {
            ...
            case KindModuleDeclaration:
                return (original === Node_Name(original.Parent));
        }
    }
    return false;
}
 ...
 * }
 */
export function rewriteModuleSpecifier(emitContext, node, compilerOptions) {
    if (node === undefined || !IsStringLiteral(node) || !ShouldRewriteModuleSpecifier(Node_Text(node), compilerOptions)) {
        return node;
    }
    const nodeText = Node_Text(node);
    const updatedText = ChangeExtension(nodeText, GetOutputExtension(nodeText, compilerOptions.Jsx));
    if (updatedText !== nodeText) {
        const updated = NewStringLiteral(emitContext.Factory.__tsgoEmbedded0, updatedText, AsStringLiteral(node).TokenFlags);
        EmitContext_SetOriginal(emitContext, updated, node);
        EmitContext_AssignCommentAndSourceMapRanges(emitContext, updated, node);
        return updated;
    }
    return node;
}
...
export function createEmptyImports(factory) {
    ...
    return NewExportDeclaration(f, undefined, /*modifiers*/ false, /*isTypeOnly*/ NewNamedExports(f, NodeFactory_NewNodeList(f, [])), undefined, /*moduleSpecifier*/ undefined);
}
/**
...
export function getExternalModuleNameLiteral(factory, importNode, sourceFile, host, resolver, compilerOptions) {
    ...
    if (moduleName !== undefined && IsStringLiteral(moduleName)) {
        ...
        if (name === undefined) { // !!! propagate token flags (will produce new diffs)
            name = NewStringLiteral(factory.__tsgoEmbedded0, Node_Text(moduleName), TokenFlagsNone);
        }
        return name;
    }
    return undefined;
 ...
 * }
 */
export function tryGetModuleNameFromDeclaration(declaration, host, factory, resolver, compilerOptions) {
 ...
 * }
 */
export function getExternalModuleNameFromPath(host, fileName, referencePath) {
    // !!!
    return "";
}
/**
 ...
 * }
 */
export function tryRenameExternalModule(factory, moduleName, sourceFile) {
    // !!!
    return undefined;
}
/**
 ...
 * }
 */
export function isFileLevelReservedGeneratedIdentifier(emitContext, name) {
    ...
    return (info !== undefined &&
        ...
        GeneratedIdentifierFlags_IsReservedInNestedScopes(info.Flags));
}
/**
