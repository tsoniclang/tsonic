...
import { KindAmpersandAmpersandEqualsToken, KindArrowFunction, KindBarBarEqualsToken, KindBinaryExpression, KindBindingElement, KindClassExpression, KindEqualsToken, KindExportAssignment, KindFunctionExpression, KindParameter, KindPropertyAssignment, KindPropertyDeclaration, KindQuestionQuestionEqualsToken, KindShorthandPropertyAssignment, KindVariableDeclaration, } from "../../ast/generated/kinds.js";
 ...
 * }
 */
export function isClassNamedEvaluationHelperBlock(emitContext, node) {
    if (!IsClassStaticBlockDeclaration(node)) {
        return false;
    }
    const csbd = AsClassStaticBlockDeclaration(node);
    const bodyStatements = Node_Statements(csbd.Body);
 ...
 * }
 */
export function classHasExplicitlyAssignedName(emitContext, node) {
    if (EmitContext_AssignedName(emitContext, node) !== undefined) {
 ...
 * }
 */
export function classHasDeclaredOrExplicitlyAssignedName(emitContext, node) {
    return Node_Name(node) !== undefined || classHasExplicitlyAssignedName(emitContext, node);
}
/**
 ...
 * }
 */
export function isAnonymousFunctionDefinition(emitContext, node, cb) {
    ...
    switch (skipped.Kind) {
        ...
        case KindFunctionExpression:
            if (Node_Name(AsFunctionExpression(skipped)) !== undefined) {
                return false;
            }
            break;
...
export function isNamedEvaluationSource(node) {
    switch (node.Kind) {
        ...
        case KindVariableDeclaration:
            return IsIdentifier(AsVariableDeclaration(node).name) && Node_Initializer(node) !== undefined;
        case KindParameter:
            return IsIdentifier(AsParameterDeclaration(node).name) && Node_Initializer(node) !== undefined && AsParameterDeclaration(node).DotDotDotToken === undefined;
        case KindBindingElement:
            return IsIdentifier(AsBindingElement(node).name) && Node_Initializer(node) !== undefined && AsBindingElement(node).DotDotDotToken === undefined;
 ...
 * }
 */
export function isNamedEvaluation(emitContext, node) {
 ...
 * }
 */
export function isNamedEvaluationAnd(emitContext, node, cb) {
    if (!isNamedEvaluationSource(node)) {
        return false;
    }
    switch (node.Kind) {
 ...
 * }
 */
export function getAssignedNameOfIdentifier(emitContext, name, expression) {
    const original = EmitContext_MostOriginal(emitContext, SkipOuterExpressions(expression, OEKAll));
    if ((IsClassDeclaration(original) || IsFunctionDeclaration(original)) &&
        ...
        return NewStringLiteral(emitContext.Factory.__tsgoEmbedded0, "default", TokenFlagsNone);
    }
    return NodeFactory_NewStringLiteralFromNode(emitContext.Factory, name);
}
/**
 ...
 * }
 */
export function getAssignedNameOfPropertyName(emitContext, name, assignedNameText) {
    const factory = emitContext.Factory;
    if (assignedNameText.length > 0) {
        const assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
        return [assignedName, name];
    }
    if (IsPropertyNameLiteral(name) || IsPrivateIdentifier(name)) {
        const assignedName = NodeFactory_NewStringLiteralFromNode(factory, name);
        return [assignedName, name];
    }
    const expression = Node_Expression(name);
    if (expression !== undefined && IsPropertyNameLiteral(expression) && !IsIdentifier(expression)) {
        const assignedName = NodeFactory_NewStringLiteralFromNode(factory, expression);
        return [assignedName, name];
    }
    // Must be a computed property name
    const assignedName = NodeFactory_NewGeneratedNameForNode(factory, name);
    EmitContext_AddVariableDeclaration(emitContext, assignedName);
    const key = NodeFactory_NewPropKeyHelper(factory, expression);
    const assignment = NodeFactory_NewAssignmentExpression(factory, assignedName, key);
    const updatedName = NodeFactory_UpdateComputedPropertyName(factory.__tsgoEmbedded0, AsComputedPropertyName(name), assignment);
    return [assignedName, updatedName];
}
/**
 ...
 * }
 */
export function createClassNamedEvaluationHelperBlock(emitContext, assignedName, thisExpression) {
    const factory = emitContext.Factory;
 ...
 * }
 */
export function injectClassNamedEvaluationHelperBlockIfMissing(emitContext, node, assignedName, thisExpression) {
    if (classHasExplicitlyAssignedName(emitContext, node)) {
        return node;
    }
    const factory = emitContext.Factory;
    ...
    if (nodeName !== undefined) {
        ...
        const bodyStmts = Node_Statements(csbd.Body);
    ...
    const newMembers = [...leading, namedEvaluationBlock, ...trailing];
    const membersList = NodeFactory_NewNodeList(astFactory, newMembers);
    membersList.Loc = Node_MemberList(node).Loc;
    const oldNode = node;
    let updatedNode;
    if (IsClassDeclaration(node)) {
        updatedNode = NodeFactory_UpdateClassDeclaration(astFactory, AsClassDeclaration(node), Node_Modifiers(node), node.name, Node_TypeParameterList(node), node.HeritageClauses, membersList);
    }
    else {
        updatedNode = NodeFactory_UpdateClassExpression(astFactory, AsClassExpression(node), Node_Modifiers(node), node.name, Node_TypeParameterList(node), node.HeritageClauses, membersList);
    }
    EmitContext_SetAssignedName(emitContext, updatedNode, assignedName);
    // Transfer ClassThis from old to new node
    const ct = EmitContext_ClassThis(emitContext, oldNode);
    if (ct !== undefined) {
        EmitContext_SetClassThis(emitContext, updatedNode, ct);
    }
    return updatedNode;
}
/**
 ...
 * }
 */
export function finishTransformNamedEvaluation(emitContext, expression, assignedName, ignoreEmptyStringLiteral) {
    if (ignoreEmptyStringLiteral && IsStringLiteral(assignedName) && Node_Text(assignedName).length === 0) {
        return expression;
    }
    const factory = emitContext.Factory;
    ...
    if (IsClassExpression(innerExpression)) {
        updatedExpression = injectClassNamedEvaluationHelperBlockIfMissing(emitContext, innerExpression, assignedName, undefined);
    }
    else {
        updatedExpression = NodeFactory_NewSetFunctionNameHelper(factory, innerExpression, assignedName, "");
    }
    return NodeFactory_RestoreOuterExpressions(factory, expression, updatedExpression, OEKAll);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfPropertyAssignment(context, node, ignoreEmptyStringLiteral, assignedNameText) {
    ...
    return NodeFactory_UpdatePropertyAssignment(factory.__tsgoEmbedded0, node, undefined, // modifiers
    name, undefined, // postfixToken
    undefined, // typeNode
    initializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfShorthandAssignmentProperty(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
    }
    else {
        assignedName = getAssignedNameOfIdentifier(emitContext, node.name, node.ObjectAssignmentInitializer);
    }
    const objectAssignmentInitializer = finishTransformNamedEvaluation(emitContext, node.ObjectAssignmentInitializer, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateShorthandPropertyAssignment(factory.__tsgoEmbedded0, node, undefined, // modifiers
    node.name, undefined, // postfixToken
    undefined, // typeNode
    node.EqualsToken, objectAssignmentInitializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfVariableDeclaration(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
    ...
    const initializer = finishTransformNamedEvaluation(emitContext, node.Initializer, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateVariableDeclaration(factory.__tsgoEmbedded0, node, node.name, undefined, // exclamationToken
    undefined, // typeNode
    initializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfParameterDeclaration(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
    ...
    const initializer = finishTransformNamedEvaluation(emitContext, node.Initializer, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateParameterDeclaration(factory.__tsgoEmbedded0, node, undefined, // modifiers
    node.DotDotDotToken, node.name, undefined, // questionToken
    undefined, // typeNode
    initializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfBindingElement(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
    ...
    const initializer = finishTransformNamedEvaluation(emitContext, node.Initializer, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateBindingElement(factory.__tsgoEmbedded0, node, node.DotDotDotToken, node.PropertyName, node.name, initializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfPropertyDeclaration(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    const [assignedName, name] = getAssignedNameOfPropertyName(emitContext, node.name, assignedNameText);
    const initializer = finishTransformNamedEvaluation(emitContext, node.Initializer, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdatePropertyDeclaration(factory.__tsgoEmbedded0, node, Node_Modifiers(node), name, undefined, // postfixToken
    undefined, // typeNode
    initializer);
}
/**
 ...
 * }
 */
export function transformNamedEvaluationOfAssignmentExpression(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
    ...
    const right = finishTransformNamedEvaluation(emitContext, node.Right, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateBinaryExpression(factory.__tsgoEmbedded0, node, undefined, // modifiers
    node.Left, undefined, // typeNode
 ...
 * }
 */
export function transformNamedEvaluationOfExportAssignment(emitContext, node, ignoreEmptyStringLiteral, assignedNameText) {
    const factory = emitContext.Factory;
    let assignedName;
    if (assignedNameText.length > 0) {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, assignedNameText, TokenFlagsNone);
        ...
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, "", TokenFlagsNone);
    }
    else {
        assignedName = NewStringLiteral(factory.__tsgoEmbedded0, "default", TokenFlagsNone);
    }
    const expression = finishTransformNamedEvaluation(emitContext, node.Expression, assignedName, ignoreEmptyStringLiteral);
    return NodeFactory_UpdateExportAssignment(factory.__tsgoEmbedded0, node, undefined, // modifiers
    node.IsExportEquals, undefined, // typeNode
    expression);
}
/**
...
//# sourceMappingURL=namedevaluation.js.map