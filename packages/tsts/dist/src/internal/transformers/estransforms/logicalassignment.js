 ...
 * }
 */
export function logicalAssignmentTransformer_visit(receiver, node) {
 ...
 * }
 */
export function logicalAssignmentTransformer_visitBinaryExpression(receiver, node) {
    ...
    if (IsAccessExpression(left)) {
        ...
        if (!propertyAccessTargetSimpleCopiable) {
            ...
            propertyAccessTargetAssignment = NodeFactory_NewAssignmentExpression(printerFactory, propertyAccessTarget, Node_Expression(left));
        }
        if (IsPropertyAccessExpression(left)) {
            assignmentTarget = NewPropertyAccessExpression(astFactory, propertyAccessTarget, undefined, Node_Name(left), NodeFlagsNone);
            left = NewPropertyAccessExpression(astFactory, propertyAccessTargetAssignment, undefined, Node_Name(left), NodeFlagsNone);
        }
        else {
            ...
            if (!elementAccessArgumentSimpleCopiable) {
                ...
                argumentExpr = NodeFactory_NewAssignmentExpression(printerFactory, elementAccessArgument, eae.ArgumentExpression);
            }
            assignmentTarget = NewElementAccessExpression(astFactory, propertyAccessTarget, undefined, elementAccessArgument, NodeFlagsNone);
            left = NewElementAccessExpression(astFactory, propertyAccessTargetAssignment, undefined, argumentExpr, NodeFlagsNone);
        }
    }
    return NewBinaryExpression(astFactory, undefined, left, undefined, NewToken(astFactory, nonAssignmentOperator), NewParenthesizedExpression(astFactory, NodeFactory_NewAssignmentExpression(printerFactory, assignmentTarget, right)));
}
/**
 ...
 * }
 */
export function newLogicalAssignmentTransformer(opts) {
    const tx = {
...
//# sourceMappingURL=logicalassignment.js.map