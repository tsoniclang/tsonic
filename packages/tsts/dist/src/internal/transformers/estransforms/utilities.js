...
import { NodeFactory_NewGeneratedPrivateNameForNodeEx, NodeFactory_NewAssignmentExpression, NodeFactory_NewThisExpression, NodeFactory_NewStrictEqualityExpression, NodeFactory_NewStrictInequalityExpression, NodeFactory_NewVoidZeroExpression, } from "../../printer/factory.js";
import { NewBinaryExpression, NewClassExpression, NewPropertyAccessExpression, NewKeywordExpression, NewIdentifier, NewToken, NewArrowFunction, NewParameterDeclaration, NewPropertyAssignment, NewObjectLiteralExpression, NewCallExpression, NewVariableDeclaration, NewVariableDeclarationList, NewVariableStatement, } from "../../ast/generated/factory.js";
...
import { KindSuperKeyword, KindNullKeyword, KindEqualsGreaterThanToken, KindCallExpression as KindCallExpressionKind, KindPropertyAccessExpression as KindPropertyAccessExpressionKind, KindElementAccessExpression as KindElementAccessExpressionKind, KindBinaryExpression as KindBinaryExpressionKind, KindPrefixUnaryExpression as KindPrefixUnaryExpressionKind, KindPostfixUnaryExpression as KindPostfixUnaryExpressionKind, KindFunctionExpression, KindFunctionDeclaration, KindMethodDeclaration, KindGetAccessor, KindSetAccessor, KindConstructor, KindClassDeclaration, KindClassExpression, KindAmpersandAmpersandToken, KindBarBarToken, } from "../../ast/generated/kinds.js";
 ...
 * }
 */
export function convertClassDeclarationToClassExpression(emitContext, node) {
    const f = emitContext.Factory.__tsgoEmbedded0;
    const updated = NewClassExpression(f, ExtractModifiers(emitContext, node.modifiers, (~ModifierFlagsExportDefault)), node.name, node.TypeParameters, node.HeritageClauses, node.Members);
    EmitContext_SetOriginal(emitContext, updated, node);
    updated.Loc = node.Loc;
    return updated;
}
/**
 ...
 * }
 */
export function createNotNullCondition(emitContext, left, right, invert) {
    const f = emitContext.Factory.__tsgoEmbedded0;
    const pf = emitContext.Factory;
    const nullKeyword = NewKeywordExpression(f, KindNullKeyword);
    const voidZero = NodeFactory_NewVoidZeroExpression(pf);
    let innerLeft;
    let innerRight;
    let opToken;
    if (invert) {
        innerLeft = NodeFactory_NewStrictEqualityExpression(pf, left, nullKeyword);
        innerRight = NodeFactory_NewStrictEqualityExpression(pf, right, voidZero);
        opToken = NewToken(f, KindBarBarToken);
    }
    else {
        innerLeft = NodeFactory_NewStrictInequalityExpression(pf, left, nullKeyword);
        innerRight = NodeFactory_NewStrictInequalityExpression(pf, right, voidZero);
 ...
 * }
 */
export function superAccessState_initSuperAccessVisitor(receiver, emitContext, factory) {
    receiver.factory = factory;
    receiver.superAccessVisitor = EmitContext_NewNodeVisitor(emitContext, (node) => superAccessState_visitSuperAccessNode(receiver, node));
}
/**
 ...
 * }
 */
export function superAccessState_visitSuperAccessNode(receiver, node) {
    const f = receiver.factory.__tsgoEmbedded0;
    const visitor = receiver.superAccessVisitor;
    switch (node.Kind) {
        ...
        case KindPropertyAccessExpressionKind: {
            if (Node_Expression(node).Kind === KindSuperKeyword) {
                return NewPropertyAccessExpression(f, receiver.superBinding, undefined, Node_Name(node), NodeFlagsNone);
            }
            return NodeVisitor_VisitEachChild(visitor, node);
        }
        case KindElementAccessExpressionKind: {
            if (Node_Expression(node).Kind === KindSuperKeyword) {
                return superAccessState_createSuperElementAccessInAsyncMethod(receiver, AsElementAccessExpression(node).ArgumentExpression);
            }
            return NodeVisitor_VisitEachChild(visitor, node);
        }
 ...
 * }
 */
export function superAccessState_substituteSuperAccessesInBody(receiver, body) {
    return NodeVisitor_VisitNode(receiver.superAccessVisitor, body);
}
/**
 ...
 * }
 */
export function superAccessState_substituteCallExpressionWithSuperAccess(receiver, call, visitor) {
    const f = receiver.factory.__tsgoEmbedded0;
    ...
    if (IsPropertyAccessExpression(expression)) {
        target = NewPropertyAccessExpression(f, receiver.superBinding, undefined, AsPropertyAccessExpression(expression).name, NodeFlagsNone);
    }
    else if (IsElementAccessExpression(expression)) {
        target = superAccessState_createSuperElementAccessInAsyncMethod(receiver, AsElementAccessExpression(expression).ArgumentExpression);
    }
    else {
        return NodeVisitor_VisitEachChild(concreteVisitor, call);
    }
    const callTarget = NewPropertyAccessExpression(f, target, undefined, NewIdentifier(f, "call"), NodeFlagsNone);
    let allArgs = [NodeFactory_NewThisExpression(pf)];
    if (call.Arguments !== undefined) {
        const visitedArgs = NodeVisitor_VisitNodes(concreteVisitor, call.Arguments);
        if (visitedArgs !== undefined) {
            allArgs = [...allArgs, ...visitedArgs.Nodes];
        }
    }
    const result = NewCallExpression(f, callTarget, undefined, undefined, NodeFactory_NewNodeList(f, allArgs), NodeFlagsNone);
    result.Loc = call.Loc;
    return result;
}
/**
 ...
 * }
 */
export function superAccessState_createSuperElementAccessInAsyncMethod(receiver, argumentExpression) {
    const f = receiver.factory.__tsgoEmbedded0;
    const superIndexCall = NewCallExpression(f, receiver.superIndexBinding, undefined, undefined, NodeFactory_NewNodeList(f, [argumentExpression]), NodeFlagsNone);
    if (receiver.hasSuperPropertyAssignment) {
        return NewPropertyAccessExpression(f, superIndexCall, undefined, NewIdentifier(f, "value"), NodeFlagsNone);
    }
    return superIndexCall;
}
/**
 ...
 * }
 */
export function superAccessState_createSuperAccessVariableStatement(receiver) {
    const f = receiver.factory.__tsgoEmbedded0;
    const pf = receiver.factory;
    let accessors = [];
    OrderedSet_Values(receiver.capturedSuperProperties)((name) => {
        let descriptorProperties = [];
        const getterBody = NewPropertyAccessExpression(f, NewKeywordExpression(f, KindSuperKeyword), undefined, NewIdentifier(f, name), NodeFlagsNone);
        const getterArrow = NewArrowFunction(f, undefined, undefined, NodeFactory_NewNodeList(f, []), undefined, undefined, NewToken(f, KindEqualsGreaterThanToken), getterBody);
        const getter = NewPropertyAssignment(f, undefined, NewIdentifier(f, "get"), undefined, undefined, getterArrow);
        descriptorProperties = [...descriptorProperties, getter];
        if (receiver.hasSuperPropertyAssignment) {
            const vParam = NewParameterDeclaration(f, undefined, undefined, NewIdentifier(f, "v"), undefined, undefined, undefined);
            const superProp = NewPropertyAccessExpression(f, NewKeywordExpression(f, KindSuperKeyword), undefined, NewIdentifier(f, name), NodeFlagsNone);
            const assignExpr = NodeFactory_NewAssignmentExpression(pf, superProp, NewIdentifier(f, "v"));
            const setterArrow = NewArrowFunction(f, undefined, undefined, NodeFactory_NewNodeList(f, [vParam]), undefined, undefined, NewToken(f, KindEqualsGreaterThanToken), assignExpr);
            const setter = NewPropertyAssignment(f, undefined, NewIdentifier(f, "set"), undefined, undefined, setterArrow);
            descriptorProperties = [...descriptorProperties, setter];
        }
        const descriptor = NewObjectLiteralExpression(f, NodeFactory_NewNodeList(f, descriptorProperties), false);
        const accessor = NewPropertyAssignment(f, undefined, NewIdentifier(f, name), undefined, undefined, descriptor);
        accessors = [...accessors, accessor];
        // Go `for ... range` iterates every captured property.
        return true;
    });
    const descriptorsObject = NewObjectLiteralExpression(f, NodeFactory_NewNodeList(f, accessors), true);
    const objectCreateCall = NewCallExpression(f, NewPropertyAccessExpression(f, NewIdentifier(f, "Object"), undefined, NewIdentifier(f, "create"), NodeFlagsNone), undefined, undefined, NodeFactory_NewNodeList(f, [
        NewKeywordExpression(f, KindNullKeyword),
        descriptorsObject,
    ]), NodeFlagsNone);
    const decl = NewVariableDeclaration(f, receiver.superBinding, undefined, undefined, objectCreateCall);
    const declList = NewVariableDeclarationList(f, NodeFactory_NewNodeList(f, [decl]), NodeFlagsConst);
 ...
 * }
 */
export function superAccessState_trackSuperAccess(receiver, node) {
    ...
    switch (node.Kind) {
        ...
        case KindBinaryExpressionKind: {
            ...
            if (IsAssignmentOperator(be.OperatorToken.Kind) && assignmentTargetContainsSuperProperty(be.Left)) {
                receiver.hasSuperPropertyAssignment = true;
            }
            break;
        }
        case KindPrefixUnaryExpressionKind:
            if (isUpdateExpression(node) && assignmentTargetContainsSuperProperty(AsPrefixUnaryExpression(node).Operand)) {
                receiver.hasSuperPropertyAssignment = true;
            }
            break;
        case KindPostfixUnaryExpressionKind:
            if (isUpdateExpression(node) && assignmentTargetContainsSuperProperty(AsPostfixUnaryExpression(node).Operand)) {
                receiver.hasSuperPropertyAssignment = true;
            }
            break;
    }
 ...
 * }
 */
export function createAccessorPropertyBackingField(f, node, modifiers, initializer) {
    return NodeFactory_UpdatePropertyDeclaration(f.__tsgoEmbedded0, node, modifiers, NodeFactory_NewGeneratedPrivateNameForNodeEx(f, node.name, { Flags: GeneratedIdentifierFlagsNone, Prefix: "", Suffix: "_accessor_storage" }), undefined, undefined, initializer);
}
