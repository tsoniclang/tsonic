 ...
 * }
 */
export function exponentiationTransformer_visit(receiver, node) {
 ...
 * }
 */
export function exponentiationTransformer_visitBinaryExpression(receiver, node) {
    const visitor = Transformer_Visitor(receiver.__tsgoEmbedded0);
    switch (node.OperatorToken.Kind) {
 ...
 * }
 */
export function exponentiationTransformer_visitExponentiationAssignmentExpression(receiver, node) {
    const visitor = Transformer_Visitor(receiver.__tsgoEmbedded0);
    ...
    if (IsElementAccessExpression(left)) {
        ...
        const objExpr = NodeFactory_NewAssignmentExpression(pf, expressionTemp, Node_Expression(left));
        objExpr.Loc = Node_Expression(left).Loc;
        const accessExpr = NodeFactory_NewAssignmentExpression(pf, argumentExpressionTemp, AsElementAccessExpression(left).ArgumentExpression);
        accessExpr.Loc = AsElementAccessExpression(left).ArgumentExpression.Loc;
        target = NewElementAccessExpression(af, objExpr, undefined, accessExpr, NodeFlagsNone);
        value = NewElementAccessExpression(af, expressionTemp, undefined, argumentExpressionTemp, NodeFlagsNone);
        value.Loc = left.Loc;
    }
        ...
        const expressionTemp = NodeFactory_NewTempVariable(pf);
        EmitContext_AddVariableDeclaration(emitCtx, expressionTemp);
        const assignment = NodeFactory_NewAssignmentExpression(pf, expressionTemp, Node_Expression(left));
        assignment.Loc = Node_Expression(left).Loc;
        target = NewPropertyAccessExpression(af, assignment, undefined, Node_Name(left), NodeFlagsNone);
        target.Loc = left.Loc;
        value = NewPropertyAccessExpression(af, expressionTemp, undefined, Node_Name(left), NodeFlagsNone);
        value.Loc = left.Loc;
    }
    else {
        // Transforms `a **= b` into `a = Math.pow(a, b)`
 ...
 * }
 */
export function exponentiationTransformer_visitExponentiationExpression(receiver, node) {
    const visitor = Transformer_Visitor(receiver.__tsgoEmbedded0);
    const pf = Transformer_Factory(receiver.__tsgoEmbedded0);
    const left = NodeVisitor_VisitNode(visitor, node.Left);
    const right = NodeVisitor_VisitNode(visitor, node.Right);
 ...
 * }
 */
export function newExponentiationTransformer(opts) {
    const tx = {
