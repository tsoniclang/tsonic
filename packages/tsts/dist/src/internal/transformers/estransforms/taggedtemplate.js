...
import { NewArrayLiteralExpression, NewCallExpression, NewStringLiteral, NewVariableDeclaration, NewVariableDeclarationList, NewVariableStatement, } from "../../ast/generated/factory.js";
...
import { NodeFactory_NewAssignmentExpression, NodeFactory_NewLogicalORExpression, NodeFactory_NewTemplateObjectHelper, NodeFactory_NewUniqueName, NodeFactory_NewVoidZeroExpression, } from "../../printer/factory.js";
 ...
 */
export const newlineNormalizer = strings.NewReplacer("\r\n", "\n", "\r", "\n");
/**
 ...
 * }
 */
export function newTaggedTemplateLiftRestrictionTransformer(opts) {
    const tx = {
 ...
 * }
 */
export function taggedTemplateTransformer_visit(receiver, node) {
 ...
 * }
 */
export function taggedTemplateTransformer_visitSourceFile(receiver, node) {
    ...
    let visited = NodeVisitor_VisitEachChild(visitor, node);
    if (receiver.taggedTemplateStringDeclarations.length > 0) {
        ...
        const statements = [
            ...
            NewVariableStatement(af, undefined, NewVariableDeclarationList(af, NodeFactory_NewNodeList(af, receiver.taggedTemplateStringDeclarations), NodeFlagsNone)),
        ];
 ...
 * }
 */
export function taggedTemplateTransformer_visitTaggedTemplateExpression(receiver, node) {
 ...
 * }
 */
export function taggedTemplateTransformer_processTaggedTemplateExpression(receiver, node) {
    const visitor = Transformer_Visitor(receiver.__tsgoEmbedded0);
    const pf = Transformer_Factory(receiver.__tsgoEmbedded0);
    const af = pf.__tsgoEmbedded0;
    const tag = NodeVisitor_VisitNode(visitor, node.Tag);
    const template = node.Template;
    if (!hasInvalidEscape(template)) {
        return NodeVisitor_VisitEachChild(visitor, node);
    }
    // Build up the template arguments and the raw and cooked strings for the template.
    const templateArguments = [undefined]; // placeholder for the template object
    const cookedStrings = [];
    const rawStrings = [];
    if (IsNoSubstitutionTemplateLiteral(template)) {
        ...
        cookedStrings.push(createTemplateCooked(pf, Node_TemplateLiteralLikeData(te.Head)));
        rawStrings.push(getRawLiteral(pf, te.Head));
        for (const span of te.TemplateSpans.Nodes) {
            const ts = AsTemplateSpan(span);
            cookedStrings.push(createTemplateCooked(pf, Node_TemplateLiteralLikeData(ts.Literal)));
            rawStrings.push(getRawLiteral(pf, ts.Literal));
            templateArguments.push(NodeVisitor_VisitNode(visitor, ts.Expression));
        }
    }
    const helperCall = NodeFactory_NewTemplateObjectHelper(pf, NewArrayLiteralExpression(af, NodeFactory_NewNodeList(af, cookedStrings), false), NewArrayLiteralExpression(af, NodeFactory_NewNodeList(af, rawStrings), false));
    // Create a variable to cache the template object if we're in a module.
    // Do not do this in the global scope, as any variable we currently generate could conflict with
    // variables from outside of the current compilation. In the future, we can revisit this behavior.
    if (IsExternalModule(receiver.currentSourceFile)) {
        const tempVar = NodeFactory_NewUniqueName(pf, "templateObject");
        receiver.taggedTemplateStringDeclarations.push(NewVariableDeclaration(af, tempVar, undefined, undefined, undefined));
        templateArguments[0] = NodeFactory_NewLogicalORExpression(pf, tempVar, NodeFactory_NewAssignmentExpression(pf, tempVar, helperCall));
    }
    else {
        templateArguments[0] = helperCall;
    }
 ...
 * }
 */
export function createTemplateCooked(f, template) {
 ...
 * }
 */
export function getRawLiteral(f, node) {
 ...
 * }
 */
export function hasInvalidEscape(template) {
    if (IsNoSubstitutionTemplateLiteral(template)) {
        return ((Node_TemplateLiteralLikeData(template).TemplateFlags & TokenFlagsContainsInvalidEscape) !== 0);
    }
    const te = AsTemplateExpression(template);
...
//# sourceMappingURL=taggedtemplate.js.map