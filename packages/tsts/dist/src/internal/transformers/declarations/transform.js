...
export function NewDeclarationTransformer(host, context, compilerOptions, declarationFilePath, declarationMapPath) {
    ...
    const tx = {
        ...
        expandoHosts: { entries: undefined },
        rawReferencedFiles: [],
...
export function DeclarationTransformer_visitSourceFile(receiver, node) {
    ...
    receiver.expandoHosts = { entries: undefined };
    ...
    if (inner !== undefined && inner.Kind === KindModuleBlock) {
        ...
        const lateStatements = (!IsGlobalScopeAugmentation(input) && !receiver.resultHasScopeMarker && !hasScopeMarker(lateStatementsResult))
            ? (receiver.needsScopeFixMarker
                ...
                : NodeVisitor_VisitNodes(EmitContext_NewNodeVisitor(emitContext, (s) => DeclarationTransformer_stripExportModifiers(receiver, s)), lateStatementsResult))
...
export function DeclarationTransformer_transformVariableStatement(receiver, input) {
    ...
    if (IsVarUsing(input.DeclarationList) || IsVarAwaitUsing(input.DeclarationList)) {
        ...
        if (nodes !== declList.Declarations || declList.Flags !== input.DeclarationList.Flags) {
...
export function DeclarationTransformer_transformEnumDeclaration(receiver, input) {
    ...
    const newMembers = MapNonNil(input.Members.Nodes, (m) => {
        ...
        const newInitializer = (typeof value === 'number')
            ? (value >= 0
...
export function DeclarationTransformer_ensureModifierFlags(receiver, node) {
    ...
    const additions = parentIsFile
        ? (IsImplicitlyExportedJSTypeAlias(node) ? (baseAdditions | ModifierFlagsExport) : baseAdditions)
 ...
 * }
 */
export function DeclarationTransformer_filterBindingPatternInitializers(receiver, node) {
    ...
    for (const elem of Node_Elements(node)) {
        ...
        elements.push(updateNode(NewBindingElement(astFactory, bindingElem.DotDotDotToken, bindingElem.PropertyName, DeclarationTransformer_filterBindingPatternInitializers(receiver, elemName), undefined), elem, astFactory.hooks));
    }
...
export function DeclarationTransformer_transformExpandoAssignment(receiver, node) {
    ...
    const varDeclList = NewVariableDeclarationList(astFactory, NodeFactory_NewNodeList(astFactory, [varDecl]), NodeFlagsNone);
    const varStatement = NewVariableStatement(astFactory, undefined, varDeclList);
    const statements = isNonContextualKeywordName
        ? [
            varStatement,
            NewExportDeclaration(astFactory, undefined, false, NewNamedExports(astFactory, NodeFactory_NewNodeList(astFactory, [NewExportSpecifier(astFactory, false, exportName, NewIdentifier(astFactory, Node_Text(Node_Name(left))))])), undefined, undefined),
        ]
        : [varStatement];
...
export function DeclarationTransformer_transformExpandoHost(receiver, name, declaration) {
    ...
    const finalReplacement = defaultExport
        ? [...replacement, NewExportAssignment(astFactory, undefined, false, undefined, name)]
        : replacement;
