...
import { KindExportAssignment, KindExportDeclaration, KindExportSpecifier, KindImportClause, KindImportDeclaration, KindImportEqualsDeclaration, KindImportSpecifier, KindModuleBlock, KindModuleDeclaration, KindNamedExports, KindNamedImports, KindNamespaceImport, KindSourceFile, } from "../../ast/generated/kinds.js";
 ...
 * }
 */
export function NewImportElisionTransformer(opt) {
 ...
 * }
 */
export function ImportElisionTransformer_visit(receiver, node) {
    const tx = receiver;
 ...
 * }
 */
export function ImportElisionTransformer_shouldEmitAliasDeclaration(receiver, node) {
    return (IsInJSFile(node) || ImportElisionTransformer_isReferencedAliasDeclaration(receiver, node));
}
/**
 ...
 * }
 */
export function ImportElisionTransformer_shouldEmitImportEqualsDeclaration(receiver, node) {
    // preserve old compiler's behavior: emit import declaration (even if we do not consider them referenced) when
    // - current file is not external module
    // - import declaration is top level and target is value imported by entity name
    return (ImportElisionTransformer_shouldEmitAliasDeclaration(receiver, node) ||
        (!IsExternalModule(receiver.currentSourceFile) && ImportElisionTransformer_isTopLevelValueImportEqualsWithEntityName(receiver, node)));
}
/**
 ...
 * }
 */
export function ImportElisionTransformer_isReferencedAliasDeclaration(receiver, node) {
 ...
 * }
 */
export function ImportElisionTransformer_isValueAliasDeclaration(receiver, node) {
    const parsed = EmitContext_ParseNode(Transformer_EmitContext(receiver.__tsgoEmbedded0), node);
 ...
 * }
 */
export function ImportElisionTransformer_isTopLevelValueImportEqualsWithEntityName(receiver, node) {
    const parsed = EmitContext_ParseNode(Transformer_EmitContext(receiver.__tsgoEmbedded0), node);
    return parsed !== undefined && receiver.emitResolver.IsTopLevelValueImportEqualsWithEntityName(parsed);
}
//# sourceMappingURL=importelision.js.map