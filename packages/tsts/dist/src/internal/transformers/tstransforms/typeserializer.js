 ...
 * }
 */
export function newMetadataSerializer(resolver, f, ec, languageVersion, strictNullChecks) {
    return {
        resolver: resolver,
 ...
 * }
 */
export function metadataSerializer_setContext(receiver, ctx) {
    receiver.c = ctx;
}
/**
 ...
 * }
 */
export function metadataSerializer_SerializeTypeOfNode(receiver, ctx, node, container) {
 ...
 * }
 */
export function metadataSerializer_SerializeParameterTypesOfNode(receiver, ctx, node, container) {
    const oldCtx = receiver.c;
    receiver.c = ctx;
    try {
 ...
 * }
 */
export function metadataSerializer_SerializeReturnTypeOfNode(receiver, ctx, node) {
    const oldCtx = receiver.c;
    receiver.c = ctx;
    try {
 ...
 * }
 */
export function getAccessorTypeNode(node, container) {
    const members = Node_Members(container) ?? [];
    const accessors = GetAllAccessorDeclarations(members, node);
 ...
 * }
 */
export function metadataSerializer_serializeTypeOfNode(receiver, node, container) {
 ...
 * }
 */
export function metadataSerializer_serializeParameterTypesOfNode(receiver, node, container) {
    const f = receiver.f.__tsgoEmbedded0;
    const valueDeclaration = IsClassLike(node) ? GetFirstConstructorWithBody(node) :
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (i === 0 && IsIdentifier(Node_Name(parameter)) && Node_Text(Node_Name(parameter)) === "this") {
            continue;
        }
 ...
 * }
 */
export function getParametersOfDecoratedDeclaration(node, container) {
    if (container !== undefined && node.Kind === KindGetAccessor) {
        const members = Node_Members(container) ?? [];
        const acc = GetAllAccessorDeclarations(members, node);
        if (acc.SetAccessor !== undefined) {
            return acc.SetAccessor.Parameters;
        }
    }
    return Node_ParameterList(node);
}
/**
 ...
 * }
 */
export function metadataSerializer_serializeReturnTypeOfNode(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
 ...
 * }
 */
export function metadataSerializer_serializeTypeNode(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
    if (node === undefined) {
    ...
    switch (skipped.Kind) {
        ...
        case KindConditionalType: {
            ...
            try {
                return metadataSerializer_serializeUnionOrIntersectionConstituents(receiver, [AsConditionalTypeNode(skipped).TrueType, AsConditionalTypeNode(skipped).FalseType], false);
            }
            finally {
 ...
 * }
 */
export function metadataSerializer_serializeUnionOrIntersectionConstituents(receiver, types, isIntersection) {
    const f = receiver.f.__tsgoEmbedded0;
    let serializedType = undefined;
 ...
 * }
 */
export function metadataSerializer_serializeLiteralOfLiteralTypeNode(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
    switch (node.Kind) {
 ...
 * }
 */
export function metadataSerializer_serializeTypeReferenceNode(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
    const serialScope = receiver.c.currentNameScope !== undefined ? receiver.c.currentNameScope : receiver.c.currentLexicalScope;
    const kind = receiver.resolver.GetTypeReferenceSerializationKind(EmitContext_ParseNode(receiver.ec, node.TypeName), EmitContext_ParseNode(receiver.ec, serialScope));
    switch (kind) {
        case TypeReferenceSerializationKindUnknown: {
            ...
            return NewConditionalExpression(f, NodeFactory_NewTypeCheck(receiver.f, NodeFactory_NewAssignmentExpression(receiver.f, temp, serialized), "function"), NewToken(f, KindQuestionToken), temp, NewToken(f, KindColonToken), NewIdentifier(f, "Object"));
        }
 ...
 * }
 */
export function metadataSerializer_serializeBigIntConstructor(receiver) {
    const f = receiver.f.__tsgoEmbedded0;
    if (receiver.languageVersion >= ScriptTargetES2020) {
        return NewIdentifier(f, "BigInt");
    }
    return NewConditionalExpression(f, NodeFactory_NewTypeCheck(receiver.f, NewIdentifier(f, "BigInt"), "function"), NewToken(f, KindQuestionToken), NewIdentifier(f, "BigInt"), NewToken(f, KindColonToken), NewIdentifier(f, "Object"));
}
/**
 ...
 * }
 */
export function metadataSerializer_serializeEntityNameAsExpression(receiver, node) {
    switch (node.Kind) {
        case KindIdentifier: {
            const f = receiver.f.__tsgoEmbedded0;
            const coercible = { AsNodeFactory: () => f };
            const name = Node_Clone(node, coercible);
            name.Loc = node.Loc;
        ...
        case KindQualifiedName:
            return metadataSerializer_serializeQualifiedNameAsExpression(receiver, AsQualifiedName(node));
        default:
            return undefined;
    }
}
 ...
 * }
 */
export function metadataSerializer_serializeQualifiedNameAsExpression(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
    return NewPropertyAccessExpression(f, metadataSerializer_serializeEntityNameAsExpression(receiver, node.Left), undefined, node.Right, NodeFlagsNone);
}
/**
 ...
 * }
 */
export function metadataSerializer_serializeEntityNameAsExpressionFallback(receiver, node) {
    const f = receiver.f.__tsgoEmbedded0;
    ...
    if (AsQualifiedName(node).Left.Kind === KindIdentifier) {
        return metadataSerializer_createCheckedValue(receiver, metadataSerializer_serializeEntityNameAsExpression(receiver, AsQualifiedName(node).Left), metadataSerializer_serializeEntityNameAsExpression(receiver, node));
    }
    const left = metadataSerializer_serializeEntityNameAsExpressionFallback(receiver, AsQualifiedName(node).Left);
    const temp = NodeFactory_NewTempVariable(receiver.f);
    EmitContext_AddVariableDeclaration(receiver.ec, temp);
    return NodeFactory_NewLogicalANDExpression(receiver.f, NodeFactory_NewLogicalANDExpression(receiver.f, AsBinaryExpression(left).Left, NodeFactory_NewStrictInequalityExpression(receiver.f, NodeFactory_NewAssignmentExpression(receiver.f, temp, AsBinaryExpression(left).Right), NodeFactory_NewVoidZeroExpression(receiver.f))), NewPropertyAccessExpression(f, temp, undefined, AsQualifiedName(node).Right, NodeFlagsNone));
}
/**
 ...
 * }
 */
export function metadataSerializer_createCheckedValue(receiver, left, right) {
    const f = receiver.f.__tsgoEmbedded0;
    return NodeFactory_NewLogicalANDExpression(receiver.f, NodeFactory_NewStrictInequalityExpression(receiver.f, NewTypeOfExpression(f, left), NewStringLiteral(f, "undefined", TokenFlagsNone)), right);
}
/**
 ...
 * }
 */
export function metadataSerializer_equateSerializedTypeNodes(receiver, left, right) {
    ...
    if (IsPropertyAccessExpression(left)) {
        return IsPropertyAccessExpression(right) &&
            ...
            metadataSerializer_equateSerializedTypeNodes(receiver, Node_Name(left), Node_Name(right));
    }
