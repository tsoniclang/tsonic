 ...
 */
export const USE_NEW_TYPE_METADATA_FORMAT = false;
/**
 ...
 * }
 */
export function NewMetadataTransformer(opt) {
    const tx = {
        __tsgoEmbedded0: { emitContext: undefined, factory: undefined, visitor: undefined },
        legacyDecorators: Tristate_IsTrue(opt.CompilerOptions.ExperimentalDecorators),
        resolver: opt.EmitResolver,
        serializer: undefined,
        strictNullChecks: CompilerOptions_GetStrictOptionValue(opt.CompilerOptions, opt.CompilerOptions.StrictNullChecks),
        parent: undefined,
 ...
 * }
 */
export function MetadataTransformer_visit(receiver, node) {
    ...
    switch (node.Kind) {
        case KindMethodDeclaration:
            return MetadataTransformer_visitMethodDeclaration(receiver, node);
        case KindSetAccessor:
            return MetadataTransformer_visitSetAccessor(receiver, node);
        case KindGetAccessor:
            return MetadataTransformer_visitGetAccessor(receiver, node);
        case KindPropertyDeclaration:
            return MetadataTransformer_visitPropertyDeclaration(receiver, node);
        case KindSourceFile: {
        ...
        case KindClassDeclaration:
            return MetadataTransformer_visitClassDeclaration(receiver, node);
        case KindClassExpression:
            return MetadataTransformer_visitClassExpression(receiver, node);
        default:
 ...
 * }
 */
export function MetadataTransformer_setParent(receiver, node) {
    const tx = receiver;
    tx.parent = node;
}
/**
 ...
 * }
 */
export function MetadataTransformer_setCurrentLexicalScope(receiver, node) {
    const tx = receiver;
    tx.currentLexicalScope = node;
}
/**
 ...
 * }
 */
export function MetadataTransformer_visitClassExpression(receiver, node) {
    const tx = receiver;
    ...
    try {
        if (!ClassOrConstructorParameterIsDecorated(tx.legacyDecorators, nodeAsNode)) {
            ...
            result = NodeFactory_UpdateClassExpression(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNodes(visitor, node.TypeParameters), NodeVisitor_VisitNodes(visitor, node.HeritageClauses), NodeVisitor_VisitNodes(visitor, node.Members));
        }
    }
    finally {
 ...
 * }
 */
export function MetadataTransformer_visitClassDeclaration(receiver, node) {
    const tx = receiver;
    const oldParent = tx.parent;
    tx.parent = node;
    const visitor = Transformer_Visitor(tx.__tsgoEmbedded0);
    ...
    try {
        if (!ClassOrConstructorParameterIsDecorated(tx.legacyDecorators, nodeAsNode)) {
            ...
            const modifiers = MetadataTransformer_injectClassTypeMetadata(receiver, NodeVisitor_VisitModifiers(visitor, Node_Modifiers(nodeAsNode)), nodeAsNode);
            result = NodeFactory_UpdateClassDeclaration(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNodes(visitor, node.TypeParameters), NodeVisitor_VisitNodes(visitor, node.HeritageClauses), NodeVisitor_VisitNodes(visitor, node.Members));
        }
    }
    finally {
        MetadataTransformer_setParent(receiver, oldParent);
 ...
 * }
 */
export function MetadataTransformer_visitPropertyDeclaration(receiver, node) {
    const tx = receiver;
    const visitor = Transformer_Visitor(tx.__tsgoEmbedded0);
    const astFactory = Transformer_Factory(tx.__tsgoEmbedded0).__tsgoEmbedded0;
    const nodeAsNode = node;
    ...
    return NodeFactory_UpdatePropertyDeclaration(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNode(visitor, node.PostfixToken), NodeVisitor_VisitNode(visitor, node.Type), NodeVisitor_VisitNode(visitor, node.Initializer));
}
/**
 ...
 * }
 */
export function MetadataTransformer_visitMethodDeclaration(receiver, node) {
    const tx = receiver;
    const visitor = Transformer_Visitor(tx.__tsgoEmbedded0);
    const astFactory = Transformer_Factory(tx.__tsgoEmbedded0).__tsgoEmbedded0;
    const nodeAsNode = node;
    ...
    const modifiers = MetadataTransformer_injectClassElementTypeMetadata(receiver, NodeVisitor_VisitModifiers(visitor, Node_Modifiers(nodeAsNode)), nodeAsNode, tx.parent);
    return NodeFactory_UpdateMethodDeclaration(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.AsteriskToken), NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNode(visitor, node.PostfixToken), NodeVisitor_VisitNodes(visitor, node.TypeParameters), NodeVisitor_VisitNodes(visitor, node.Parameters), NodeVisitor_VisitNode(visitor, node.Type), NodeVisitor_VisitNode(visitor, node.FullSignature), NodeVisitor_VisitNode(visitor, node.Body));
}
/**
 ...
 * }
 */
export function MetadataTransformer_visitSetAccessor(receiver, node) {
    const tx = receiver;
    const visitor = Transformer_Visitor(tx.__tsgoEmbedded0);
    const astFactory = Transformer_Factory(tx.__tsgoEmbedded0).__tsgoEmbedded0;
    const nodeAsNode = node;
    ...
    const modifiers = MetadataTransformer_injectClassElementTypeMetadata(receiver, NodeVisitor_VisitModifiers(visitor, Node_Modifiers(nodeAsNode)), nodeAsNode, tx.parent);
    return NodeFactory_UpdateSetAccessorDeclaration(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNodes(visitor, node.TypeParameters), NodeVisitor_VisitNodes(visitor, node.Parameters), NodeVisitor_VisitNode(visitor, node.Type), NodeVisitor_VisitNode(visitor, node.FullSignature), NodeVisitor_VisitNode(visitor, node.Body));
}
/**
 ...
 * }
 */
export function MetadataTransformer_visitGetAccessor(receiver, node) {
    const tx = receiver;
    const visitor = Transformer_Visitor(tx.__tsgoEmbedded0);
    const astFactory = Transformer_Factory(tx.__tsgoEmbedded0).__tsgoEmbedded0;
    const nodeAsNode = node;
    ...
    const modifiers = MetadataTransformer_injectClassElementTypeMetadata(receiver, NodeVisitor_VisitModifiers(visitor, Node_Modifiers(nodeAsNode)), nodeAsNode, tx.parent);
    return NodeFactory_UpdateGetAccessorDeclaration(astFactory, node, modifiers, NodeVisitor_VisitNode(visitor, node.name), NodeVisitor_VisitNodes(visitor, node.TypeParameters), NodeVisitor_VisitNodes(visitor, node.Parameters), NodeVisitor_VisitNode(visitor, node.Type), NodeVisitor_VisitNode(visitor, node.FullSignature), NodeVisitor_VisitNode(visitor, node.Body));
}
/**
 ...
 * }
 */
export function MetadataTransformer_injectClassTypeMetadata(receiver, list, node) {
    const tx = receiver;
 ...
 * }
 */
export function MetadataTransformer_injectClassElementTypeMetadata(receiver, list, node, container) {
    const tx = receiver;
    const factory = Transformer_Factory(tx.__tsgoEmbedded0);
 ...
 * }
 */
export function MetadataTransformer_getTypeMetadata(receiver, node, container) {
    const tx = receiver;
    // Decorator metadata is not yet supported for ES decorators.
    if (!tx.legacyDecorators) {
        return [];
 ...
 * }
 */
export function MetadataTransformer_getOldTypeMetadata(receiver, node, container) {
    const tx = receiver;
    let decorators = [];
    const factory = Transformer_Factory(tx.__tsgoEmbedded0);
    if (MetadataTransformer_shouldAddTypeMetadata(tx, node)) {
        const typeMetadata = NodeFactory_NewMetadataHelper(factory, "design:type", metadataSerializer_SerializeTypeOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node, container));
    ...
    if (MetadataTransformer_shouldAddParamTypesMetadata(tx, node)) {
        const paramTypesMetadata = NodeFactory_NewMetadataHelper(factory, "design:paramtypes", metadataSerializer_SerializeParameterTypesOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node, container));
    ...
    if (MetadataTransformer_shouldAddReturnTypeMetadata(tx, node)) {
        const returnTypeMetadata = NodeFactory_NewMetadataHelper(factory, "design:returntype", metadataSerializer_SerializeReturnTypeOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node));
 ...
 * }
 */
export function MetadataTransformer_getNewTypeMetadata(receiver, node, container) {
    const tx = receiver;
    let properties = [];
    const factory = Transformer_Factory(tx.__tsgoEmbedded0);
    ...
    if (MetadataTransformer_shouldAddTypeMetadata(tx, node)) {
        properties = [
            ...properties,
            NewPropertyAssignment(astFactory, undefined, NewIdentifier(astFactory, "type"), undefined, undefined, NewArrowFunction(astFactory, undefined, undefined, NodeFactory_NewNodeList(astFactory, []), undefined, undefined, NewToken(astFactory, KindEqualsGreaterThanToken), metadataSerializer_SerializeTypeOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node, container))),
        ];
    }
    if (MetadataTransformer_shouldAddParamTypesMetadata(tx, node)) {
        properties = [
            ...properties,
            NewPropertyAssignment(astFactory, undefined, NewIdentifier(astFactory, "paramTypes"), undefined, undefined, NewArrowFunction(astFactory, undefined, undefined, NodeFactory_NewNodeList(astFactory, []), undefined, undefined, NewToken(astFactory, KindEqualsGreaterThanToken), metadataSerializer_SerializeParameterTypesOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node, container))),
        ];
    }
    if (MetadataTransformer_shouldAddReturnTypeMetadata(tx, node)) {
        properties = [
            ...properties,
            NewPropertyAssignment(astFactory, undefined, NewIdentifier(astFactory, "returnType"), undefined, undefined, NewArrowFunction(astFactory, undefined, undefined, NodeFactory_NewNodeList(astFactory, []), undefined, undefined, NewToken(astFactory, KindEqualsGreaterThanToken), metadataSerializer_SerializeReturnTypeOfNode(tx.serializer, { currentLexicalScope: tx.currentLexicalScope, currentNameScope: container, serializingConditionalTypeBranch: false }, node))),
        ];
    }
    if (properties.length > 0) {
        const typeInfoMetadata = NodeFactory_NewMetadataHelper(factory, "design:typeinfo", NewObjectLiteralExpression(astFactory, NodeFactory_NewNodeList(astFactory, properties), true));
 ...
 * }
 */
export function MetadataTransformer_shouldAddTypeMetadata(receiver, node) {
    switch (node.Kind) {
        case KindMethodDeclaration:
        case KindGetAccessor:
        case KindSetAccessor:
 ...
 * }
 */
export function MetadataTransformer_shouldAddReturnTypeMetadata(receiver, node) {
 ...
 * }
 */
export function MetadataTransformer_shouldAddParamTypesMetadata(receiver, node) {
    switch (node.Kind) {
        case KindClassDeclaration:
        case KindClassExpression:
