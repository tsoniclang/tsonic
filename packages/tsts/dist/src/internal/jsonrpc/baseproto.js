 ...
 * )
 */
export const ErrInvalidHeader = errors.New("jsonrpc: invalid header");
export const ErrInvalidContentLength = errors.New("jsonrpc: invalid content length");
export const ErrNoContentLength = errors.New("jsonrpc: no content length");
/**
 ...
 * }
 */
export function Reader_Read(receiver) {
    let contentLength = 0;
    for (;;) {
        ...
        if (bytes.Equal(line, [13, 10])) {
            break;
        }
        const [key, value, ok] = bytes.Cut(line, [":".charCodeAt(0)]);
        if (!ok) {
            return [[], fmt.Errorf("%w: %q", ErrInvalidHeader, line)];
        }
        if (bytes.Equal(key, Array.from(new TextEncoder().encode("Content-Length")))) {
    ...
    const data = new globalThis.Array(contentLength).fill(0);
 ...
 * }
 */
export function Writer_Write(receiver, data) {
...
//# sourceMappingURL=baseproto.js.map