 ...
 * }
 */
export function FailBadSyntaxKind(node, ...message) {
    const msg = message.length === 0 ? "Unexpected node." : Sprint(...message);
    Fail(Sprintf("%s\nNode %s was unexpected.", msg, node.KindString()));
}
/**
 ...
 * }
 */
export function AssertNever(member, ...message) {
    const msg = message.length === 0 ? "Illegal value:" : Sprint(...message);
    const mKind = member;
    const mStr = member;
    const detail = typeof mKind?.KindString === "function" ? mKind.KindString()
 ...
 * }
 */
export function assertSlow(...message) {
    // See https://dave.cheney.net/2020/05/02/mid-stack-inlining-in-go
    const msg = message.length > 0 ? "False expression: " + Sprint(...message) : "False expression.";
    Fail(msg);
}
