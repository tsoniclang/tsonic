...
import { CompareDiagnostics, Diagnostic_Code as ASTDiagnostic_Code, Diagnostic_Category as ASTDiagnostic_Category, Diagnostic_End as ASTDiagnostic_End, Diagnostic_File as ASTDiagnostic_File_inner, Diagnostic_Len as ASTDiagnostic_Len, Diagnostic_Localize as ASTDiagnostic_Localize, Diagnostic_MessageChain as ASTDiagnostic_MessageChain_inner, Diagnostic_Pos as ASTDiagnostic_Pos, Diagnostic_RelatedInformation as ASTDiagnostic_RelatedInformation_inner, } from "../ast/diagnostic.js";
import { UTF16Len } from "../core/core.js";
import { byteLen, byteSlice } from "../parser/utilities.js";
import { Tristate_IsTrue } from "../core/tristate.js";
import { Category_Name, CategoryError, CategoryMessage, CategorySuggestion, CategoryWarning, } from "../diagnostics/diagnostics.js";
import { Errors_Files, File_appears_to_be_binary, File_change_detected_Starting_incremental_compilation, Found_0_errors, Found_0_errors_in_1_files, Found_0_errors_in_the_same_file_starting_at_Colon_1, Found_1_error, Found_1_error_in_0, Starting_compilation_in_watch_mode, } from "../diagnostics/generated/messages.js";
import { Message_Code, Message_Localize } from "../diagnostics/diagnostics.js";
import { GetECMALineAndUTF16CharacterOfPosition, GetECMALineOfPosition, GetECMAPositionOfLineAndByteOffset, } from "../scanner/scanner.js";
 ...
 * }
 */
export function ASTDiagnostic_RelatedInformation(receiver) {
    const related = ASTDiagnostic_RelatedInformation_inner(receiver.__tsgoEmbedded0);
    const result = new Array(related.length);
    for (let i = 0; i < related.length; i++) {
        result[i] = WrapASTDiagnostic(related[i]);
    }
    return result;
}
/**
 ...
 * }
 */
export function ASTDiagnostic_File(receiver) {
 ...
 * }
 */
export function ASTDiagnostic_MessageChain(receiver) {
    const chain = ASTDiagnostic_MessageChain_inner(receiver.__tsgoEmbedded0);
    const result = new Array(chain.length);
    for (let i = 0; i < chain.length; i++) {
        result[i] = WrapASTDiagnostic(chain[i]);
    }
    return result;
}
/**
 ...
 * }
 */
export function WrapASTDiagnostic(d) {
    const result = {
        __tsgoEmbedded0: d,
    };
    result.File = () => ASTDiagnostic_File(result);
    result.Pos = () => ASTDiagnostic_Pos(result.__tsgoEmbedded0);
    result.End = () => ASTDiagnostic_End(result.__tsgoEmbedded0);
    result.Len = () => ASTDiagnostic_Len(result.__tsgoEmbedded0);
    result.Code = () => ASTDiagnostic_Code(result.__tsgoEmbedded0);
    result.Category = () => ASTDiagnostic_Category(result.__tsgoEmbedded0);
    result.Localize = (locale_) => ASTDiagnostic_Localize(result.__tsgoEmbedded0, locale_);
    result.MessageChain = () => ASTDiagnostic_MessageChain(result);
    result.RelatedInformation = () => ASTDiagnostic_RelatedInformation(result);
    return result;
}
/**
 ...
 * }
 */
export function WrapASTDiagnostics(diags) {
    const result = new Array(diags.length);
 ...
 * }
 */
export function FromASTDiagnostics(diags) {
    const result = new Array(diags.length);
    for (let i = 0; i < diags.length; i++) {
        result[i] = WrapASTDiagnostic(diags[i]);
    }
 ...
 * }
 */
export function ToDiagnostics(diags) {
    const result = new Array(diags.length);
    for (let i = 0; i < diags.length; i++) {
        result[i] = diags[i];
    }
 ...
 * }
 */
export function CompareASTDiagnostics(a, b) {
 ...
 * )
 */
export const foregroundColorEscapeGrey = "[90m";
export const foregroundColorEscapeRed = "[91m";
export const foregroundColorEscapeYellow = "[93m";
export const foregroundColorEscapeBlue = "[94m";
export const foregroundColorEscapeCyan = "[96m";
/**
 ...
 * )
 */
export const gutterStyleSequence = "[7m";
 ...
 * }
 */
export function FormatDiagnosticsWithColorAndContext(output, diags, formatOpts) {
    if (diags.length === 0) {
        return;
    }
    for (let i = 0; i < diags.length; i++) {
 ...
 * }
 */
export function FormatDiagnosticWithColorAndContext(output, diagnostic, formatOpts) {
 ...
 * }
 */
export function writeCodeSnippet(writer, sourceFile, start, length, squiggleColor, indent, formatOpts) {
 ...
 * }
 */
export function FlattenDiagnosticMessage(d, newLine, locale) {
 ...
 * }
 */
export function WriteFlattenedASTDiagnosticMessage(writer, diagnostic, newline, locale) {
    WriteFlattenedDiagnosticMessage(writer, WrapASTDiagnostic(diagnostic), newline, locale);
}
/**
 ...
 * }
 */
export function WriteFlattenedDiagnosticMessage(writer, diagnostic, newline, locale) {
 ...
 * }
 */
export function flattenDiagnosticMessageChain(writer, chain, newLine, locale, level) {
 ...
 * }
 */
export function getCategoryFormat(category) {
    switch (category) {
        case CategoryError:
            return foregroundColorEscapeRed;
 ...
 * }
 */
export function writeWithStyleAndReset(output, text, formatStyle) {
 ...
 * }
 */
export function WriteLocation(output, file, pos, formatOpts, writeWithStyleAndReset) {
    const [firstLine, firstChar] = GetECMALineAndUTF16CharacterOfPosition(file, pos);
    let relativeFileName;
    if (formatOpts !== undefined) {
        relativeFileName = ConvertToRelativePath(file.FileName(), formatOpts.__tsgoEmbedded0);
    }
    else {
        relativeFileName = file.FileName();
    }
 ...
 * }
 */
export function WriteErrorSummaryText(output, allDiagnostics, formatOpts) {
 ...
 * }
 */
export function getErrorSummary(diags) {
    let totalErrorCount = 0;
    let globalErrors = [];
    let errorsByFile = undefined;
    ...
    const effectiveErrorsByFile = errorsByFile ?? new Map();
    const sortedFiles = SortedFunc(Keys(effectiveErrorsByFile), (a, b) => {
 ...
 * }
 */
export function writeTabularErrorsDisplay(output, errorSummary, formatOpts) {
 ...
 * }
 */
export function prettyPathForFileError(file, fileErrors, formatOpts) {
    if (file === undefined || fileErrors.length === 0) {
        return "";
    }
    const line = GetECMALineOfPosition(file, fileErrors[0].Pos());
    let fileName = file.FileName();
    if (PathIsAbsolute(fileName) && PathIsAbsolute(formatOpts.__tsgoEmbedded0.CurrentDirectory)) {
        fileName = ConvertToRelativePath(file.FileName(), formatOpts.__tsgoEmbedded0);
    }
    return Sprintf("%s%s:%d%s", fileName, foregroundColorEscapeGrey, line + 1, resetEscapeSequence);
}
/**
 ...
 * }
 */
export function WriteFormatDiagnostics(output, diagnostics, formatOpts) {
 ...
 * }
 */
export function WriteFormatDiagnostic(output, diagnostic, formatOpts) {
    if (diagnostic.File() !== undefined) {
        const [line, character] = GetECMALineAndUTF16CharacterOfPosition(diagnostic.File(), diagnostic.Pos());
        const fileName = diagnostic.File().FileName();
        const relativeFileName = ConvertToRelativePath(fileName, formatOpts.__tsgoEmbedded0);
 ...
 * }
 */
export function FormatDiagnosticsStatusWithColorAndTime(output, time, diag, formatOpts) {
    Fprint(output, "[");
    writeWithStyleAndReset(output, time, foregroundColorEscapeGrey);
    Fprint(output, "] ");
    WriteFlattenedDiagnosticMessage(output, diag, formatOpts.NewLine, formatOpts.Locale);
 ...
 * }
 */
export function FormatDiagnosticsStatusAndTime(output, time, diag, formatOpts) {
    Fprint(output, time, " - ");
    WriteFlattenedDiagnosticMessage(output, diag, formatOpts.NewLine, formatOpts.Locale);
}
/**
 ...
 * }
 */
export let ScreenStartingCodes = [
 ...
 * }
 */
export function TryClearScreen(output, diag, options) {
    if (!Tristate_IsTrue(options.PreserveWatchOutput) &&
