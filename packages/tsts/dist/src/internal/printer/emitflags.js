 ...
 * )
 */
export const EFSingleLine = 1 << 0; // The contents of this node should be emitted on a single line.
export const EFMultiLine = 1 << 1; // The contents of this node should be emitted on multiple lines.
export const EFNoLeadingSourceMap = 1 << 2; // Do not emit a leading source map location for this node.
export const EFNoTrailingSourceMap = 1 << 3; // Do not emit a trailing source map location for this node.
export const EFNoNestedSourceMaps = 1 << 4; // Do not emit source map locations for children of this node.
export const EFNoTokenLeadingSourceMaps = 1 << 5; // Do not emit leading source map location for token nodes.
export const EFNoTokenTrailingSourceMaps = 1 << 6; // Do not emit trailing source map location for token nodes.
export const EFNoLeadingComments = 1 << 7; // Do not emit leading comments for this node.
export const EFNoTrailingComments = 1 << 8; // Do not emit trailing comments for this node.
export const EFNoNestedComments = 1 << 9; // Do not emit nested comments for children of this node.
export const EFHelperName = 1 << 10; // The Identifier refers to an *unscoped* emit helper (one that is emitted at the top of the file)
export const EFExportName = 1 << 11; // Ensure an export prefix is added for an identifier that points to an exported declaration with a local name (see SymbolFlags.ExportHasLocal).
export const EFLocalName = 1 << 12; // Ensure an export prefix is not added for an identifier that points to an exported declaration.
export const EFIndented = 1 << 13; // Adds an explicit extra indentation level for class and function bodies when printing (used to match old emitter).
export const EFNoIndentation = 1 << 14; // Do not indent the node.
export const EFReuseTempVariableScope = 1 << 15; // Reuse the existing temp variable scope during emit.
export const EFCustomPrologue = 1 << 16; // Treat the statement as if it were a prologue directive (NOTE: Prologue directives are *not* transformed).
export const EFNoAsciiEscaping = 1 << 17; // When synthesizing nodes that lack an original node or textSourceNode, we want to write the text on the node with ASCII escaping substitutions.
export const EFExternalHelpers = 1 << 18; // This source file has external helpers
export const EFStartOnNewLine = 1 << 19; // Start this node on a new line
export const EFIndirectCall = 1 << 20; // Emit CallExpression as an indirect call: `(0, f)()`
export const EFAsyncFunctionBody = 1 << 21; // The node was originally an async function body.
export const EFNoLexicalArguments = 1 << 22; // Do not capture `arguments` for this arrow function. Set on arrows lowered from class static blocks, where `arguments` is an error; preserves Strada's emit behavior.
export const EFTransformPrivateStaticElements = 1 << 23; // Indicates static private elements in a file or class should be transformed regardless of --target (used by esDecorators transform).
export const EFNoLexicalThis = 1 << 24; // Do not capture `this` for this node's subtree. Set on relocated static initializers, where `this` is handled by the class fields transform.
/**
 ...
 * )
 */
export const EFNone = 0;
export const EFNoSourceMap = EFNoLeadingSourceMap | EFNoTrailingSourceMap; // Do not emit a source map location for this node.
export const EFNoTokenSourceMaps = EFNoTokenLeadingSourceMaps | EFNoTokenTrailingSourceMaps; // Do not emit source map locations for tokens of this node.
export const EFNoComments = EFNoLeadingComments | EFNoTrailingComments; // Do not emit comments for this node.
//# sourceMappingURL=emitflags.js.map