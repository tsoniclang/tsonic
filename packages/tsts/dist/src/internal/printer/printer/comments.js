 ...
 * }
 */
export function Printer_writeComment(receiver, text) {
 ...
 * }
 */
export function Printer_writeCommentRange(receiver, comment) {
 ...
 * }
 */
export function Printer_writeCommentRangeWorker(receiver, text, lineMap, kind, loc) {
    if (kind === KindMultiLineCommentTrivia) {
        ...
        for (; pos < TextRange_End(loc); currentLine++) {
            const nextLineStart = currentLine + 1 === lineCount ? byteLen(text) + 1 : lineMap[currentLine + 1];
 ...
 * }
 */
export function Printer_shouldEmitComments(receiver, node) {
 ...
 * }
 */
export function Printer_shouldWriteComment(receiver, comment) {
    return (!receiver.Options.OnlyPrintJSDocStyle ||
        ...
        (receiver.currentSourceFile !== undefined && IsPinnedComment(SourceFile_Text(receiver.currentSourceFile), comment)));
}
/**
 ...
 * }
 */
export function Printer_shouldEmitLeadingComments(receiver, node) {
    return ((EmitContext_EmitFlags(receiver.emitContext, node) & EFNoLeadingComments) === 0);
}
/**
 ...
 * }
 */
export function Printer_shouldEmitTrailingComments(receiver, node) {
    return ((EmitContext_EmitFlags(receiver.emitContext, node) & EFNoTrailingComments) === 0);
}
/**
 ...
 * }
 */
export function Printer_shouldEmitNestedComments(receiver, node) {
    return ((EmitContext_EmitFlags(receiver.emitContext, node) & EFNoNestedComments) === 0);
}
/**
 ...
 * }
 */
export function Printer_shouldEmitDetachedComments(receiver, node) {
 ...
 * }
 */
export function Printer_hasCommentsAtPosition(receiver, pos) {
    if (receiver.currentSourceFile === undefined) {
        return false;
    }
    ...
    const text = SourceFile_Text(receiver.currentSourceFile);
    let found = false;
    GetTrailingCommentRanges(nf, text, pos + 1)((_comment) => {
        found = true;
        return false;
    });
    if (found) {
    ...
    GetLeadingCommentRanges(nf, text, pos + 1)((_comment) => {
        found = true;
        return false;
    });
    return found;
 ...
 * }
 */
export function Printer_syntheticCommentWillEmitNewLine(receiver, comment) {
 ...
 * }
 */
export function Printer_emitPrologueDirectives(receiver, statements) {
    ...
    return statements.Nodes.length;
}
/**
 ...
 * }
 */
export function Printer_emitTripleSlashDirectives(receiver, node) {
 ...
 * }
 */
export function Printer_emitDirective(receiver, kind, refs) {
    for (const ref of refs) {
        ...
        Printer_writeComment(receiver, `/// <reference ${kind}="${ref.FileName}" ${resolutionMode}${IfElse(ref.Preserve, `preserve="true" `, "")}/>`);
        Printer_writeLine(receiver);
    }
}
/**
 ...
 * }
 */
export function Printer_emitCommentsBeforeNode(receiver, node) {
    ...
    const c = Arena_New(receiver.commentStateArena);
 ...
 * }
 */
export function Printer_emitCommentsAfterNode(receiver, node, state) {
    if (state === undefined) {
        return;
    }
 ...
 * }
 */
export function Printer_emitCommentsBeforeToken(receiver, _token, pos, contextNode, flags) {
    if ((flags & tefNoComments) !== 0 || receiver.commentsDisabled) {
        // Still skip trivia so that the returned pos correctly identifies the token position.
        // This is needed for trailing source map positions (writeTokenText advances pos by token length).
        if (receiver.currentSourceFile !== undefined && !PositionIsSynthesized(pos)) {
            pos = SkipTrivia(SourceFile_Text(receiver.currentSourceFile), pos);
        }
        return [undefined, pos];
    }
    const startPos = pos;
    if (receiver.currentSourceFile !== undefined) {
        pos = SkipTrivia(SourceFile_Text(receiver.currentSourceFile), startPos);
    }
    ...
    if (Node_Pos(contextNode) !== startPos) {
        ...
        const needsIndent = (indentLeading && receiver.currentSourceFile !== undefined && !PositionsAreOnSameLine(startPos, pos, receiver.currentSourceFile));
        Printer_increaseIndentIf(receiver, needsIndent);
        Printer_emitLeadingComments(receiver, startPos, false);
        Printer_decreaseIndentIf(receiver, needsIndent);
    }
    return [Arena_New(receiver.commentStateArena), pos];
}
/**
 ...
 * }
 */
export function Printer_emitCommentsAfterToken(receiver, token, pos, contextNode, state) {
    if (state === undefined) {
        return;
    }
    if (Node_End(contextNode) !== pos) {
        const isJsxExprContext = (contextNode.Kind === KindJsxExpression);
 ...
 * }
 */
export function Printer_emitDetachedCommentsBeforeStatementList(receiver, node, detachedRange) {
 ...
 * }
 */
export function Printer_emitDetachedCommentsAfterStatementList(receiver, _node, detachedRange, state) {
    if (state === undefined) {
        return;
    }
    const emitFlags = state.emitFlags;
    ...
    if (!skipTrailingComments) {
        const hasWrittenComment = Printer_emitLeadingComments(receiver, TextRange_End(detachedRange), false);
 ...
 * }
 */
export function Printer_emitLeadingCommentsOfNode(receiver, node, emitFlags, commentRange) {
    ...
    if ((!PositionIsSynthesized(pos) || !PositionIsSynthesized(end)) && pos !== end) {
        ...
        // has not opted out from emitting leading comments.
        if (!skipLeadingComments) {
            Printer_emitLeadingComments(receiver, pos, (node.Kind === KindNotEmittedStatement));
        }
        if (!skipLeadingComments || (pos >= 0 && (emitFlags & EFNoLeadingComments) !== 0)) {
            // Advance the container position if comments get emitted or if they've been disabled explicitly using NoLeadingComments.
            receiver.containerPos = pos;
        }
        if (!skipTrailingComments || (end >= 0 && (emitFlags & EFNoTrailingComments) !== 0)) {
            // Advance the container end if comments get emitted or if they've been disabled explicitly using NoTrailingComments.
            receiver.containerEnd = end;
            // To avoid invalid comment emit in a down-level binding pattern, we
            // keep track of the last declaration list container's end
            if (node.Kind === KindVariableDeclarationList) {
                receiver.declarationListContainerEnd = end;
            }
        }
    }
}
 ...
 * }
 */
export function Printer_emitTrailingCommentsOfNode(receiver, node, emitFlags, commentRange, containerPos, containerEnd, declarationListContainerEnd) {
    const pos = TextRange_Pos(commentRange);
    const end = TextRange_End(commentRange);
 ...
 * }
 */
export function Printer_emitLeadingSyntheticCommentsOfNode(receiver, node, emitFlags) {
    if ((emitFlags & EFNoLeadingComments) !== 0) {
        return;
    }
 ...
 * }
 */
export function Printer_emitLeadingSynthesizedComment(receiver, comment) {
 ...
 * }
 */
export function Printer_emitTrailingSyntheticCommentsOfNode(receiver, node, emitFlags) {
    if ((emitFlags & EFNoTrailingComments) !== 0) {
        return;
    }
 ...
 * }
 */
export function Printer_emitTrailingSynthesizedComment(receiver, comment) {
 ...
 * }
 */
export function formatSynthesizedComment(comment) {
 ...
 * }
 */
export function Printer_writeSynthesizedComment(receiver, comment) {
    const text = formatSynthesizedComment(comment);
    const lineMap = comment.Kind === KindMultiLineCommentTrivia ? ComputeECMALineStarts(text) : [];
...
export function Printer_emitLeadingComments(receiver, pos, elided) {
    ...
    // skip detached comments
    const detachedStack = receiver.detachedCommentsInfo;
    let adjustedPos = pos;
    if (Stack_Len(detachedStack) > 0) {
        const info = Stack_Peek(detachedStack);
        if (info.nodePos === adjustedPos) {
            adjustedPos = Stack_Pop(detachedStack).detachedCommentEndPos;
        }
    }
    const nf = NodeFactory_AsNodeFactory(receiver.emitContext.Factory.__tsgoEmbedded0);
    const text = SourceFile_Text(receiver.currentSourceFile);
 ...
 * }
 */
export function Printer_shouldEmitCommentIfTripleSlash(receiver, comment, tripleSlash) {
    switch (tripleSlash) {
        case TSTrue:
 ...
 * }
 */
export function Printer_shouldEmitNewLineBeforeLeadingCommentOfPosition(receiver, pos, commentPos) {
    // If the leading comments start on different line than the start of node, write new line
    return (receiver.currentSourceFile !== undefined &&
        pos !== commentPos &&
        ComputeLineOfPosition(SourceFile_ECMALineMap(receiver.currentSourceFile), pos) !== ComputeLineOfPosition(SourceFile_ECMALineMap(receiver.currentSourceFile), commentPos));
}
/**
 ...
 * }
 */
export function Printer_emitLeadingCommentsOfPosition(receiver, pos) {
    if (receiver.commentsDisabled || pos === -1) {
        return;
    }
    Printer_emitLeadingComments(receiver, pos, false);
}
/**
 ...
 * }
 */
export function Printer_emitTrailingComments(receiver, pos, commentSeparator) {
 ...
 * }
 */
export function Printer_emitTrailingCommentsOfPosition(receiver, pos, prefixSpace, forceNoNewline) {
 ...
 * }
 */
export function Printer_emitDetachedCommentsAndUpdateCommentsInfo(receiver, textRange) {
    if (receiver.currentSourceFile === undefined) {
        return;
    }
    const [currentDetachedCommentInfo, ok] = Printer_emitDetachedComments(receiver, textRange);
    if (ok) {
        Stack_Push(receiver.detachedCommentsInfo, currentDetachedCommentInfo);
    }
}
/**
 ...
 * }
 */
export function Printer_emitDetachedComments(receiver, textRange) {
    const result = { nodePos: 0, detachedCommentEndPos: 0 };
    let hasResult = false;
    if (receiver.currentSourceFile === undefined) {
        return [result, hasResult];
    }
    ...
    if (leadingComments.length > 0) {
        ...
        if (detachedComments.length > 0) {
            ...
            // sure there is at least one blank line between it and the node. If not, it's not
            // a copyright header.
            const lastDetachedComment = LastOrNil(detachedComments);
 ...
 * }
 */
export function Printer_emitComments(receiver, comments, commentSeparator) {
    let interveningSeparator = false;
    if (comments.length === 0) {
        return false;
    }
    if (commentSeparator === commentSeparatorBefore) {
 ...
 * }
 */
export function Printer_emitComment(receiver, comment) {
 ...
 * }
 */
export function Printer_isTripleSlashComment(receiver, comment) {
    return (receiver.currentSourceFile !== undefined &&
        IsRecognizedTripleSlashComment(SourceFile_Text(receiver.currentSourceFile), comment));
}
