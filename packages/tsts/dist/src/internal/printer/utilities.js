...
import { KindClassDeclaration, KindClassExpression, KindGetAccessor, KindMethodDeclaration, KindParameter, KindPropertyDeclaration, KindSetAccessor, KindAmpersandAmpersandToken, KindBarBarToken, KindQuestionQuestionToken, KindParenthesizedExpression, KindBinaryExpression, KindNewExpression, KindSingleLineCommentTrivia, KindMultiLineCommentTrivia, KindBigIntLiteral, KindNoSubstitutionTemplateLiteral, KindTemplateHead, KindTemplateMiddle, KindTemplateTail, KindStringLiteral, KindNumericLiteral, KindRegularExpressionLiteral, KindTypeParameter, KindTemplateLiteralTypeSpan, KindTemplateSpan, KindDecorator, KindHeritageClause, KindTypeLiteral, KindInterfaceDeclaration, KindUnionType, KindIntersectionType, KindArrayLiteralExpression, KindTupleType, KindNamedImports, KindNamedExports, KindObjectLiteralExpression, KindJsxAttributes, KindCallExpression, KindJsxElement, KindJsxFragment, KindJsxOpeningElement, KindJsxSelfClosingElement, KindBlock, KindModuleBlock, KindCaseClause, KindDefaultClause, KindCaseBlock, KindEnumDeclaration, KindSourceFile, } from "../ast/generated/kinds.js";
import { IsCallExpression, IsArrowFunction, IsFunctionExpression, IsInterfaceDeclaration, IsEnumMember, IsInferTypeNode, } from "../ast/generated/predicates.js";
import { AsBinaryExpression, AsCaseBlock, AsCallExpression, AsNewExpression, AsUnionTypeNode, AsIntersectionTypeNode, AsTemplateExpression, AsTemplateLiteralTypeNode, AsInterfaceDeclaration, AsStringLiteral, AsNumericLiteral, AsNoSubstitutionTemplateLiteral, AsTemplateHead, AsTemplateMiddle, AsTemplateTail, } from "../ast/generated/casts.js";
import { IsPrologueDirective, PositionIsSynthesized, NodeIsSynthesized, SkipPartiallyEmittedExpressions, IsFunctionLike, IsClassLike, IsClassElement, IsTypeElement, IsJsxChild, IsStatement, IsTypeNode, IsModifier, IsUnterminatedLiteral, IsFunctionExpressionOrArrowFunction, } from "../ast/utilities.js";
...
const decodeRuneInStringAt = (s, pos) => {
    const info = getUtf8ByteInfo(s);
    if (info.ascii) {
        return pos >= s.length ? [0xfffd, 0] : [s.charCodeAt(pos), 1];
    }
 ...
 * )
 */
export const getLiteralTextFlagsNone = 0;
export const getLiteralTextFlagsNeverAsciiEscape = 1 << 0;
export const getLiteralTextFlagsJsxAttributeEscape = 1 << 1;
export const getLiteralTextFlagsTerminateUnterminatedLiterals = 1 << 2;
export const getLiteralTextFlagsAllowNumericSeparator = 1 << 3;
/**
 ...
 * )
 */
export const QuoteCharSingleQuote = 0x27; // '\''
export const QuoteCharDoubleQuote = 0x22; // '"'
export const QuoteCharBacktick = 0x60; // '`'
/**
 ...
 * }
 */
export const jsxEscapedCharsMap = new globalThis.Map([
    [0x22 /* '"' */, "&quot;"],
    [0x27 /* '\'' */, "&apos;"],
]);
/**
 ...
 * }
 */
export const escapedCharsMap = new globalThis.Map([
    [0x09 /* '\t' */, "\\t"],
    [0x0b /* '\v' */, "\\v"],
    [0x0c /* '\f' */, "\\f"],
    [0x08 /* '\b' */, "\\b"],
 ...
 * }
 */
export function encodeJsxCharacterEntity(b, charCode) {
 ...
 * }
 */
export function encodeUtf16EscapeSequence(b, charCode) {
    const hexCharCode = ToUpper(FormatUint(charCode, 16));
    b.WriteString("\\u");
 ...
 * }
 */
export function escapeStringWorker(s, quoteChar, flags, b) {
 ...
 * }
 */
export function EscapeString(s, quoteChar) {
    const b = new Builder();
    b.Grow(byteLen(s) + 2);
    escapeStringWorker(s, quoteChar, getLiteralTextFlagsNeverAsciiEscape, b);
    return b.String();
 ...
 * }
 */
export function escapeNonAsciiString(s, quoteChar) {
    const b = new Builder();
    b.Grow(byteLen(s) + 2);
    escapeStringWorker(s, quoteChar, getLiteralTextFlagsNone, b);
    return b.String();
 ...
 * }
 */
export function escapeJsxAttributeString(s, quoteChar) {
    const b = new Builder();
    b.Grow(byteLen(s) + 2);
    escapeStringWorker(s, quoteChar, getLiteralTextFlagsJsxAttributeEscape | getLiteralTextFlagsNeverAsciiEscape, b);
    return b.String();
 ...
 * }
 */
export function canUseOriginalText(node, flags) {
    // A synthetic node has no original text, nor does a node without a parent as we would be unable to find the
    // containing SourceFile. We also cannot use the original text if the literal was unterminated and the caller has
    // requested proper termination of unterminated literals
 ...
 * }
 */
export function getLiteralText(node, sourceFile, flags) {
    // If we don't need to downlevel and we can reach the original source text using
    // the node's parent reference, then simply get the text as it was originally written.
    if (sourceFile !== undefined && canUseOriginalText(node, flags)) {
        return GetSourceTextOfNodeFromSourceFile(sourceFile, node, false /*includeTrivia*/);
    }
    // If we can't reach the original source text, use the canonical form if it's a number,
    // or a (possibly escaped) quoted form of the original text if it's string-like.
    switch (node.Kind) {
        case KindStringLiteral: {
            const b = new Builder();
            const quoteChar = (AsStringLiteral(node).TokenFlags & TokenFlagsSingleQuote) !== 0
 ...
 * }
 */
export function isNotPrologueDirective(node) {
 ...
 * }
 */
export function RangeIsOnSingleLine(r, sourceFile) {
    return rangeStartIsOnSameLineAsRangeEnd(r, r, sourceFile);
}
/**
 ...
 * }
 */
export function rangeStartPositionsAreOnSameLine(range1, range2, sourceFile) {
 ...
 * }
 */
export function rangeEndPositionsAreOnSameLine(range1, range2, sourceFile) {
 ...
 * }
 */
export function rangeStartIsOnSameLineAsRangeEnd(range1, range2, sourceFile) {
 ...
 * }
 */
export function rangeEndIsOnSameLineAsRangeStart(range1, range2, sourceFile) {
 ...
 * }
 */
export function getStartPositionOfRange(r, sourceFile, includeComments) {
 ...
 * }
 */
export function PositionsAreOnSameLine(pos1, pos2, sourceFile) {
    return GetLinesBetweenPositions(sourceFile, pos1, pos2) === 0;
}
/**
 ...
 * }
 */
export function GetLinesBetweenPositions(sourceFile, pos1, pos2) {
    if (pos1 === pos2) {
        return 0;
    }
 ...
 * }
 */
export function getLinesBetweenRangeEndAndRangeStart(range1, range2, sourceFile, includeSecondRangeComments) {
 ...
 * }
 */
export function getLinesBetweenPositionAndPrecedingNonWhitespaceCharacter(pos, stopPos, sourceFile, includeComments) {
 ...
 * }
 */
export function getLinesBetweenPositionAndNextNonWhitespaceCharacter(pos, stopPos, sourceFile, includeComments) {
 ...
 * }
 */
export function getPreviousNonWhitespacePosition(pos, stopPos, sourceFile) {
 ...
 * }
 */
export function siblingNodePositionsAreComparable(previousNode, nextNode) {
 ...
 * }
 */
export function getContainingNodeArray(node) {
 ...
 * }
 */
export function originalNodesHaveSameParent(nodeA, nodeB) {
    // TODO(rbuckton): nodeA = getOriginalNode(nodeA)
...
export function tryGetEnd(node) {
    if (node === undefined) {
        return [0, false];
    }
    const value = node;
    ...
    if (value["Kind"] !== undefined) {
        return [Node_End(node), true];
    }
    if (value["Nodes"] !== undefined) {
        return [NodeList_End(node), true];
    }
    if (value["end"] !== undefined) {
        return [TextRange_End(node), true];
    }
 ...
 * }
 */
export function greatestEnd(end, ...nodes) {
 ...
 * }
 */
export function skipSynthesizedParentheses(node) {
 ...
 * }
 */
export function isBinaryOperation(node, token) {
 ...
 * }
 */
export function mixingBinaryOperatorsRequiresParentheses(a, b) {
 ...
 * }
 */
export function isImmediatelyInvokedFunctionExpressionOrArrowFunction(node) {
    node = SkipPartiallyEmittedExpressions(node);
 ...
 * }
 */
export function IsFileLevelUniqueName(sourceFile, name, hasGlobalName) {
 ...
 * }
 */
export function hasLeadingHash(text) {
 ...
 * }
 */
export function removeLeadingHash(text) {
    if (hasLeadingHash(text)) {
 ...
 * }
 */
export function ensureLeadingHash(text) {
    if (hasLeadingHash(text)) {
        return text;
    }
    else {
 ...
 * }
 */
export function FormatGeneratedName(privateName, prefix, base, suffix) {
 ...
 * }
 */
export function isASCIIWordCharacter(ch) {
 ...
 * }
 */
export function makeIdentifierFromModuleName(moduleName) {
 ...
 * }
 */
export function findSpanEndWithEmitContext(c, array, test, start) {
    let i = start;
 ...
 * }
 */
export function findSpanEnd(array, test, start) {
    let i = start;
...
export function skipWhiteSpaceSingleLine(text, pos) {
    const box = pos;
 ...
 * }
 */
export function matchWhiteSpaceSingleLine(text, pos) {
    const box = pos;
 ...
 * }
 */
export function matchRune(text, pos, expected) {
    const box = pos;
    const [ch, size] = decodeRuneInStringAt(text, box.v);
    if (ch === expected) {
        box.v = (box.v + size);
 ...
 * }
 */
export function matchString(text, pos, expected) {
    const box = pos;
    const textLen = byteLen(text);
    ...
    while (expectedPos < expectedLen) {
        ...
        const tempBox = { v: textPos };
        if (!matchRune(text, tempBox, expectedRune)) {
            return false;
        }
        textPos = tempBox.v;
        expectedPos += expectedSize;
 ...
 * }
 */
export function matchQuotedString(text, pos) {
    const box = pos;
    let textPos = box.v;
    const textLen = byteLen(text);
    let quoteChar;
    const tempBox1 = { v: textPos };
    if (matchRune(text, tempBox1, 0x27 /* '\'' */)) {
        textPos = tempBox1.v;
        quoteChar = 0x27;
    }
    else {
        const tempBox2 = { v: textPos };
        if (matchRune(text, tempBox2, 0x22 /* '"' */)) {
            textPos = tempBox2.v;
            quoteChar = 0x22;
        }
        else {
 ...
 * }
 */
export function IsRecognizedTripleSlashComment(text, commentRange) {
    const textBytes = utf8Encoder.encode(text);
 ...
 * }
 */
export function isJSDocLikeText(text, comment) {
    const textBytes = utf8Encoder.encode(text);
 ...
 * }
 */
export function IsPinnedComment(text, comment) {
    const textBytes = utf8Encoder.encode(text);
    return comment.Kind === KindMultiLineCommentTrivia &&
        (TextRange_End(comment) - TextRange_Pos(comment)) > 5 &&
 ...
 * }
 */
export function calculateIndent(text, pos, end) {
 ...
 * }
 */
export function newLineCharacterCache(source) {
    return {
        lineMap: source.ECMALineMap(),
        text: source.Text(),
        cachedLine: 0,
 ...
 * }
 */
export function lineCharacterCache_getLineAndCharacter(receiver, pos) {
