 ...
 * )
 */
export const missingPosition = -1;
/**
 ...
 * }
 */
export function MappedPosition_isSourceMappedPosition(receiver) {
    const m = receiver;
    return m.sourceIndex !== MissingSource && m.sourcePosition !== missingPosition;
}
/**
 ...
 * }
 */
export function createDocumentPositionMapper(host, sourceMap, mapPath) {
    ...
    const sourceFileAbsolutePaths = coreMap(sourceMap.Sources, (source) => {
        return GetNormalizedAbsolutePath(source, sourceRoot);
    });
    const useCaseSensitiveFileNames = host.UseCaseSensitiveFileNames();
    const sourceToSourceIndexMap = new globalThis.Map();
    ...
    const sourceMappings = new globalThis.Map();
    // getDecodedMappings()
    const decoder = DecodeMappings(sourceMap.Mappings);
    MappingsDecoder_Values(decoder)((mapping) => {
        // processMapping()
        let generatedPosition = -1;
        let lineInfo = host.GetECMALineInfo(generatedAbsoluteFilePath);
        if (lineInfo !== undefined) {
            generatedPosition = ComputePositionOfLineAndUTF16Character(lineInfo.lineStarts, mapping.GeneratedLine, mapping.GeneratedCharacter, lineInfo.text, true /*allowEdits*/);
        }
        let sourcePosition = -1;
        if (Mapping_IsSourceMapping(mapping)) {
            lineInfo = host.GetECMALineInfo(sourceFileAbsolutePaths[mapping.SourceIndex]);
            if (lineInfo !== undefined) {
                const pos = ComputePositionOfLineAndUTF16Character(lineInfo.lineStarts, mapping.SourceLine, mapping.SourceCharacter, lineInfo.text, true /*allowEdits*/);
                sourcePosition = pos;
            }
        }
    ...
    // getGeneratedMappings()
    generatedMappings = decodedMappings;
    slicesSortFunc(generatedMappings, (a, b) => {
        return a.generatedPosition - b.generatedPosition;
    });
    generatedMappings = DeduplicateSorted(generatedMappings, (a, b) => {
        return a.generatedPosition === b.generatedPosition &&
            a.sourceIndex === b.sourceIndex &&
            a.sourcePosition === b.sourcePosition;
    });
 ...
 * }
 */
export function DocumentPositionMapper_GetSourcePosition(receiver, loc) {
    const d = receiver;
    if (d === undefined) {
        return undefined;
    }
    ...
    const [targetIndex] = slicesBinarySearchFunc(d.generatedMappings, loc.Pos, (m, pos) => {
        return m.generatedPosition - pos;
    });
    if (targetIndex < 0 || targetIndex >= d.generatedMappings.length) {
        return undefined;
    }
    const mapping = d.generatedMappings[targetIndex];
    if (!MappedPosition_isSourceMappedPosition(mapping)) {
        return undefined;
    }
    // Closest position
 ...
 * }
 */
export function DocumentPositionMapper_GetGeneratedPosition(receiver, loc) {
    const d = receiver;
    if (d === undefined) {
        return undefined;
    }
    const canonical = GetCanonicalFileName(loc.FileName, d.useCaseSensitiveFileNames);
    const ok = d.sourceToSourceIndexMap.has(canonical);
    const sourceIndex = ok ? d.sourceToSourceIndexMap.get(canonical) : 0;
    if (!ok) {
        return undefined;
    }
    ...
    const sourceMappings = d.sourceMappings.get(sourceIndex) ?? [];
    const [targetIndex] = slicesBinarySearchFunc(sourceMappings, loc.Pos, (m, pos) => {
        return m.sourcePosition - pos;
    });
 ...
 * }
 */
export function GetDocumentPositionMapper(host, generatedFileName) {
    ...
    for (const location of possibleMapLocations) {
        const resolvedMapFileName = GetNormalizedAbsolutePath(location, GetDirectoryPath(generatedFileName));
 ...
 * }
 */
export function convertDocumentToSourceMapper(host, contents, mapFileName) {
    ...
    // Don't support source maps that contain inlined sources
    if (Some(sourceMap.SourcesContent, (s) => { return s !== undefined; })) {
        return undefined;
    }
    return createDocumentPositionMapper(host, sourceMap, mapFileName);
}
 ...
 * }
 */
export function tryParseRawSourceMap(contents) {
    const sourceMap = {
    ...
    const err = jsonUnmarshal(stringToBytes(contents), sourceMap);
    if (err !== undefined) {
        return undefined;
    }
    if (sourceMap.Version !== 3) {
 ...
 * }
 */
export function tryGetSourceMappingURL(host, fileName) {
 ...
 * }
 */
export function tryParseBase64Url(url) {
    let found;
...
//# sourceMappingURL=source_mapper.js.map