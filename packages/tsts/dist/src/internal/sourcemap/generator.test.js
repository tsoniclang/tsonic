...
import { Generator_AddGeneratedMapping, Generator_AddName, Generator_AddNamedSourceMapping, Generator_AddSource, Generator_AddSourceMapping, Generator_RawSourceMap, Generator_SetSourceContent, Generator_String, NewGenerator, } from "./generator.js";
function rawFields(m) {
    return {
        Version: m.Version,
        File: m.File,
...
function newGen() {
    ...
    return NewGenerator("main.js", "/", "/", { UseCaseSensitiveFileNames: false, CurrentDirectory: "" });
}
function assertNilError(err) {
...
test("SourceMapGenerator_SetSourceContent_ForSecondSourceOnly_Serialized", () => {
    ...
    const sourceIndex = Generator_AddSource(gen, "/main.ts");
    assertNilError(Generator_SetSourceContent(gen, sourceIndex, "foo"));
    assert.equal(Generator_String(gen), '{"version":3,"file":"main.js","sourceRoot":"/","sources":["skipped.ts","main.ts"],"names":[],"mappings":"","sourcesContent":[null,"foo"]}');
});
