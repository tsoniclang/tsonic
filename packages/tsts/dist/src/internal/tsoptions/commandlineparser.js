...
import { Cannot_read_file_0, Option_0_can_only_be_specified_in_tsconfig_json_file_or_set_to_false_or_null_on_command_line, Option_0_can_only_be_specified_in_tsconfig_json_file_or_set_to_null_on_command_line, Option_0_requires_value_to_be_greater_than_1, Options_0_and_1_cannot_be_combined, Unterminated_quoted_string_in_response_file_0, } from "../diagnostics/generated/messages.js";
import { GetNormalizedAbsolutePath } from "../tspath/path.js";
import { IsWhiteSpaceLike } from "../stringutil/util.js";
import { CommandLineOption_Elements, CommandLineOption_EnumMap, CommandLineOptionTypeBoolean, CommandLineOptionTypeList, CommandLineOptionTypeListOrElement, CommandLineOptionTypeNumber, CommandLineOptionTypeString, } from "./commandlineoption.js";
...
import { buildOptionsParser_as_optionParser, compilerOptionsParser_as_optionParser, convertToOptionsWithAbsolutePaths, ParseCompilerOptions, watchOptionsParser_as_optionParser, } from "./parsinghelpers.js";
 ...
 * }
 */
export function commandLineParser_AlternateMode(receiver) {
    const p = receiver;
    return p.workerDiagnostics.didYouMean.alternateMode;
}
/**
 ...
 * }
 */
export function commandLineParser_OptionsDeclarations(receiver) {
    const p = receiver;
    return p.workerDiagnostics.didYouMean.OptionDeclarations;
}
/**
 ...
 * }
 */
export function commandLineParser_UnknownOptionDiagnostic(receiver) {
    const p = receiver;
    return p.workerDiagnostics.didYouMean.UnknownOptionDiagnostic;
}
/**
 ...
 * }
 */
export function commandLineParser_UnknownDidYouMeanDiagnostic(receiver) {
    const p = receiver;
    return p.workerDiagnostics.didYouMean.UnknownDidYouMeanDiagnostic;
}
/**
 ...
 * }
 */
export function ParseCommandLine(commandLine, host) {
    ...
    const optionsWithAbsolutePaths = convertToOptionsWithAbsolutePaths(OrderedMap_Clone(parser.options), CommandLineCompilerOptionsMap, host.GetCurrentDirectory());
    const compilerParser = { __tsgoEmbedded0: {} };
    ...
    const watchParser = { __tsgoEmbedded0: {} };
...
export function ParseBuildCommandLine(commandLine, host) {
    ...
    const buildParser = { __tsgoEmbedded0: {} };
    convertMapToOptions(parser.options, buildOptionsParser_as_optionParser(buildParser));
    const watchParser = { __tsgoEmbedded0: {} };
    convertMapToOptions(parser.options, watchOptionsParser_as_optionParser(watchParser));
    let result = {
 ...
 * }
 */
export function parseCommandLineWorker(parseCommandLineWithDiagnostics, commandLine, fs, currentDirectory) {
    const parser = {
        fs: fs,
        currentDirectory: currentDirectory,
        workerDiagnostics: parseCommandLineWithDiagnostics,
        fileNames: [],
        options: newMapWithSizeHint(0),
        errors: [],
 ...
 * }
 */
export function commandLineParser_parseStrings(receiver, args) {
    const p = receiver;
    let i = 0;
    while (i < args.length) {
        const s = args[i];
 ...
 * }
 */
export function getInputOptionName(input) {
    // removes at most two leading '-' from the input string
    return strings.TrimPrefix(strings.TrimPrefix(input, "-"), "-");
}
/**
 ...
 * }
 */
export function commandLineParser_parseResponseFile(receiver, fileName) {
    const p = receiver;
    fileName = GetNormalizedAbsolutePath(fileName, p.currentDirectory);
    const [fileContents, errors] = tryReadFile(fileName, (fn) => {
 ...
 * }
 */
export function tryReadFile(fileName, readFile, errors) {
    // this function adds a compiler diagnostic if the file cannot be read
 ...
 * }
 */
export function commandLineParser_parseOptionValue(receiver, args, i, opt, diag) {
    const p = receiver;
    if (opt.IsTSConfigOnly && i <= args.length) {
        ...
        if (optValue === "null") {
            OrderedMap_Set(p.options, opt.Name, undefined);
            i++;
        }
        else if (opt.Kind === CommandLineOptionTypeBoolean) {
            if (optValue === "false") {
                OrderedMap_Set(p.options, opt.Name, false);
                i++;
            }
            else {
                if (optValue === "true") {
        ...
        if (i >= args.length) {
            if (opt.Kind !== CommandLineOptionTypeBoolean) {
                ...
                if (opt.Kind === CommandLineOptionTypeList) {
                    OrderedMap_Set(p.options, opt.Name, []);
                }
                else if (opt.Kind === "enum") {
                ...
                OrderedMap_Set(p.options, opt.Name, true);
            }
            return i;
        }
        if (args[i] !== "null") {
            switch (opt.Kind) {
                case CommandLineOptionTypeNumber: {
                    ...
                    if (e === undefined) {
                        if (num >= opt.minValue) {
                            OrderedMap_Set(p.options, opt.Name, num);
                        }
                        else {
                ...
                case CommandLineOptionTypeString: {
                    ...
                    if (err === undefined || err.length === 0) {
                        OrderedMap_Set(p.options, opt.Name, val);
                    }
                    else {
                ...
                case CommandLineOptionTypeList: {
                    ...
                    OrderedMap_Set(p.options, opt.Name, result);
                    p.errors = [...p.errors, ...err];
 ...
 * }
 */
export function commandLineParser_parseListTypeOption(receiver, opt, value) {
    return ParseListTypeOption(opt, value);
}
/**
 ...
 * }
 */
export function ParseListTypeOption(opt, value) {
    value = strings.TrimSpace(value);
    let errors = [];
    if (strings.HasPrefix(value, "-")) {
        return [[], errors];
    ...
    switch (elementsKind) {
        case CommandLineOptionTypeString: {
            const elements = MapFiltered(values, (v) => {
        ...
        default: {
            const result = MapFiltered(values, (v) => {
 ...
 * }
 */
export function convertJsonOptionOfEnumType(opt, value, valueExpression, sourceFile) {
    if (value === "") {
    ...
    const [val, ok] = OrderedMap_Get(typeMap, key);
    if (ok) {
        return validateJsonOptionValue(opt, val, valueExpression, sourceFile);
    }
