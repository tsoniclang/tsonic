...
import { CommandLineOptionTypeBoolean, CommandLineOptionTypeList, CommandLineOptionTypeListOrElement, CommandLineOptionTypeObject, CommandLineOptionTypeString, CommandLineOptionTypeEnum, commandLineOptionsToMap as commandLineOptionsToMapImpl, extraValidationLocale, extraValidationNone, extraValidationSpec, newCommandLineOption, CommandLineOption_DisallowNullOrUndefined, CommandLineOption_Elements, CommandLineOption_EnumMap, } from "./commandlineoption.js";
import { OptionsDeclarations } from "./declscompiler.js";
import { typeAcquisitionDeclaration } from "./declstypeacquisition.js";
import { CreateDiagnosticForNodeInSourceFile, CreateDiagnosticForNodeInSourceFileOrCompilerDiagnostic, getCompilerOptionValueTypeString, createUnknownOptionError, extraKeyDiagnostics, extraKeyDidYouMeanDiagnostics, } from "./errors.js";
import { convertJsonOptionOfEnumType, tryReadFile, } from "./commandlineparser.js";
import { parseJsonToStringKey, parseProjectReference, mergeCompilerOptions, compilerOptionsParser_as_optionParser, typeAcquisitionParser_as_optionParser, ParseCompilerOptions, ParseTypeAcquisition, } from "./parsinghelpers.js";
 ...
 */
export const CommandLineCompilerOptionsMap = commandLineOptionsToMap(OptionsDeclarations);
/**
 ...
 * }
 */
export const compilerOptionsDeclaration = newCommandLineOption({
    Name: "compilerOptions",
    Kind: CommandLineOptionTypeObject,
    ElementOptions: CommandLineCompilerOptionsMap,
});
 ...
 * }
 */
export const compileOnSaveCommandLineOption = newCommandLineOption({
    Name: "compileOnSave",
    Kind: CommandLineOptionTypeBoolean,
    DefaultValueDescription: false,
});
 ...
 * }
 */
export const extendsOptionDeclaration = newCommandLineOption({
    Name: "extends",
    Kind: CommandLineOptionTypeListOrElement,
    Category: diagnostics.File_Management,
 ...
 * }
 */
export const tsconfigRootOptionsMap = newCommandLineOption({
    Name: "undefined", // should never be needed since this is root
    Kind: CommandLineOptionTypeObject,
    ElementOptions: commandLineOptionsToMap([
        compilerOptionsDeclaration,
 ...
 * }
 */
export function configFileSpecs_matchesExclude(receiver, fileName, comparePathsOptions) {
 ...
 * }
 */
export function configFileSpecs_getMatchedIncludeSpec(receiver, fileName, comparePathsOptions) {
 ...
 * }
 */
export function configFileSpecs_getMatchedFileSpec(receiver, fileName, comparePathsOptions) {
 ...
 * }
 */
export function ExtendedConfigCacheEntry_ExtendedFileNames(receiver) {
 ...
 * }
 */
export function parseOwnConfigOfJsonSourceFile(sourceFile, host, basePath, configFileName) {
    ...
    let extendedConfigPath = undefined;
    const errors = [];
    const onPropertySet = (keyText, value, propertyAssignment, parentOption, option) => {
        // Ensure value is verified except for extends which is handled in its own way for error reporting
        let currentValue = value;
        const propertySetErrors = [];
        ...
        if (parentOption !== undefined && parentOption.Name !== "undefined" && currentValue !== undefined && currentValue !== null) {
            if (option !== undefined && option.Name !== "") {
                ...
                if (parentOption.ElementOptions !== undefined) {
                    ...
                    if (possibleOption !== undefined && possibleOption.Name !== keyText) {
                        propertySetErrors.push(CreateDiagnosticForNodeInSourceFileOrCompilerDiagnostic(sourceFile, PropertyAssignment_Name(propertyAssignment), extraKeyDidYouMeanDiagnostics(parentOption.Name), keyText, possibleOption.Name));
                    }
                    else {
                        propertySetErrors.push(createUnknownOptionError(keyText, unknownNameDiag, "" /*unknownOptionErrorText*/, PropertyAssignment_Name(propertyAssignment), sourceFile, undefined /*alternateMode*/));
                    }
                }
                else {
                    // errors = append(errors, ast.NewCompilerDiagnostic(diagnostics.Unknown_compiler_option_0_Did_you_mean_1, keyText, core.FindKey(parentOption.ElementOptions, keyText)))
            ...
            if (option === extendsOptionDeclaration) {
                const [configPath, err] = getExtendsConfigPathOrArray(currentValue, host, basePath, configFileName, propertyAssignment, propertyAssignment.Initializer, sourceFile);
                extendedConfigPath = configPath;
                propertySetErrors.push(...err);
            }
            else if (option === undefined) {
                if (keyText === "excludes") {
                    propertySetErrors.push(CreateDiagnosticForNodeInSourceFile(sourceFile, propertyAssignment, diagnostics.Unknown_option_excludes_Did_you_mean_exclude));
                }
                if (core.Find(OptionsDeclarations, (opt) => opt.Name === keyText) !== undefined) {
 ...
 * }
 */
export function NewTsconfigSourceFileFromFilePath(configFileName, configPath, configSourceText) {
    const sourceFile = ParseSourceFile({ FileName: configFileName, Path: configPath }, configSourceText, ScriptKindJSON);
    return {
        ExtendedSourceFiles: [],
 ...
 * }
 */
export function convertConfigFileToObject(sourceFile, jsonConversionNotifier) {
    let rootExpression = undefined;
    if ((sourceFile.Statements.Nodes?.length ?? 0) > 0) {
        rootExpression = Node_Expression(sourceFile.Statements.Nodes[0]);
    }
    if (rootExpression !== undefined && rootExpression.Kind !== KindObjectLiteralExpression) {
        ...
        const errors = [NewCompilerDiagnostic(diagnostics.The_root_value_of_a_0_file_must_be_an_object, baseFileName)];
        // Last-ditch error recovery. Somewhat useful because the JSON parser will recover from some parse errors by
        // synthesizing a top-level array literal expression. There's a reasonable chance the first element of that
        // array is a well-formed configuration object, made into an array element by stray characters.
        if (IsArrayLiteralExpression(rootExpression)) {
            const firstObject = core.Find((Node_Elements(rootExpression) ?? []), (n) => IsObjectLiteralExpression(n));
            if (firstObject !== undefined) {
                return convertToJson(sourceFile, firstObject, true /*returnValue*/, jsonConversionNotifier);
            }
        }
        return [newMapWithSizeHint(0), errors];
    }
    return convertToJson(sourceFile, rootExpression, true, jsonConversionNotifier);
}
/**
...
export const orderedMapType = reflect_TypeFor();
export function isOrderedMap(value) {
 ...
 * }
 */
export function isCompilerOptionsValue(option, value) {
    if (option !== undefined) {
 ...
 * }
 */
export function validateJsonOptionValue(opt, val, valueExpression, sourceFile) {
    ...
    switch (opt.extraValidation) {
        case extraValidationSpec: {
            const diag = specToDiagnostic(val, false);
            if (diag !== undefined) {
 ...
 * }
 */
export function convertJsonOptionOfListType(option, values, basePath, propertyAssignment, valueExpression, sourceFile) {
    let expression = undefined;
    const errors = [];
    if (Array.isArray(values)) {
        const valuesArr = values;
        const mappedValues = core.MapIndex(valuesArr, (v, index) => {
            if (valueExpression !== undefined) {
                const elems = Node_Elements(valueExpression);
                expression = elems !== undefined ? elems[index] : undefined;
            }
            const [result, err] = convertJsonOption(CommandLineOption_Elements(option), v, basePath, propertyAssignment, expression, sourceFile);
            errors.push(...err);
            return result;
        });
        let filteredValues = mappedValues;
        if (!option.listPreserveFalsyValues) {
            filteredValues = core.Filter(mappedValues, (v) => {
 ...
 */
export const configDirTemplate = "${configDir}";
/**
 ...
 * }
 */
export function startsWithConfigDirTemplate(value) {
 ...
 * }
 */
export function normalizeNonListOptionValue(option, basePath, value) {
    let result = value;
    if (option.IsFilePath) {
        result = NormalizeSlashes(result);
        if (!startsWithConfigDirTemplate(result)) {
            result = GetNormalizedAbsolutePath(result, basePath);
        }
        if (result === "") {
            result = ".";
        }
 ...
 * }
 */
export function convertJsonOption(opt, value, basePath, propertyAssignment, valueExpression, sourceFile) {
    if (opt.IsCommandLineOnly) {
        let nodeValue = undefined;
        if (propertyAssignment !== undefined) {
            nodeValue = propertyAssignment;
        }
    ...
    if (isCompilerOptionsValue(opt, value)) {
        switch (opt.Kind) {
            ...
            case CommandLineOptionTypeEnum:
                ...
                return convertJsonOptionOfEnumType(opt, value, valueExpression, sourceFile);
        }
 ...
 * }
 */
export function getExtendsConfigPathOrArray(value, host, basePath, configFileName, propertyAssignment, valueExpression, sourceFile) {
    const extendedConfigPathArray = [];
 ...
 * }
 */
export function getExtendsConfigPath(extendedConfig, host, basePath, valueExpression, sourceFile) {
    extendedConfig = NormalizeSlashes(extendedConfig);
    const errors = [];
    let errorFile = undefined;
    ...
    // If the path isn't a rooted or relative path, resolve like a module
    const resolverHostInst = { __tsgoEmbedded0: host };
    const resolverHostAsResolution = {
        FS: () => host.FS(),
 ...
 * }
 */
export function CommandLineOptionNameMap_Get(receiver, name) {
    const m = receiver;
 ...
 * }
 */
export function convertMapToOptions(compilerOptions, result) {
    // this assumes any `key`, `value` pair in `options` will have `value` already be the correct type. this function should no error handling
    OrderedMap_Entries(compilerOptions)((key, value) => {
        result.ParseOption(key, value);
        return true;
    });
    return result;
 ...
 * }
 */
export function convertOptionsFromJson(optionsNameMap, jsonOptions, basePath, result) {
    ...
    const jsonMap = jsonOptions;
    const errors = [];
    OrderedMap_Entries(jsonMap)((key, value) => {
        ...
        if (commandLineOptionEnumMapVal !== undefined) {
            if (typeof value === "string") {
                const [val, ok] = OrderedMap_Get(commandLineOptionEnumMapVal, strings.ToLower(value));
                if (ok) {
 ...
 * }
 */
export function convertArrayLiteralExpressionToJson(sourceFile, elements, elementOption, returnValue) {
    if (!returnValue) {
 ...
 * }
 */
export function directoryOfCombinedPath(fileName, basePath) {
    // Use the `getNormalizedAbsolutePath` function to avoid canonicalizing the path, as it must remain noncanonical
    // until consistent casing errors are reported
 ...
 * }
 */
export function ParseConfigFileTextToJson(fileName, path, jsonText) {
    const jsonSourceFile = ParseSourceFile({ FileName: fileName, Path: path }, jsonText, ScriptKindJSON);
 ...
 */
export function resolverHost_Trace(receiver, msg) { }
/**
 ...
 * }
 */
export function ParseJsonSourceFileConfigFileContent(sourceFile, host, basePath, existingOptions, existingOptionsRaw, configFileName, resolutionStack, extraFileExtensions, extendedConfigCache) {
    // tracing?.push(tracing.Phase.Parse, "parseJsonSourceFileConfigFileContent", { path: sourceFile.fileName });
    const result = parseJsonConfigFileContentWorker(undefined /*json*/, sourceFile, host, basePath, existingOptions, existingOptionsRaw, configFileName, resolutionStack, extraFileExtensions, extendedConfigCache);
    // tracing?.pop();
    return result;
}
/**
 ...
 * }
 */
export function convertObjectLiteralExpressionToJson(sourceFile, returnValue, node, objectOption, jsonConversionNotifier) {
    let result = undefined;
    if (returnValue) {
        result = newMapWithSizeHint(0);
    }
    const errors = [];
    for (const element of (node.Properties.Nodes ?? [])) {
        ...
        if (keyText !== "") {
            if (returnValue) {
                OrderedMap_Set(result, keyText, value);
            }
            // Notify key value set, if user asked for it
 ...
 * }
 */
export function convertToJson(sourceFile, rootExpression, returnValue, jsonConversionNotifier) {
 ...
 * }
 */
export function convertPropertyValueToJson(sourceFile, valueExpression, option, returnValue, jsonConversionNotifier) {
    switch (valueExpression.Kind) {
        ...
        case KindArrayLiteralExpression: {
            const [result, errors] = convertArrayLiteralExpressionToJson(sourceFile, (Node_Elements(valueExpression) ?? []), option, returnValue);
            return [result, errors];
        }
    }
    // Not in expected format
 ...
 * }
 */
export function ParseJsonConfigFileContent(json, host, basePath, existingOptions, configFileName, resolutionStack, extraFileExtensions, extendedConfigCache) {
    const result = parseJsonConfigFileContentWorker(parseJsonToStringKey(json), undefined /*sourceFile*/, host, basePath, existingOptions, undefined /*existingOptionsRaw*/, configFileName, resolutionStack, extraFileExtensions, extendedConfigCache);
    return result;
}
/**
 ...
 * }
 */
export function getDefaultCompilerOptions(configFileName) {
    let options = {};
 ...
 * }
 */
export function getDefaultTypeAcquisition(configFileName) {
    const options = {};
    if (configFileName !== "" && GetBaseFileName(configFileName) === "jsconfig.json") {
 ...
 * }
 */
export function convertCompilerOptionsFromJsonWorker(jsonOptions, basePath, configFileName) {
    const options = getDefaultCompilerOptions(configFileName);
    const parser = { __tsgoEmbedded0: options };
 ...
 * }
 */
export function convertTypeAcquisitionFromJsonWorker(jsonOptions, basePath, configFileName) {
    const options = getDefaultTypeAcquisition(configFileName);
    const parser = { __tsgoEmbedded0: options };
    const [, errors] = convertOptionsFromJson(typeAcquisitionDeclaration.ElementOptions, jsonOptions, basePath, typeAcquisitionParser_as_optionParser(parser));
    return [options, errors];
}
/**
 ...
 * }
 */
export function parseOwnConfigOfJson(json, host, basePath, configFileName) {
    const errors = [];
    const jsonMap = json;
    ...
    if (extendsVal !== undefined && extendsVal !== null && extendsVal !== "") {
        const [configPath, extErr] = getExtendsConfigPathOrArray(extendsVal, host, basePath, configFileName, undefined, undefined, undefined);
        extendedConfigPath = configPath;
        errors.push(...extErr);
    }
    const parsedConfig = {
 ...
 * }
 */
export function readJsonConfigFile(fileName, path, readFile) {
        ...
        return [
            {
                ExtendedSourceFiles: [],
                configFileSpecs: undefined,
                SourceFile: ParseSourceFile({ FileName: fileName, Path: path }, text, ScriptKindJSON),
            },
            diagnostic,
        ];
    }
    else {
        const factory = { hooks: {}, nodeCount: 0, textCount: 0 };
        const endOfFileToken = NodeFactory_NewToken(factory, KindEndOfFile);
        const sourceFileNode = NodeFactory_NewSourceFile(factory, { FileName: fileName, Path: path }, "", undefined, endOfFileToken);
 ...
 * }
 */
export function getExtendedConfig(sourceFile, extendedConfigFileName, host, resolutionStack, extendedConfigCache, result) {
    const errors = [];
 ...
 * }
 */
export function ParseExtendedConfig(fileName, path, resolutionStack, host, extendedConfigCache) {
 ...
 * }
 */
export function parseConfig(json, sourceFile, host, basePath, configFileName, resolutionStack, extendedConfigCache) {
    ...
    const applyExtendedConfig = (result, extendedConfigPath) => {
        ...
        if (extendedConfig !== undefined && extendedConfig.options !== undefined) {
            ...
            const setPropertyValue = (propertyName) => {
                const ownRawMap = ownConfig.raw;
                if (isOrderedMap(ownRawMap) && OrderedMap_Has(ownRawMap, propertyName)) {
                    return;
                }
                if (propertyName === "include" || propertyName === "exclude" || propertyName === "files") {
                    if (isOrderedMap(extendsRaw) && OrderedMap_Has(extendsRaw, propertyName)) {
                        const sliceVal = OrderedMap_GetOrZero(extendsRaw, propertyName);
                        if (Array.isArray(sliceVal) && sliceVal !== undefined) {
                            const slice = sliceVal;
                            const value = core.Map(slice, (path) => {
            ...
            setPropertyValue("exclude");
            setPropertyValue("files");
            if (isOrderedMap(extendsRaw) && OrderedMap_Has(extendsRaw, "compileOnSave")) {
                const compileOnSave = OrderedMap_GetOrZero(extendsRaw, "compileOnSave");
    ...
    if (ownConfig.extendedConfigPath !== undefined && ownConfig.extendedConfigPath !== null) {
        ...
        const ownRawMap = ownConfig.raw;
        if (result.include !== undefined && result.include.length > 0) {
            OrderedMap_Set(ownRawMap, "include", result.include);
        }
        if (result.exclude !== undefined && result.exclude.length > 0) {
 ...
 * }
 */
export function parseJsonConfigFileContentWorker(json, sourceFile, host, basePath, existingOptions, existingOptionsRaw, configFileName, resolutionStack, extraFileExtensions, extendedConfigCache) {
    const basePathForFileNames = configFileName !== ""
    ...
    const getPropFromRaw = (prop, validateElement, elementTypeName) => {
        ...
        if (exists && value !== undefined && value !== null) {
            if (Array.isArray(value)) {
                ...
                return { sliceValue: result, wrongValue: "" };
            }
    ...
    const referencesOfRaw = getPropFromRaw("references", (element) => isOrderedMap(element), "object");
    let fileSpecs = getPropFromRaw("files", (element) => typeof element === "string", "string");
    if (fileSpecs.sliceValue !== undefined || fileSpecs.wrongValue === "") {
        const hasZeroOrNoReferences = referencesOfRaw.wrongValue === "no-prop" || referencesOfRaw.wrongValue === "not-array" || (referencesOfRaw.sliceValue ?? []).length === 0;
        const hasExtends = OrderedMap_GetOrZero(rawConfig, "extends");
        if ((fileSpecs.sliceValue ?? []).length === 0 && hasZeroOrNoReferences && (hasExtends === undefined || hasExtends === null)) {
            if (sourceFile !== undefined) {
                const fileName = configFileName !== "" ? configFileName : "tsconfig.json";
                const diagnosticMessage = diagnostics.The_files_list_in_config_file_0_is_empty;
                const nodeValue = ForEachTsConfigPropArray(tsconfigToSourceFile(sourceFile), "files", (property) => property.Initializer);
    ...
    const includeSpecsRaw = getPropFromRaw("include", (element) => typeof element === "string", "string");
    const excludeSpecsRaw = getPropFromRaw("exclude", (element) => typeof element === "string", "string");
    let includeSpecs = includeSpecsRaw;
    let excludeSpecs = excludeSpecsRaw;
    ...
    let validatedIncludeSpecs = [];
    let validatedIncludeSpecsBeforeSubstitution = [];
    let validatedExcludeSpecs = [];
    let validatedFilesSpec = [];
    let validatedFilesSpecBeforeSubstitution = [];
    ...
    if (fileSpecs.sliceValue !== undefined) {
        const stringFileSpecs = core.Filter(fileSpecs.sliceValue, (spec) => typeof spec === "string");
    ...
    const getFileNames = (_basePath) => {
        const [fileNames, litLen] = getFileNamesFromConfigSpecs(cfgSpecs, _basePath, parsedConfig.options, host.FS(), extraFileExtensions);
        if (shouldReportNoInputFiles(fileNames, canJsonReportNoInputFiles(rawConfig), resolutionStack)) {
            const incSpecs = cfgSpecs.includeSpecs !== undefined ? cfgSpecs.includeSpecs : [];
            const excSpecs = cfgSpecs.excludeSpecs !== undefined ? cfgSpecs.excludeSpecs : [];
            errors.push(NewCompilerDiagnostic(diagnostics.No_inputs_were_found_in_config_file_0_Specified_include_paths_were_1_and_exclude_paths_were_2, configFileName, core.Must(...core.StringifyJson(incSpecs, "", "")), core.Must(...core.StringifyJson(excSpecs, "", ""))));
        }
        return [fileNames, litLen];
    };
    const getProjectReferences = (_basePath) => {
        const newReferencesOfRaw = getPropFromRaw("references", (element) => isOrderedMap(element), "object");
        if (newReferencesOfRaw.sliceValue === undefined) {
            return undefined;
        }
        const projectReferences = [];
        for (const reference of newReferencesOfRaw.sliceValue) {
 ...
 * }
 */
export function canJsonReportNoInputFiles(rawConfig) {
 ...
 * }
 */
export function shouldReportNoInputFiles(fileNames, canJsonReportNoInputFiles, resolutionStack) {
 ...
 * }
 */
export function validateSpecs(specs, disallowTrailingRecursion, jsonSourceFile, specKey) {
    const createDiagnostic = (message, spec) => {
 ...
 * }
 */
export function specToDiagnostic(spec, disallowTrailingRecursion) {
    if (disallowTrailingRecursion && invalidTrailingRecursion(spec)) {
        return diagnostics.File_specification_cannot_end_in_a_recursive_directory_wildcard_Asterisk_Asterisk_Colon_0;
    }
    if (invalidDotDotAfterRecursiveWildcard(spec)) {
 ...
 * }
 */
export function invalidTrailingRecursion(spec) {
    // Matches **, /**, ** /, and /** /, but not a**b.
    // Strip optional trailing slash, then check if it ends with /** or is just **
 ...
 * }
 */
export function invalidDotDotAfterRecursiveWildcard(s) {
    // We used to use the regex /(^|\/)\*\*\/(.*\/)?\.\.($|\/)/ to check for this case, but
    // in v8, that has polynomial performance because the recursive wildcard match - **/ -
    // can be matched in many arbitrary positions when multiple are present, resulting
    // in bad backtracking (and we don't care which is matched - just that some /.. segment
 ...
 * }
 */
export function GetTsConfigPropArrayElementValue(tsConfigSourceFile, propKey, elementValue) {
 ...
 * }
 */
export function ForEachTsConfigPropArray(tsConfigSourceFile, propKey, callback) {
    if (tsConfigSourceFile !== undefined) {
        return ForEachPropertyAssignment(getTsConfigObjectLiteralExpression(tsConfigSourceFile), propKey, callback);
    }
    return undefined;
 ...
 * }
 */
export function CreateDiagnosticAtReferenceSyntax(config, index, message, ...args) {
    return ForEachTsConfigPropArray(config.ConfigFile.SourceFile, "references", (property) => {
 ...
 * }
 */
export function GetCallbackForFindingPropertyAssignmentByValue(value) {
    return (property) => {
        if (IsArrayLiteralExpression(property.Initializer)) {
            return core.Find((Node_Elements(property.Initializer) ?? []), (element) => (IsStringLiteral(element) && Node_Text(element) === value));
        }
        return undefined;
    };
}
 ...
 * }
 */
export function GetOptionsSyntaxByArrayElementValue(objectLiteral, propKey, elementValue) {
    return ForEachPropertyAssignment(objectLiteral, propKey, GetCallbackForFindingPropertyAssignmentByValue(elementValue));
}
/**
 ...
 * }
 */
export function ForEachPropertyAssignment(objectLiteral, key, callback, ...key2) {
 ...
 * }
 */
export function getTsConfigObjectLiteralExpression(tsConfigSourceFile) {
 ...
 * }
 */
export function getSubstitutedPathWithConfigDirTemplate(value, basePath) {
 ...
 * }
 */
export function getSubstitutedStringArrayWithConfigDirTemplate(list, basePath) {
    let result = undefined;
 ...
 * }
 */
export function handleOptionConfigDirTemplateSubstitution(compilerOptions, basePath) {
    if (compilerOptions === undefined) {
        return;
    }
    const options = compilerOptions;
    // !!! don't hardcode this; use options declarations?
    OrderedMap_Entries(options.Paths)((k, v) => {
        const substitution = getSubstitutedStringArrayWithConfigDirTemplate(v, basePath);
        if (substitution !== undefined) {
            OrderedMap_Set(options.Paths, k, substitution);
        }
        return true;
    });
 ...
 * }
 */
export function hasFileWithHigherPriorityExtension(file, extensions, hasFile) {
    let extensionGroup = [];
 ...
 * }
 */
export function removeWildcardFilesWithLowerPriorityExtension(file, wildcardFiles, extensions, keyMapper) {
    let extensionGroup = [];
    for (const group of extensions) {
        if (FileExtensionIsOneOf(file, group)) {
            extensionGroup = extensionGroup.concat(group);
    ...
    for (let i = extensionGroup.length - 1; i >= 0; i--) {
        ...
        OrderedMap_Delete(wildcardFiles, lowerPriorityPath);
    }
}
/**
 ...
 * }
 */
export function getFileNamesFromConfigSpecs(configFileSpecs, basePath, options, host, extraFileExtensions) {
    // Go overrides extraFileExtensions with empty slice in this function
    extraFileExtensions = [];
    basePath = NormalizePath(basePath);
    const keyMapper = (value) => GetCanonicalFileName(value, host.UseCaseSensitiveFileNames());
    // Literal file names (provided via the "files" array in tsconfig.json) are stored in a
    // file map with a possibly case insensitive key. We use this map later when including wildcard paths.
    const literalFileMap = newMapWithSizeHint(0);
    // Wildcard paths
    const wildcardFileMap = newMapWithSizeHint(0);
    // Wildcard paths of json files
    const wildCardJsonFileMap = newMapWithSizeHint(0);
    ...
    for (const fileName of (validatedFilesSpec ?? [])) {
        ...
        OrderedMap_Set(literalFileMap, keyMapper(fileName), file);
    }
    let jsonOnlyIncludeMatchers = undefined;
        ...
        for (const file of files) {
            if (FileExtensionIs(file, ExtensionJson)) {
                if (jsonOnlyIncludeMatchers === undefined) {
                    const includes = core.Filter(validatedIncludeSpecs, (include) => strings.HasSuffix(include, ExtensionJson));
                ...
                if (includeIndex !== -1) {
                    ...
                    if (!OrderedMap_Has(literalFileMap, key) && !OrderedMap_Has(wildCardJsonFileMap, key)) {
                        OrderedMap_Set(wildCardJsonFileMap, key, file);
                    }
                }
                continue;
            }
            // If we have already included a literal or wildcard path with a higher priority extension, skip this file.
            if (hasFileWithHigherPriorityExtension(file, supportedExtensions, (fn) => {
                const canonicalFileName = keyMapper(fn);
                return (OrderedMap_Has(literalFileMap, canonicalFileName) || OrderedMap_Has(wildcardFileMap, canonicalFileName));
            })) {
                continue;
            }
            ...
            const key = keyMapper(file);
            if (!OrderedMap_Has(literalFileMap, key) && !OrderedMap_Has(wildcardFileMap, key)) {
                OrderedMap_Set(wildcardFileMap, key, file);
            }
        }
    }
    const files = [];
    OrderedMap_Values(literalFileMap)((file) => {
        files.push(file);
        return true;
    });
    OrderedMap_Values(wildcardFileMap)((file) => {
        files.push(file);
        return true;
    });
    OrderedMap_Values(wildCardJsonFileMap)((file) => {
        files.push(file);
        return true;
    });
    return [files, OrderedMap_Size(literalFileMap)];
}
/**
 ...
 * }
 */
export function GetSupportedExtensions(compilerOptions, extraFileExtensions) {
 ...
 * }
 */
export function GetSupportedExtensionsWithJsonIfResolveJsonModule(compilerOptions, supportedExtensions) {
 ...
 * }
 */
export function GetParsedCommandLineOfConfigFile(configFileName, options, optionsRaw, sys, extendedConfigCache) {
 ...
 * }
 */
export function GetParsedCommandLineOfConfigFilePath(configFileName, path, options, optionsRaw, sys, extendedConfigCache) {
    ...
    return [ParseJsonSourceFileConfigFileContent(tsConfigSourceFile, sys, GetDirectoryPath(configFileName), options, optionsRaw, configFileName, [], undefined, extendedConfigCache), undefined];
}
//# sourceMappingURL=tsconfigparsing.js.map