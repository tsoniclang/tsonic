...
function vfsParseConfigHost(files, currentDirectory, useCaseSensitiveFileNames) {
...
class memoCache {
    m;
    GetExtendedConfig(fileName, path, resolutionStack, host) {
        if (this.m === undefined) {
            this.m = new Map();
        }
...
function makeParseConfig(host) {
    return (configFileName, cache) => {
        ...
        const tsConfigSourceFile = {
            ExtendedSourceFiles: [],
            configFileSpecs: undefined,
            SourceFile: ParseSourceFile({ FileName: configFileName, Path: cfgPath }, jsonText, ScriptKindJSON),
        };
        return ParseJsonSourceFileConfigFileContent(tsConfigSourceFile, host, host.GetCurrentDirectory(), undefined, undefined, configFileName, [], [], cache);
    };
}
...
test("ExtendedConfigErrorsAppearOnCacheHit / single config parsed twice", () => {
  ...
  "excludes": ["**/*.ts"]
    ...
    const host = vfsParseConfigHost(files, "/", true /*useCaseSensitiveFileNames*/);
...
//# sourceMappingURL=tsconfigparsing.test.js.map