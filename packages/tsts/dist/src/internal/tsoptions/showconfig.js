...
import { CompilerOptions_GetAllowImportingTsExtensions, CompilerOptions_GetAllowJS, CompilerOptions_GetAreDeclarationMapsEnabled, CompilerOptions_GetEmitDeclarations, CompilerOptions_GetEmitModuleDetectionKind, CompilerOptions_GetEmitModuleKind, CompilerOptions_GetIsolatedModules, CompilerOptions_GetModuleResolutionKind, CompilerOptions_GetResolveJsonModule, CompilerOptions_GetResolvePackageJsonExports, CompilerOptions_GetResolvePackageJsonImports, CompilerOptions_GetUseDefineForClassFields, CompilerOptions_IsIncremental, CompilerOptions_ShouldPreserveConstEnums, } from "../core/compileroptions.js";
import { GetNormalizedAbsolutePath, GetRelativePathFromFile } from "../tspath/path.js";
import { CommandLineOption_EnumMap, CommandLineOptionTypeBoolean, CommandLineOptionTypeList, CommandLineOptionTypeListOrElement, CommandLineOptionTypeNumber, CommandLineOptionTypeString, CommandLineOption_Elements, } from "./commandlineoption.js";
...
import { ParsedCommandLine_CompilerOptions, ParsedCommandLine_FileNames, ParsedCommandLine_GetCurrentDirectory, ParsedCommandLine_ProjectReferences, ParsedCommandLine_UseCaseSensitiveFileNames, } from "./parsedcommandline.js";
/**
 ...
 * }
 */
export function ConvertToTSConfig(configParseResult, configFileName) {
    ...
    if (refs.length > 0) {
        const references = [];
        for (const r of refs) {
            const ref = NewOrderedMapWithSizeHint(2);
 ...
 * }
 */
export function filterSameAsDefaultInclude(specs) {
    if (specs.length === 0) {
        return [];
    }
 ...
 * }
 */
export function getNameOfCompilerOptionValue(value, enumMap) {
    const found = { value: "" };
    const matched = { value: false };
    OrderedMap_Entries(enumMap)((k, v) => {
        if (v === value) {
            found.value = k;
            matched.value = true;
            return false;
 ...
 * }
 */
export function serializeCompilerOptions(options, configFilePath, comparePathsOptions) {
    const result = NewOrderedMapWithSizeHint(32);
    ...
    for (const fieldName of globalThis.Object.keys(optionsObj)) {
        ...
        if (globalThis.Array.isArray(fieldValue) && fieldValue.length === 0) {
            continue;
        }
 ...
 * }
 */
export function serializeEnumValue(value, enumMap) {
    if (typeof value === "number") {
 ...
 * }
 */
export function addImpliedOptions(optionMap, options, arg, arg1) {
    // Build the set of explicitly provided option JSON names (e.g., "module", "target").
    const provided = new globalThis.Map();
    OrderedMap_Keys(optionMap)((k) => {
        provided.set(k, true);
        return true;
    });
    const defaultOpts = {};
 ...
 * }
 */
export function anyDependencyProvided(dependencies, provided) {
 ...
 * }
 */
export function serializeImpliedOptionValue(optionDecl, value) {
...
//# sourceMappingURL=showconfig.js.map