...
import { CommandLineOptionTypeBoolean, CommandLineOptionTypeEnum, CommandLineOptionTypeList, CommandLineOptionTypeNumber, CommandLineOptionTypeObject, CommandLineOptionTypeString, extraValidationLocale, extraValidationNone, newCommandLineOption, } from "./commandlineoption.js";
/**
...
export let OptionsDeclarations = undefined;
/**
 ...
 * }
 */
export const optionsForCompiler = [
    //******* compilerOptions not common with --build *******
    // CommandLine only options
    newCommandLineOption({
        Name: "all",
 ...
 */
export const optionsType = reflect_TypeFor();
const compilerOptionFieldMap = buildCompilerOptionFieldMap();
/**
 ...
 * }
 */
export function optionsHaveChanges(oldOptions, newOptions, declFilter) {
    if (oldOptions === newOptions) {
        return false;
    }
    ...
    return ForEachCompilerOptionValue(newOptions, declFilter, (option, value, _i) => {
        const fieldName = compilerOptionFieldName(option.Name);
        const newValue = value.Interface();
        const oldValue = oldOptions[fieldName];
        if (option.strictFlag) {
            return CompilerOptions_GetStrictOptionValue(oldOptions, oldValue) !== CompilerOptions_GetStrictOptionValue(newOptions, newValue);
        }
        if (option.allowJsFlag) {
 ...
 * }
 */
export function ForEachCompilerOptionValue(options, declFilter, fn) {
    ...
    for (const [fieldName, optionDeclaration] of compilerOptionFieldMap) {
        if (optionDeclaration !== undefined && declFilter(optionDeclaration)) {
            if (fn(optionDeclaration, reflect_ValueOf(values[fieldName]), index)) {
                return true;
            }
        }
        index = index + 1;
 ...
 * }
 */
export function CompilerOptionsAffectSemanticDiagnostics(oldOptions, newOptions) {
    return optionsHaveChanges(oldOptions, newOptions, (option) => option.AffectsSemanticDiagnostics);
}
/**
 ...
 * }
 */
export function CompilerOptionsAffectDeclarationPath(oldOptions, newOptions) {
    return optionsHaveChanges(oldOptions, newOptions, (option) => option.AffectsDeclarationPath);
}
/**
 ...
 * }
 */
export function CompilerOptionsAffectEmit(oldOptions, newOptions) {
    return optionsHaveChanges(oldOptions, newOptions, (option) => option.AffectsEmit);
}
function buildCompilerOptionFieldMap() {
