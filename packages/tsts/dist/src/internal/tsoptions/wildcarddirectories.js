...
import { CombinePaths, ContainsPath, DirectorySeparator, NormalizeSlashes, RemoveTrailingDirectorySeparator, } from "../tspath/path.js";
 ...
 * }
 */
export function getWildcardDirectories(include, exclude, comparePathsOptions) {
    // We watch a directory recursively if it contains a wildcard anywhere in a directory segment
    // of the pattern:
    //
    //  /a/b/**/d   - Watch /a/b recursively to catch changes to any d in any subfolder recursively
    ...
    //  /a/b/*      - Watch /a/b directly to catch any new file
    //  /a/b/a?z    - Watch /a/b directly to catch any new file matching a?z
    const excludeMatcher = NewSpecMatcher(exclude, comparePathsOptions.CurrentDirectory, UsageExclude, comparePathsOptions.UseCaseSensitiveFileNames);
    const wildcardDirectories = new globalThis.Map();
    if (include.length === 0) {
        return wildcardDirectories;
    }
    const wildCardKeyToPath = new globalThis.Map();
    const recursiveKeys = [];
 ...
 * }
 */
export function toCanonicalKey(path, useCaseSensitiveFileNames) {
    if (useCaseSensitiveFileNames) {
        return path;
    }
    return strings.ToLower(path);
 ...
 * }
 */
export function getWildcardDirectoryFromSpec(spec, useCaseSensitiveFileNames) {
    // Find the first occurrence of a wildcard character
...
//# sourceMappingURL=wildcarddirectories.js.map