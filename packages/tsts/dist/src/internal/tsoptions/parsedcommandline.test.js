...
import { ParsedCommandLine_FileNames, ParsedCommandLine_LiteralFileNames, ParsedCommandLine_PossiblyMatchesFileName, ParsedCommandLine_ReloadFileNamesOfParsedCommandLine, } from "./parsedcommandline.js";
...
function parseConfig(jsonText, files = sourceFiles) {
...
function assertMatches(parsedCommandLine, files, matches) {
    const expectedMatches = new Set(matches);
    for (const fileName of files.keys()) {
        assert.equal(ParsedCommandLine_PossiblyMatchesFileName(parsedCommandLine, fileName), expectedMatches.has(fileName), `fileName: ${fileName}`);
    }
    for (const fileName of matches) {
        if (!files.has(fileName)) {
            assert.equal(ParsedCommandLine_PossiblyMatchesFileName(parsedCommandLine, fileName), true, `fileName: ${fileName}`);
        }
    }
}
test("ParsedCommandLine.PossiblyMatchesFileName mirrors TS-Go literal files and include lists", () => {
    const noFilesFS = FromMap(new Map(), true);
...
//# sourceMappingURL=parsedcommandline.test.js.map