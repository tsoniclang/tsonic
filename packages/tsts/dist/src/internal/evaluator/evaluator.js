...
import { KindAmpersandToken, KindAsteriskAsteriskToken, KindAsteriskToken, KindBarToken, KindBinaryExpression, KindCaretToken, KindElementAccessExpression, KindGreaterThanGreaterThanGreaterThanToken, KindGreaterThanGreaterThanToken, KindIdentifier, KindLessThanLessThanToken, KindMinusToken, KindNoSubstitutionTemplateLiteral, KindNumericLiteral, KindPercentToken, KindPlusToken, KindPrefixUnaryExpression, KindPropertyAccessExpression, KindSlashToken, KindStringLiteral, KindTemplateExpression, KindTildeToken, } from "../ast/generated/kinds.js";
import { IsEntityNameExpression, OEKParentheses, SkipOuterExpressions } from "../ast/utilities.js";
import { IfElse } from "../core/core.js";
import { Number_BitwiseAND, Number_BitwiseNOT, Number_BitwiseOR, Number_BitwiseXOR, Number_Exponentiate, Number_IsNaN, Number_LeftShift, Number_Remainder, Number_SignedRightShift, Number_UnsignedRightShift, } from "../jsnum/jsnum.js";
 ...
 * }
 */
export function NewResult(value, isSyntacticallyString, resolvedOtherFiles, hasExternalReferences) {
    return {
        Value: value,
 ...
 * }
 */
export function NewEvaluator(evaluateEntity, outerExpressionsToSkip) {
    let evaluate;
    evaluate = (expr, location) => {
        ...
        switch (expr.Kind) {
            case KindPrefixUnaryExpression: {
                ...
                const result = evaluate(prefix.Operand, location);
                resolvedOtherFiles = result.ResolvedOtherFiles;
                hasExternalReferences = result.HasExternalReferences;
            ...
            case KindBinaryExpression: {
                ...
                const left = evaluate(binary.Left, location);
                const right = evaluate(binary.Right, location);
                const operator = binary.OperatorToken.Kind;
                isSyntacticallyString = ((left.IsSyntacticallyString || right.IsSyntacticallyString) && binary.OperatorToken.Kind === KindPlusToken);
                resolvedOtherFiles = (left.ResolvedOtherFiles || right.ResolvedOtherFiles);
                hasExternalReferences = (left.HasExternalReferences || right.HasExternalReferences);
                const leftIsNum = typeof left.Value === "number";
                const rightIsNum = typeof right.Value === "number";
                const leftNum = leftIsNum ? left.Value : 0;
                const rightNum = rightIsNum ? right.Value : 0;
                if (leftIsNum && rightIsNum) {
                    switch (operator) {
                        case KindBarToken:
                ...
                let leftStr = leftIsStr ? left.Value : "";
                let rightStr = rightIsStr ? right.Value : "";
 ...
 * }
 */
export function evaluateTemplateExpression(expr, location, evaluate) {
    let sb = "";
    ...
    for (const span of templateExpression.TemplateSpans.Nodes) {
        ...
        const spanResult = evaluate(templateSpan.Expression, location);
