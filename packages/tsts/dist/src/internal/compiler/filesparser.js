...
import { fileLoader_toPath, fileLoader_isSupportedExtension, fileLoader_loadSourceFileMetaData, fileLoader_parseSourceFile, fileLoader_resolveTripleslashPathReference, fileLoader_resolveTypeReferenceDirectives, fileLoader_resolveAutomaticTypeDirectives, fileLoader_resolveImportsAndModuleAugmentations, fileLoader_pathForLibFile, fileLoader_sortLibs, } from "./fileloader.js";
...
import { includeProcessor_addProcessingDiagnosticsForFileCasing, } from "./includeprocessor.js";
 ...
 * }
 */
export function parseTask_FileName(receiver) {
 ...
 * }
 */
export function parseTask_Path(receiver) {
 ...
 * }
 */
export function parseTask_load(receiver, loader) {
 ...
 * }
 */
export function parseTask_redirect(receiver, loader, fileName) {
 ...
 * }
 */
export function parseTask_loadAutomaticTypeDirectives(receiver, loader) {
    // loader.opts.Tracing is omitted (single-threaded, tracing is a no-op)
 ...
 * }
 */
export function parseTask_addSubTask(receiver, ref, libFile) {
 ...
 * }
 */
export const parseTaskDataPool = (() => {
    const pool = new Pool();
    pool.New = () => ({
        tasks: new globalThis.Map(),
        mu: new Mutex(),
        lowestDepth: 0,
        startedSubTasks: false,
        packageId: { Name: "", SubModuleName: "", Version: "", PeerDependencies: "" },
 ...
 * }
 */
export function filesParser_parse(receiver, loader, tasks) {
 ...
 * }
 */
export function filesParser_start(receiver, loader, tasks, depth) {
    for (let i = 0; i < tasks.length; i++) {
        const task = tasks[i];
        task.path = fileLoader_toPath(loader, task.normalizedFilePath);
        const candidate = getParseTaskData(task);
        const [data, loaded] = SyncMap_LoadOrStore(receiver.taskDataByPath, task.path, candidate);
        if (loaded) {
            putParseTaskData(candidate);
        }
        const capturedI = i;
 ...
 * }
 */
export function filesParser_getProcessedFiles(receiver, loader) {
    const totalFileCount = loader.totalFileCount.Load();
    const libFileCount = loader.libFileCount.Load();
    let missingFiles = [];
    let duplicateSourceFiles = [];
    const files = [];
    const libFiles = [];
    const filesByPath = new globalThis.Map();
    let tasksSeenByNameIgnoreCase = undefined;
    if (loader.comparePathsOptions.UseCaseSensitiveFileNames) {
        tasksSeenByNameIgnoreCase = new globalThis.Map();
    }
    const inclProcessor = {
        fileIncludeReasons: new globalThis.Map(),
        processingDiagnostics: [],
        reasonToReferenceLocation: { __tsgoBlank0: [], __tsgoBlank1: [], m: new sync_Map() },
        includeReasonToRelatedInfo: { __tsgoBlank0: [], __tsgoBlank1: [], m: new sync_Map() },
        redirectAndFileFormat: { __tsgoBlank0: [], __tsgoBlank1: [], m: new sync_Map() },
    ...
    let outputFileToProjectReferenceSource = undefined;
    if (!(ProgramOptions_canUseProjectReferenceSource(loader.opts))) {
        outputFileToProjectReferenceSource = new globalThis.Map();
    }
    const resolvedModules = new globalThis.Map();
    const typeResolutionsInFile = new globalThis.Map();
    const sourceFileMetaDatas = new globalThis.Map();
    let jsxRuntimeImportSpecifiers = undefined;
    let importHelpersImportSpecifiers = undefined;
    const sourceFilesFoundSearchingNodeModules = { M: new globalThis.Map() };
    const libFilesMap = new globalThis.Map();
    let redirectTargetsMap = undefined;
    let redirectFilesByPath = undefined;
    let packageIdToSourceFile = undefined;
    if (!Tristate_IsFalse(ParsedCommandLine_CompilerOptions(loader.opts.Config).DeduplicatePackages)) {
        redirectTargetsMap = new globalThis.Map();
        packageIdToSourceFile = new globalThis.Map();
    }
    const collectFiles = (tasks, seen) => {
        for (let task of tasks) {
            ...
            const [data] = SyncMap_Load(receiver.taskDataByPath, task.path);
            if (!task.loaded) {
                continue;
            }
            // ensure we only walk each task once
            ...
            if (tasksSeenByNameIgnoreCase !== undefined) {
                const pathLowerCase = ToFileNameLowerCase(task.path);
            ...
            if (packageIdToSourceFile !== undefined && dataPackageId.Name !== "") {
                ...
                if (packageIdFile !== undefined) {
                    ...
                    if (redirectFilesByPath === undefined) {
                        redirectFilesByPath = new globalThis.Map();
                    }
            ...
            if (task.jsxRuntimeImportSpecifier !== undefined) {
                if (jsxRuntimeImportSpecifiers === undefined) {
                    jsxRuntimeImportSpecifiers = new globalThis.Map();
                }
            ...
            if (task.importHelpersImportSpecifier !== undefined) {
                if (importHelpersImportSpecifiers === undefined) {
                    importHelpersImportSpecifiers = new globalThis.Map();
                }
    ...
    collectFiles(loader.rootTasks, new globalThis.Map());
    fileLoader_sortLibs(loader, libFiles);
    const allFiles = [...libFiles, ...files];
    if (redirectFilesByPath !== undefined) {
        const redirectFilesByPathDefined = redirectFilesByPath;
        for (const redirectFile of redirectFilesByPathDefined.values()) {
            redirectFile.index += libFiles.length;
        }
    }
    const keys = Collect(SyncMap_Keys(loader.pathForLibFileResolutions));
    Sort(keys);
    for (const key of keys) {
        const [value] = SyncMap_Load(loader.pathForLibFileResolutions, key);
        const modeAwareCache = NewGoStructMap();
        modeAwareCache.set({ Name: value.libraryName, Mode: ModuleKindCommonJS }, value.resolution);
        resolvedModules.set(key, modeAwareCache);
 ...
 * }
 */
export function filesParser_addIncludeReason(receiver, includeProcessor, task, reason) {
    if (task.redirectedParseTask !== undefined) {
...
//# sourceMappingURL=filesparser.js.map