 ...
 * }
 */
export function projectReferenceFileMapper_getParseFileRedirect(receiver, file) {
 ...
 * }
 */
export function projectReferenceFileMapper_getResolvedProjectReferences(receiver) {
 ...
 * }
 */
export function projectReferenceFileMapper_getProjectReferenceFromSource(receiver, path) {
 ...
 * }
 */
export function projectReferenceFileMapper_getProjectReferenceFromOutputDts(receiver, path) {
 ...
 * }
 */
export function projectReferenceFileMapper_isSourceFromProjectReference(receiver, path) {
 ...
 * }
 */
export function projectReferenceFileMapper_getCompilerOptionsForFile(receiver, file) {
    const redirect = projectReferenceFileMapper_getRedirectParsedCommandLineForResolution(receiver, file);
    const redirectedReference = redirect !== undefined
 ...
 * }
 */
export function projectReferenceFileMapper_getRedirectParsedCommandLineForResolution(receiver, file) {
 ...
 * }
 */
export function projectReferenceFileMapper_getRedirectForResolution(receiver, file) {
 ...
 * }
 */
export function projectReferenceFileMapper_getResolvedReferenceFor(receiver, path) {
 ...
 * }
 */
export function projectReferenceFileMapper_rangeResolvedProjectReference(receiver, f) {
    if (receiver.opts.Config.ConfigFile === undefined) {
        return false;
    }
    const seenRef = NewSetWithSizeHint(receiver.referencesInConfigFile?.size ?? 0);
 ...
 * }
 */
export function projectReferenceFileMapper_rangeResolvedReferenceWorker(receiver, references, f, parent, seenRef) {
    for (let index = 0; index < references.length; index++) {
        const path = references[index];
        if (!Set_AddIfAbsent(seenRef, path)) {
            continue;
        }
        const config = receiver.configToProjectReference?.get(path);
        if (!f(path, config, parent, index)) {
 ...
 * }
 */
export function projectReferenceFileMapper_rangeResolvedProjectReferenceInChildConfig(receiver, childConfig, f) {
 ...
 * }
 */
export function projectReferenceFileMapper_getSourceToDtsIfSymlink(receiver, file) {
    // If preserveSymlinks is true, module resolution wont jump the symlink
    // but the resolved real path may be the .d.ts from project reference
    // Note:: Currently we try the real path only if the
    // file is from node_modules to avoid having to run real path on all file paths
    const path = file.Path();
    const [realpathDtsToSource, ok] = SyncMap_Load(receiver.realpathDtsToSource, path);
    if (ok) {
        return realpathDtsToSource;
    }
    if (receiver.loader !== undefined && Tristate_IsTrue(ParsedCommandLine_CompilerOptions(receiver.opts.Config).PreserveSymlinks)) {
        ...
        if (!strings.Contains(fileName, "/node_modules/")) {
            SyncMap_Store(receiver.realpathDtsToSource, path, undefined);
        }
        else {
            ...
            if (realDeclarationPath === path) {
                ...
                if (realpathDtsToSourceResult !== undefined) {
                    SyncMap_Store(receiver.realpathDtsToSource, path, realpathDtsToSourceResult);
...
//# sourceMappingURL=projectreferencefilemapper.js.map