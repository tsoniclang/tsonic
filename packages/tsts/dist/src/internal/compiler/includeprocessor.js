...
export function updateFileIncludeProcessor(p) {
    ...
    p.__tsgoEmbedded0.includeProcessor = {
        ...
        reasonToReferenceLocation: { __tsgoBlank0: [], __tsgoBlank1: [], m: new SyncMapImpl() },
        includeReasonToRelatedInfo: { __tsgoBlank0: [], __tsgoBlank1: [], m: new SyncMapImpl() },
        redirectAndFileFormat: { __tsgoBlank0: [], __tsgoBlank1: [], m: new SyncMapImpl() },
 ...
 * }
 */
export function includeProcessor_getDiagnostics(receiver, p) {
    receiver.computedDiagnosticsOnce.Do(() => {
        const coll = {
            mu: new (class {
                Lock() { }
                Unlock() { }
 ...
 * }
 */
export function includeProcessor_addProcessingDiagnostic(receiver, ...d) {
 ...
 * }
 */
export function includeProcessor_addProcessingDiagnosticsForFileCasing(receiver, file, existingCasing, currentCasing, reason) {
    const reasons = receiver.fileIncludeReasons.get(file);
    if (!FileIncludeReason_isReferencedFile(reason) && ContainsFunc(reasons ?? [], (r) => FileIncludeReason_isReferencedFile(r))) {
 ...
 * }
 */
export function includeProcessor_getReferenceLocation(receiver, r, program) {
    const [existing, ok] = SyncMap_Load(receiver.reasonToReferenceLocation, r);
    if (ok) {
        return existing;
    }
    const [loc] = SyncMap_LoadOrStore(receiver.reasonToReferenceLocation, r, FileIncludeReason_getReferencedLocation(r, program));
    return loc;
}
/**
 ...
 * }
 */
export function includeProcessor_getCompilerOptionsObjectLiteralSyntax(receiver, program) {
    receiver.compilerOptionsSyntaxOnce.Do(() => {
 ...
 * }
 */
export function includeProcessor_getRelatedInfo(receiver, r, program) {
    const [existing, ok] = SyncMap_Load(receiver.includeReasonToRelatedInfo, r);
    if (ok) {
        return existing;
    }
    const [relatedInfo] = SyncMap_LoadOrStore(receiver.includeReasonToRelatedInfo, r, FileIncludeReason_toRelatedInfo(r, program));
    return relatedInfo;
}
/**
 ...
 * }
 */
export function includeProcessor_explainRedirectAndImpliedFormat(receiver, program, filePath, toFileName) {
    const [existing, ok] = SyncMap_Load(receiver.redirectAndFileFormat, filePath);
    if (ok) {
        return existing;
    }
    let file = undefined;
    ...
    if (source !== file.FileName()) {
        result = [...result, NewCompilerDiagnostic(File_is_output_of_project_reference_source_0, toFileName(source))];
    }
    if (redirectsFile !== undefined) {
        const targetFile = Program_GetSourceFileByPath(program, redirectsFile.target);
        result = [...result, NewCompilerDiagnostic(File_redirects_to_file_0, toFileName(SourceFile_FileName(targetFile)))];
    }
    if (sourceFile !== undefined && IsExternalOrCommonJSModule(sourceFile)) {
        ...
        if (impliedFormat === ModuleKindESNext) {
            if (metaData.PackageJsonType === "module") {
                result = [...result, NewCompilerDiagnostic(File_is_ECMAScript_module_because_0_has_field_type_with_value_module, toFileName(metaData.PackageJsonDirectory + "/package.json"))];
            }
        }
    ...
    const [stored] = SyncMap_LoadOrStore(receiver.redirectAndFileFormat, filePath, result);
    return stored;
}
