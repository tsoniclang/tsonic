 ...
 * )
 */
export const fileIncludeKindImport = 0;
export const fileIncludeKindReferenceFile = 1;
export const fileIncludeKindTypeReferenceDirective = 2;
export const fileIncludeKindLibReferenceDirective = 3;
export const fileIncludeKindRootFile = 4;
export const fileIncludeKindLibFile = 5;
export const fileIncludeKindAutomaticTypeDirectiveFile = 6;
/**
 ...
 * }
 */
export function referenceFileLocation_text(receiver) {
 ...
 * }
 */
export function referenceFileLocation_diagnosticAt(receiver, message, ...args) {
    if (receiver.node !== undefined) {
 ...
 * }
 */
export function FileIncludeReason_asIndex(receiver) {
 ...
 * }
 */
export function FileIncludeReason_asLibFileIndex(receiver) {
    if (typeof receiver.data === "number") {
        return [receiver.data, true];
    }
    return [0, false];
}
/**
 ...
 * }
 */
export function FileIncludeReason_isReferencedFile(receiver) {
 ...
 * }
 */
export function FileIncludeReason_asReferencedFileData(receiver) {
    return receiver.data;
}
/**
 ...
 * }
 */
export function FileIncludeReason_asAutomaticTypeDirectiveFileData(receiver) {
    return receiver.data;
}
/**
 ...
 * }
 */
export function FileIncludeReason_getReferencedLocation(receiver, program) {
 ...
 * }
 */
export function FileIncludeReason_toDiagnostic(receiver, program, relativeFileName) {
    if (relativeFileName) {
        receiver.relativeFileNameDiagOnce.Do(() => {
            receiver.relativeFileNameDiag = FileIncludeReason_computeDiagnostic(receiver, program, (fileName) => GetRelativePathFromDirectory(Program_GetCurrentDirectory(program), fileName, program.comparePathsOptions));
        });
        return receiver.relativeFileNameDiag;
    }
    else {
        receiver.diagOnce.Do(() => {
            receiver.diag = FileIncludeReason_computeDiagnostic(receiver, program, (fileName) => fileName);
        });
 ...
 * }
 */
export function FileIncludeReason_computeDiagnostic(receiver, program, toFileName) {
 ...
 * }
 */
export function FileIncludeReason_computeReferenceFileDiagnostic(receiver, program, toFileName) {
    ...
    switch (receiver.kind) {
        case fileIncludeKindImport:
            if (!referenceLocation.isSynthetic) {
                ...
                if (specifier !== undefined && specifier === referenceLocation.node) {
                    if (referenceLocation.packageId.Name !== "") {
 ...
 * }
 */
export function FileIncludeReason_toRelatedInfo(receiver, program) {
    if (FileIncludeReason_isReferencedFile(receiver)) {
    ...
    switch (receiver.kind) {
        ...
        case fileIncludeKindAutomaticTypeDirectiveFile: {
            if (!CompilerOptions_UsesWildcardTypes(Program_Options(program))) {
                const data = FileIncludeReason_asAutomaticTypeDirectiveFileData(receiver);
                const typesSyntax = GetOptionsSyntaxByArrayElementValue(includeProcessor_getCompilerOptionsObjectLiteralSyntax(program.__tsgoEmbedded0.includeProcessor, program), "types", data.typeReference);
        ...
        case fileIncludeKindLibFile: {
            const [index, ok] = FileIncludeReason_asLibFileIndex(receiver);
            if (ok) {
                const libSyntax = GetOptionsSyntaxByArrayElementValue(includeProcessor_getCompilerOptionsObjectLiteralSyntax(program.__tsgoEmbedded0.includeProcessor, program), "lib", Program_Options(program).Lib[index]);
                ...
                const target = ScriptTarget_String(CompilerOptions_GetEmitScriptTarget(Program_Options(program)));
                if (target !== "") {
                    const targetValueSyntax = ForEachPropertyAssignment(includeProcessor_getCompilerOptionsObjectLiteralSyntax(program.__tsgoEmbedded0.includeProcessor, program), "target", GetCallbackForFindingPropertyAssignmentByValue(target));
 ...
 * }
 */
export function FileIncludeReason_computeReferenceFileRelatedInfo(receiver, program) {
    const referenceLocation = includeProcessor_getReferenceLocation(program.__tsgoEmbedded0.includeProcessor, receiver, program);
    if (referenceLocation.isSynthetic) {
        return undefined;
    }
...
//# sourceMappingURL=fileInclude.js.map