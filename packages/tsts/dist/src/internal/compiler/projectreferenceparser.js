 ...
 * }
 */
export function projectReferenceParseTask_parse(receiver, projectReferenceParser) {
 ...
 * }
 */
export function createProjectReferenceParseTasks(projectReferences) {
    return projectReferences.map((configName) => ({
        configName,
        resolved: undefined,
        subTasks: [],
    }));
 ...
 * }
 */
export function projectReferenceParser_parse(receiver, tasks) {
 ...
 * }
 */
export function projectReferenceParser_start(receiver, tasks) {
    for (let i = 0; i < tasks.length; i++) {
        const task = tasks[i];
        const path = fileLoader_toPath(receiver.loader, task.configName);
        const [loadedTask, loaded] = SyncMap_LoadOrStore(receiver.tasksByFileName, path, task);
        if (loaded) {
            // dedup tasks to ensure correct file order, regardless of which task would be started first
            tasks[i] = loadedTask;
        }
 ...
 * }
 */
export function projectReferenceParser_initMapper(receiver, tasks) {
    const totalReferences = SyncMap_Size(receiver.tasksByFileName) + 1;
    receiver.loader.projectReferenceFileMapper.configToProjectReference = new globalThis.Map();
    receiver.loader.projectReferenceFileMapper.referencesInConfigFile = new globalThis.Map();
    receiver.loader.projectReferenceFileMapper.sourceToProjectReference = new globalThis.Map();
    receiver.loader.projectReferenceFileMapper.outputDtsToProjectReference = new globalThis.Map();
    const seen = NewSetWithSizeHint(tasks.length);
    receiver.loader.projectReferenceFileMapper.referencesInConfigFile.set(SourceFile_Path(receiver.loader.opts.Config.ConfigFile.SourceFile), projectReferenceParser_initMapperWorker(receiver, tasks, seen));
 ...
 * }
 */
export function projectReferenceParser_initMapperWorker(receiver, tasks, seen) {
    if (tasks.length === 0) {
        return [];
    }
    const results = [];
    for (const task of tasks) {
        ...
        if (task.resolved !== undefined && receiver.loader.opts.Config.ConfigFile !== task.resolved.ConfigFile) {
            ...
            if (ProgramOptions_canUseProjectReferenceSource(receiver.loader.opts)) {
                ...
                if (declDir !== "") {
                    Set_AddIfAbsent(receiver.loader.dtsDirectories, fileLoader_toPath(receiver.loader, declDir));
                }
            }
        }
...
//# sourceMappingURL=projectreferenceparser.js.map