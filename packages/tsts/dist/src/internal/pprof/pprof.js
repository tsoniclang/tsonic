 ...
 * }
 */
export function BeginProfiling(profileDir, logWriter) {
 ...
 * }
 */
export function ProfileSession_Stop(receiver) {
    const p = receiver;
    pprof.StopCPUProfile();
    p.cpuFile.Close();
    if (p.memFilePath !== "") {
 ...
 * }
 */
export function CPUProfiler_StartCPUProfile(receiver, profileDir) {
    const c = receiver;
    c.mu.Lock();
    try {
 ...
 * }
 */
export function CPUProfiler_StopCPUProfile(receiver) {
    const c = receiver;
    c.mu.Lock();
    try {
        if (c.session === undefined) {
 ...
 * }
 */
export function SaveHeapProfile(profileDir) {
    const mkdirErr = os.MkdirAll(profileDir, 0o755);
    if (mkdirErr !== undefined) {
        return ["", fmt.Errorf("failed to create profile directory: %w", mkdirErr)];
    }
 ...
 * }
 */
export function SaveAllocProfile(profileDir) {
    const mkdirErr = os.MkdirAll(profileDir, 0o755);
    if (mkdirErr !== undefined) {
        return ["", fmt.Errorf("failed to create profile directory: %w", mkdirErr)];
    }
