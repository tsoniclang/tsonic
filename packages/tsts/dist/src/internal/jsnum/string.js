 ...
 * }
 */
export function Number_String(receiver) {
    switch (true) {
    ...
    if (MinSafeInteger <= receiver && receiver <= MaxSafeInteger) {
        const i = globalThis.Math.trunc(receiver);
        if (i === receiver) {
            return FormatInt(i, 10);
        }
    }
    // Otherwise, the Go json package handles this correctly.
    const [b] = Marshal(receiver);
 ...
 * }
 */
export function isStrWhiteSpace(r) {
    // This is different than stringutil.IsWhiteSpaceLike.
    // https://tc39.es/ecma262/2024/multipage/ecmascript-language-lexical-grammar.html#prod-LineTerminator
    // https://tc39.es/ecma262/2024/multipage/ecmascript-language-lexical-grammar.html#prod-WhiteSpace
    switch (r) {
 ...
 */
export let errUnknownPrefix = errors_New("unknown number prefix");
/**
 ...
 * }
 */
export function parseFloatString(s) {
    let hasDot;
    let hasExp = false;
    // <a>
    // <a>.<b>
 ...
 * }
 */
export function cutAny(s, cutset) {
 ...
 * }
 */
export function trimLeadingZeros(s) {
    if (HasPrefix(s, "0")) {
        s = TrimLeft(s, "0");
        if (s === "") {
            return "0";
 ...
 * }
 */
export function isAllBinaryDigits(s) {
    for (const r of rangeRunes(s)) {
 ...
 * }
 */
export function isAllOctalDigits(s) {
    for (const r of rangeRunes(s)) {
        if (!IsOctalDigit(r)) {
            return false;
        }
 ...
 * }
 */
export function isAllHexDigits(s) {
    for (const r of rangeRunes(s)) {
        if (!IsHexDigit(r)) {
            return false;
        }
 ...
 * }
 */
export function isNumberRune(r) {
    if (IsDigit(r)) {
        return true;
    }
