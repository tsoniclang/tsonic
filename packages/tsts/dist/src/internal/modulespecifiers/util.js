 ...
 * )
 */
export const regexPatternCacheMu = new RWMutex();
export const regexPatternCache = NewGoStructMap();
/**
 ...
 * }
 */
export function comparePathsByRedirect(a, b, useCaseSensitiveFileNames) {
    if (a.IsRedirect === b.IsRedirect) {
 ...
 * }
 */
export function IsExcludedByRegex(moduleSpecifier, excludes) {
...
const _regexPatternCacheInternal = new Map();
export function stringToRegex(pattern) {
 ...
 * }
 */
export function GetJSExtensionForDeclarationFileExtension(ext) {
    switch (ext) {
        case ExtensionDts:
            return ExtensionJs;
        case ExtensionDmts:
 ...
 * }
 */
export function getJSExtensionForFile(fileName, options) {
 ...
 * }
 */
export function tryGetAnyFileFromPath(host, path) {
    // !!! TODO: shouldn't this use readdir instead of fileexists for perf?
    // We check all js, `node` and `json` extensions in addition to TS, since node module resolution would also choose those over the directory
    const extGroups = GetSupportedExtensions({ AllowJs: TSTrue }, [
        {
            Extension: "node",
            IsMixedContent: false,
 ...
 * }
 */
export function getPathsRelativeToRootDirs(path, rootDirs, useCaseSensitiveFileNames) {
    const results = [];
 ...
 * }
 */
export function isPathRelativeToParent(path) {
    return strings.HasPrefix(path, "..");
}
/**
 ...
 * }
 */
export function getRelativePathIfInSameVolume(path, directoryPath, useCaseSensitiveFileNames) {
 ...
 * }
 */
export function packageJsonPathsAreEqual(a, b, options) {
    if (a === b) {
        return true;
    }
 ...
 * }
 */
export function prefersTsExtension(allowedEndings) {
 ...
 * }
 */
export function replaceFirstStar(s, replacement) {
    return strings.Replace(s, "*", replacement, 1);
}
/**
 ...
 * )
 */
export const nodeModulesPathParseStateBeforeNodeModules = 0;
export const nodeModulesPathParseStateNodeModules = 1;
export const nodeModulesPathParseStateScope = 2;
export const nodeModulesPathParseStatePackageContent = 3;
/**
 ...
 * }
 */
export function GetNodeModulePathParts(fullPath) {
    // If fullPath can't be valid module file within node_modules, returns undefined.
    // Example of expected pattern: /base/path/node_modules/[@scope/otherpackage/@otherscope/node_modules/]package/[subdirectory/]file.js
    // Returns indices:                       ^            ^                                                      ^             ^
    ...
    let partStart = 0;
    let partEnd = 0;
    let state = nodeModulesPathParseStateBeforeNodeModules;
    for (; partEnd >= 0;) {
        partStart = partEnd;
 ...
 * }
 */
export function GetPackageNameFromDirectory(fileOrDirectoryPath) {
 ...
 * }
 */
export function ProcessEntrypointEnding(entrypoint, prefs, host, options, importingSourceFile, allowedEndings) {
    let specifier = entrypoint.ModuleSpecifier;
    if (entrypoint.Ending === EndingFixed) {
        return specifier;
    }
    if (allowedEndings.length === 0) {
        allowedEndings = GetAllowedEndingsInPreferredOrder(prefs, host, options, importingSourceFile, "", host.GetDefaultResolutionModeForFile(importingSourceFile));
    }
