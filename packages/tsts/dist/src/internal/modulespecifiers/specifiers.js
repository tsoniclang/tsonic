...
import { CompilerOptions_GetModuleResolutionKind, CompilerOptions_GetPathsBasePath, CompilerOptions_GetResolvePackageJsonExports, CompilerOptions_GetResolvePackageJsonImports, ModuleResolutionKindBundler, ModuleResolutionKindNodeNext, ResolutionModeCommonJS, ResolutionModeESM, ResolutionModeNone, } from "../core/compileroptions.js";
...
import { ChangeExtension, ChangeFullExtension, ExtensionCjs, ExtensionCts, ExtensionDcts, ExtensionDmts, ExtensionDts, ExtensionJson, ExtensionMjs, ExtensionMts, ExtensionTs, ExtensionsNotSupportingExtensionlessResolution, FileExtensionIsOneOf, GetDeclarationFileExtension, HasImplementationTSFileExtension, HasTSFileExtension, IsDeclarationFileName, RemoveExtension, RemoveFileExtension, TryGetExtensionFromPath, } from "../tspath/extension.js";
import { CombinePaths, ComparePaths, ContainsPath, EnsureTrailingDirectorySeparator, ForEachAncestorDirectoryStoppingAtGlobalCache, GetBaseFileName, GetDirectoryPath, GetNormalizedAbsolutePath, GetRelativePathFromDirectory, NormalizePath, PathIsRelative, RemoveTrailingDirectorySeparator, ResolvePath, StartsWithDirectory, ToPath, } from "../tspath/path.js";
import * as tspath from "../tspath/path.js";
import { getModuleSpecifierPreferences } from "./preferences.js";
import { MatchingModeDirectory, MatchingModeExact, MatchingModePattern, ModuleSpecifierEndingIndex, ModuleSpecifierEndingJsExtension, ModuleSpecifierEndingMinimal, ModuleSpecifierEndingTsExtension, RelativePreferenceNonRelative, RelativePreferenceRelative, RelativePreferenceExternalNonRelative, ResultKindAmbient, ResultKindNodeModules, ResultKindNone, ResultKindPaths, ResultKindRedirect, ResultKindRelative, } from "./types.js";
import { CountPathComponents } from "./compare.js";
import { GetNodeModulePathParts, IsExcludedByRegex, PathIsBareSpecifier, ensurePathIsNonModuleName, getJSExtensionForFile, getPathsRelativeToRootDirs, getRelativePathIfInSameVolume, isPathRelativeToParent, packageJsonPathsAreEqual, prefersTsExtension, replaceFirstStar, tryGetAnyFileFromPath, TryGetRealFileNameForNonJSDeclarationFileName, GetJSExtensionForDeclarationFileExtension, comparePathsByRedirect, } from "./util.js";
/**
 ...
 * }
 */
export function GetModuleSpecifiers(moduleSymbol, checker, compilerOptions, importingSourceFile, host, userPreferences, options, forAutoImports) {
 ...
 * }
 */
export function GetModuleSpecifiersWithInfo(moduleSymbol, checker, compilerOptions, importingSourceFile, host, userPreferences, options, forAutoImports) {
 ...
 * }
 */
export function GetModuleSpecifiersForFileWithInfo(importingSourceFile, moduleFileName, compilerOptions, host, userPreferences, options, forAutoImports) {
    const modulePaths = getAllModulePathsWorker(getInfo(host.GetSourceOfProjectReferenceIfOutputIncluded(importingSourceFile), host), moduleFileName, host, compilerOptions, options);
 ...
 * }
 */
export function tryGetModuleNameFromAmbientModule(moduleSymbol, checker) {
 ...
 * }
 */
export function getInfo(importingSourceFileName, host) {
 ...
 * }
 */
export function getAllModulePaths(info, importedFileName, host, compilerOptions, preferences, options) {
 ...
 * }
 */
export function getAllModulePathsWorker(info, importedFileName, host, compilerOptions, options) {
    ...
    const comparePaths_ = (a, b) => comparePathsByRedirect(a, b, useCaseSensitiveFileNames);
    // Sort by paths closest to importing file Name directory
    const sortedPaths = [];
    ...
    while (allFileNames.size !== 0) {
        ...
        if (pathsInDirectory.length > 0) {
            ...
            for (const p of pathsInDirectory)
                sortedPaths.push(p);
        }
 ...
 * }
 */
export function containsIgnoredPath(s) {
    return strings.Contains(s, "/node_modules/.") ||
        strings.Contains(s, "/.git") ||
        strings.Contains(s, ".#");
}
 ...
 * }
 */
export function ContainsNodeModules(s) {
    return strings.Contains(s, "/node_modules/");
}
/**
 ...
 * }
 */
export function GetEachFileNameOfModule(importingFileName, importedFileName, host, preferSymlinks) {
    ...
    if (symlinkCache !== undefined) {
        ForEachAncestorDirectoryStoppingAtGlobalCache(host.GetGlobalTypingsCacheLocation(), GetDirectoryPath(fullImportedFileName), (realPathDirectory) => {
            const [symlinkSet, ok] = SyncMap_Load(KnownSymlinks_DirectoriesByRealpath(symlinkCache), EnsureTrailingDirectorySeparator(ToPath(realPathDirectory, cwd, host.UseCaseSensitiveFileNames())));
            if (!ok) {
            ...
            for (const target of targets) {
                ...
                SyncSet_Range(symlinkSet, (symlinkDirectory) => {
 ...
 * }
 */
export function computeModuleSpecifiers(modulePaths, compilerOptions, importingSourceFile, host, userPreferences, options, forAutoImport) {
    ...
    //   4. Relative paths
    const pathsSpecifiers = [];
    const redirectPathsSpecifiers = [];
    const nodeModulesSpecifiers = [];
    ...
    for (const modulePath of modulePaths) {
        ...
        const local = getLocalModuleSpecifier(modulePath.FileName, info, compilerOptions, host, importMode, preferences, 
 ...
 * }
 */
export function getLocalModuleSpecifier(moduleFileName, info, compilerOptions, host, importMode, preferences, pathsOnly) {
 ...
 * }
 */
export function processEnding(fileName, allowedEndings, options, host) {
 ...
 * }
 */
export function tryGetModuleNameFromRootDirs(rootDirs, moduleFileName, sourceDirectory, allowedEndings, compilerOptions, host) {
 ...
 * }
 */
export function tryGetModuleNameAsNodeModule(pathObj, info, importingSourceFile, host, options, userPreferences, packageNameOnly, overrideMode) {
 ...
 * }
 */
export function tryDirectoryWithPackageJson(parts, pathObj, importingSourceFile, host, overrideMode, options, allowedEndings) {
    ...
    if (CompilerOptions_GetResolvePackageJsonExports(options)) {
        ...
        if (packageJsonContent !== undefined && packageJsonPathFields.Exports.__tsgoEmbedded0.Type !== JSONValueTypeNotPresent) {
            const fromExports = tryGetModuleNameFromExports(options, host, pathObj.FileName, packageRootPath, packageName, packageJsonPathFields.Exports, conditions);
    ...
    if (packageJsonContent !== undefined && packageJsonPathFields.TypesVersions.Type === JSONValueTypeObject) {
        ...
        if (paths !== undefined) {
            ...
            const fromPaths = tryGetModuleNameFromPaths(subModuleName, paths, allowedEndings, packageRootPath, host, options);
    ...
    if (mainFileRelative.length > 0) {
        ...
        if (!blockedByTypesVersions) {
            ...
            if (ComparePaths(RemoveFileExtension(mainExportFile), RemoveFileExtension(moduleFileToTry), compareOpt) === 0) {
                ...
                stringutilHasPrefix(moduleFileToTry, mainExportFile, host.UseCaseSensitiveFileNames()) &&
 ...
 * }
 */
export function tryGetModuleNameFromExports(options, host, targetFilePath, packageDirectory, packageName, exports, conditions) {
    if (ExportsOrImports_IsSubpaths(exports)) {
        const obj = ExportsOrImports_AsObject(exports);
        let result = "";
        OrderedMap_Entries(obj)((k, subk) => {
    ...
    return tryGetModuleNameFromExportsOrImports(options, host, targetFilePath, packageDirectory, packageName, exports, conditions, MatchingModeExact, false, false);
}
/**
 ...
 * }
 */
export function tryGetModuleNameFromPackageJsonImports(moduleFileName, sourceDirectory, options, host, importMode, preferTsExtension) {
    ...
    switch (imports.__tsgoEmbedded0.Type) {
        ...
        case JSONValueTypeObject: {
            ...
            let result = "";
            OrderedMap_Entries(top)((k, value) => {
                if (k === "#" || k === "#/" || !strings.HasPrefix(k, "#")) {
                    return true; // continue
                }
                ...
                const mode = strings.HasSuffix(k, "/") ? MatchingModeDirectory : strings.Contains(k, "*") ? MatchingModePattern : MatchingModeExact;
                result = tryGetModuleNameFromExportsOrImports(options, host, moduleFileName, ancestorDirectoryWithPackageJson, k, value, conditions, mode, true, preferTsExtension);
                if (result.length > 0) {
                    return false; // break
                }
                return true; // continue
 ...
 * }
 */
export function tryGetModuleNameFromPaths(relativeToBaseUrl, paths, allowedEndings, baseDirectory, host, compilerOptions) {
    const caseSensitive = host.UseCaseSensitiveFileNames();
    let finalResult = "";
    OrderedMap_Entries(paths)((key, values) => {
        for (const patternText of values) {
            ...
            for (const ending of allowedEndings) {
                const result = processEnding(relativeToBaseUrl, [ending], compilerOptions, host);
            ...
            else if (Some(candidates, (c) => c.ending !== ModuleSpecifierEndingMinimal && pattern === c.value) ||
                Some(candidates, (c) => c.ending === ModuleSpecifierEndingMinimal && pattern === c.value && validateEnding(c, relativeToBaseUrl, compilerOptions, host))) {
                finalResult = key;
                return false; // break outer
            }
        }
 ...
 * }
 */
export function validateEnding(c, relativeToBaseUrl, compilerOptions, host) {
 ...
 * }
 */
export function tryGetModuleNameFromExportsOrImports(options, host, targetFilePath, packageDirectory, packageName, exports, conditions, mode, isImports, preferTsExtension) {
    switch (exports.__tsgoEmbedded0.Type) {
        case JSONValueTypeNotPresent:
            return "";
        case JSONValueTypeString: {
            const strValue = exports.__tsgoEmbedded0.Value;
            let outputFile = "";
            let declarationFile = "";
            if (isImports) {
        ...
        case JSONValueTypeObject: {
            const obj = ExportsOrImports_AsObject(exports);
            let result = "";
            OrderedMap_Entries(obj)((key, value) => {
 ...
 * }
 */
export function GetModuleSpecifier(compilerOptions, host, importingSourceFile, importingSourceFileName, oldImportSpecifier, toFileName, options) {
    return getModuleSpecifierWithPreferences(compilerOptions, host, importingSourceFile, importingSourceFileName, oldImportSpecifier, toFileName, {}, options);
}
/**
 ...
 * }
 */
export function UpdateModuleSpecifier(compilerOptions, host, importingSourceFile, importingSourceFileName, oldImportSpecifier, toFileName, userPreferences, options) {
    return getModuleSpecifierWithPreferences(compilerOptions, host, importingSourceFile, importingSourceFileName, oldImportSpecifier, toFileName, userPreferences, options);
}
/**
 ...
 * }
 */
export function getModuleSpecifierWithPreferences(compilerOptions, host, importingSourceFile, importingSourceFileName, oldImportSpecifier, toFileName, userPreferences, options) {
