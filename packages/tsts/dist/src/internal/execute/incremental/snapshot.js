...
import { Diagnostic_Category, Diagnostic_Code, Diagnostic_File, Diagnostic_Len, Diagnostic_MessageArgs, Diagnostic_MessageChain, Diagnostic_MessageKey, Diagnostic_Pos, Diagnostic_RelatedInformation, NewDiagnosticFromSerialized, RepopulateModeMismatch, RepopulateModuleNotFound, } from "../../ast/diagnostic.js";
...
import { Program_GetSourceFileByPath, Program_GetSourceFiles, Program_IsSourceFileDefaultLibrary, } from "../../compiler/program.js";
import { CompilerOptions_GetEmitDeclarations, CompilerOptions_IsIncremental, } from "../../core/compileroptions.js";
...
import { CreateModeMismatchDetails, CreateModuleNotFoundChain, } from "../../checker/utilities.js";
import { EnsurePathIsNonModuleName, GetDirectoryPath, GetRelativePathFromDirectory, } from "../../tspath/path.js";
 ...
 */
export function FileInfo_Version(receiver) {
 ...
 */
export function FileInfo_Signature(receiver) {
 ...
 */
export function FileInfo_AffectsGlobalScope(receiver) {
 ...
 */
export function FileInfo_ImpliedNodeFormat(receiver) {
 ...
 * }
 */
export function ComputeHash(text, hashWithText) {
 ...
 * )
 */
export const FileEmitKindNone = 0;
export const FileEmitKindJs = (1 << 0);
export const FileEmitKindJsMap = (1 << 1);
export const FileEmitKindJsInlineMap = (1 << 2);
export const FileEmitKindDtsErrors = (1 << 3);
export const FileEmitKindDtsEmit = (1 << 4);
export const FileEmitKindDtsMap = (1 << 5);
export const FileEmitKindDts = (FileEmitKindDtsErrors | FileEmitKindDtsEmit);
export const FileEmitKindAllJs = (FileEmitKindJs | FileEmitKindJsMap | FileEmitKindJsInlineMap);
export const FileEmitKindAllDtsEmit = (FileEmitKindDtsEmit | FileEmitKindDtsMap);
export const FileEmitKindAllDts = (FileEmitKindDts | FileEmitKindDtsMap);
export const FileEmitKindAll = (FileEmitKindAllJs | FileEmitKindAllDts);
/**
 ...
 * }
 */
export function getPendingEmitKindWithOptions(options, oldOptions) {
 ...
 * }
 */
export function getPendingEmitKind(emitKind, oldEmitKind) {
    if (oldEmitKind === emitKind) {
        return FileEmitKindNone;
    }
    if (oldEmitKind === 0 || emitKind === 0) {
    ...
    // If there is diff in Js emit, pending emit is js emit flags
    if ((diff & FileEmitKindAllJs) !== 0) {
        result = (result | (emitKind & FileEmitKindAllJs));
    }
    // If dts errors pending, add dts errors flag
    if ((diff & FileEmitKindDtsErrors) !== 0) {
        result = (result | (emitKind & FileEmitKindAllDts));
    }
    // If there is diff in Dts emit, pending emit is dts emit flags
    if ((diff & FileEmitKindAllDtsEmit) !== 0) {
        result = (result | (emitKind & FileEmitKindAllDtsEmit));
    }
    return result;
}
/**
 ...
 * }
 */
export function emitSignature_getNewEmitSignature(receiver, oldOptions, newOptions) {
 ...
 * }
 */
export function buildInfoDiagnosticWithFileName_toDiagnostic(receiver, p, file) {
    let fileForDiagnostic = undefined;
    if (receiver.file !== "") {
    ...
    return NewDiagnosticFromSerialized(fileForDiagnostic, NewTextRange(receiver.pos, receiver.end), receiver.code, receiver.category, receiver.messageKey, receiver.messageArgs, messageChain, relatedInformation, receiver.reportsUnnecessary, receiver.reportsDeprecated, receiver.skippedOnNoEmit);
}
/**
 ...
 * }
 */
export function repopulateDiagnosticChain(b, p, file) {
 ...
 * }
 */
export function buildInfoDiagnosticWithFileName_toDiagnosticWithoutRepopulate(receiver, p, file) {
    const messageChain = [];
    for (const msg of receiver.messageChain) {
    ...
    return NewDiagnosticFromSerialized(file, NewTextRange(receiver.pos, receiver.end), receiver.code, receiver.category, receiver.messageKey, receiver.messageArgs, messageChain, relatedInformation, receiver.reportsUnnecessary, receiver.reportsDeprecated, receiver.skippedOnNoEmit);
}
/**
 ...
 * }
 */
export function repopulateModeMismatchChain(b, p, file) {
    if (file === undefined) {
        return buildInfoDiagnosticWithFileName_toDiagnosticWithoutRepopulate(b, p, file);
    }
    const details = CreateModeMismatchDetails(p, file);
    const nextChain = [];
    for (const msg of b.messageChain) {
        nextChain.push(buildInfoDiagnosticWithFileName_toDiagnostic(msg, p, file));
    }
    return NewDiagnosticFromSerialized(file, NewTextRange(b.pos, b.end), Message_Code(details.Message), Message_Category(details.Message), Message_Key(details.Message), StringifyArgs(details.Args), nextChain, [], false, false, false);
}
/**
 ...
 * }
 */
export function repopulateModuleNotFoundChain(b, p, file, info) {
    if (file === undefined) {
        return buildInfoDiagnosticWithFileName_toDiagnosticWithoutRepopulate(b, p, file);
    }
    ...
    const details = CreateModuleNotFoundChain(p, file, info.ModuleReference, info.Mode, packageName);
    const nextChain = [];
    for (const msg of b.messageChain) {
        nextChain.push(buildInfoDiagnosticWithFileName_toDiagnostic(msg, p, file));
    }
 ...
 * }
 */
export function DiagnosticsOrBuildInfoDiagnosticsWithFileName_getDiagnostics(receiver, p, file) {
    if (receiver.diagnostics !== undefined && receiver.diagnostics !== null) {
        return receiver.diagnostics;
    }
    // Convert and cache the diagnostics
    receiver.diagnostics = receiver.buildInfoDiagnostics.map((diag) => buildInfoDiagnosticWithFileName_toDiagnostic(diag, p, file));
    return receiver.diagnostics;
}
/**
 ...
 * }
 */
export function snapshot_addFileToChangeSet(receiver, filePath) {
    SyncSet_Add(receiver.changedFilesSet, filePath);
 ...
 * }
 */
export function snapshot_addFileToAffectedFilesPendingEmit(receiver, filePath, emitKind) {
    const [existingKind] = SyncMap_Load(receiver.affectedFilesPendingEmit, filePath);
    SyncMap_Store(receiver.affectedFilesPendingEmit, filePath, ((existingKind ?? 0) | emitKind));
    if ((emitKind & FileEmitKindDtsErrors) !== 0) {
 ...
 * }
 */
export function snapshot_getAllFilesExcludingDefaultLibraryFile(receiver, program, firstSourceFile) {
 ...
 * }
 */
export function getTextHandlingSourceMapForSignature(text, data) {
    if (data.SourceMapUrlPos !== -1) {
 ...
 * }
 */
export function snapshot_computeSignatureWithDiagnostics(receiver, file, text, data) {
 ...
 * }
 */
export function diagnosticToStringBuilder(diagnostic, file, builder) {
    if (diagnostic === undefined) {
        return;
    }
    builder.WriteString("\n");
    if (Diagnostic_File(diagnostic) !== file) {
        builder.WriteString(EnsurePathIsNonModuleName(GetRelativePathFromDirectory(GetDirectoryPath(SourceFile_Path(file)), Diagnostic_File(diagnostic) !== undefined ? SourceFile_Path(Diagnostic_File(diagnostic)) : "", { UseCaseSensitiveFileNames: false, CurrentDirectory: "" })));
    }
 ...
 * }
 */
export function snapshot_computeHash(receiver, text) {
 ...
 * }
 */
export function snapshot_canUseIncrementalState(receiver) {
