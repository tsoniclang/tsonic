...
import { Diagnostic_Code, Diagnostic_Category, Diagnostic_File, Diagnostic_MessageArgs, Diagnostic_MessageChain, Diagnostic_MessageKey, Diagnostic_Pos, Diagnostic_End, Diagnostic_RelatedInformation, Diagnostic_RepopulateInfo, Diagnostic_ReportsDeprecated, Diagnostic_ReportsUnnecessary, Diagnostic_SkippedOnNoEmit, } from "../../ast/diagnostic.js";
 ...
 * }
 */
export function snapshotToBuildInfo(snapshot, program, buildInfoFileName) {
    const buildInfo = {
        Version: Version(),
        Errors: false,
        CheckPending: false,
    ...
    const to = {
        ...
        fileNamesToFileIdListId: new Map(),
        roots: new Map(),
    };
 ...
 * }
 */
export function toBuildInfo_relativeToBuildInfo(receiver, path) {
 ...
 * }
 */
export function toBuildInfo_toFileId(receiver, path) {
    let fileId = receiver.fileNameToFileId.get(path);
    if (fileId === undefined || fileId === 0) {
        ...
        if (libFile !== undefined && !libFile.Replaced) {
            ...
            receiver.buildInfo.FileNames.push(toBuildInfo_relativeToBuildInfo(receiver, path));
        }
        fileId = receiver.buildInfo.FileNames.length;
        receiver.fileNameToFileId.set(path, fileId);
    }
    return fileId;
}
/**
 ...
 * }
 */
export function toBuildInfo_toFileIdListId(receiver, set_) {
    const fileIds = core.Map(slices.Collect(maps.Keys(Set_Keys(set_))), (id) => toBuildInfo_toFileId(receiver, id));
    slices.Sort(fileIds);
    const key = core.Map(fileIds, (id) => String(id)).join(",");
    ...
    if (fileIdListId === undefined || fileIdListId === 0) {
        ...
        fileIdListId = receiver.buildInfo.FileIdsList.length;
 ...
 * }
 */
export function toBuildInfo_toRelativeToBuildInfoCompilerOptionValue(receiver, option, v) {
    if (option.Kind === CommandLineOptionTypeList) {
        if (CommandLineOption_Elements(option).IsFilePath) {
            const arr = v;
            if (arr !== undefined) {
                return core.Map(arr, (s) => toBuildInfo_relativeToBuildInfo(receiver, s));
            }
        }
    }
    else if (option.IsFilePath) {
 ...
 * }
 */
export function toBuildInfo_toBuildInfoDiagnosticsFromFileNameDiagnostics(receiver, diagnostics) {
    return core.Map(diagnostics, (d) => {
        let file = 0;
        if (d.file !== "") {
 ...
 * }
 */
export function toBuildInfo_toBuildInfoDiagnosticsFromDiagnostics(receiver, filePath, diagnostics) {
    return core.Map(diagnostics, (d) => {
        let file = 0;
        let noFile = false;
 ...
 * }
 */
export function toBuildInfo_toBuildInfoDiagnosticsOfFile(receiver, filePath, diags) {
 ...
 * }
 */
export function toBuildInfo_collectRootFiles(receiver) {
 ...
 * }
 */
export function toBuildInfo_setFileInfoAndEmitSignatures(receiver) {
    receiver.buildInfo.FileInfos = core.Map(Program_GetSourceFiles(receiver.program), (file) => {
        const [info] = SyncMap_Load(receiver.snapshot.fileInfos, SourceFile_Path(file));
        const fileId = toBuildInfo_toFileId(receiver, SourceFile_Path(file));
        if (receiver.buildInfo.FileNames[fileId - 1] !== toBuildInfo_relativeToBuildInfo(receiver, SourceFile_Path(file))) {
            ...
            if (libFile === undefined || libFile.Replaced || receiver.buildInfo.FileNames[fileId - 1] !== libFile.Name) {
                throw new globalThis.Error(`File name at index ${fileId - 1} does not match expected relative path or libName: ${receiver.buildInfo.FileNames[fileId - 1]} != ${toBuildInfo_relativeToBuildInfo(receiver, SourceFile_Path(file))}`);
            }
        }
        if (Tristate_IsTrue(receiver.snapshot.options.Composite)) {
            if (!IsJsonSourceFile(file) && Program_SourceFileMayBeEmitted(receiver.program, file, false)) {
                const [emitSignature, loaded] = SyncMap_Load(receiver.snapshot.emitSignatures, SourceFile_Path(file));
                if (!loaded) {
                    receiver.buildInfo.EmitSignatures.push({ FileId: fileId, Signature: "", DiffersOnlyInDtsMap: false, DiffersInOptions: false });
                }
                else if (emitSignature.signature !== info.signature) {
                    const incrementalEmitSignature = { FileId: fileId, Signature: "", DiffersOnlyInDtsMap: false, DiffersInOptions: false };
                    if (emitSignature.signature !== "") {
                        incrementalEmitSignature.Signature = emitSignature.signature;
                    }
                    else if (emitSignature.signatureWithDifferentOptions[0] === info.signature) {
 ...
 * }
 */
export function toBuildInfo_setRootOfIncrementalProgram(receiver) {
    const keys = slices.Collect(maps.Keys(receiver.roots));
    slices.SortFunc(keys, (a, b) => {
        return toBuildInfo_toFileId(receiver, SourceFile_Path(a)) - toBuildInfo_toFileId(receiver, SourceFile_Path(b));
    });
 ...
 * }
 */
export function toBuildInfo_setCompilerOptions(receiver) {
    ForEachCompilerOptionValue(receiver.snapshot.options, (option) => option.AffectsBuildInfo, (option, value, _i) => {
        if (value.IsZero()) {
            return false;
        }
        if (receiver.buildInfo.Options === undefined) {
            receiver.buildInfo.Options = newMapWithSizeHint(0);
        }
        OrderedMap_Set(receiver.buildInfo.Options, option.Name, toBuildInfo_toRelativeToBuildInfoCompilerOptionValue(receiver, option, value.Interface()));
        return false;
    });
}
/**
 ...
 * }
 */
export function toBuildInfo_setReferencedMap(receiver) {
    const keys = referenceMap_getPathsWithReferences(receiver.snapshot.referencedMap);
    slices.Sort(keys);
    receiver.buildInfo.ReferencedMap = core.Map(keys, (filePath) => {
        const [references] = referenceMap_getReferences(receiver.snapshot.referencedMap, filePath);
        return {
            FileId: toBuildInfo_toFileId(receiver, filePath),
 ...
 * }
 */
export function toBuildInfo_setChangeFileSet(receiver) {
    const files = slices.Collect(SyncSet_Keys(receiver.snapshot.changedFilesSet));
    slices.Sort(files);
    receiver.buildInfo.ChangeFileSet = core.Map(files, (f) => toBuildInfo_toFileId(receiver, f));
}
/**
 ...
 * }
 */
export function toBuildInfo_setSemanticDiagnostics(receiver) {
    for (const file of Program_GetSourceFiles(receiver.program)) {
        const [value, ok] = SyncMap_Load(receiver.snapshot.semanticDiagnosticsPerFile, SourceFile_Path(file));
        if (!ok) {
            if (!SyncSet_Has(receiver.snapshot.changedFilesSet, SourceFile_Path(file))) {
 ...
 * }
 */
export function toBuildInfo_setEmitDiagnostics(receiver) {
    const files = slices.Collect(SyncMap_Keys(receiver.snapshot.emitDiagnosticsPerFile));
    slices.Sort(files);
    receiver.buildInfo.EmitDiagnosticsPerFile = core.Map(files, (filePath) => {
        const [value] = SyncMap_Load(receiver.snapshot.emitDiagnosticsPerFile, filePath);
 ...
 * }
 */
export function toBuildInfo_setAffectedFilesPendingEmit(receiver) {
    ...
    for (const filePath of files) {
        ...
        const [pendingEmit] = SyncMap_Load(receiver.snapshot.affectedFilesPendingEmit, filePath);
 ...
 * }
 */
export function toBuildInfo_setRootOfNonIncrementalProgram(receiver) {
    receiver.buildInfo.Root = core.Map(ParsedCommandLine_FileNames(Program_CommandLine(receiver.program)), (fileName) => {
        return {
            Start: 0,
            End: 0,
            NonIncremental: toBuildInfo_relativeToBuildInfo(receiver, ToPath(fileName, receiver.comparePathsOptions.CurrentDirectory, receiver.comparePathsOptions.UseCaseSensitiveFileNames)),
        };
    });
}
//# sourceMappingURL=snapshottobuildinfo.js.map