 ...
 * }
 */
export function buildInfoToSnapshot(buildInfo, config, host) {
    ...
    to.filePaths = core.Map(buildInfo.FileNames, (fileName) => {
    ...
    to.filePathSet = core.Map(buildInfo.FileIdsList, (fileIdList) => {
        const fileSet = NewSetWithSizeHint(fileIdList.length);
        for (const fileId of fileIdList) {
            fileSet.M.set(toSnapshot_toFilePath(to, fileId), {});
        }
        return fileSet;
    });
 ...
 * }
 */
export function toSnapshot_toAbsolutePath(receiver, path) {
 ...
 * }
 */
export function toSnapshot_toFilePath(receiver, fileId) {
 ...
 * }
 */
export function toSnapshot_toFilePathSet(receiver, fileIdListId) {
 ...
 * }
 */
export function toSnapshot_toBuildInfoDiagnosticsWithFileName(receiver, diagnostics) {
 ...
 * }
 */
export function toSnapshot_toDiagnosticsOrBuildInfoDiagnosticsWithFileName(receiver, dig) {
    return {
        diagnostics: [],
 ...
 * }
 */
export function toSnapshot_setCompilerOptions(receiver) {
 ...
 * }
 */
export function toSnapshot_setFileInfoAndEmitSignatures(receiver) {
    ...
    for (let index = 0; index < receiver.buildInfo.FileInfos.length; index++) {
        ...
        const path = toSnapshot_toFilePath(receiver, (index + 1));
        const info = BuildInfoFileInfo_GetFileInfo(buildInfoFileInfo);
        SyncMap_Store(receiver.snapshot.fileInfos, path, info);
        if (info.signature !== "" && isComposite) {
 ...
 * }
 */
export function toSnapshot_setReferencedMap(receiver) {
 ...
 * }
 */
export function toSnapshot_setChangeFileSet(receiver) {
    for (const fileId of receiver.buildInfo.ChangeFileSet) {
        const filePath = toSnapshot_toFilePath(receiver, fileId);
        SyncSet_Add(receiver.snapshot.changedFilesSet, filePath);
    }
}
/**
 ...
 * }
 */
export function toSnapshot_setSemanticDiagnostics(receiver) {
    SyncMap_Range(receiver.snapshot.fileInfos, (path, _info) => {
        // Initialize to have no diagnostics if its not changed file
        if (!SyncSet_Has(receiver.snapshot.changedFilesSet, path)) {
            SyncMap_Store(receiver.snapshot.semanticDiagnosticsPerFile, path, { diagnostics: [], buildInfoDiagnostics: [] });
        }
        return true;
    });
    for (const diagnostic of receiver.buildInfo.SemanticDiagnosticsPerFile) {
        if (diagnostic.FileId !== 0) {
            ...
            SyncMap_Delete(receiver.snapshot.semanticDiagnosticsPerFile, filePath); // does not have cached diagnostics
        }
        else {
            const filePath = toSnapshot_toFilePath(receiver, diagnostic.Diagnostics.FileId);
            SyncMap_Store(receiver.snapshot.semanticDiagnosticsPerFile, filePath, toSnapshot_toDiagnosticsOrBuildInfoDiagnosticsWithFileName(receiver, diagnostic.Diagnostics));
        }
    }
}
/**
 ...
 * }
 */
export function toSnapshot_setEmitDiagnostics(receiver) {
    for (const diagnostic of receiver.buildInfo.EmitDiagnosticsPerFile) {
        const filePath = toSnapshot_toFilePath(receiver, diagnostic.FileId);
        SyncMap_Store(receiver.snapshot.emitDiagnosticsPerFile, filePath, toSnapshot_toDiagnosticsOrBuildInfoDiagnosticsWithFileName(receiver, diagnostic));
    }
}
/**
 ...
 * }
 */
export function toSnapshot_setAffectedFilesPendingEmit(receiver) {
    ...
    for (const pendingEmit of receiver.buildInfo.AffectedFilesPendingEmit) {
        SyncMap_Store(receiver.snapshot.affectedFilesPendingEmit, toSnapshot_toFilePath(receiver, pendingEmit.FileId), core.IfElse(pendingEmit.EmitKind === 0, ownOptionsEmitKind, pendingEmit.EmitKind));
    }
}
//# sourceMappingURL=buildinfotosnapshot.js.map