...
import { Diagnostic_Category, Diagnostic_Clone, Diagnostic_Code, Diagnostic_End, Diagnostic_MessageArgs, Diagnostic_MessageChain, Diagnostic_MessageKey, Diagnostic_Pos, Diagnostic_RepopulateInfo, Diagnostic_SetMessageChain, } from "../../ast/diagnostic.js";
...
import { Program_GetCurrentDirectory, Program_GetParseFileRedirect, Program_GetResolvedTypeReferenceDirectives, Program_GetSourceFileByPath, Program_GetSourceFileMetaData, Program_GetSourceFiles, Program_GetTypeCheckerForFileExclusive, Program_IsSourceFileDefaultLibrary, Program_Options, Program_SingleThreaded, Program_UseCaseSensitiveFileNames, } from "../../compiler/program.js";
import { CompilerOptions_IsIncremental } from "../../core/compileroptions.js";
import { TSUnknown, Tristate_IsTrue } from "../../core/tristate.js";
import { CompilerOptionsAffectDeclarationPath, CompilerOptionsAffectEmit, CompilerOptionsAffectSemanticDiagnostics, } from "../../tsoptions/declscompiler.js";
import { GetDirectoryPath, ToPath } from "../../tspath/path.js";
import { BindSourceFile } from "../../binder/binder.js";
import { GetSourceFileOfNode, IsExternalOrCommonJSModule, IsGlobalScopeAugmentation, IsJsonSourceFile, IsModuleWithStringLiteralName, } from "../../ast/utilities.js";
import { IsStringLiteral } from "../../ast/generated/predicates.js";
import { emitSignature_getNewEmitSignature, GetFileEmitKind, getPendingEmitKindWithOptions, repopulateDiagnosticChain, snapshot_addFileToAffectedFilesPendingEmit, snapshot_addFileToChangeSet, snapshot_canUseIncrementalState, snapshot_computeHash, snapshot_getAllFilesExcludingDefaultLibraryFile, } from "./snapshot.js";
 ...
 * }
 */
export function programToSnapshot(program, oldProgram, hashWithText) {
 ...
 * }
 */
export function toProgramSnapshot_reuseFromOldProgram(receiver) {
    if (receiver.oldProgram !== undefined) {
        ...
        // Copy old snapshot's changed files set
        SyncSet_Range(receiver.oldProgram.snapshot.changedFilesSet, (key) => {
            SyncSet_Add(receiver.snapshot.changedFilesSet, key);
            return true;
        });
        SyncMap_Range(receiver.oldProgram.snapshot.affectedFilesPendingEmit, (key, emitKind) => {
            SyncMap_Store(receiver.snapshot.affectedFilesPendingEmit, key, emitKind);
            return true;
        });
 ...
 * }
 */
export function toProgramSnapshot_computeProgramFileChanges(receiver) {
    ...
    for (const file of files) {
        wg.Queue(() => {
            ...
            if (receiver.oldProgram !== undefined) {
                const [oldFileInfo, ok] = SyncMap_Load(receiver.oldProgram.snapshot.fileInfos, SourceFile_Path(file));
                if (ok) {
                    ...
                    if (oldFileInfo.version !== version || oldFileInfo.affectsGlobalScope !== affectsGlobalScope || oldFileInfo.impliedNodeFormat !== impliedNodeFormat) {
                        ...
                        if (!Set_Equals(newReferences, oldReferences)) {
                            ...
                            for (const [refPath] of Set_Keys(newReferences)) {
                                if (Program_GetSourceFileByPath(receiver.program, refPath) === undefined) {
                                    const [, hadOldInfo] = SyncMap_Load(receiver.oldProgram.snapshot.fileInfos, refPath);
                                    if (hadOldInfo) {
                ...
                if (!SyncSet_Has(receiver.snapshot.changedFilesSet, SourceFile_Path(file))) {
                    const [emitDiagnostics, hasEmitDiag] = SyncMap_Load(receiver.oldProgram.snapshot.emitDiagnosticsPerFile, SourceFile_Path(file));
                    if (hasEmitDiag) {
                    ...
                    if (canCopySemanticDiagnostics) {
                        if ((!file.IsDeclarationFile || copyDeclarationFileDiagnostics) &&
                            ...
                            const [diagnostics, hasDiag] = SyncMap_Load(receiver.oldProgram.snapshot.semanticDiagnosticsPerFile, SourceFile_Path(file));
                            if (hasDiag) {
                ...
                if (canCopyEmitSignatures) {
                    const [oldEmitSignature, hasOldEmit] = SyncMap_Load(receiver.oldProgram.snapshot.emitSignatures, SourceFile_Path(file));
                    if (hasOldEmit) {
 ...
 * }
 */
export function toProgramSnapshot_handleFileDelete(receiver) {
    if (receiver.oldProgram !== undefined) {
        // If the global file is removed, add all files as changed
        SyncMap_Range(receiver.oldProgram.snapshot.fileInfos, (filePath, oldInfo) => {
            const [, ok] = SyncMap_Load(receiver.snapshot.fileInfos, filePath);
            if (!ok) {
                if (oldInfo.affectsGlobalScope) {
 ...
 * }
 */
export function toProgramSnapshot_handlePendingEmit(receiver) {
 ...
 * }
 */
export function toProgramSnapshot_handlePendingCheck(receiver) {
    if (receiver.oldProgram !== undefined &&
        SyncMap_Size(receiver.snapshot.semanticDiagnosticsPerFile) !== Program_GetSourceFiles(receiver.program).length &&
 ...
 * }
 */
export function fileAffectsGlobalScope(file) {
    BindSourceFile(file);
    if (core.Some(file.ModuleAugmentations, (augmentation) => {
    ...
    return file.Statements !== undefined &&
        ...
        core.Some(file.Statements.Nodes, (stmt) => {
 ...
 * }
 */
export function addReferencedFilesFromSymbol(file, referencedFiles, symbol_) {
    ...
    for (const declaration of symbol_.Declarations ?? []) {
        ...
        if (file !== fileOfDecl) {
            Set_Add(referencedFiles, SourceFile_Path(fileOfDecl));
        }
    }
}
/**
 ...
 * }
 */
export function addReferencedFilesFromImportLiteral(file, referencedFiles, checker, importName) {
    const symbol = Checker_GetSymbolAtLocation(checker, importName);
    addReferencedFilesFromSymbol(file, referencedFiles, symbol);
}
/**
 ...
 * }
 */
export function addReferencedFileFromFileName(program, fileName, referencedFiles, sourceFileDirectory) {
    const redirect = Program_GetParseFileRedirect(program, fileName);
    if (redirect !== "") {
        Set_Add(referencedFiles, ToPath(redirect, Program_GetCurrentDirectory(program), Program_UseCaseSensitiveFileNames(program)));
    }
    else {
        Set_Add(referencedFiles, ToPath(fileName, sourceFileDirectory, Program_UseCaseSensitiveFileNames(program)));
    }
}
/**
 ...
 * }
 */
export function getReferencedFiles(program, file) {
    const referencedFiles = NewSetWithSizeHint(0);
    const [checker, done] = Program_GetTypeCheckerForFileExclusive(program, TODO(), file);
    try {
 ...
 * }
 */
export function repopulateDiagnosticsOfFile(diags, p, file) {
 ...
 * }
 */
export function repopulateDiagnosticsList(diags, p, file) {
    let changed = false;
 ...
 * }
 */
export function repopulateDiagnosticMessageChain(chain, p, file) {
    if (chain.length === 0) {
        return undefined;
    }
    let changed = false;
 ...
 * }
 */
export function astDiagToBuildInfoDiag(d) {
    const b = {
        file: "",
        noFile: false,
...
//# sourceMappingURL=programtosnapshot.js.map