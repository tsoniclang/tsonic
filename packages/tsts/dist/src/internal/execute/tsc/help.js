...
import { NewOrderedMapWithSizeHint, OrderedMap_Entries, OrderedMap_GetOrZero, OrderedMap_Set, OrderedMap_Size, OrderedMap_Values, } from "../../collections/ordered_map.js";
import { Set_Has } from "../../collections/set.js";
import { Version } from "../../core/version.js";
import { Message_Localize, } from "../../diagnostics/diagnostics.js";
import { X_tsc_Colon_The_TypeScript_Compiler, Version_0, COMMON_COMMANDS, COMMAND_LINE_FLAGS, COMMON_COMPILER_OPTIONS, ALL_COMPILER_OPTIONS, WATCH_OPTIONS, BUILD_OPTIONS, X_default_Colon, X_type_Colon, X_one_or_more_Colon, X_one_of_Colon, Command_line_Options, Compiles_the_current_project_tsconfig_json_in_the_working_directory, Ignoring_tsconfig_json_compiles_the_specified_files_with_default_compiler_options, Build_a_composite_project_in_the_working_directory, Creates_a_tsconfig_json_with_the_recommended_settings_in_the_working_directory, Compiles_the_TypeScript_project_located_at_the_specified_path, An_expanded_version_of_this_information_showing_all_possible_compiler_options, Compiles_the_current_project_with_additional_settings, You_can_learn_about_all_of_the_compiler_options_at_0, Including_watch_w_will_start_watching_the_current_project_for_the_file_changes_Once_set_you_can_config_watch_mode_with_Colon, Using_build_b_will_make_tsc_behave_more_like_a_build_orchestrator_than_a_compiler_This_is_used_to_trigger_building_composite_projects_which_you_can_learn_more_about_at_0, } from "../../diagnostics/generated/messages.js";
import { CommandLineOptionTypeBoolean, CommandLineOptionTypeEnum, CommandLineOptionTypeList, CommandLineOptionTypeListOrElement, CommandLineOptionTypeNumber, CommandLineOptionTypeObject, CommandLineOptionTypeString, CommandLineOption_DeprecatedKeys, CommandLineOption_Elements, CommandLineOption_EnumMap, } from "../../tsoptions/commandlineoption.js";
 ...
 * }
 */
export function PrintVersion(sys, locale) {
 ...
 * }
 */
export function PrintHelp(sys, locale, commandLine) {
 ...
 * }
 */
export function getOptionsForHelp(commandLine) {
    let opts = [...(Clone(OptionsDeclarations) ?? [])];
    opts = [...opts, TscBuildOption];
    const compilerOptions = ParsedCommandLine_CompilerOptions(commandLine);
    if (Tristate_IsTrue(compilerOptions.All)) {
        SortFunc(opts, (a, b) => strings.Compare(strings.ToLower(a.Name), strings.ToLower(b.Name)));
        return opts;
    }
    else {
        return Filter(opts, (opt) => opt.ShowInSimplifiedHelpView);
    }
}
/**
 ...
 * }
 */
export function getHeader(sys, message) {
    ...
    if (terminalWidth >= message.length + tsIconLength) {
        ...
        header.push(`${message.padEnd(leftAlign)}`, tsIconFirstLine, "\n", strings.Repeat(" ", leftAlign), tsIconSecondLine, "\n");
    }
    else {
        header.push(message, "\n", "\n");
    }
 ...
 * }
 */
export function printEasyHelp(sys, locale, simpleOptions) {
    const clrs = createColors(sys);
    const output = [];
    const example = (examples, desc) => {
 ...
 * }
 */
export function printAllHelp(sys, locale, options) {
    const output = [];
    const msg = Message_Localize(X_tsc_Colon_The_TypeScript_Compiler, locale) + " - " + Message_Localize(Version_0, locale, Version());
    output.push(...getHeader(sys, msg));
    // ALL COMPILER OPTIONS section
    ...
    const buildOptions = Filter(OptionsForBuild, (option) => option !== TscBuildOption);
 ...
 * }
 */
export function PrintBuildHelp(sys, locale, buildOptions) {
    const output = [];
    ...
    const options = Filter(buildOptions, (option) => option !== TscBuildOption);
 ...
 * }
 */
export function generateSectionOptionsOutput(sys, locale, sectionName, options, subCategory, beforeOptionsDescription, afterOptionsDescription) {
    const output = [];
 ...
 * }
 */
export function generateGroupOptionOutput(sys, locale, optionsList) {
    let maxLength = 0;
 ...
 * }
 */
export function generateOptionOutput(sys, locale, option, rightAlignOfLeft, leftAlignOfRight) {
    const text = [];
    const clrs = createColors(sys);
    // name and description
    ...
    if (option.DefaultValueDescription !== null && option.DefaultValueDescription !== undefined && typeof option.DefaultValueDescription === "object" && "code" in option.DefaultValueDescription) {
        // It's a Message object
        defaultValueDescription = Message_Localize(option.DefaultValueDescription, locale);
    }
    else {
        defaultValueDescription = formatDefaultValue(option.DefaultValueDescription, IfElse(option.Kind === CommandLineOptionTypeList || option.Kind === CommandLineOptionTypeListOrElement, CommandLineOption_Elements(option), option));
    }
    const terminalWidth = sys.GetWidthOfTerminal();
    if (terminalWidth >= 80) {
 ...
 * }
 */
export function formatDefaultValue(defaultValue, option) {
    ...
    if (option.Kind === CommandLineOptionTypeEnum) {
        ...
        OrderedMap_Entries(enumMap)((name, value) => {
            if (value === defaultValue) {
                names.push(name);
            }
            return true;
 ...
 * }
 */
export function showAdditionalInfoOutput(valueCandidates, option) {
 ...
 * }
 */
export function getValueCandidate(sys, locale, option) {
    if (option.Kind === CommandLineOptionTypeObject) {
        return undefined;
    }
    const res = { valueType: "", possibleValues: "" };
 ...
 * }
 */
export function getPossibleValues(option) {
    switch (option.Kind) {
        case CommandLineOptionTypeString:
        case CommandLineOptionTypeNumber:
        case CommandLineOptionTypeBoolean:
        ...
        default: {
            ...
            // Group synonyms: es6/es2015
            const enumMap = CommandLineOption_EnumMap(option);
            const inverted = NewOrderedMapWithSizeHint(OrderedMap_Size(enumMap));
            const deprecatedKeys = CommandLineOption_DeprecatedKeys(option);
            OrderedMap_Entries(enumMap)((name, value) => {
                if (deprecatedKeys === undefined || !Set_Has(deprecatedKeys, name)) {
                    OrderedMap_Set(inverted, value, GoAppend(OrderedMap_GetOrZero(inverted, value), name));
                }
                return true;
            });
            const syns = [];
            OrderedMap_Values(inverted)((synonyms) => {
 ...
 * }
 */
export function getPrettyOutput(colors, left, right, rightAlignOfLeft, leftAlignOfRight, terminalWidth, colorLeft) {
    const res = [];
 ...
 * }
 */
export function getDisplayNameTextOfOption(option) {
