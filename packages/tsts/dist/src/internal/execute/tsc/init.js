...
import { OrderedMap_Get, OrderedMap_GetOrZero, OrderedMap_Has, OrderedMap_Keys, } from "../../collections/ordered_map.js";
import { ModuleDetectionKindForce, ModuleKindNodeNext, ScriptTargetESNext, JsxEmitReactJSX, } from "../../core/compileroptions.js";
import { Message_Localize } from "../../diagnostics/diagnostics.js";
import { A_tsconfig_json_file_is_already_defined_at_Colon_0, Visit_https_Colon_Slash_Slashaka_ms_Slashtsconfig_to_read_more_about_this_file, File_Layout, Environment_Settings, See_also_https_Colon_Slash_Slashaka_ms_Slashtsconfig_Slashmodule, For_nodejs_Colon, X_and_npm_install_D_types_Slashnode, Other_Outputs, Stricter_Typechecking_Options, Style_Options, Recommended_Options, } from "../../diagnostics/generated/messages.js";
import { CommandLineOption_EnumMap, CommandLineOption_Elements, CommandLineOptionTypeEnum, } from "../../tsoptions/commandlineoption.js";
import { OptionsDeclarations } from "../../tsoptions/declscompiler.js";
import { CombinePaths, NormalizePath, } from "../../tspath/path.js";
 ...
 * }
 */
export function WriteConfigFile(sys, locale, reportDiagnostic, options) {
 ...
 * }
 */
export function generateTSConfig(options, locale) {
    const tab = "  ";
    const result = [];
    const allSetOptions = [];
    OrderedMap_Keys(options)((k) => {
        if (k !== "init" && k !== "help" && k !== "watch") {
            allSetOptions.push(k);
        }
        return true;
    });
    const emitHeader = (header) => {
    ...
    const formatSingleValue = (value, enumMap) => {
    ...
    const formatValueOrArray = (settingName, value) => {
    ...
    const emitOption = (setting, defaultValue, commented) => {
        if (commented > 2) {
