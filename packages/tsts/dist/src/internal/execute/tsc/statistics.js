...
import { Program_IdentifierCount, Program_InstantiationCount, Program_LineCount, Program_SourceFiles, Program_SymbolCount, Program_TypeCount, } from "../../compiler/program.js";
 ...
 * }
 */
export function table_add(receiver, name, value) {
    const t = receiver;
 ...
 * }
 */
export function table_print(receiver, w) {
    const t = receiver;
    let nameWidth = 0;
    let valueWidth = 0;
 ...
 * }
 */
export function identifierCount(p) {
    let count = 0;
 ...
 * }
 */
export function statisticsFromProgram(input, memStats) {
    const mem = memStats;
    return {
        isAggregate: false,
        Projects: 0,
        ...
        memoryUsed: mem?.Alloc ?? 0,
        memoryAllocs: mem?.Mallocs ?? 0,
        compileTimes: input.CompileTimes,
    };
}
/**
 ...
 * }
 */
export function Statistics_Report(receiver, w, testing) {
    const s = receiver;
    ...
    try {
        ...
        table_add(t, prefix + "Memory used", Sprintf("%vK", s.memoryUsed / 1024));
 ...
 * }
 */
export function Statistics_Aggregate(receiver, stat) {
    const s = receiver;
    s.isAggregate = true;
    ...
    s.types += stat.types;
    s.instantiations += stat.instantiations;
    s.memoryUsed = (s.memoryUsed + stat.memoryUsed);
    s.memoryAllocs = (s.memoryAllocs + stat.memoryAllocs);
    s.compileTimes.ConfigTime = (s.compileTimes.ConfigTime + stat.compileTimes.ConfigTime);
    s.compileTimes.BuildInfoReadTime = (s.compileTimes.BuildInfoReadTime + stat.compileTimes.BuildInfoReadTime);
    s.compileTimes.ParseTime = (s.compileTimes.ParseTime + stat.compileTimes.ParseTime);
    s.compileTimes.bindTime = (s.compileTimes.bindTime + stat.compileTimes.bindTime);
    s.compileTimes.checkTime = (s.compileTimes.checkTime + stat.compileTimes.checkTime);
    s.compileTimes.emitTime = (s.compileTimes.emitTime + stat.compileTimes.emitTime);
    s.compileTimes.ChangesComputeTime = (s.compileTimes.ChangesComputeTime + stat.compileTimes.ChangesComputeTime);
}
/**
 ...
 * }
 */
export function Statistics_SetTotalTime(receiver, totalTime) {
    const s = receiver;
    if (s.compileTimes === undefined) {
        s.compileTimes = { ConfigTime: 0, ParseTime: 0, bindTime: 0, checkTime: 0, totalTime: 0, emitTime: 0, BuildInfoReadTime: 0, ChangesComputeTime: 0 };
    }
