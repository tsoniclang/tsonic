...
import { ParsedCommandLine_Locale, ParsedCommandLine_CompilerOptions, ParsedCommandLine_ExtendedSourceFiles, ParsedCommandLine_FileNames, ParsedCommandLine_PossiblyMatchesDirectoryName, ParsedCommandLine_PossiblyMatchesFileName, ParsedCommandLine_WildcardDirectories, ParsedCommandLine_ReloadFileNamesOfParsedCommandLine, } from "../tsoptions/parsedcommandline.js";
import { ContainsPath, GetDirectoryPath, GetNormalizedAbsolutePath, GetPathComponents, IsVolumeCharacter, NormalizeSlashes, ToPath, } from "../tspath/path.js";
 ...
 * }
 */
export function fswatchBackend_WatchDirectory(receiver, dir, fn, recursive, ignore) {
    let opts = [];
    if (recursive) {
...
export function fswatchBackend_as_WatchBackend(receiver) {
    return {
        WatchDirectory: (dir, fn, recursive, ignore) => fswatchBackend_WatchDirectory(receiver, dir, fn, recursive, ignore),
    };
}
/**
 ...
 * }
 */
export function createWatcher(sys, configParseResult, compilerOptionsFromCommandLine, commandLineRaw, reportDiagnostic, reportErrorSummary, testing) {
    ...
    const w = {
        ...
        doCycleCh: {},
        debugLog: undefined,
        changedMu: { Lock: () => { }, Unlock: () => { }, TryLock: () => true },
...
function asCommandLineTestingWithWatchBackend(testing) {
 ...
 * }
 */
export function Watcher_start(receiver, ctx) {
 ...
 * }
 */
export function Watcher_computeDesiredWatches(receiver, seenFilePaths) {
    const cwd = receiver.sys.GetCurrentDirectory();
    const desiredDirs = new Map(); // dir → recursive
    // Wildcard directories from tsconfig (recursive or non-recursive)
 ...
 * }
 */
export function Watcher_reconcileWatches(receiver, seenFilePaths) {
    if (receiver.backend === undefined) {
        return;
    }
    const desiredDirs = Watcher_computeDesiredWatches(receiver, seenFilePaths);
    // Reconcile directory watches using DiffMaps, performing effects inline
    DiffMapsFunc(receiver.watchedDirs, desiredDirs, (wd, recursive) => (wd.recursive === recursive), (dir, recursive) => {
        if (receiver.debugLog !== undefined) {
 ...
 * }
 */
export function Watcher_comparePathsOptions(receiver) {
    return {
 ...
 * }
 */
export function Watcher_resolveDesiredDirs(receiver, desiredDirs) {
    ...
    for (const [dir, recursive] of desiredDirs) {
        ...
        if (resolved.has(watchDir)) {
            ...
            resolved.set(watchDir, (existing || watchRecursive));
        }
        else {
 ...
 * }
 */
export function canWatchDirectory(dir) {
 ...
 * }
 */
export function perceivedOsRootLengthForWatching(components) {
    const length = components.length;
    if (length <= 1) {
        return 1;
    }
 ...
 * }
 */
export function Watcher_createDirWatch(receiver, dir, recursive) {
    const entry = { closer: undefined, recursive };
    const cb = (events, err) => {
 ...
 * }
 */
export function Watcher_closeAllWatches(receiver) {
    // mu.Lock() / Unlock() omitted: TSTS is single-threaded
    let dirs = [];
 ...
 * }
 */
export function Watcher_handleWatchTerminated(receiver, dir, identity) {
    if (receiver.debugLog !== undefined) {
        Fprintf(receiver.debugLog, "[watch] watch terminated: %s\n", dir);
    }
    let staleCloser = undefined;
    // mu.Lock() / Unlock() omitted: TSTS is single-threaded
 ...
 * }
 */
export function Watcher_onWatchEvents(receiver, events, err) {
    if (err !== undefined) {
 ...
 * }
 */
export function Watcher_signalDoCycle(receiver) {
...
export function Watcher_DoCycle(receiver) {
    ...
    receiver.changedOverflow = false;
    ...
    if (hasEvents && !overflow && !receiver.configModified) {
        // Filter fswatch events against known dependencies
        if (Watcher_isRelevantChange(receiver, changedPaths ?? new Map())) {
            Watcher_evictChangedSourceFiles(receiver, changedPaths ?? new Map());
        }
        else {
            if (receiver.debugLog !== undefined) {
 ...
 * }
 */
export function Watcher_isRelevantChange(receiver, changedPaths) {
...
export function Watcher_doBuild(receiver) {
    ...
    receiver.seenFiles = NewSetWithSizeHint(seenSlice.length);
    for (const p of seenSlice) {
        Set_Add(receiver.seenFiles, ToPath(p, cwd, caseSensitive));
    }
    receiver.configMtimes = new Map();
    for (const cfgPath of receiver.configFilePaths) {
 ...
 * }
 */
export function Watcher_evictChangedSourceFiles(receiver, changedPaths) {
    const caseSensitive = receiver.sys.FS().UseCaseSensitiveFileNames();
    const cwd = receiver.sys.GetCurrentDirectory();
    for (const [eventPath] of changedPaths) {
        const p = ToPath(eventPath, cwd, caseSensitive);
        const [, ok] = SyncMap_Load(receiver.sourceFileCache, p);
        if (ok) {
            if (receiver.debugLog !== undefined) {
 ...
 * }
 */
export function Watcher_parseConfigFile(receiver) {
    const extendedConfigCache = { m: newSyncMap() };
    const [configParseResult, errors] = GetParsedCommandLineOfConfigFile(receiver.configFileName, receiver.compilerOptionsFromCommandLine, receiver.commandLineRaw, receiver.sys, ExtendedConfigCache_as_tsoptions_ExtendedConfigCache(extendedConfigCache));
    if (errors.length > 0) {
