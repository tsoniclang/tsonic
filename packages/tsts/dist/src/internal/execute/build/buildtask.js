...
import { upToDateStatus_isError, upToDateStatus_isPseudoBuild, upToDateStatus_inputOutputFileAndTime, upToDateStatus_inputOutputName, upToDateStatus_upstreamErrors, upToDateStatus_oldestOutputFileName, upToDateStatusTypeConfigFileNotFound, upToDateStatusTypeBuildErrors, upToDateStatusTypeUpstreamErrors, upToDateStatusTypeUpToDate, upToDateStatusTypeUpToDateWithUpstreamTypes, upToDateStatusTypeUpToDateWithInputFileText, upToDateStatusTypeInputFileMissing, upToDateStatusTypeOutputMissing, upToDateStatusTypeInputFileNewer, upToDateStatusTypeOutOfDateBuildInfoWithPendingEmit, upToDateStatusTypeOutOfDateBuildInfoWithErrors, upToDateStatusTypeOutOfDateOptions, upToDateStatusTypeOutOfDateRoots, upToDateStatusTypeTsVersionOutputOfDate, upToDateStatusTypeForceBuild, upToDateStatusTypeSolution, } from "./uptodatestatus.js";
/**
 ...
 * )
 */
export const updateKindNone = 0;
export const updateKindConfig = 1;
export const updateKindUpdate = 2;
/**
 ...
 * )
 */
export const buildKindNone = 0;
export const buildKindPseudo = 1;
export const buildKindProgram = 2;
/**
 ...
 * }
 */
export function BuildTask_waitOnUpstream(receiver) {
 ...
 * }
 */
export function BuildTask_unblockDownstream(receiver) {
    receiver.pending.Store(false);
 ...
 * }
 */
export function BuildTask_reportDiagnostic(receiver, err) {
 ...
 * }
 */
export function BuildTask_report(receiver, orchestrator, configPath, buildResult) {
 ...
 * }
 */
export function BuildTask_buildProject(receiver, orchestrator, path) {
 ...
 * }
 */
export function BuildTask_updateDownstream(receiver, orchestrator, path) {
    ...
    for (const downStream of receiver.downStream) {
        ...
        if (downStream.status !== undefined) {
            switch (downStream.status.kind) {
                case upToDateStatusTypeUpToDate:
                    if (!receiver.result.program.HasChangedDtsFile()) {
                ...
                case upToDateStatusTypeUpToDateWithInputFileText:
                    if (receiver.result.program.HasChangedDtsFile()) {
                        downStream.status = { kind: upToDateStatusTypeInputFileNewer, data: { input: receiver.config, output: upToDateStatus_oldestOutputFileName(downStream.status) } };
                    }
                    break;
                case upToDateStatusTypeUpstreamErrors: {
 ...
 * }
 */
export function BuildTask_compileAndEmit(receiver, orchestrator, path) {
    receiver.errors = [];
    ...
    if (!Tristate_IsTrue(orchestrator.opts.Command.BuildOptions.Force)) {
        oldProgram = ReadBuildInfoProgram(receiver.resolved, host_as_incremental_BuildInfoReader(orchestrator.host), host_as_compiler_CompilerHost(orchestrator.host));
    }
    compileTimes.BuildInfoReadTime = orchestrator.opts.Sys.Now().Sub(buildInfoReadStart);
    const parseStart = orchestrator.opts.Sys.Now();
    const trace = GetTraceWithWriterFromSys(receiver.result.builder, ParsedBuildCommandLine_Locale_fn(orchestrator.opts.Command), orchestrator.opts.Testing);
    const buildCompilerHost = compilerHost_as_compiler_CompilerHost({ host: orchestrator.host, trace });
    ...
    receiver.result.program = incremental_NewProgram(program, oldProgram, orchestrator.host, (orchestrator.opts.Testing !== undefined && orchestrator.opts.Testing !== null));
    compileTimes.ChangesComputeTime = orchestrator.opts.Sys.Now().Sub(changesComputeStart);
    ...
    const noEmitOnError = programOptions !== undefined && programOptions.NoEmitOnError?.IsTrue() === true;
 ...
 * }
 */
export function BuildTask_handleStatusThatDoesntRequireBuild(receiver, orchestrator) {
    switch (receiver.status.kind) {
        ...
        case upToDateStatusTypeUpstreamErrors: {
            ...
            if (Tristate_IsTrue(orchestrator.opts.Command.BuildOptions.Verbose)) {
                receiver.result.reportStatus(NewCompilerDiagnostic(IfElse(upstreamStatus.refHasUpstreamErrors, diagnostics.Skipping_build_of_project_0_because_its_dependency_1_was_not_built, diagnostics.Skipping_build_of_project_0_because_its_dependency_1_has_errors), Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, upstreamStatus.ref)));
            }
            return true;
        }
        case upToDateStatusTypeSolution:
 ...
 * }
 */
export function BuildTask_getUpToDateStatus(receiver, orchestrator, configPath) {
    ...
    for (const upstream of receiver.upStream) {
        if (Tristate_IsTrue(orchestrator.opts.Command.BuildOptions.StopBuildOnErrors) && upToDateStatus_isError(upstream.task.status)) {
            return { kind: upToDateStatusTypeUpstreamErrors, data: { ref: ParsedCommandLine_ProjectReferences(receiver.resolved)[upstream.refIndex].Path, refHasUpstreamErrors: upstream.task.status.kind === upToDateStatusTypeUpstreamErrors } };
        }
    }
    if (Tristate_IsTrue(orchestrator.opts.Command.BuildOptions.Force)) {
    ...
    let oldestOutputFileAndTime = { file: buildInfoPath, time: buildInfoTime };
    let newestInputFileAndTime = { file: "", time: new TimeClass() };
    const seenRoots = {};
    let buildInfoRootInfoReader = undefined;
    for (const inputFile of ParsedCommandLine_FileNames(receiver.resolved)) {
        ...
        if (inputTime.After(oldestOutputFileAndTime.time)) {
            ...
            if (BuildInfo_IsIncremental(buildInfo)) {
                ...
                const fileInfo = buildInfoFileInfo !== undefined ? buildInfoFileInfo.fileInfo !== undefined ? { version: () => buildInfoFileInfo.version ?? "" } : undefined : undefined;
                const fi = buildInfoFileInfo !== undefined ? buildInfoFileInfo : undefined;
                if (fi !== undefined) {
                    const fileInfoObj = fi.fileInfo;
                    if (fileInfoObj !== undefined && fileInfoObj.Version !== "") {
                        version = fileInfoObj.Version;
                        const fsObj = host_FS_fn(orchestrator.host);
                        const [text, ok] = fsObj.ReadFile(String(resolvedInputPath));
                        if (ok) {
                            currentVersion = ComputeHash(text, (orchestrator.opts.Testing !== undefined && orchestrator.opts.Testing !== null));
                            if (version === currentVersion) {
                                inputTextUnchanged = true;
                            }
                        }
            ...
            if (version === "" || version !== currentVersion) {
                return { kind: upToDateStatusTypeInputFileNewer, data: { input: inputFile, output: buildInfoPath } };
            }
        }
        if (inputTime.After(newestInputFileAndTime.time)) {
    ...
    for (const root of slicesCollect(BuildInfoRootInfoReader_Roots(buildInfoRootInfoReader))) {
        if (!Set_Has(seenRoots, root)) {
            return { kind: upToDateStatusTypeOutOfDateRoots, data: { input: String(root), output: buildInfoPath } };
        }
    }
    if (!CompilerOptions_IsIncremental(resolvedCompilerOptions)) {
        for (const outputFile of slicesCollect(ParsedCommandLine_GetOutputFileNames(receiver.resolved))) {
            ...
            if (outputTime.Before(newestInputFileAndTime.time)) {
                return { kind: upToDateStatusTypeInputFileNewer, data: { input: newestInputFileAndTime.file, output: outputFile } };
            }
            if (outputTime.Before(oldestOutputFileAndTime.time)) {
    ...
    for (const upstream of receiver.upStream) {
        ...
        if (BuildTask_hasConflictingBuildInfo(receiver, orchestrator, upstream.task)) {
            return { kind: upToDateStatusTypeInputFileNewer, data: { input: ParsedCommandLine_ProjectReferences(receiver.resolved)[upstream.refIndex].Path, output: oldestOutputFileAndTime.file } };
        }
    ...
    const checkInputFileTime = (inputFile) => {
        const inputTime = host_GetMTime(orchestrator.host, inputFile);
        if (inputTime.After(oldestOutputFileAndTime.time)) {
            return { kind: upToDateStatusTypeInputFileNewer, data: { input: inputFile, output: oldestOutputFileAndTime.file } };
        }
        return undefined;
    };
    ...
    return {
        kind: IfElse(refDtsUnchanged, upToDateStatusTypeUpToDateWithUpstreamTypes, IfElse(inputTextUnchanged, upToDateStatusTypeUpToDateWithInputFileText, upToDateStatusTypeUpToDate)),
        data: { input: newestInputFileAndTime, output: oldestOutputFileAndTime, buildInfo: buildInfoPath },
    };
}
/**
 ...
 * }
 */
export function BuildTask_reportUpToDateStatus(receiver, orchestrator) {
    if (!Tristate_IsTrue(orchestrator.opts.Command.BuildOptions.Verbose)) {
        return;
    }
    switch (receiver.status.kind) {
        case upToDateStatusTypeConfigFileNotFound:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_config_file_does_not_exist, Orchestrator_relativeFileName(orchestrator, receiver.config)));
            break;
        case upToDateStatusTypeUpstreamErrors: {
            const upstreamStatus = upToDateStatus_upstreamErrors(receiver.status);
            receiver.result.reportStatus(NewCompilerDiagnostic(IfElse(upstreamStatus.refHasUpstreamErrors, diagnostics.Project_0_can_t_be_built_because_its_dependency_1_was_not_built, diagnostics.Project_0_can_t_be_built_because_its_dependency_1_has_errors), Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, upstreamStatus.ref)));
            break;
        }
        case upToDateStatusTypeBuildErrors:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_it_has_errors, Orchestrator_relativeFileName(orchestrator, receiver.config)));
            break;
        case upToDateStatusTypeUpToDate: {
            const ioFileAndTime = upToDateStatus_inputOutputFileAndTime(receiver.status);
            if (ioFileAndTime !== undefined) {
                receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_up_to_date_because_newest_input_1_is_older_than_output_2, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, ioFileAndTime.input.file), Orchestrator_relativeFileName(orchestrator, ioFileAndTime.output.file)));
            }
            break;
        }
        case upToDateStatusTypeUpToDateWithUpstreamTypes:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_up_to_date_with_d_ts_files_from_its_dependencies, Orchestrator_relativeFileName(orchestrator, receiver.config)));
            break;
        case upToDateStatusTypeUpToDateWithInputFileText:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_up_to_date_but_needs_to_update_timestamps_of_output_files_that_are_older_than_input_files, Orchestrator_relativeFileName(orchestrator, receiver.config)));
            break;
        case upToDateStatusTypeInputFileMissing:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_input_1_does_not_exist, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data)));
            break;
        case upToDateStatusTypeOutputMissing:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_output_file_1_does_not_exist, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data)));
            break;
        case upToDateStatusTypeInputFileNewer: {
            const inputOutput = upToDateStatus_inputOutputName(receiver.status);
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_output_1_is_older_than_input_2, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, inputOutput.output), Orchestrator_relativeFileName(orchestrator, inputOutput.input)));
            break;
        }
        case upToDateStatusTypeOutOfDateBuildInfoWithPendingEmit:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_buildinfo_file_1_indicates_that_some_of_the_changes_were_not_emitted, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data)));
            break;
        case upToDateStatusTypeOutOfDateBuildInfoWithErrors:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_buildinfo_file_1_indicates_that_program_needs_to_report_errors, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data)));
            break;
        case upToDateStatusTypeOutOfDateOptions:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_buildinfo_file_1_indicates_there_is_change_in_compilerOptions, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data)));
            break;
        case upToDateStatusTypeOutOfDateRoots: {
            const inputOutput2 = upToDateStatus_inputOutputName(receiver.status);
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_buildinfo_file_1_indicates_that_file_2_was_root_file_of_compilation_but_not_any_more, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, inputOutput2.output), Orchestrator_relativeFileName(orchestrator, inputOutput2.input)));
            break;
        }
        case upToDateStatusTypeTsVersionOutputOfDate:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_out_of_date_because_output_for_it_was_generated_with_version_1_that_differs_with_current_version_2, Orchestrator_relativeFileName(orchestrator, receiver.config), Orchestrator_relativeFileName(orchestrator, receiver.status.data), core_Version()));
            break;
        case upToDateStatusTypeForceBuild:
            receiver.result.reportStatus(NewCompilerDiagnostic(diagnostics.Project_0_is_being_forcibly_rebuilt, Orchestrator_relativeFileName(orchestrator, receiver.config)));
            break;
        case upToDateStatusTypeSolution:
            // Does not need to report status
            break;
 ...
 * }
 */
export function BuildTask_canUpdateJsDtsOutputTimestamps(receiver) {
    return (!Tristate_IsTrue(ParsedCommandLine_CompilerOptions(receiver.resolved).NoEmit) && !CompilerOptions_IsIncremental(ParsedCommandLine_CompilerOptions(receiver.resolved)));
}
/**
 ...
 * }
 */
export function BuildTask_updateTimeStamps(receiver, orchestrator, emittedFiles, verboseMessage) {
    const emitted = NewSetFromItems(...(emittedFiles ?? []));
 ...
 * }
 */
export function BuildTask_cleanProject(receiver, orchestrator, path) {
    if (receiver.resolved === undefined) {
        BuildTask_reportDiagnostic(receiver, NewCompilerDiagnostic(diagnostics.File_0_not_found, receiver.config));
        receiver.result.exitStatus = ExitStatusDiagnosticsPresent_OutputsSkipped;
        return;
    }
    const inputs = NewSetFromItems(...core_Map(ParsedCommandLine_FileNames(receiver.resolved), (f) => Orchestrator_toPath(orchestrator, f)));
    for (const outputFile of slicesCollect(ParsedCommandLine_GetOutputFileNames(receiver.resolved))) {
 ...
 * }
 */
export function BuildTask_cleanProjectOutput(receiver, orchestrator, outputFile, inputs) {
 ...
 * }
 */
export function BuildTask_updateWatch(receiver, orchestrator, oldCache) {
    receiver.configTime = host_loadOrStoreMTime(orchestrator.host, receiver.config, oldCache, false);
    if (receiver.resolved !== undefined) {
        receiver.extendedConfigTimes = core_Map(ParsedCommandLine_ExtendedSourceFiles(receiver.resolved), (p) => {
            return host_loadOrStoreMTime(orchestrator.host, p, oldCache, false);
        });
        receiver.inputFiles = core_Map(ParsedCommandLine_FileNames(receiver.resolved), (p) => {
            return host_loadOrStoreMTime(orchestrator.host, p, oldCache, false);
        });
        if (BuildTask_canUpdateJsDtsOutputTimestamps(receiver)) {
            for (const outputFile of slicesCollect(ParsedCommandLine_GetOutputFileNames(receiver.resolved))) {
 ...
 * }
 */
export function BuildTask_resetStatus(receiver) {
    receiver.status = undefined;
    receiver.pending.Store(true);
    receiver.errors = [];
}
/**
 ...
 * }
 */
export function BuildTask_resetConfig(receiver, orchestrator, path) {
    receiver.dirty = true;
 ...
 * }
 */
export function BuildTask_hasUpdate(receiver, orchestrator, path) {
    let needsConfigUpdate = false;
    let needsUpdate = false;
    const configTime = host_GetMTime(orchestrator.host, receiver.config);
    if (configTime !== receiver.configTime && !configTime.equal?.(receiver.configTime)) {
    ...
    if (receiver.resolved !== undefined) {
        ...
        for (let idx = 0; idx < extendedFiles.length; idx++) {
            ...
            if (mtime !== receiver.extendedConfigTimes[idx] && !mtime.equal?.(receiver.extendedConfigTimes[idx])) {
                BuildTask_resetConfig(receiver, orchestrator, path);
                needsConfigUpdate = true;
            }
        }
        ...
        for (let idx = 0; idx < fileNames.length; idx++) {
            ...
            const mtime = host_GetMTime(orchestrator.host, file);
            if (mtime !== receiver.inputFiles[idx] && !mtime.equal?.(receiver.inputFiles[idx])) {
        ...
        if (!needsConfigUpdate) {
            ...
            const fs = host_FS_fn(orchestrator.host);
            const newConfig = receiver.resolved.ReloadFileNamesOfParsedCommandLine(fs);
 ...
 * }
 */
export function BuildTask_loadOrStoreBuildInfo(receiver, orchestrator, configPath, buildInfoFileName) {
    ...
    const reader = NewBuildInfoReader(orchestrator.host);
 ...
 * }
 */
export function BuildTask_onBuildInfoEmit(receiver, orchestrator, buildInfoFileName, buildInfo, hasChangedDtsFile) {
    receiver.buildInfoEntryMu.Lock();
    const mTime = orchestrator.opts.Sys.Now();
    let dtsTime = undefined;
    if (hasChangedDtsFile) {
        dtsTime = mTime;
    }
 ...
 * }
 */
export function BuildTask_hasConflictingBuildInfo(receiver, orchestrator, upstream) {
    if (receiver.buildInfoEntry !== undefined && upstream.buildInfoEntry !== undefined) {
        return (receiver.buildInfoEntry.path === upstream.buildInfoEntry.path);
    }
    return false;
}
/**
 ...
 * }
 */
export function BuildTask_getLatestChangedDtsMTime(receiver, orchestrator) {
    receiver.buildInfoEntryMu.Lock();
    ...
    const dtsTime = host_GetMTime(orchestrator.host, GetNormalizedAbsolutePath(receiver.buildInfoEntry.buildInfo.LatestChangedDtsFile, GetDirectoryPath(String(receiver.buildInfoEntry.path))));
 ...
 * }
 */
export function BuildTask_storeOutputTimeStamp(receiver, orchestrator) {
    return (Tristate_IsTrue(orchestrator.opts.Command.CompilerOptions.Watch) && !CompilerOptions_IsIncremental(ParsedCommandLine_CompilerOptions(receiver.resolved)));
}
/**
 ...
 * }
 */
export function BuildTask_writeFile(receiver, orchestrator, fileName, text, data) {
    const fs = host_FS_fn(orchestrator.host);
    const err = fs.WriteFile(fileName, text);
    if (err === undefined || err === null) {
        if (data !== undefined && data !== null && data.BuildInfo !== undefined) {
            const hasChangedDts = receiver.result.program.HasChangedDtsFile();
            BuildTask_onBuildInfoEmit(receiver, orchestrator, fileName, data.BuildInfo, hasChangedDts);
        }
        else if (BuildTask_storeOutputTimeStamp(receiver, orchestrator)) {
