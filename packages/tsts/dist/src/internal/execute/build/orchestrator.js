 ...
 * }
 */
export function orchestratorResult_report(receiver, o) {
    if (Tristate_IsTrue(o.opts.Command.CompilerOptions.Watch)) {
        o.watchStatusReporter(NewCompilerDiagnostic(IfElse(receiver.errors.length === 1, diagnostics.Found_1_error_Watching_for_file_changes, diagnostics.Found_0_errors_Watching_for_file_changes), receiver.errors.length));
    }
    else {
        o.errorSummaryReporter(receiver.errors);
    }
    if (receiver.filesToDelete !== undefined && receiver.filesToDelete !== null) {
        Orchestrator_createBuilderStatusReporter(o, undefined)(NewCompilerDiagnostic(diagnostics.A_non_dry_build_would_delete_the_following_files_Colon_0, strings.Join(core_Map(receiver.filesToDelete, (f) => "\r\n * " + f), "")));
    }
 ...
 */
export let __a05f111f_0 = Orchestrator_as_tsc_Watcher(undefined);
export function Orchestrator_as_tsc_Watcher(receiver) {
    return {
        DoCycle: () => Orchestrator_DoCycle(receiver),
    };
}
/**
 ...
 * }
 */
export function Orchestrator_relativeFileName(receiver, fileName) {
 ...
 * }
 */
export function Orchestrator_toPath(receiver, fileName) {
 ...
 * }
 */
export function Orchestrator_Order(receiver) {
 ...
 * }
 */
export function Orchestrator_Upstream(receiver, configName) {
 ...
 * }
 */
export function Orchestrator_Downstream(receiver, configName) {
    const path = Orchestrator_toPath(receiver, configName);
    const task = Orchestrator_getTask(receiver, path);
 ...
 * }
 */
export function Orchestrator_getTask(receiver, path) {
 ...
 * }
 */
export function Orchestrator_createBuildTasks(receiver, oldTasks, configs, wg) {
        ...
        wg.Queue(() => {
            ...
            if (task === undefined) {
                ...
                task = {
                    ...
                    buildInfoEntryMu: { Lock: () => { }, Unlock: () => { }, TryLock: () => true },
                    errors: [],
                    pending,
                    isInitialCycle: oldTasks === undefined,
                    downStreamUpdateMu: { Lock: () => { }, Unlock: () => { }, TryLock: () => true },
                    dirty: false,
                };
                task.pending.Store(true);
                task.buildInfoEntry = buildInfo;
 ...
 * }
 */
export function Orchestrator_setupBuildTask(receiver, configName, downStream, inCircularContext, completed, analyzing, circularityStack) {
    const path = Orchestrator_toPath(receiver, configName);
    const task = Orchestrator_getTask(receiver, path);
    if (!Set_Has(completed, path)) {
        if (Set_Has(analyzing, path)) {
            if (!inCircularContext) {
                receiver.errors.push(NewCompilerDiagnostic(diagnostics.Project_references_may_not_form_a_circular_graph_Cycle_detected_Colon_0, strings.Join(circularityStack, "\n")));
            }
            return undefined;
        }
        ...
        if (task.resolved !== undefined) {
            ...
            for (let index = 0; index < subRefs.length; index++) {
                ...
                const upstream = Orchestrator_setupBuildTask(receiver, subReference, task, (inCircularContext || projectRefs[index].Circular), completed, analyzing, circularityStack);
 ...
 * }
 */
export function Orchestrator_GenerateGraphReusingOldTasks(receiver) {
    const tasks = receiver.tasks;
    receiver.tasks = newSyncMap();
    receiver.order = [];
    receiver.errors = [];
 ...
 * }
 */
export function Orchestrator_GenerateGraph(receiver, oldTasks) {
    ...
    const completed = {};
    const analyzing = {};
    const circularityStack = [];
    for (const project of projects) {
        Orchestrator_setupBuildTask(receiver, project, undefined, false, completed, analyzing, circularityStack);
    }
}
/**
 ...
 * }
 */
export function Orchestrator_Start(receiver, ctx) {
 ...
 * }
 */
export function Orchestrator_Watch(receiver, ctx) {
    void ctx;
 ...
 * }
 */
export function Orchestrator_updateWatch(receiver) {
    const oldCache = receiver.host.mTimes;
    receiver.host.mTimes = newSyncMap();
    Orchestrator_rangeTask(receiver, (_path, task) => {
 ...
 * }
 */
export function Orchestrator_resetCaches(receiver) {
    const cachesVfs = receiver.host.host.FS();
 ...
 * }
 */
export function Orchestrator_buildOrClean(receiver) {
    if (!Tristate_IsTrue(receiver.opts.Command.BuildOptions.Clean) && Tristate_IsTrue(receiver.opts.Command.BuildOptions.Verbose)) {
        Orchestrator_createBuilderStatusReporter(receiver, undefined)(NewCompilerDiagnostic(diagnostics.Projects_in_this_build_Colon_0, strings.Join(core_Map(Orchestrator_Order(receiver), (p) => "\r\n    * " + Orchestrator_relativeFileName(receiver, p)), "")));
    }
    const buildResult = {
        result: { Status: 0, Watcher: undefined },
        errors: [],
        statistics: {
            Projects: 0,
            ProjectsBuilt: 0,
 ...
 * }
 */
export function Orchestrator_rangeTask(receiver, f) {
 ...
 * }
 */
export function Orchestrator_buildOrCleanProject(receiver, task, path, buildResult) {
    task.result = {
        builder: new Builder(),
 ...
 * }
 */
export function Orchestrator_getWriter(receiver, task) {
 ...
 * }
 */
export function Orchestrator_createBuilderStatusReporter(receiver, task) {
 ...
 * }
 */
export function Orchestrator_createDiagnosticReporter(receiver, task) {
...
function newParseCache() {
    return { entries: newSyncMap() };
}
export function NewOrchestrator(opts) {
    const orchestrator = {
        opts,
        comparePathsOptions: {
            CurrentDirectory: opts.Sys.GetCurrentDirectory(),
    ...
    const innerHost = {
        orchestrator,
        host: NewCachedFSCompilerHost(orchestrator.opts.Sys.GetCurrentDirectory(), orchestrator.opts.Sys.FS(), orchestrator.opts.Sys.DefaultLibraryPath(), undefined, undefined),
        extendedConfigCache,
