...
import { ParsedCommandLine_Locale, ParsedCommandLine_CompilerOptions, ParsedCommandLine_FileNames, } from "../tsoptions/parsedcommandline.js";
...
import { Options_0_and_1_cannot_be_combined, Option_project_cannot_be_mixed_with_source_files_on_a_command_line, Cannot_find_a_tsconfig_json_file_at_the_current_directory_Colon_0, The_specified_path_does_not_exist_Colon_0, X_tsconfig_json_is_present_but_will_not_be_loaded_if_files_are_specified_on_commandline_Use_ignoreConfig_to_skip_this_error, } from "../diagnostics/generated/messages.js";
import { ReadBuildInfoProgram, NewBuildInfoReader, } from "./incremental/incremental.js";
import { NewProgram as IncrementalNewProgram, Program_GetProgram, Program_as_compiler_ProgramLike as IncrementalProgram_as_compiler_ProgramLike, } from "./incremental/program.js";
...
import { ExitStatusSuccess, ExitStatusDiagnosticsPresent_OutputsSkipped, ExitStatusDiagnosticsPresent_OutputsGenerated, ExitStatusNotImplemented, } from "./tsc/compile.js";
 ...
 * }
 */
export function startTracingIfNeeded(sys, config, testing) {
 ...
 * }
 */
export function stopTracing(sys, tr) {
    if (tr === undefined) {
        return;
    }
 ...
 * }
 */
export function CommandLine(ctx, sys, commandLineArgs, testing) {
 ...
 * }
 */
export function fmtMain(sys, input, output) {
 ...
 * }
 */
export function tscBuildCompilation(ctx, sys, buildCommand, testing) {
 ...
 * }
 */
export function tscCompilation(ctx, sys, commandLine, testing) {
    let configFileName = "";
    ...
    try {
        if (Tristate_IsTrue(ParsedCommandLine_CompilerOptions(commandLine).Init)) {
            WriteConfigFile(sys, locale, reportDiagnostic, commandLine.Raw);
            return { Status: ExitStatusSuccess, Watcher: undefined };
        }
        ...
        const extendedConfigCache = { m: { __tsgoBlank0: [], __tsgoBlank1: [], m: new SyncGoMap() } };
        ...
        if (configFileName !== "") {
            ...
            if (raw !== undefined && raw !== null) {
                ...
                if (rawMap.keys !== undefined) {
                    // Wrap command line options in a "compilerOptions" key to match tsconfig.json structure
                    const wrapped = NewOrderedMapWithSizeHint(0);
        ...
        if (Tristate_IsTrue(ParsedCommandLine_CompilerOptions(configForCompilation).Watch)) {
            const watcher = createWatcher(sys, configForCompilation, compilerOptionsFromCommandLine, commandLineRaw, reportDiagnostic, reportErrorSummary, testing);
            ...
            return performIncrementalCompilation(sys, configForCompilation, reportDiagnostic, reportErrorSummary, extendedConfigCache, compileTimes, testing);
        }
 ...
 * }
 */
export function findConfigFile(searchPath, fileExists, configName) {
 ...
 * }
 */
export function getTraceFromSys(sys, locale, testing) {
 ...
 * }
 */
export function performIncrementalCompilation(sys, config, reportDiagnostic, reportErrorSummary, extendedConfigCache, compileTimes, testing) {
    ...
    const program = NewProgram({ Config: config, Host: host, Tracing: tr });
    compileTimes.ParseTime = sys.Now().Sub(parseStart);
 ...
 * }
 */
export function performCompilation(sys, config, reportDiagnostic, reportErrorSummary, extendedConfigCache, compileTimes, testing) {
    const host = NewCachedFSCompilerHost(sys.GetCurrentDirectory(), sys.FS(), sys.DefaultLibraryPath(), ExtendedConfigCache_as_tsoptions_ExtendedConfigCache(extendedConfigCache), getTraceFromSys(sys, ParsedCommandLine_Locale(config), testing));
    const tr = startTracingIfNeeded(sys, config, testing);
    const parseStart = sys.Now();
    const program = NewProgram({ Config: config, Host: host, Tracing: tr });
 ...
 * }
 */
export function showConfig(sys, config, configFileName) {
