 ...
 * }
 */
export function CommandLine(sys, commandLineArgs, testing) {
 ...
 * }
 */
export function tscBuildCompilation(sys, buildCommand, testing) {
 ...
 * }
 */
export function tscCompilation(sys, commandLine, testing) {
    let configFileName = "";
    ...
    try {
        ...
        if (Tristate_IsTrue(ParsedCommandLine_CompilerOptions(configForCompilation).Watch)) {
            const watcher = createWatcher(sys, configForCompilation, compilerOptionsFromCommandLine, reportDiagnostic, reportErrorSummary, testing);
