 ...
 * }
 */
export function shouldRescanLessThanLessThanToken(s, containingNode, token) {
    return (token === KindLessThanLessThanToken && IsJsxChild(containingNode));
}
/**
 ...
 * }
 */
export function scanNavigationToken(s, containingNode) {
...
export function GetTouchingPropertyName(sourceFile, position) {
    return getTokenAtPosition(sourceFile, position, false /*allowPositionInLeadingTrivia*/, (node) => {
        return (IsPropertyNameLiteral(node) || IsKeywordKind(node.Kind) || IsPrivateIdentifier(node));
    });
}
/**
 ...
 * }
 */
export function getTokenAtPosition(sourceFile, position, allowPositionInLeadingTrivia, includePrecedingTokenAtEndPosition) {
    let next = undefined;
    let prevSubtree = undefined;
    let current = Node_AsNode(sourceFile);
    let left = 0;
    let nodeAfterLeft = undefined;
    const testNode = (node) => {
    ...
    const visitNodeList = (nodeList, _v) => {
        ...
        if (next === undefined) {
            if (NodeList_End(nodeList) === position && includePrecedingTokenAtEndPosition !== undefined) {
                ...
                if (match && (nodes[index].Flags & NodeFlagsReparsed) !== 0) {
                    ...
                    [index, match] = BinarySearchUniqueFunc(nodes, (middle, node) => {
                        const cmp = testNode(node);
                        if (cmp < 0) {
                            left = Node_End(node);
    ...
    for (;;) {
        ...
        if (next === undefined) {
            ...
            while (left < end) {
                ...
                const flags = Scanner_TokenFlags(s);
                if (tokenEnd > end) {
                    break;
                }
                if (tokenStart <= position && position < tokenEnd) {
 ...
 * }
 */
export function getPosition(node, sourceFile, allowPositionInLeadingTrivia) {
    if (allowPositionInLeadingTrivia) {
 ...
 * }
 */
export function findRightmostNode(node) {
    let next = undefined;
    let current = node;
    const visitNode = (n, _v) => {
        if (n !== undefined) {
            next = n;
        }
        return n;
    };
    const visitNodes = (nodeList, _visitor) => {
 ...
 * }
 */
export function VisitEachChildAndJSDoc(node, sourceFile, visitNode, visitNodes) {
    const visitor = getNodeVisitor(visitNode, visitNodes);
 ...
 * )
 */
export const comparisonLessThan = -1;
export const comparisonEqualTo = 0;
export const comparisonGreaterThan = 1;
/**
 ...
 * }
 */
export function FindPrecedingTokenEx(sourceFile, position, startNode, excludeJSDoc) {
    const find = (n) => {
        ...
        const visitNodesFn = (nodeList, _v) => {
 ...
 * }
 */
export function isValidPrecedingNode(node, sourceFile) {
    if (node.Kind === KindEndOfFile) {
        return (Node_JSDoc(node, sourceFile).length > 0);
    }
 ...
 * }
 */
export function GetStartOfNode(node, file, includeJSDoc) {
 ...
 * }
 */
export function findRightmostValidToken(endPos, sourceFile, containingNode, position, excludeJSDoc) {
    if (position === -1) {
    ...
    const find = (n, ep) => {
        ...
        let rightmostValidNode = undefined;
        let rightmostVisitedNodes = []; // Nodes after the last valid node.
        let hasChildren = false;
        const shouldVisitNode = (node) => {
            // Node is synthetic or out of the desired range: don't visit it.
            return (!(((node.Flags & NodeFlagsReparsed) !== 0) ||
                Node_End(node) > ep || GetStartOfNode(node, sourceFile, !excludeJSDoc /*includeJSDoc*/) >= position));
        };
        const visitNodeFn = (node, _v) => {
            if (node === undefined || (node.Flags & NodeFlagsReparsed) !== 0) {
                return node;
        ...
        const visitNodesFn = (nodeList, _v) => {
            if (nodeList !== undefined && nodeList.Nodes.length > 0) {
                hasChildren = true;
                const [index, _match] = BinarySearchUniqueFunc(nodeList.Nodes, (_middle, node) => {
                    if (Node_End(node) > ep) {
                        return comparisonGreaterThan;
                    }
                    return comparisonLessThan;
 ...
 * }
 */
export function FindNextToken(previousToken, parent, file) {
    const find = (n) => {
        ...
        const visitNodesFn = (nodeList, _v) => {
            if (nodeList !== undefined && nodeList.Nodes.length > 0 && foundNode === undefined) {
                const nodes = nodeList.Nodes;
                const [index, match] = BinarySearchUniqueFunc(nodes, (_i, node) => {
                    if ((node.Flags & NodeFlagsReparsed) !== 0) {
                        return comparisonLessThan;
                    }
 ...
 * }
 */
export function getNodeVisitor(visitNode, visitNodes) {
    let wrappedVisitNode = undefined;
    let wrappedVisitNodes = undefined;
    if (visitNode !== undefined) {
        wrappedVisitNode = (n, v) => {
    ...
    const hooks = {
        ...
        VisitToken: wrappedVisitNode,
        VisitNodes: wrappedVisitNodes,
        VisitModifiers: (modifiers, visitor) => {
            if (modifiers !== undefined && wrappedVisitNodes !== undefined) {
                wrappedVisitNodes(modifiers, visitor);
            }
            return modifiers;
        },
 ...
 * }
 */
export function shouldSkipChild(node) {
 ...
 * }
 */
export function FindChildOfKind(containingNode, kind, sourceFile) {
    let lastNodePos = Node_Pos(containingNode);
    const scan = GetScannerForSourceFile(sourceFile, lastNodePos);
    let foundChild = undefined;
    const visitNodeFn = (node) => {
        if (node === undefined || (node.Flags & NodeFlagsReparsed) !== 0) {
            return false;
        }
        // Look for child in preceding tokens.
        ...
        while (startPos < Node_Pos(node)) {
            ...
            if (tokenKind === kind) {
                ...
                const flags = Scanner_TokenFlags(scan);
