...
test("GetTokenAtPosition / JSDoc type assertion", () => {
    ...
    const file = ParseSourceFile({ FileName: "/test.js", Path: "/test.js" }, fileText, ScriptKindJS);
    ...
    assert.ok(token.Kind === KindIdentifier || token.Kind === KindParenthesizedExpression, `Expected identifier or parenthesized expression, got ${token.Kind}`);
});
...
test("GetTokenAtPosition / pointer equality", () => {
    const fileText = `
		...
		`;
    const file = ParseSourceFile({ FileName: "/file.ts", Path: "/file.ts" }, fileText, ScriptKindTS);
