...
export function collectExtensionDiagnosticsForSourceFile(program, sourceFile) {
...
function convertExtensionDiagnostic(diagnostic, sourceFile) {
    ...
    if (isExtensionDiagnosticSourceSpan(location)) {
        ...
        return NewDiagnostic(location.sourceFile, NewTextRange(location.pos, location.end), message);
    }
    return undefined;
}
function createExtensionDiagnosticMessage(diagnostic) {
...
function mapExtensionDiagnosticCategory(category) {
    switch (category) {
        case "error":
...
function isExtensionDiagnosticSourceSpan(value) {
    return typeof value === "object"
        && value !== null
        && "sourceFile" in value
        && "pos" in value
