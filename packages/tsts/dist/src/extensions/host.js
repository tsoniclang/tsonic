...
export class ExtensionFactStore {
    ...
    [factStoreSetSourceAnalyzerAccessGuard](accessGuard, enumerationGuard) {
...
export class ProviderRegistry {
    ...
    get hasSourceDeclarationProviders() {
        return this.#sourceDeclarationProviders.size !== 0;
    }
    requiresProviderForModule(specifier) {
    ...
    #loadProviderDeclarationCandidate(specifier, context, planningCandidates) {
        ...
        const declarationValidation = validateProviderDeclarationModel(declarationModel, resolution);
...
export class ExtensionHost {
    ...
    #registerSourceDeclarationProviderForExtension(extensionId, provider) {
        this.#assertRegistrationOwner(extensionId, "source declaration provider");
        return runWithoutExtensionOwnerAuthority(this.#ownerAuthority, () => this.providers.registerSourceDeclarationProvider(provider));
    }
    ...
    #completeRolledBackFactAttempt(attempt, preserveDiagnostics) {
        const state = this.#assertActiveFactAttempt(attempt);
        const rollbackErrors = [];
...
function renderProviderClassMembers(declaration, context) {
    ...
    if ((declaration.members ?? []).some((member) => getProviderPropertyNameText(member.name) === nominalMemberName)) {
        throw new Error(`Provider declaration identity '${declaration.id}' collides with host-owned type-family nominal member '${nominalMemberName}'.`);
    }
...
function validateProviderIdentity(identity) {
    const invalidFields = [];
...
function validateProviderDeclarationModel(value, resolution) {
    ...
    const invalidImportIndex = (value.imports ?? []).findIndex((declaration) => !isValidProviderImportDeclaration(declaration));
    ...
    const invalidExportIndex = value.exports.findIndex((declaration) => !isValidProviderExportDeclaration(declaration));
    ...
    const invalidScopeIndex = value.exports.findIndex((declaration) => !hasValidProviderExportTypeParameterScope(declaration));
    ...
    const invalidReferenceIndex = value.exports.findIndex((declaration) => !hasValidProviderReferenceBindingsForExport(declaration, context));
    ...
    const invalidHeritageIndex = value.exports.findIndex((declaration) => !hasValidProviderValueHeritageReferences(declaration, context));
    ...
    if (!isValidProviderTypeFamilyDeclarations(value.moduleSpecifier, value.exports, value.imports ?? [])) {
...
function isValidProviderTypeFamilyDeclarations(moduleSpecifier, exports, imports) {
    ...
    for (const [familyName, variants] of familyGroups) {
        ...
        if (variants.some((variant) => {
            ...
            const nominalMemberName = getProviderTypeFamilyVariantNominalMemberName(moduleSpecifier, variant);
            return (variant.members ?? []).some((member) => getProviderPropertyNameText(member.name) === nominalMemberName);
        })) {
            return false;
        }
