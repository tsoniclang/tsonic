...
export function analyzeRawMemoryCall(call: MemorySourceCall, analysis: MemorySourceAnalysis): void {
  ...
  if (name === "torawptr" || name === "reinterpretrawptr") {
    ...
    publishMemoryFact(call, tsonicRawMemoryOperationFactKey, name === "torawptr"
  ...
  const dataLayoutOperand = args[name === "offsetrawptr" ? 2 : 1];
  const dataLayout = dataLayoutOperand === undefined ? undefined : selectedDataLayout(dataLayoutOperand.expression, context, analysis.registrations);
  ...
  if (name === "offsetrawptr") {
  ...
  } else if (name === "rawptrtoaddressinteger" || name === "addressintegertorawptr") {
    ...
    publishMemoryFact(call, tsonicRawMemoryOperationFactKey, name === "rawptrtoaddressinteger"
