...
| Values | Primitives, arrays, fixed arrays, tuples, structural records, string-literal enums, discriminated unions, nullable values, finite broad values |
...
| TypeScript types | Closed mapped/utility types including `Partial`, `Required`, `Readonly`, `Pick`, `Omit`, `Record`, `Exclude`, `Extract`, `NonNullable`, callable utilities, and `Awaited` |
