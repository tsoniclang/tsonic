...
import { formatArchitectureFindings } from "./tooling/architecture-rules.mjs";
import { evaluatePublicExportInventory } from "./tooling/public-export-inventory.mjs";
...
test("shared packages expose only audience-owned public entrypoints", async () => {
  const targetApiManifest = JSON.parse(await readFile(
    join(repoRoot, "packages/target-api/package.json"),
    "utf8",
  ));
  const targetApiFindings = evaluatePublicExportInventory({
    ...
    sourceTextByEntrypoint: new Map(await Promise.all([
      ...
      `packages/target-api/src/public/${entrypoint}.ts`,
      await readFile(
        join(repoRoot, `packages/target-api/src/public/${entrypoint}.ts`),
        "utf8",
      ),
    ]))),
    forbiddenNamesByEntrypoint: new Map([
      ...
      ["packages/target-api/src/public/source.ts", [
        "TargetArtifactContractGraph",
        "TargetProviderContext",
      ]],
    ]),
  ...
  const sourceCoreManifest = JSON.parse(await readFile(
    join(repoRoot, "packages/source-core/package.json"),
    "utf8",
  ));
  const sourceCoreFindings = evaluatePublicExportInventory({
    manifest: sourceCoreManifest,
    expectedEntrypoints: [".", "./extension", "./facts", "./package.json"],
    sourceTextByEntrypoint: new Map(await Promise.all([
      "index",
      "extension",
      "facts",
    ].map(async (entrypoint) => [
      `packages/source-core/src/public/${entrypoint}.ts`,
      await readFile(
        join(repoRoot, `packages/source-core/src/public/${entrypoint}.ts`),
        "utf8",
      ),
    ]))),
    forbiddenNamesByEntrypoint: new Map([
      ["packages/source-core/src/public/index.ts", [
        "analyzeSafetyBuilderCalls",
        "tsonicSafetyBuilderFactKey",
      ]],
      ["packages/source-core/src/public/extension.ts", [
        "tsonicSafetyBuilderFactKey",
        "TsonicUnsafeContextFact",
      ]],
      ["packages/source-core/src/public/facts.ts", [
        "analyzeSafetyBuilderCalls",
  ...
  const findings = [...targetApiFindings, ...sourceCoreFindings];
...
test("neutral source marker catalog contains no target-flavoured aliases", async () => {
  const text = await readFile(
    join(repoRoot, "packages/source-core/src/extension/source-modules.ts"),
    "utf8",
  );
...
test("source tests fixtures and documentation do not import retired neutral markers", async () => {
  ...
  for (const sourceFile of await neutralMarkerContractFiles()) {
    if (repoRelative(sourceFile) === "test/architecture/dependency-boundaries.test.mjs") {
      continue;
    }
    const text = await readFile(sourceFile, "utf8");
