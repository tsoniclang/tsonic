...
type HasCustomAttributes = {
  GetCustomAttributes(inherit: boolean): readonly unknown[];
};
...
Console.WriteLine(`User.Serializable: ${ok(hasAttribute(userType, "System.SerializableAttribute"))}`);
...
Console.WriteLine(
  `User.save.Obsolete: ${ok(!!saveMethod && hasAttribute(saveMethod, "System.ObsoleteAttribute"))}`
);

const nameProp = userType.GetProperty("name");
Console.WriteLine(
  `User.name.Obsolete: ${ok(!!nameProp && hasAttribute(nameProp, "System.ObsoleteAttribute"))}`
);
