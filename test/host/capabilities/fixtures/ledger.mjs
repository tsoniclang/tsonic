...
export const capabilityClosedRuntimeCarriers = Object.freeze([
...
const bannedOpenRuntimeMechanismPattern = /QuickJS|Reflection|dynamic|GetProperty|GetProperties|GetMethod|GetMethods|MethodInfo\.Invoke|Activator\.CreateInstance|Assembly\.Load/u;
const mapSetCapabilityId = "surface.js.map-set";
const mapSetRequiredPossibleLanes = Object.freeze(["closed-runtime", "hard-reject"]);
const mapSetRequiredClosedRuntimeFacts = Object.freeze([
  "selected Map/Set source declaration identity",
  "selected Map/Set operation fact",
  "closed JS Map/Set runtime carrier",
  "JS SameValueZero equality metadata",
]);
const mapSetRequiredHardRejectReasons = Object.freeze([
  "missing-selected-map-set-operation",
  "missing-same-value-zero-semantics",
...
export const coreIntrinsicCoverage = Object.freeze([
  ...
  { moduleSpecifier: coreLangIntrinsicModuleSpecifier, exportName: "allocateptr", factSlug: "allocate-pointer", sourceKind: "call-marker", capabilityId: "source-core.lang.portable-intrinsics.allocate-pointer" },
  { moduleSpecifier: coreLangIntrinsicModuleSpecifier, exportName: "loadptr", factSlug: "load-pointer", sourceKind: "call-marker", capabilityId: "source-core.lang.portable-intrinsics.load-pointer" },
  { moduleSpecifier: coreLangIntrinsicModuleSpecifier, exportName: "storeptr", factSlug: "store-pointer", sourceKind: "call-marker", capabilityId: "source-core.lang.portable-intrinsics.store-pointer" },
  { moduleSpecifier: coreLangIntrinsicModuleSpecifier, exportName: "equalptr", factSlug: "equal-pointer", sourceKind: "call-marker", capabilityId: "source-core.lang.portable-intrinsics.equal-pointer" },
...
const baseCapabilityDefinitions = Object.freeze([
  ...
  ["host.config.surface-selection", "Select target-supported source surfaces", "complete", "tsonic-host"],
  ...
  ["source-core.lang.portable-intrinsics.allocate-pointer", "allocateptr attaches fresh typed-location facts", "complete", "source-core-provider"],
  ["source-core.lang.portable-intrinsics.load-pointer", "loadptr attaches exact typed-location read facts", "complete", "source-core-provider"],
  ["source-core.lang.portable-intrinsics.store-pointer", "storeptr attaches exact typed-location write facts", "complete", "source-core-provider"],
  ["source-core.lang.portable-intrinsics.equal-pointer", "equalptr attaches exact typed-location identity-comparison facts", "complete", "source-core-provider"],
  ...
  ["carrier.array.public-abi-policy", "Source T[] remains TS Array<T>; public target ABI uses fact-backed IEnumerable/IReadOnlyList/List/native-array/closed-runtime lanes without leaking JSArray by default", "complete", "target-provider"],
  ...
  ["carrier.any-tsvalue", "any uses the canonical closed TsValue carrier", "complete", "target-provider"],
  ...
  ["surface.node.buffer-crypto-os", "Buffer, crypto, and os use selected Node provider-package facts", "complete", "surface-provider"],
  ["surface.node.fs", "node:fs uses selected Node provider-package facts", "complete", "surface-provider"],
  ["surface.node.fs-stats-date", "node:fs Stats Date members use selected Node provider-package and JS surface facts", "complete", "surface-provider"],
  ["surface.node.process", "node:process uses selected Node provider-package facts", "complete", "surface-provider"],
  ["surface.node.util", "node:util uses selected Node provider-package facts and rejects open-carrier helpers without fallback", "complete", "surface-provider"],
  ["surface.node.url", "node:url uses selected Node provider-package facts and rejects open-object URL helpers without fallback", "complete", "surface-provider"],

  ["dynamic-value.canonical", "TypeScript any uses one canonical closed-runtime policy", "complete", "target-provider"],
  ["dynamic-value.any.property", "any property operations use closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.dynamic-get", "any property reads use explicit closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.dynamic-set", "any property writes use explicit closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.call-construct", "any call/new use closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.dynamic-call", "any calls use explicit closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.operators", "any operators use closed-runtime carrier facts", "complete", "target-provider"],
  ["dynamic-value.any.typed-boundary-cast", "any typed-boundary casts are explicit", "complete", "target-provider"],
  ["dynamic-value.object.no-dynamic-access", "object is not treated like any", "complete", "target-provider"],
  ["dynamic-value.unknown.no-dynamic-access", "unknown is not treated like any", "complete", "target-provider"],
  ["dynamic-value.prototype-mutation", "Prototype mutation is explicit runtime support or diagnostic", "complete", "target-provider"],
  ["dynamic-value.proxy-eval-function-with", "proxy, eval, Function, and with are rejected unless explicit runtime exists", "complete", "target-provider"],
  ...
  ["runtime.dynamic.carrier", "TypeScript any closed-runtime carrier is explicit runtime capability", "complete", "target-provider"],
  ...
  ["runtime.csharp.js", "C# always includes its closed JS-value runtime; the js surface selects source declarations only", "complete", "csharp-runtime"],
  ...
  ["diagnostic.dynamic-operation-facts", "Dynamic operations require exact closed operation facts", "complete", "target-provider"],
  ["diagnostic.unsupported-dynamic-operation", "Unsupported dynamic operations fail closed", "complete", "target-provider"],
...
const slice6WholeProgramClosurePositiveTests = Object.freeze([
  "test/cli-build/toolchain/whole-program-csharp-closure.test.mjs",
  "test/cli-build/core/declarations/classes.test.mjs",
  "test/cli-build/providers/source-profile/target-config.test.mjs",
  "test/cli-build/toolchain/runtime-toolchain-proof.test.mjs",
  "test/host/dependencies/surface-composition.test.mjs",
  "../tsonic-csharp/test/backend/printer/csharp-printer.test.mjs",
...
function slice6WholeProgramClosureEvidence() {
  return {
    ...
    ...slice6EvidenceForRows(slice6WholeProgramRuntimeRows, {
      tstsDecision:
        "TSTS validates source imports and calls; host composition selects the JS surface runtime artifacts and C# policy resolves each checked call.",
      providerFacts: Object.freeze([
    ...
    ...slice6EvidenceForRows(slice6WholeProgramDiagnosticRows, {
      sourceExamples: Object.freeze([
        ...
        "backend requires a closed target selection before emission",
      ]),
      tstsDecision:
      ...
      providerFacts: Object.freeze([
        ...
        "unsupportedTargetOperationDiagnostic",
        "diagnosticEvidenceFact",
      ]),
      backendContract:
        "Missing source/provider evidence, unsupported surfaces, rejected target operations, and backend diagnostics stop artifact and toolchain output deterministically.",
      notes:
...
const reviewedCapabilityEvidence = Object.freeze({
  ...
  "carrier.primitive": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "test/cli-build/providers/tsts-type-forms/core.test.mjs",
    ]),
  ...
  "performance.long-operator-chain": Object.freeze({
    ...
    tstsDecision:
      "TSTS checks each binary + operation, operand type, and result type. Shared source semantics exposes that checked operator decision; C# policy selects the target operator from the exact operands and result without reimplementing TypeScript operator semantics.",
    providerFacts: Object.freeze([
      "selectedSourceOperator",
      "selectedCsharpOperator",
      "resolvedOperandTargetTypes",
      "sourcePrimitiveFact",
    ]),
    backendContract:
      "C# backend emits long operator chains only from the immutable C# operator selection. Missing target policy produces diagnostics; the backend does not split source or infer target operators from token spelling.",
    runtimeContract:
  ...
  "carrier.tuple": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/statements/direct-translation-bindings.test.mjs",
      "test/cli-build/core/object-shapes/basic.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
      "test/cli-build/core/control-flow/expressions.test.mjs",
      "test/cli-build/providers/tsts-type-forms/core.test.mjs",
  ...
  "carrier.dictionary-record": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/core/object-shapes/basic.test.mjs",
      "test/cli-build/runtime/iteration/facts.test.mjs",
  ...
  "host.project.clean-rebuild": Object.freeze({
    ...
    positiveTests: Object.freeze([
      "test/cli-build/providers/source-profile/target-config.test.mjs",
      "test/host/publication/output-publication.test.mjs",
      "test/architecture/dependency-boundaries.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/providers/source-profile/target-config.test.mjs",
  ...
  "tsts.package.public-root-artifact": Object.freeze({
    ...
    positiveTests: Object.freeze([
      "test/architecture/package-artifacts.test.mjs",
      "test/host/dependencies/surface-composition.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/architecture/package-artifacts.test.mjs",
      "test/host/dependencies/surface-composition.test.mjs",
      "test/host/capabilities/ledger.test.mjs",
    ]),
    oldEvidence: Object.freeze([
      "packages/frontend/src/program/creation-cases/tsts-source-program.test.ts",
    ]),
  ...
  "source.marker.struct": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "packages/source-core/src/extension/source-extension-virtual-modules-and-primitives.test.ts",
      "test/cli-build/providers/source-semantics/core.test.mjs",
      "test/cli-build/core/classes/value-types.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "packages/source-core/src/extension/source-extension-flow-and-structure.test.ts",
      "packages/source-core/src/extension/source-extension-pointers-and-fixed-arrays.test.ts",
  ...
  "source-core.lang.portable-intrinsics": Object.freeze({
    sourceExamples: Object.freeze([
      "import { writeonlyref, readwriteref, readonlyref, sharedborrow, mutableborrow, move, struct, field, attribute, defaultvalue, addressof, allocateptr, loadptr, storeptr, equalptr } from \"@tsonic/core/lang.js\";",
      ...
      "const alias = addressof(value); const fresh = allocateptr<int32>(0); storeptr(alias, loadptr(fresh)); equalptr(alias, addressof(value));",
    ]),
    tstsDecision:
  ...
  "source-core.lang.portable-intrinsics.allocate-pointer": coreIntrinsicEvidence({
    moduleSpecifier: coreLangIntrinsicModuleSpecifier,
    exportName: "allocateptr",
    factSlug: "allocate-pointer",
    sourceKind: "call-marker",
    sourceExamples: [
      "import { allocateptr } from \"@tsonic/core/lang.js\";",
      "const pointer = allocateptr<int32>(1);",
    ],
    sourceContract:
      "Core owns allocateptr<T>(initial) as a portable operation that creates one fresh typed location initialized from exact T evidence.",
    providerFacts: [
      "sourceCorePointerOperationFact",
      "pointerPointeeTypeFact",
      "freshLocationFact",
  ...
  "source-core.lang.portable-intrinsics.load-pointer": coreIntrinsicEvidence({
    ...
    sourceExamples: [
      ...
      "const value: int32 = loadptr(pointer);",
    ],
    sourceContract:
      "Core owns loadptr<T>(pointer) as a portable read from the exact selected Pointer<T> location.",
    providerFacts: [
      "sourceCorePointerOperationFact",
      "pointerPointeeTypeFact",
  ...
  "source-core.lang.portable-intrinsics.store-pointer": coreIntrinsicEvidence({
    ...
    sourceContract:
      "Core owns storeptr<T>(pointer,value) as a portable write through the exact selected Pointer<T> location after ordinary TypeScript checking proves T.",
    providerFacts: [
      "sourceCorePointerOperationFact",
      "pointerPointeeTypeFact",
  ...
  "source-core.lang.portable-intrinsics.equal-pointer": coreIntrinsicEvidence({
    ...
    sourceExamples: [
      ...
      "const same = equalptr(pointer, addressof(value));",
    ],
    sourceContract:
      "Core owns equalptr<T>(left,right) as portable identity comparison for two Pointer<T> or undefined operands; it compares location identity, not current values or wrapper identity.",
    providerFacts: [
      "sourceCorePointerOperationFact",
      "pointerPointeeTypeFact",
  ...
  "declaration.class.abstract": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/declarations/direct-translation-declarations.test.mjs",
      "test/cli-build/providers/source-profile/target-config.test.mjs",
      "test/cli-build/toolchain/whole-program-csharp-closure.test.mjs",
    ]),
    negativeTests: Object.freeze([
  ...
  "operation.spread.object": Object.freeze({
    ...
    oldEvidenceAbsence: Object.freeze({
      ...
      searchEvidence: Object.freeze([
        ...
        "current object-spread coverage is stronger than old coverage because it proves positive spread, subset spread, nested spread, readonly spread, dictionary hard-reject, closedRuntime-any hard-reject, and single-evaluation hard-reject through finalized facts",
      ]),
      reviewerNotes:
  ...
  "surface.js.array.length-index": Object.freeze({
    ...
    laneClassification: freezeLaneClassification({
      ...
      possibleLanes: Object.freeze(["static-native", "closed-runtime", "hard-reject"]),
      canonical: {
        lane: "static-native",
        requiredFacts: Object.freeze([
      ...
      closedRuntime: {
        lane: "closed-runtime",
        requiredFacts: Object.freeze([
          "selected-dynamic-value-operation",
  ...
  "surface.js.array.sparse-delete-holes": Object.freeze({
    ...
    laneClassification: freezeLaneClassification({
      ...
      possibleLanes: Object.freeze(["closed-runtime", "hard-reject"]),
      canonical: {
        lane: "hard-reject",
        reasons: Object.freeze([
  ...
  "analysis.abstraction.policy-enforcement": Object.freeze({
    ...
    backendContract:
      "Backends consume immutable policy/provider selections and target AST. They do not rediscover target members from source spelling, source-family branches, target member names, or recovery inference.",
    positiveTests: Object.freeze([
    ...
    notes:
      "Complete proof: the C# architecture validator now reports zero findings across product code after JS surface and Node provider-package member routing and object-shape member lookup were moved behind selected identity, finalized facts, declarative metadata, generic selectors, or structured missing diagnostics. Lazy generic analysis tests prove checked-source structural queries without policy conclusions, and surface/object-shape tests prove selected declarations/facts are required for JS surface, Node provider-package, and object-shape operations.",
  }),
  ...
  "surface.js.string-methods": Object.freeze({
    ...
    notes:
      "Reviewed proof: selected JS surface facts cover string element access, code-point for-of, selected string instance/helper calls including trim/toUpperCase chaining, normalize/locale case/localeCompare/search/well-formed helpers, split returning the selected JS surface List<string> array-return ABI, Object.toString delegation for closed string primitive receivers, and fail-closed rejection without closed string receiver facts. String(value) and zero-argument String() map only from selected StringConstructor call identity plus closed conversion argument facts to Tsonic.CSharp.Js.Globals.String, with runtime tests proving explicit undefined/null conversion remains distinct from the zero-argument empty string result. new String(value) remains hard-rejected until an explicit wrapper-object carrier exists, so wrapper construction never falls back to object/dynamic/native string semantics. String.raw remains hard-rejected until template-object semantics have a closed carrier. String match, matchAll, replace, replaceAll, search, and split consume the canonical JS RegExp profile's selected overload and protocol evidence, with closed RegExp result, iterator, group, indices, callback, and lastIndex carriers. CLI evidence dotnet-builds selected string calls and conversions through finalized facts and asserts no InvalidExpression, __unsupported, dynamic/reflection, or source-spelling fallback appears in generated output.",
  }),
  ...
  "surface.js.map-set": Object.freeze({
    ...
    laneClassification: freezeLaneClassification({
      ...
      possibleLanes: ["closed-runtime", "hard-reject"],
      canonical: {
        lane: "closed-runtime",
        requiredFacts: [
          "selected Map/Set source declaration identity",
  ...
  "surface.node.fs": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/accessSync.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/chmod.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/closeSync.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/cp.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/openSync.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/readlink.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/realpath.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/rmdir.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/symlink.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/fs/truncateSync.tests.cs",
  ...
  "surface.node.fs-stats-date": Object.freeze({
    ...
    tstsDecision:
      "TSTS validates node:fs Stats members from selected Node provider-package virtual declarations and Date instance calls from selected JS declarations; nullish Date unions must preserve the closed Date carrier across both surfaces.",
    providerFacts: Object.freeze([
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed complete proof: selected Node provider-package declarations expose Stats.mtime as the JS Date source shape, property mapping requires the selected provider member identity, JS Date instance calls require selected JS declarations, Date | undefined nullish coalescing preserves the closed JS Date carrier across surfaces, CLI/toolchain emission produces Tsonic.CSharp.Js.Date rather than DateTime/string/dynamic carriers, no-provider-package Node imports fail before artifact emission, and runtime tests cover Stats Date values plus JS Date behavior.",
  }),
  "surface.node.process": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/arch.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/argv.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/chdir.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/cwd.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/env.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/execPath.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/exit.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/exitCode.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/kill.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/metrics.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/pid.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/platform.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/ppid.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/process/version.tests.cs",
  ...
  "surface.node.buffer-crypto-os": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/buffer/buffer.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/buffer/buffer.module.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/createHash.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/createHmac.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/getCiphers.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/getCurves.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/getHashes.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/randomBytes.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/randomFillSync.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/randomInt.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/randomUUID.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/crypto/timingSafeEqual.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/os/os.tests.cs",
  ...
  "surface.node.util": Object.freeze({
    positiveTests: Object.freeze([
      ...
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/util/util.extras.tests.cs",
    ...
    negativeTests: Object.freeze([
      ...
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/util/util.tests.cs",
      "../csharp-nodejs/csharp/test/Tsonic.CSharp.Node.Tests/util/util.extras.tests.cs",
    ]),
    oldEvidence: Object.freeze([
      "test/fixtures/nodejs-surface-alias-coverage/",
  ...
  "backend.csharp.runtime-artifacts": Object.freeze({
    ...
    notes:
      "Reviewed proof: host composition includes provider, selected-surface, and selected provider-package runtime contributions before backend/toolchain handoff, omits unselected surface/package contributions, emits no target artifacts when TSTS rejects the source program, always includes the C# runtime and closed JS-value runtime, and includes the Node runtime only when the Node provider package is active. Focused diagnostics reject invalid runtime contributions and target/toolchain combinations without test-local reference configuration or hidden discovery.",
  }),
  "runtime.csharp.js": Object.freeze({
    positiveTests: Object.freeze([
      "test/host/dependencies/surface-composition.test.mjs",
    ...
    notes:
      "Reviewed proof: the C# provider always contributes the real csharp-runtime and csharp-js assemblies because TypeScript any uses the canonical closed TsValue carrier; this does not activate JS source declarations. Selecting the JS surface changes the source profile and maps its proven operations onto the same closed runtime. Generated projects exclude csharp-nodejs unless the Node provider package is active, JS-only and JS+Node executables run through the exact selected runtime operations, and unsupported operations diagnose before artifact publication.",
  }),
  ...
  "operation.operator.checked-target-operation": Object.freeze({
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed proof: binary, unary, assignment, compound, comparison, logical, nullish, typeof, instanceof, and closedRuntime any operators emit only from finalized TSTS/provider targetOperation facts plus matching C# target-operation facts; unsupported in-operator, structural, generic type-parameter, unproven bitwise, and unsupported closedRuntime operators fail closed with deterministic diagnostics; backend emission uses Roslyn operator-token AST nodes and rejects semantic-string/operator-token drift.",
  }),
  "operation.iteration.for-of.sync": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/iteration/facts.test.mjs",
      "test/cli-build/core/control-flow/statements.test.mjs",
      "test/cli-build/runtime/js-surface/rejections.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/runtime/iteration/facts.test.mjs",
  ...
  "operation.await.promise-task": Object.freeze({
    ...
    positiveTests: Object.freeze([
      ...
      "test/cli-build/core/control-flow/statements.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
      "test/host/cli/async-build.test.mjs",
      "test/cli-build/runtime/nodejs/modules-path-fs.test.mjs",
      "test/cli-build/runtime/nodejs/process-buffer-crypto-os.test.mjs",
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
      "test/cli-build/runtime/nodejs/rejections.test.mjs",
  ...
  "function.this-binding": Object.freeze({
    ...
    positiveTests: Object.freeze([
      "test/cli-build/core/control-flow/functions-this-async.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/core/control-flow/functions-this-async.test.mjs",
    ]),
  ...
  "expression.literal.bigint-regex-template": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/core/control-flow/expressions.test.mjs",
      "../tsonic-csharp/test/policy/operations/regexp-literals.test.mjs",
      "../tsonic-csharp/test/source/facts/source-literal-values.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/core/control-flow/expressions.test.mjs",
      "../tsonic-csharp/test/policy/operations/regexp-literals.test.mjs",
  ...
  "statement.switch": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/core/control-flow/statements.test.mjs",
      "test/cli-build/core/closure/control-flow.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/core/control-flow/statements.test.mjs",
      "test/cli-build/core/closure/control-flow.test.mjs",
  ...
  "carrier.object-shape": Object.freeze({
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed proof: generated object-shape adapters are expression-local carriers, while shared semantic Type and Symbol subjects retain declared interface/class/struct carriers. CLI proof includes interface-backed object literal adapters, generic interface object literal adapters, inline structural parameters, nested object extraction, rest object carriers, object spread carriers, readonly utility-projected shape copies, nested spread carriers, nullable optional members, Record dictionary separation, and non-Node executables that apply object-shape default extraction through generated C# behavior. This prevents imported interface object literals from conflicting with declaration carriers and keeps unknown, any closedRuntime spread/destructuring, computed/accessor members, dictionary spread without copy facts, and missing shape/provider facts fail-closed instead of falling back to source spelling.",
  }),
  "carrier.any-tsvalue": Object.freeze({
    ...
    notes:
      "Reviewed proof: TypeScript any always receives the closed TsValue source/runtime carrier and lowers only through exact TsValue/TsObject/TsArray/TsUnion/TsFunction/JSArray operation facts. csharp-js runtime tests prove property/element/call/construct and union forwarding behavior without reflection or dynamic dispatch, CLI/toolchain proof emits TsValue public boundaries plus closed runtime calls without selecting the JS source surface, and object and unknown remain non-dynamic.",
  }),
  ...
  "dynamic-value.prototype-mutation": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/runtime/js-surface/arrays.test.mjs",
    ]),
  ...
  "dynamic-value.canonical": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "../tsonic-csharp/test/toolchain/project-model/project-artifacts.test.mjs",
    ...
    notes:
      "Reviewed proof: TypeScript any always resolves to the closed TsValue carrier, C# always includes the csharp-js runtime without activating the JS source surface, exact finalized facts drive property read/write, element read/write, direct calls, member calls, construction, operators, and typed-boundary conversion, missing or incompatible facts fail closed, object and unknown do not acquire dynamic operations, and CLI/toolchain proof emits buildable C# without reflection, C# dynamic, or source-name fallback.",
  }),
  "dynamic-value.any.property": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
    ...
    notes:
      "Reviewed proof: any property and element operations are not source-owned fallbacks. Source semantics records closed get/set facts only for explicit any subjects, missing operation facts fail closed, backend property reads and property/element writes emit only from exact closed operation facts with argument projections, csharp-js runtime tests prove closed object/array slot behavior including undefined for missing properties, and CLI/toolchain proof builds the emitted operations.",
  }),
  "dynamic-value.any.call-construct": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "../csharp-js/tests/Tsonic.CSharp.Js.Tests/Values/TsValueTests.cs",
    ...
    notes:
      "Reviewed proof: calls and new expressions through explicit any require exact closed target operation facts. Source semantics records InvokeDynamic/ConstructDynamic facts, backend AST emission requires those finalized facts plus explicit argument projection, runtime tests prove closed TsFunction invocation/construction and missing-method TypeError behavior, and CLI/toolchain proof covers direct any calls, member calls through closed property-read carriers, and construction.",
  }),
  "dynamic-value.any.dynamic-get": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
    ...
    notes:
      "Reviewed proof: any property/element reads require exact finalized get facts before backend AST output. Backend C# AST planning renders property and element reads only from closed carrier facts with explicit key projections, runtime tests prove missing-property undefined and closed key conversion, and CLI/toolchain proof builds generated ReadDynamicSlot/ReadDynamicElement calls.",
  }),
  "dynamic-value.any.dynamic-set": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "../csharp-js/tests/Tsonic.CSharp.Js.Tests/Values/TsValueTests.cs",
  ...
  "dynamic-value.any.dynamic-call": Object.freeze({
    ...
    notes:
      "Reviewed proof: calls through explicit any emit deterministic missing-operation diagnostics unless a finalized closed carrier operation fact exists. Direct calls and member-style calls through prior property-read carriers both lower through InvokeDynamic, runtime tests prove closed TsFunction invocation and missing-method TypeError behavior, and CLI/toolchain proof builds the generated calls without source-name fallback.",
  }),
  "dynamic-value.any.operators": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/backend/planner/expressions/direct-translation-core.test.mjs",
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
    ...
    notes:
      "Reviewed proof: supported explicit-any binary, prefix, typeof, void, and instanceof operators lower only through finalized closed TsValue operation facts; backend planner tests require static runtime-helper facts including ApplyDynamicTypeof, ApplyDynamicVoid, and exact generic IsDynamicInstanceOf<T>; csharp-js runtime tests prove closed primitive/typeof/void/type-test semantics without reflection or dynamic dispatch; and source/CLI diagnostics hard-reject unsupported <<, &, **, +=, **=, in, comma/sequence, and delete forms before backend fallback.",
  }),
  "dynamic-value.any.typed-boundary-cast": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/policy/conversions/direct-source-literal-policy.test.mjs",
      "test/cli-build/providers/dotnet/constraints-conversions.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
    ...
    notes:
      "Reviewed proof: typed boundaries between any and typed target carriers are never emitted from backend inference. Assertions, assignments, initializers, and returns require finalized target conversion facts; any-to-typed boundaries lower through TsValue.CastDynamic<T> or TsUnion.CastDynamic<T...>, typed-to-any boundaries lower through TsValue.from, backend conversion tests require the finalized generic method fact, runtime tests prove successful scalar/union casts and deterministic mismatch/nullish failures, and CLI/toolchain proof builds the conversions without reflection, dynamic dispatch, or source-name fallback.",
  }),
  "dynamic-value.object.no-dynamic-access": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
      "test/cli-build/core/object-shapes/basic.test.mjs",
  ...
  "dynamic-value.unknown.no-dynamic-access": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
      "test/cli-build/providers/source-semantics/core.test.mjs",
      "test/cli-build/core/object-shapes/basic.test.mjs",
    ...
    notes:
      "Reviewed proof: unknown uses the canonical closed TsValue storage carrier but receives no selected dynamic operations; unknown.foo therefore remains a TSTS source diagnostic before backend emission. Exact object literals crossing an unknown boundary lower each finalized data property into a nested TsObject/TsValue carrier and execute through CLI plus dotnet proof without object, C# dynamic, reflection, dictionaries, anonymous target types, or generated structural declarations.",
  }),
  "runtime.dynamic.carrier": Object.freeze({
    ...
    notes:
      "Reviewed proof: the runtime carrier fact for TypeScript any is non-renderable by itself; every operation requires a separate exact closed operation fact. TsValue/TsObject/TsArray/TsUnion/TsFunction/JSArray runtime artifacts implement deterministic behavior without reflection or dynamic dispatch, backend AST paths consume finalized property/element/call/construct facts, and CLI/toolchain proof builds emitted code with the canonical C# JS-value runtime reference.",
  }),
  "diagnostic.dynamic-operation-facts": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
    ]),
    negativeTests: Object.freeze([
    ...
    notes:
      "Reviewed proof: dynamic any diagnostics are surfaced through CLI builds before C# artifacts whenever exact operation evidence is absent. Current tests cover property, element, call, construct, operator, and typed-boundary families and require closed target facts instead of backend fallback.",
  }),
  ...
  "backend.diagnostics": Object.freeze({
    ...
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/policy/types/direct-binding-projection-policy.test.mjs",
      "../tsonic-csharp/test/backend/planner/artifacts/direct-translation-artifacts.test.mjs",
      "test/cli-build/core/object-shapes/basic.test.mjs",
      "../tsonic-csharp/test/architecture/roslyn-boundary.test.mjs",
      "test/cli-build/core/control-flow/statements.test.mjs",
      "test/host/dependencies/surface-composition.test.mjs",
  ...
  "toolchain.csharp.build-run": Object.freeze({
    ...
    notes:
      "Reviewed partial proof: provider-dotnet CLI suite now builds generated C# projects for provider-owned static/instance/nested/byref/generic/delegate/attribute/exception operations and runs provider-backed exception semantics; JS surface and Node provider-package suites provide separate selected-runtime build proof.",
  }),
  "toolchain.csharp.library": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/toolchain/runtime-toolchain-proof.test.mjs",
  ...
  "diagnostic.missing-target-fact": Object.freeze({
    ...
    blockers: Object.freeze([
      "diagnostic.missing-target-fact remains partial until missing-fact negatives cover each backend operation family separately: provider calls/properties/indexers/conversions, JS surface and Node provider-package operations, object/array/tuple/union/nullish carriers, iteration/spread/destructuring, runtime artifacts, precise source spans, and no artifact/toolchain handoff.",
    ]),
    laneClassification: freezeLaneClassification({
  ...
  "target.shared.operation-contract": Object.freeze({
    ...
    tstsDecision:
      "TSTS checks the source operation and shared source semantics exposes its exact selected member, signature, operands, arguments, and result. Target policy consumes that immutable source decision directly.",
    providerFacts: Object.freeze([
      "selectedSourceOperation",
      "selectedSourceSignature",
      "resolvedTargetType",
      "selectedCsharpOperation",
    ]),
    backendContract:
      "Backends emit only the immutable target selection returned by target policy. They do not re-query source identity, match source spelling, or infer a target helper after selection.",
    positiveTests: Object.freeze([
      "test/host/dependencies/surface-composition.test.mjs",
      "../tsonic-csharp/test/integration/provider-selection/direct-provider-call-selection-parameter-and-generic-evidence.test.mjs",
      "../tsonic-csharp/test/integration/provider-selection/direct-provider-call-selection-relations-and-conversions.test.mjs",
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed complete proof for the current shared target contract: neutral host tests prove targets receive checked source semantics through the shared query contract; C# call/property/element/operator tests prove target policy must return one closed selection before translation; old extension, dictionary, nullish, and unsupported operator evidence map stale emitter cases to direct query and policy gates rather than target-name shortcuts.",
  }),
  "diagnostic.unsupported-surface": Object.freeze({
    sourceExamples: Object.freeze([
  ...
  "diagnostic.unsupported-selected-surface-operation": Object.freeze({
    ...
    notes:
      "Reviewed proof: all current first-party JS and Node provider-package rows are complete, and unsupported selected operations fail closed through the owning surface/provider diagnostic path rather than backend lookup. C# JS and Node provider package tests hard-reject declared unsupported selected operations with CSHARP_JS_SURFACE_OPERATION_UNSUPPORTED or CSHARP_NODEJS_PROVIDER_PACKAGE_OPERATION_UNSUPPORTED and diagnostic evidence naming selected source/provider identity, required facts, reason, and capability id. Selected JS and Node unsupported-operation diagnostics carry checked operation nodes through ExtensionDiagnostic.nodeOrSpan, so CLI rendering uses exact source spans instead of parsing evidence text. CLI evidence hard-rejects selected JS Object descriptor/prototype operations, String.raw, JSON.stringify carrier gaps, and selected unsupported Node provider-package operations before artifacts, with no placeholder calls, runtime fallback, source-name recovery, dynamic dispatch, or reflection; String.match and String.matchAll instead require exact selected RegExp or protocol evidence.",
  }),
  "diagnostic.unsupported-target-operation": Object.freeze({
    positiveTests: Object.freeze([
      "../tsonic-csharp/test/providers/native/target-bindings/dotnet-provider-cache-properties-and-events.test.mjs",
  ...
  "diagnostic.unsupported-dynamic-operation": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli-build/runtime/dynamic-values/runtime.test.mjs",
    ]),
    negativeTests: Object.freeze([
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed proof: unsupported dynamic operators diagnose before project output, while supported property, element, call, construct, operator, and typed-boundary operations emit only from exact closed TsValue/TsObject/TsArray/TsFunction facts. CLI/runtime proof builds canonical closed runtime-carrier calls without QuickJS, reflection dispatch, C# dynamic, source-name guessing, or fallback artifact emission.",
  }),
  ...
  "downstream.smoke.simple-apps": Object.freeze({
    ...
    positiveTests: Object.freeze([
      ...
      "test/cli-build/runtime/language/core.test.mjs",
    ]),
    negativeTests: Object.freeze([
      "test/cli-build/toolchain/downstream-smoke.test.mjs",
    ]),
  ...
  "downstream.nodejs-source": Object.freeze({
    ...
    backendContract:
      "Generated C# for Node-style source must consume selected Node provider-package facts and runtime artifacts, then fail closed when the package is unselected or a selected operation is unsupported.",
    positiveTests: Object.freeze([
      "test/cli-build/runtime/nodejs/modules-path-fs.test.mjs",
      "test/cli-build/runtime/nodejs/process-buffer-crypto-os.test.mjs",
      "test/cli-build/runtime/nodejs/util-url.test.mjs",
    ...
    blockers: Object.freeze([]),
    notes:
      "Reviewed proof: Node provider-package proof builds and runs existing Node-style source with selected js surface plus nodejs provider package, including node:fs, node:path, node:process namespace imports, node:os namespace imports, node:buffer, node:crypto, node:http, and node:timers through selected provider declarations, generated C# runtime project references, dotnet build, dotnet run, and exact stdout. Type-only node:http imports prove provider-owned source declarations do not add a Node runtime reference. Adjacent negatives prove node:* imports fail when the Node provider package is unselected and unsupported child_process, dgram, dns, events, net, querystring, readline, stream, tls, and zlib modules fail without C# artifacts or reflection/dynamic fallback.",
  }),
...
function capabilityDefaults(capabilityId, owner) {
  ...
  if (capabilityId.startsWith("operation.") || capabilityId.startsWith("expression.") || capabilityId.startsWith("statement.") || capabilityId.startsWith("binding.") || capabilityId.startsWith("function.")) {
    return {
      ...
      providerFacts: ["selectedTargetOperation", "selectedConversion", "resolvedTargetType"],
      backendContract:
        "Backend emits target AST from source AST plus immutable target selections; no source-name guessing or semantic recovery is allowed.",
    };
  }
  ...
  if (capabilityId.startsWith("dynamic-value.") || capabilityId.startsWith("runtime.")) {
    return {
      sourceExamples: ["const value: any = source; value.missing();"],
      tstsDecision:
        "TSTS checks TypeScript source and preserves the exact selected dynamic operation.",
      providerFacts: ["resolvedJsValueTargetType", "selectedDynamicValueOperationFact"],
      backendContract:
        "Backend emits a dynamic operation only from the exact finalized closed-operation and JS-value carrier facts.",
      runtimeContract:
...
function laneClassificationDefaults(capabilityId, owner) {
  ...
  const closedRuntime = laneClosedRuntimeBehavior(capabilityId);
  const canonical = laneCanonicalBehavior(
    capabilityId,
    staticRequiredFacts,
    hardRejectReasons,
    closedRuntime,
  );
  const hasStaticNativeLane = canonical.lane === "static-native" || !isClosedRuntimeOnlyCapability(capabilityId);
  const possibleLanes = Object.freeze([
    ...(hasStaticNativeLane ? ["static-native"] : []),
    ...(closedRuntime === undefined ? [] : ["closed-runtime"]),
    "hard-reject",
  ]);
  ...
  return freezeLaneClassification({
    ...
    ...(hasStaticNativeLane
      ...
      : {}),
    ...(closedRuntime === undefined ? {} : { closedRuntime }),
    hardReject: {
      lane: "hard-reject",
...
function laneCanonicalBehavior(
  capabilityId,
  staticRequiredFacts,
  hardRejectReasons,
  closedRuntime,
) {
  if (isClosedRuntimeOnlyCapability(capabilityId)) {
...
function isClosedRuntimeOnlyCapability(capabilityId) {
  return capabilityId.startsWith("dynamic-value.") ||
    capabilityId.startsWith("runtime.dynamic") ||
    capabilityId.startsWith("carrier.any");
}

function lanePatternKind(capabilityId) {
  if (capabilityId.startsWith("dynamic-value.any.dynamic-call") || capabilityId.startsWith("dynamic-value.any.call-construct")) {
    return "dynamic-call-or-construct";
  }
  if (capabilityId.startsWith("dynamic-value.any.dynamic-get") || capabilityId.startsWith("dynamic-value.any.property")) {
    return "dynamic-property-read";
  }
  if (capabilityId.startsWith("dynamic-value.any.dynamic-set")) {
    return "dynamic-property-write";
  }
  if (capabilityId.startsWith("dynamic-value.any.operators")) {
    return "dynamic-operator";
  }
  if (capabilityId.startsWith("dynamic-value.any.typed-boundary-cast")) {
    return "dynamic-typed-boundary";
  }
  if (capabilityId.startsWith("dynamic-value.object") || capabilityId.startsWith("dynamic-value.unknown")) {
    return "broad-carrier-access";
  }
  if (capabilityId.startsWith("dynamic-value.prototype") || capabilityId.startsWith("dynamic-value.proxy")) {
...
function laneHardRejectReasons(capabilityId) {
  ...
  if (capabilityId.startsWith("dynamic-value.proxy") || capabilityId.startsWith("dynamic-value.prototype")) {
...
function laneClosedRuntimeBehavior(capabilityId) {
  const carrier = closedRuntimeCarrier(capabilityId);
  ...
  return {
    ...
    operation: closedRuntimeOperation(capabilityId),
  };
}

function closedRuntimeCarrier(capabilityId) {
  if (capabilityId.startsWith("dynamic-value.any.dynamic-call") || capabilityId.startsWith("dynamic-value.any.call-construct")) {
    return "TsFunction";
  }
  if (capabilityId.startsWith("dynamic-value.any.dynamic-get") || capabilityId.startsWith("dynamic-value.any.dynamic-set") || capabilityId.startsWith("dynamic-value.any.property") || capabilityId.startsWith("dynamic-value.any.operators") || capabilityId.startsWith("dynamic-value.any.typed-boundary-cast") || capabilityId.startsWith("carrier.any") || capabilityId.startsWith("runtime.dynamic")) {
    return "TsValue";
  }
...
function closedRuntimeOperation(capabilityId) {
  if (capabilityId.startsWith("dynamic-value.any.dynamic-call")) {
    return "CallProperty";
  }
  if (capabilityId.startsWith("dynamic-value.any.dynamic-get") || capabilityId.startsWith("dynamic-value.any.property")) {
    return "GetProperty";
  ...
  if (capabilityId.startsWith("dynamic-value.any.call-construct")) {
    return "Construct";
  }
  if (capabilityId.startsWith("dynamic-value.any.operators")) {
...
function freezeLaneClassification(classification) {
  return Object.freeze({
    ...
    canonical: freezeLaneBehavior(classification.canonical),
    ...(classification.staticNative === undefined ? {} : { staticNative: freezeLaneBehavior(classification.staticNative) }),
    ...(classification.closedRuntime === undefined ? {} : { closedRuntime: freezeLaneBehavior(classification.closedRuntime) }),
...
function validateMapSetLaneClassification(errors, entry) {
  ...
  validateIncludes(
    errors,
    isPlainObject(classification.closedRuntime) ? classification.closedRuntime.requiredFacts : undefined,
    mapSetRequiredClosedRuntimeFacts,
    "surface.js.map-set laneClassification.closedRuntime.requiredFacts",
  );
  if (!isPlainObject(classification.closedRuntime) || classification.closedRuntime.runtimeCarrier !== "SelectedSurfaceRuntime") {
    errors.push("surface.js.map-set laneClassification.closedRuntime.runtimeCarrier must be SelectedSurfaceRuntime");
  }
  validateIncludes(
    errors,
...
function validateLaneClassification(errors, entry) {
  ...
  validateLaneBehavior(errors, classification.canonical, "laneClassification.canonical");
  validateCanonicalBehavior(errors, classification.canonical, possibleLanes);
  ...
  if (possibleLanes?.includes?.("closed-runtime")) {
    validateLaneBehavior(errors, classification.closedRuntime, "laneClassification.closedRuntime", "closed-runtime");
    validateRequiredFacts(errors, classification.closedRuntime, "laneClassification.closedRuntime.requiredFacts");
    if (!isPlainObject(classification.closedRuntime) || typeof classification.closedRuntime.runtimeCarrier !== "string" || classification.closedRuntime.runtimeCarrier.length === 0) {
      errors.push("laneClassification.closedRuntime.runtimeCarrier must be a non-empty string when lane is closed-runtime");
    } else if (bannedOpenRuntimeMechanismPattern.test(classification.closedRuntime.runtimeCarrier)) {
      errors.push("laneClassification.closedRuntime.runtimeCarrier must not name a banned runtime mechanism");
    } else if (!capabilityClosedRuntimeCarrierSet.has(classification.closedRuntime.runtimeCarrier)) {
      errors.push(`laneClassification.closedRuntime.runtimeCarrier must be one of ${capabilityClosedRuntimeCarriers.join(", ")}`);
    }
    if (!isPlainObject(classification.closedRuntime) || typeof classification.closedRuntime.operation !== "string" || classification.closedRuntime.operation.length === 0) {
      errors.push("laneClassification.closedRuntime.operation must be a non-empty string when lane is closed-runtime");
    }
  }
...
function validateCanonicalBehavior(errors, behavior, possibleLanes) {
  ...
  if (Array.isArray(possibleLanes) && possibleLanes.length > 0 && !possibleLanes.includes(behavior.lane)) {
    errors.push("laneClassification.canonical.lane must be listed in laneClassification.possibleLanes");
  }
}
