...
const reviewedCapabilityEvidence = Object.freeze({
  ...
  "binding.array.fixed-rest-default": Object.freeze({
    ...
    blockers: Object.freeze([
      "binding.array.fixed-rest-default remains partial until optional/nullish tuple default facts, one-element and empty tuple rest carriers, nested default values, and remaining sparse/full-JS copy-in/copy-out extraction lanes have current positive and fail-closed proof.",
    ]),
    notes:
      "Reviewed partial proof: old array destructuring inventory maps to current binding-pattern tests for fixed positions, rest, defaults, nested array extraction, synthetic destructured parameter prelude, and sparse JSArray hole/default/rest extraction using finalized carrier facts. Current CLI runtime evidence includes non-Node fixed/default/rest destructuring after array spread from a finalized nullable carrier, JS-surface sparse literal destructuring where defaults test hasIndex instead of length, required tuple defaults that erase to the finalized Item projection, and two-or-more tuple rest binding that emits a closed tuple from finalized element carriers. Backend fact tests prove tuple fixed/rest extraction emits Item projections, optional/nullish tuple defaults and one-element tuple rest fail closed until explicit carrier facts exist, provider read-only indexable carriers destructure without array-literal metadata, and provider literal-only carriers are rejected as missing index evidence. Missing carrier facts diagnose instead of falling back to stale array destructuring lowering.",
  }),
