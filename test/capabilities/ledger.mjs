...
const reviewedCapabilityEvidence = Object.freeze({
  ...
  "surface.node.fs": Object.freeze({
    ...
    notes:
      "Reviewed partial proof: selected NodeJS provider package facts cover unchanged bare fs imports, bare fs and node:fs namespace imports, provider-backed default node:fs module object calls such as fs.existsSync, existsSync/readFileSync string and Buffer returns, statSync/fstatSync, readSync/writeSync Buffer descriptors, writeFileSync/appendFileSync/unlinkSync writes and cleanup, node:fs/promises readFile/writeFile/stat/unlink/chmod/cp/readlink/realpath/rmdir/symlink Promise-returning target facts, no-surface negative paths block Node-owned modules before artifact emission, unsupported node:vm provider-package imports fail closed, and unsupported selected fs.watchFile fails closed without runtime fallback. Current executable proof writes, stats, reads, and unlinks a file through generated C# Node runtime calls. Stats Date-valued members are tracked under surface.node.fs-stats-date. Remains partial until the complete node:fs API surface has provider facts, precise unsupported-operation diagnostics, and runtime coverage.",
  }),
  ...
  "surface.node.buffer-crypto-os": Object.freeze({
    ...
    notes:
      "Reviewed partial proof: selected NodeJS provider package facts cover Buffer provider virtual declarations, Buffer static calls including from(string), from(number[]), from(Buffer), static Buffer.compare, Buffer.isBuffer, Buffer.poolSize, buffer.transcode, Buffer instance length/toString/copy/write/compare/includes/indexOf/lastIndexOf/readUInt8/writeUInt8, bare crypto/os and canonical node:crypto/node:os imports, provider-backed default node:crypto/node:os module object calls/properties such as randomUUID and EOL, crypto.randomUUID/randomInt/randomBytes/randomFillSync/timingSafeEqual, createHash/createHmac Hash/Hmac update/digest closed Buffer/string paths, getHashes array returns, os.homedir, and os.platform by selected provider declaration/member/signature identity. Current executable proof runs Buffer.from/toString, createHash(...).update(...).digest(\"hex\"), randomUUID, and os.platform through generated C# Node runtime calls. This capability remains partial until the full Buffer/crypto/os old fixture matrix has runtime/toolchain coverage, numeric conversion facts for byte-oriented Buffer APIs are proven end-to-end, and unsupported members fail closed with precise diagnostics.",
  }),
  ...
  "surface.node.url": Object.freeze({
    ...
    oldEvidence: Object.freeze([]),
    blockers: Object.freeze([
      "surface.node.url remains partial until live URL.searchParams-to-URL mutation coupling, URLPattern, open-object url.format, urlToHttpOptions, open-object option carriers, runtime execution, and complete selected-provider-package diagnostics have closed carrier implementations or explicit unsupported diagnostics through unit, CLI, toolchain, and runtime tests.",
    ]),
    notes:
      "Reviewed partial proof: selected node:url and bare url provider modules expose URL, URLSearchParams, and module function declarations; provider-backed default node:url module object facts expose closed file-path helpers such as pathToFileURL; closed URL constructor/properties/static methods, URL.searchParams property access, URL-as-base constructor/canParse/parse overloads, URL-only url.format, URLSearchParams constructor/append/set/get/getAll/has/delete/sort/toString/size, and domain/file-path helpers map by selected provider declaration/signature identity to Tsonic.CSharp.Node.URL/url calls; executable proof runs pathToFileURL and fileURLToPath through generated C# Node runtime calls; open-object url.format, urlToHttpOptions, URLPattern, and unproven live URL.searchParams mutation coupling fail closed or remain blocked without reflection, dynamic dispatch, object dictionary projection, or generic runtime fallback.",
  }),
  "backend.csharp.runtime-artifacts": Object.freeze({
    positiveTests: Object.freeze([
      "test/cli/surface-composition.test.mjs",
