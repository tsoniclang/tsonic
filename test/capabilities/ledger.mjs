...
const reviewedCapabilityEvidence = Object.freeze({
  ...
  "binding.array.fixed-rest-default": Object.freeze({
    ...
    notes:
      "Reviewed partial proof: old array destructuring inventory maps to current binding-pattern tests for fixed positions, rest, defaults, nested array extraction, synthetic destructured parameter prelude, and sparse JSArray hole/default/rest extraction using finalized carrier facts. Current CLI runtime evidence includes non-Node fixed/default/rest destructuring after array spread from a finalized nullable carrier and JS-surface sparse literal destructuring where defaults test hasIndex instead of length. Backend fact tests now prove tuple fixed extraction emits Item projections, tuple default/rest forms fail closed until optional-element/slice facts exist, provider read-only indexable carriers destructure without array-literal metadata, and provider literal-only carriers are rejected as missing index evidence. Missing carrier facts diagnose instead of falling back to stale array destructuring lowering.",
  }),
